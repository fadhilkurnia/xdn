...
def get_pod_node_ip(name: str, namespace: str, timeout: int = 60) -> str:
    """Return the internal IP of the node where the first running pod of a deployment lives."""
    deadline = time.time() + timeout
    while time.time() < deadline:
        result = subprocess.run(
            ["kubectl", "-n", namespace, "get", "pods", "-l", f"{APP_LABEL}={name}",
        ...
        if ip:
            return ip
        time.sleep(2)
    raise TimeoutError(f"Could not determine node IP for pods of {name}")
...
def write_results(results_dir: Path, rows: List[Dict[str, float]]) -> Path:
    ...
    fieldnames = ["app", "target_rate_rps", "offered_rate_rps", "throughput_rps", "p95_ms", "p90_ms", "p50_ms", "avg_ms"]
...
def main() -> None:
    ...
    parser.add_argument("dockerImage", help="Docker image to deploy")
    ...
    results_dir = results_base / f"load_ar_{app_name.removeprefix('oebs-')}_oebs_{timestamp}"
    ...
    try:
        ...
        if not args.data_parsing_only:
            ...
            # Detect the node where the pod is running and use NodePort.
            node_ip = get_pod_node_ip(app_name, args.namespace)
            wait_for_port(node_ip, NODEPORT)
            logger.info("NodePort ready at %s:%s, starting load runs for rates: %s",
                        node_ip, NODEPORT, rates)

        target_url_base = f"http://{node_ip}:{NODEPORT}" if node_ip else f"http://127.0.0.1:{NODEPORT}"
        ...
        for rate in rates:
            ...
            if not args.data_parsing_only and args.load_generator == "k6":
                ...
                run_latency_client(
                    go_source,
                    f"{target_url_base}/{post_endpoint}",
                    payload_json,
                    duration_seconds,
                    rate,
                    latency_output_file,
            ...
            for metrics in stage_metrics:
                ...
                all_rows.append({"app": app_name, "target_rate_rps": rate, "offered_rate_rps": offered_rate_rps, **metrics})
        ...
        # Always tear down Kubernetes objects.
        if tmp_script:
            Path(tmp_script).unlink(missing_ok=True)
