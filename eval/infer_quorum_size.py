...
def infer_q(n_peers, shifts):
    """Pure quorum-size search, decoupled from any measurement I/O.

    Model: the leader blocks until the fastest q-1 of its n_peers respond, so
    delaying a set S of leader-outbound links shifts client write latency iff
    S leaves fewer than q-1 undelayed peers, i.e. iff |S| > N - q (N = leader +
    peers). The predicate P(m) = "delaying m peers shifts" is monotone in m
    with flip point m* = N - q + 1, so q = N - m* + 1, found by bisection over
    prefixes (peers are interchangeable, so any m-subset behaves like any
    other).

    `shifts(subset)` is a boolean oracle: True iff delaying exactly those peer
    indices shifts latency. Returns (q, m_star); m_star is None when q == 1
    (delaying every peer still does not shift -- no synchronous wait).
    Consumes at most 1 + ceil(log2(n_peers)) oracle calls.
    """
    n = n_peers + 1
    if not shifts(list(range(n_peers))):  # boundary: does the leader wait at all?
        return 1, None
    lo, hi = 1, n_peers  # invariant: P(lo-1) = False, P(hi) = True
    while lo < hi:
        mid = (lo + hi) // 2
        if shifts(list(range(mid))):
            hi = mid
        else:
            lo = mid + 1
    m_star = lo
    return n - m_star + 1, m_star


def _self_test():
    """Validates infer_q against synthetic quorum oracles, no infra required.

    For each (N, q), a write waits for the fastest q-1 peers, so delaying a
    subset shifts iff it leaves < q-1 peers undelayed: shift(S) = |S| > N - q.
    Asserts the search recovers q for every q in 1..N across a range of N, and
    that it never exceeds its round budget.
    """
    import math
    ...
    for n in range(2, 9):
        ...
        for q in range(1, n + 1):
            ...
            def shifts(subset, q=q, n=n):
                calls.append(len(subset))
                return len(subset) > n - q

            got_q, m_star = infer_q(n_peers, shifts)
            assert got_q == q, f"N={n} q={q}: inferred {got_q} (m*={m_star})"
            budget = 1 + math.ceil(math.log2(n_peers)) if n_peers > 1 else 1
            assert len(calls) <= budget, \
                f"N={n} q={q}: {len(calls)} oracle calls > budget {budget}"
            if q == 1:
                assert m_star is None, f"N={n} q=1 should give m*=None, got {m_star}"
    log("self-test passed: infer_q recovers q for N=2..8, q=1..N within budget")
...
def run(args):
    ...
    # The measurement oracle infer_q drives: delay peers[subset], one
    # inject-measure-clear round, return whether latency shifted. The labels
    # (boundary / bisect m=k) are cosmetic; infer_q owns the search order.
    def shifts(subset_idx):
        label = "boundary" if len(subset_idx) == len(peers) \
            else f"bisect m={len(subset_idx)}"
        return bool(measure_with([peers[i] for i in subset_idx], label))

    q, m_star = infer_q(len(peers), shifts)
    if m_star is None:
...
def main():
    ...
    # --self-test validates the search logic against synthetic oracles and
    # exits; it needs no cluster, so the other args are not required with it.
    if "--self-test" in sys.argv:
        _self_test()
        return
    ap.add_argument("--self-test", action="store_true",
                    help="validate infer_q against synthetic oracles and exit")
