...
def start_container(host: str, container_name: str, image: str, env_vars: Optional[Dict[str, str]] = None):
    ...
    cmd = (
        ...
        f"docker run -d --rm --security-opt seccomp=unconfined {env_arg} -p 8080:80 --name {shlex.quote(container_name)} {shlex.quote(image)}"
    )
    try:
...
def start_baseline_replica(host: str, container_name: str, repo_root: Path,
    ...
    # Kill any existing replica, free port 2300, and clean CRIU dirs.
    run_remote(host, kill_cmd)
    run_remote(host, "sudo fuser -k 2300/tcp >/dev/null 2>&1 || true")
    run_remote(host, "sudo rm -rf /dev/shm/chk /dev/shm/criu-inc >/dev/null 2>&1 || true")

    # Start the primary replica. Run with sudo so CRIU commands (which need root) work.
    inc_flag = "-Dcriu.incremental=true " if incremental else ""
    start_cmd = (
        f"sudo java -cp {shlex.quote(classpath)} -Djdk.httpclient.allowRestrictedHeaders=content-length "
        f"{inc_flag}"
...
def main():
    ...
    parser.add_argument(
        ...
        default=True,
        help="Use CRIU incremental checkpoint (pre-dump with soft-dirty tracking)",
    )
    ...
    # Derive a short app label consistent with other baselines (e.g., "bookcatalog" not "xdn-bookcatalog")
    app_label = args.dockerImageName.split("/")[-1].split(":")[0].removeprefix("xdn-")
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    results_dir = results_base / f"load_ar_{app_label}_criu_{timestamp}"
    ...
    for i, rate in enumerate(rates_list):
        logger.info("=" * 60)
        logger.info("  Rate point %d/%d: %d req/s (fresh container)", i + 1, len(rates_list), rate)
        logger.info("=" * 60)

        baseline_pid = None
        try:
            # Fresh container + replica for each rate point
            stop_services(target_host, container_name, None)
            run_remote(target_host, "sudo rm -rf /dev/shm/chk /dev/shm/criu-inc >/dev/null 2>&1 || true")

            logger.info("Starting container on %s", target_host)
            start_container(target_host, container_name, args.dockerImageName, env_vars)
            logger.info("Starting BaselineCriuReplica (incremental=%s)", args.incremental)
            ...
            if metrics.get("p50_ms", 0.0) > MAX_LATENCY_MS or metrics.get("avg_ms", 0.0) > MAX_LATENCY_MS:
                logger.warning(
                    "Stopping: median or average latency exceeded %.0f ms (median=%.2f ms, avg=%.2f ms)",
                    MAX_LATENCY_MS,
            ...
            logger.exception("Rate point %d req/s failed", rate)
        finally:
            logger.info("Cleaning up rate point %d req/s", rate)
            stop_services(target_host, container_name, baseline_pid)
            run_remote(target_host, "sudo rm -rf /dev/shm/chk /dev/shm/criu-inc >/dev/null 2>&1 || true",
                       capture_output=True)
