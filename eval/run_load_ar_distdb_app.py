...
def parse_latency_output(output_path: Path) -> Dict[str, float]:
    ...
    return {
        "achieved_rps": float(metrics.get("actual_achieved_rate_rps", 0.0)),
...
def main():
    ...
    for i, rate in enumerate(rates_list):
        ...
        try:
            ...
            # Kill any leftover rqlite/tikv from a previous iteration
            for host, _ in active_replicas[:3]:
                run_remote(host, "pkill -f rqlited >/dev/null 2>&1 || true", check=False)
                run_remote(
                    host,
                    f"docker rm -f $(docker ps -aq --filter 'name=tikv_') >/dev/null 2>&1 || true",
                    check=False,
                )
            if backend == "tikv":
                rc_host, _ = parse_control_plane(config_path)
                run_remote(
                    rc_host,
                    f"docker rm -f $(docker ps -aq --filter 'name=tikv_pd_') >/dev/null 2>&1 || true",
                    check=False,
                )

            # ── Start fresh DB cluster ────────────────────────────────
            db_env: Dict[str, str] = {}
            if backend == "rqlite":
            ...
            # ── Load test ─────────────────────────────────────────────
            script_path = None
            summary_file = results_dir / f"{backend}_k6_summary_rate_{default_service}_{rate}.json"
            latency_output = results_dir / f"{backend}_go_latency_rate_{default_service}_{rate}.txt"
            try:
