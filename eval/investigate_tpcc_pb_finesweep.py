...
Runs the same XDN Primary-Backup TPC-C setup as run_load_pb_tpcc_reflex.py but
with a finer rate schedule (1–100 rps) to find the exact saturation point
and build a proper load-latency curve.

Run from the eval/ directory:
    python3 investigate_tpcc_pb_finesweep.py

Outputs go to eval/results/load_pb_tpcc_reflex_finesweep/:
...
SCREEN_SESSION = "xdn_tpcc_pb_finesweep"
SCREEN_LOG_OVERRIDE = f"screen_logs/{SCREEN_SESSION}.log"

RESULTS_DIR = Path(__file__).resolve().parent / "results" / "load_pb_tpcc_reflex_finesweep"
...
def run_load_point(primary, rate):
    """Run get_latency_at_rate.go at `rate` req/s."""
...
if __name__ == "__main__":
    print("=" * 60)
    print("TPC-C Primary-Backup Fine-Grained Rate Sweep")
    print("=" * 60)
    ...
    for host in AR_HOSTS:
        if not wait_for_port(host, HTTP_PROXY_PORT, TIMEOUT_PORT_SEC):
            ...
            print(f"       Check: tail -f {SCREEN_LOG_OVERRIDE}")
            sys.exit(1)
    ...
    print(f"\n[Phase 9] Fine-grained load sweep at {FINESWEEP_RATES} req/s ...")
