"""
plot_hotelres_comparison.py — 3-way latency + throughput comparison for hotel-reservation.

Reads rate*.txt files from three results directories and generates:
  1. Latency plot: mean latency vs offered load for XDN PB, Repl. MongoDB, OpenEBS
  2. Throughput plot: actual vs offered throughput with y=x reference

Also prints a side-by-side comparison table and saves a text report.

Usage (run from eval/ directory):
    python3 plot_hotelres_comparison.py [--results-base PATH]
"""
...
DATASETS = [
    ...
    ("pb",        "XDN PB",        "#4C72B0", "hotelres_pb"),
    ("replmongo", "Repl. MongoDB", "#DD8452", "hotelres_replmongo"),
...
def plot_latency(datasets: dict, out: Path) -> None:
    ...
    styles = {
        "pb":        ("#4C72B0", "#4C72B0"),
        "replmongo": ("#DD8452", "#DD8452"),
        "openebs":   ("#55A868", "#55A868"),
    }
    markers = {"pb": "o", "replmongo": "s", "openebs": "^"}
    labels  = {"pb": "XDN PB", "replmongo": "Repl. MongoDB", "openebs": "OpenEBS"}
    ...
    ax.set_title(
        "Hotel-Reservation Replication Comparison: Load vs Latency (mean)\n"
        "POST /reservation  ·  3-way CloudLab",
        fontsize=11,
    )
...
def plot_throughput(datasets: dict, out: Path) -> None:
    ...
    colors  = {"pb": "#4C72B0", "replmongo": "#DD8452", "openebs": "#55A868"}
    labels  = {"pb": "XDN PB",  "replmongo": "Repl. MongoDB",  "openebs": "OpenEBS"}
    markers = {"pb": "o",       "replmongo": "s",               "openebs": "^"}
    ...
    ax.set_title(
        "Hotel-Reservation Replication Comparison: Offered vs Actual Throughput\n"
        "POST /reservation  ·  3-way CloudLab",
        fontsize=11,
    )
...
def print_comparison_table(datasets: dict) -> None:
    ...
    for key in active_keys:
        ...
        if rows:
            ...
            print(f"    {labels[key]:16s}: {peak:.2f} rps at {int(rate_at_peak)} rps offered")
    print()
...
def save_report(datasets: dict, out_path: Path) -> None:
    ...
    lines = []
    lines.append("Hotel-Reservation Replication Comparison — Final Report")
    ...
    for key in active_keys:
        rows = datasets.get(key, [])
        if not rows:
            lines.append(f"  {labels[key]:16s}: no data")
            continue
        ...
        lines.append(
            f"  {labels[key]:16s}: peak={peak:.2f} rps at {int(peak_rate)} rps offered; {knee_str}"
        )
    lines.append("")
...
def parse_args():
    ...
    p.add_argument(
        ...
        help="Base directory containing hotelres_pb/, hotelres_replmongo/, hotelres_openebs/ subdirs "
             "(default: eval/results/)",
    )
    p.add_argument(
        "--out", type=Path,
        default=None,
        help="Output PNG path base (default: <results-base>/hotelres_comparison)",
    )
    return p.parse_args()
...
def main():
    ...
    out_base = args.out or (results_base / "hotelres_comparison")
    ...
    for key, label, _, subdir in DATASETS:
        ...
        datasets[key] = rows
        if rows:
            print(f"  {label:16s}: {len(rows)} rate points from {results_base / subdir}")
        else:
            print(f"  {label:16s}: no data found in {results_base / subdir}")
    ...
    save_report(datasets, results_base / "hotelres_final_report.txt")
