"""
plot_tpcc_comparison.py — 3-way TPC-C latency + throughput comparison.

Reads rate*.txt files from three results directories and generates:
  1. Latency (linear scale): mean latency for XDN PB, OpenEBS, PG Sync-Rep
  2. Throughput: actual rps per offered rate, with y=x reference

Also prints a side-by-side comparison table to stdout.

Usage (run from eval/ directory):
    python3 plot_tpcc_comparison.py [--results-base PATH]
"""
...
DATASETS = [
    ...
    ("pb",     "XDN PB",      "#4C72B0", "tpcc_pb"),
    ("openebs", "OpenEBS",    "#DD8452", "tpcc_openebs"),
...
def plot_latency(datasets: dict, out: Path) -> None:
    ...
    styles = {
        "pb":     ("#4C72B0", "#4C72B0"),
        "openebs": ("#DD8452", "#DD8452"),
        "pgsync": ("#55A868", "#55A868"),
    }
    markers = {"pb": "o", "openebs": "s", "pgsync": "^"}
    labels  = {"pb": "XDN PB", "openebs": "OpenEBS", "pgsync": "PG Sync-Rep"}
    ...
    ax.set_title(
        "TPC-C Replication Comparison: Load vs Latency (mean)\n"
...
def plot_throughput(datasets: dict, out: Path) -> None:
    ...
    colors  = {"pb": "#4C72B0", "openebs": "#DD8452", "pgsync": "#55A868"}
    labels  = {"pb": "XDN PB",  "openebs": "OpenEBS",  "pgsync": "PG Sync-Rep"}
    markers = {"pb": "o",       "openebs": "s",         "pgsync": "^"}
    ...
    ax.set_title(
        "TPC-C Replication Comparison: Offered vs Actual Throughput\n"
        "New Order via HTTP POST  ·  3-way CloudLab",
        fontsize=11,
    )
...
def save_report(datasets: dict, out_path: Path) -> None:
    ...
    lines = []
    lines.append("TPC-C Replication Comparison — Final Report")
...
def parse_args():
    ...
    p.add_argument(
        ...
        help="Base directory containing tpcc_pb/, tpcc_openebs/, tpcc_pgsync/ subdirs "
             "(default: eval/results/)",
    )
    p.add_argument(
        "--out", type=Path,
        default=None,
        help="Output PNG base path (default: <results-base>/tpcc_triple_comparison)",
    )
    return p.parse_args()
...
def main():
    ...
    out_base = args.out or (results_base / "tpcc_triple_comparison")
    ...
    save_report(datasets, results_base / "tpcc_final_report.txt")
