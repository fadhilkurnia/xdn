"""
investigate_tpcc_pb_rsync.py — RSYNC recorder TPC-C benchmark.

Runs the XDN cluster with XDN_PB_STATEDIFF_RECORDER_TYPE=RSYNC instead of
FUSELOG. RSYNC doesn't intercept filesystem operations — PG writes directly
to native filesystem during execute, eliminating FUSE overhead. State diffs
are captured via rsync at each capture point.

If RSYNC mode has lower execute time (~33ms matching PG sync-rep) but higher
capture time, FUSE is confirmed as the cause of execute inflation.

Run from the eval/ directory:
    python3 investigate_tpcc_pb_rsync.py

Outputs go to eval/results/load_pb_tpcc_reflex_rsync/:
...
SCREEN_SESSION = "xdn_tpcc_pb_rsync"
SCREEN_LOG_OVERRIDE = f"screen_logs/{SCREEN_SESSION}.log"

RESULTS_DIR = Path(__file__).resolve().parent / "results" / "load_pb_tpcc_reflex_rsync"
...
ORIGINAL_RECORDER_TYPE = "FUSELOG"
...
def restore_config():
    ...
    set_config("XDN_PB_STATEDIFF_RECORDER_TYPE", ORIGINAL_RECORDER_TYPE)
...
if __name__ == "__main__":
    print("=" * 60)
    print("TPC-C Primary-Backup with RSYNC Recorder")
    print("=" * 60)
    ...
    # Phase 1b — Modify config for RSYNC recorder
    print("\n[Phase 1b] Setting XDN_PB_STATEDIFF_RECORDER_TYPE=RSYNC ...")
    set_config("XDN_PB_STATEDIFF_RECORDER_TYPE", "RSYNC")

    try:
        # Phase 2 — Start cluster
        print("\n[Phase 2] Starting XDN cluster (RSYNC recorder) ...")
        ...
        print(f"\n[Phase 9] Load sweep at {FINESWEEP_RATES} req/s (RSYNC recorder) ...")
        ...
        print(f"   Recorder  : RSYNC (no FUSE intercept during execute)")
        ...
        # Phase 11 — Summary
        print("\n[Phase 11] Summary (RSYNC recorder)")
