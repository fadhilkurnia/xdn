import warnings as _w; _w.warn("DEPRECATED: Use run_load_pb_reflex_app.py --app synth instead.", DeprecationWarning, stacklevel=2)
"""
...
def run_exp5(primary):
    """Experiment 5: Vary auto-committed ops per request.

    Each op is an independent auto-committed INSERT (no explicit txn).
    This matches how most ORMs (GORM, wpdb, ActiveRecord) work.
    XDN coordinates once per request; baselines coordinate per-op.
    """
    print("\n" + "=" * 60)
...
if __name__ == "__main__":
    ...
    # Save effective config to results folder
    from gp_config_utils import save_effective_config
    import run_load_pb_synth_common as _sc
    save_effective_config(_sc.GP_CONFIG, _sc._bc_common.GP_JVM_ARGS, RESULTS_BASE)
    ...
    if 5 in experiments:
        all_results["exp5"] = run_exp5(primary)
