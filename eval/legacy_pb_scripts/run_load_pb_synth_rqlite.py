import warnings as _w; _w.warn("DEPRECATED: Use run_load_pb_distdb_app.py --app synth instead.", DeprecationWarning, stacklevel=2)
"""
...
def run_exp5(app_url):
    ...
    results = []
    for i, ops in enumerate(EXP5_AUTOCOMMIT_OPS):
        print(f"\n   --- Point {i+1}/{len(EXP5_AUTOCOMMIT_OPS)}: ops={ops} (autocommit) ---")
        rqlite_processes = []
        try:
            rqlite_processes = setup_fresh_cluster()
