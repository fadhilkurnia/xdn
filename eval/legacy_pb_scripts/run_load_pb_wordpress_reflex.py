import warnings as _w; _w.warn("DEPRECATED: Use run_load_pb_reflex_app.py --app wordpress instead.", DeprecationWarning, stacklevel=2)
"""
...
# Fixed warmup rate to ensure MySQL buffer pool is hot before measurement,
# regardless of the measurement rate. Must be high enough to exercise the
# full write path (InnoDB redo log, binlog, statediff capture).
WARMUP_RATE = 1500
...
def setup_fresh_wordpress_cluster(enable_apache_timing=False):
    ...
    if RESULTS_DIR is not None:
        ...
        save_effective_config(_wpmod.GP_CONFIG, _wpmod.GP_JVM_ARGS, RESULTS_DIR)
    ...
    # With XDN_SKIP_INIT_SYNC=true, the sync is instant (no rsync).
    # With XDN_MIGRATE_WAIT_SEC=5, MySQL migration wait is 5s instead of 60s.
    ...
    while time.time() < init_sync_deadline:
        ...
        print(f"   WARNING: initContainerSync not detected after {INIT_SYNC_TIMEOUT}s — proceeding anyway")
    ...
    # Enable Apache %D timing header for latency analysis (if requested)
    if enable_apache_timing:
...
def parse_args():
    ...
    p.add_argument("--fixed-accumulation", action="store_true",
                   help="Disable adaptive capture accumulation (fix window at 100µs)")
    p.add_argument("--jfr", action="store_true",
                   help="Enable Java Flight Recorder profiling during measurement")
    p.add_argument("--apache-timing", action="store_true",
                   help="Enable Apache %%D response header for per-request timing")
    return p.parse_args()
...
if __name__ == "__main__":
    ...
    if args.sample_latency or args.fixed_accumulation:
        import run_load_pb_wordpress_common as _common
    if args.sample_latency:
        _common.GP_JVM_ARGS += " -DPB_SAMPLE_LATENCY=true"
    if args.fixed_accumulation:
        _common.GP_JVM_ARGS += " -DPB_CAPTURE_ACCUMULATION_MAX_US=100"
    ...
    print(f"  Rates    : {rates}")
    print(f"  Duration : {load_duration}s per rate (warmup: {warmup_duration}s, settle: {args.settle}s)")
    ...
    for i, rate in enumerate(rates):
        ...
        if jfr_file:
            ...
            if jfr_dest.exists():
                ...
                print(f"   WARNING: JFR download failed")
