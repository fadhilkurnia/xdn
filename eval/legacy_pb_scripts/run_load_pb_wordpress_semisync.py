import warnings as _w; _w.warn("DEPRECATED: Use run_load_pb_distdb_app.py --app wordpress instead.", DeprecationWarning, stacklevel=2)
"""
...
WP_PORT       = 8094  # avoid port 2300 (XDN proxy) to prevent conflicts
MYSQL_PORT    = 3306
...
def start_mysql_primary():
    ...
    _ssh(PRIMARY_HOST, f"docker network create {DOCKER_NETWORK} 2>/dev/null || true", check=False)
    cmd = (
