...
public class GreedyHeuristicTest {
  ...
  void testSingleCellSingleNode_thatNodeSelected() {
    ...
    PlacementResult result =
        greedy.selectReplicas(reads, writes, nodes, 1, "Closest", "Closest", 1);
  ...
  void testSingleCellMultipleNodes_closestNodeSelected() {
    ...
    PlacementResult result =
        greedy.selectReplicas(reads, Collections.emptyMap(), nodes, 1, "Closest", "Closest", 1);
  ...
  void testEmptyDemand_returnsNull() {
    PlacementResult result =
        greedy.selectReplicas(
            Collections.emptyMap(),
            Collections.emptyMap(),
            Map.of("A", geo(0.0, 0.0)),
            1,
            "Closest",
            "Closest",
            1);
  ...
  void testTwoHotspots_K2_oneReplicaNearEach() {
    ...
    PlacementResult result =
        greedy.selectReplicas(reads, writes, nodes, 2, "Closest", "Closest", 1);
  ...
  void testKEqualsNodeCount_allNodesSelected() {
    ...
    PlacementResult result =
        greedy.selectReplicas(reads, Collections.emptyMap(), nodes, 3, "Closest", "Closest", 2);
  ...
  void testCoordinatorAlwaysInReplicaSet_allAccessTypes() {
    ...
    for (String readType : List.of("Closest", "Source", "Majority")) {
      for (String writeType : List.of("Closest", "Source", "Majority")) {
        PlacementResult result =
            greedy.selectReplicas(reads, writes, nodes, 2, readType, writeType, 2);
  ...
  void testClosestAccessType_doesNotCrash() {
    Map<Integer, Integer> reads = Map.of(idx(500, 500), 100);
    Map<Integer, Integer> writes = Map.of(idx(500, 500), 50);
    Map<String, Geolocation> nodes =
        Map.of(
            "A", geo(0.0, 0.0),
            "B", geo(20.0, 20.0));

    assertNotNull(greedy.selectReplicas(reads, writes, nodes, 1, "Closest", "Closest", 1));
  }
  ...
  void testSourceAccessType_doesNotCrash() {
    ...
    assertNotNull(greedy.selectReplicas(reads, writes, nodes, 2, "Source", "Source", 2));
  }
  ...
  void testMajorityAccessType_doesNotCrash() {
    ...
    assertNotNull(greedy.selectReplicas(reads, writes, nodes, 3, "Majority", "Majority", 2));
  }
  ...
  void testInvalidAccessType_throwsIllegalArgumentException() {
    ...
    assertThrows(
        ...
        () ->
            greedy.selectReplicas(
                reads, Collections.emptyMap(), nodes, 1, "BadType", "Closest", 1));
  }
  ...
  void testQuorumLargerThanK_clampedSafely() {
    ...
    PlacementResult result =
        greedy.selectReplicas(reads, Collections.emptyMap(), nodes, 2, "Majority", "Majority", 10);
  ...
  void testQ1_alwaysValidQuorum() {
    ...
    PlacementResult result =
        greedy.selectReplicas(reads, writes, nodes, 2, "Majority", "Majority", 1);
  ...
  void testWriteOnlyDemand_doesNotCrash() {
    ...
    assertNotNull(
        greedy.selectReplicas(Collections.emptyMap(), writes, nodes, 1, "Source", "Source", 1));
  }
