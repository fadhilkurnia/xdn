 ...
 *
 * NOTE ON RNG FIDELITY
 * Python uses numpy's PCG64 generator; Java uses java.util.Random (LCG).
 * For the same seed the numerical values will differ, but the statistical
 * properties (distribution shape, candidate spread) are equivalent.
 * Intra-Java reproducibility is guaranteed: same seed → same workload.
 */
public final class WorkloadGeneration {
    ...
    public static WorkloadResult generateWorkload(
        ...
        double[][] readGrid  = distribute(gridRows, gridCols, totalReads,  distribution,
                hotspotCount, hotspotConcentration, hotspotRadius,
                clusterCount, clusterRadius, clusterConcentration, rng);
        double[][] writeGrid = distribute(gridRows, gridCols, totalWrites, distribution,
                hotspotCount, hotspotConcentration, hotspotRadius,
                clusterCount, clusterRadius, clusterConcentration, rng);
        ...
        int[][] candidateNodes = generateCandidateNodes(
                gridRows, gridCols, numCandidateNodes, rng);
    ...
    private static double[][] distribute(
            int gridRows, int gridCols, int total, String distribution,
            int hotspotCount, double hotspotConcentration, double hotspotRadius,
            int clusterCount, double clusterRadius, double clusterConcentration,
            Random rng) {
        switch (distribution) {
            case "uniform":
                return distributeUniform(gridRows, gridCols, total, rng);
            case "hotspot":
                return distributeHotspot(gridRows, gridCols, total,
                        hotspotCount, hotspotConcentration, hotspotRadius, rng);
            case "clustered":
                return distributeClustered(gridRows, gridCols, total,
                        clusterCount, clusterRadius, clusterConcentration, rng);
            default:
                throw new IllegalArgumentException(
    ...
    /**
     * Spread total requests uniformly with small random variation ~U(0.5,1.5)×base.
     * Rescaled to match total exactly, then rounded.
     */
    private static double[][] distributeUniform(
            int gridRows, int gridCols, int total, Random rng) {
        ...
        if (sum > 0.0) {
            ...
            for (int r = 0; r < gridRows; r++)
                for (int c = 0; c < gridCols; c++)
                    grid[r][c] = Math.round(grid[r][c] * scale);
        }
        return grid;
    }
    ...
    /**
     * hotspotCount blob centres chosen at random. Each blob uses Gaussian
     * falloff of width hotspotRadius (σ). Blob weights follow Zipf: blob k
     * gets weight 1/(k+1), normalised. concentration fraction of total traffic
     * goes into blobs; remainder is spread with uniform noise.
     */
    private static double[][] distributeHotspot(
            int gridRows, int gridCols, int total,
            int hotspotCount, double concentration, double hotspotRadius,
            Random rng) {
        ...
        double wSum = 0.0;
        for (int k = 0; k < hotspotCount; k++) { blobWeights[k] = 1.0 / (k + 1); wSum += blobWeights[k]; }
        ...
        if (remaining > 0.0) {
            ...
            for (int r = 0; r < gridRows; r++)
                for (int c = 0; c < gridCols; c++)
                    grid[r][c] += noiseScale * noise[r * gridCols + c];
        }
    ...
    /**
     * clusterCount blobs, each with Gaussian falloff of width clusterRadius.
     * Unlike hotspot, blobs are NOT individually normalised — raw Gaussian
     * values are accumulated and then collectively rescaled.
     */
    private static double[][] distributeClustered(
            int gridRows, int gridCols, int total,
            int clusterCount, double clusterRadius, double concentration,
            Random rng) {
    ...
    /**
     * Divide the grid into numNodes roughly-equal rectangular regions; pick one
     * random cell from each region. Mirrors Python's _generate_candidate_nodes.
     */
    private static int[][] generateCandidateNodes(
        ...
        int colsRegions = Math.max(1, (int) Math.round(
                Math.sqrt((double) numNodes * gridCols / gridRows)));
        ...
        candidates.sort((a, b) -> a[0] != b[0] ? Integer.compare(a[0], b[0])
