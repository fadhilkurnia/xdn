 ...
 *
 * Pipeline per run:
 *   1. WorkloadGeneration produces a dense request grid and candidate node positions.
 *   2. The dense grid is converted to the sparse format our algorithms expect.
 *   3. Each enabled algorithm selects replica nodes via {@code selectReplicas()}.
 *   4. The benchmark evaluates the cost of that placement using the standard
 *      latency cost model (Closest / Source / Majority) and writes a CSV row.
 *
 * Two things to edit before running:
 *   {@link #SCALE}      — controls grid sizes and seed count.
 *   {@link #ALGORITHMS} — comment/uncomment entries to enable or disable algorithms.
 *
 * CSV results: {@code out/placementbenchmark/<algorithm>/<model>.csv}
 */
public class PlacementAlgorithmBenchmarkTest {
  ...
  /**
   * Controls grid sizes and seed count.
   * QUICK ≈ seconds | MEDIUM ≈ 1–2 min | FULL ≈ 5 min
   */
  private static final BenchmarkScale SCALE = BenchmarkScale.QUICK;
  private static final double MS_PER_UNIT       = 10.0;

  /**
   * Algorithms to benchmark. Comment out an entry to skip it.
   * The factory receives the grid dimensions so each algorithm instance is
   * sized correctly for the current sweep step.
   */
  ...
  private static final ModelConfig[] MODELS = {
      new ModelConfig("linearizability", "Majority", "Majority",
          new double[][]{{0.8, 0.2}}),
      new ModelConfig("sequential",      "Closest",  "Majority",
          new double[][]{{0.8, 0.2}, {0.5, 0.5}}),
      new ModelConfig("eventual",        "Closest",  "Closest",
          new double[][]{{0.8, 0.2}}),
      new ModelConfig("primary-backup",  "Source",   "Source",
          new double[][]{{0.8, 0.2}}),
  };
  ...
  void runPlacementBenchmark() throws Exception {
    System.out.printf(
        "Placement benchmark — scale=%s  algorithms=%s%n%n",
        SCALE, algorithmNames());
  ...
  private void runModel(ModelConfig model) throws Exception {
    ...
    System.out.printf("%n%s%nModel: %-20s  read=%-8s  write=%s%n%s%n",
    ...
    try {
      for (AlgorithmEntry entry : ALGORITHMS) {
        ...
        pw.println("seed,grid_rows,grid_cols,distribution,read_ratio,write_ratio,"
          ...
          for (double[] ratio : model.ratios) {
            ...
            for (int seed : SCALE.seeds) {
              ...
              try {
                ...
                wl = WorkloadGeneration.generateWorkload(
                    gridRows, gridCols,
                    CANDIDATE_COUNT, totalReqs,
                    readR, writeR,
                    dist,
                    3, 0.95, 0.04 * minDim,
                    2, 0.15 * minDim, 0.7,
                ...
                System.err.printf("  Workload gen failed %dx%d dist=%s seed=%d: %s%n",
              ...
              System.out.printf("%n  %dx%d  dist=%-9s  ratio=%.2f/%.2f  seed=%d%n",
              ...
              for (AlgorithmEntry entry : ALGORITHMS) {
                ...
                if (disabled.contains(entry.name)) {
                  ...
                  pw.printf("%d,%d,%d,%s,%.2f,%.2f,%s,%s,,,,,skipped%n",
                      seed, gridRows, gridCols, dist, readR, writeR,
                ...
                final PlacementAlgorithm algo =
                    entry.factory.create(gridRows, gridCols);
                ...
                try {
                  ...
                  Future<PlacementResult> future = ex.submit(() ->
                      algo.selectReplicas(
                          sparseReads, sparseWrites, nodeGeo,
                          CLUSTER_SIZE,
                          fModel.readType, fModel.writeType,
                          QUORUM_SIZE));
                ...
                if (timedOut) {
                  System.out.printf("    %-20s TIMEOUT (%.0f ms) — disabling for %s%n",
                      entry.name, elapsedMs, model.name);
                  pw.printf("%d,%d,%d,%s,%.2f,%.2f,%s,%s,,%.2f,,,timeout%n",
                      seed, gridRows, gridCols, dist, readR, writeR,
                      model.name, entry.name, elapsedMs);
 
                } else if (errMsg != null) {
                  System.err.printf("    %-20s ERROR: %s%n", entry.name, errMsg);
                  pw.printf("%d,%d,%d,%s,%.2f,%.2f,%s,%s,,,,,%s%n",
                      seed, gridRows, gridCols, dist, readR, writeR,
                      model.name, entry.name, "error:" + errMsg.replace(',', ';'));
                  ...
                  double cost = evaluateCost(
                      ...
                      model.readType, model.writeType, QUORUM_SIZE,
                      gridRows, gridCols);
 
                  System.out.printf("    %-20s cost=%14.2f  time=%.1f ms%n",
                      entry.name, cost, elapsedMs);
                  pw.printf("%d,%d,%d,%s,%.2f,%.2f,%s,%s,%.4f,%.2f,\"%s\",%s,ok%n",
                      seed, gridRows, gridCols, dist, readR, writeR,
                      model.name, entry.name,
                      cost, elapsedMs,
      ...
      writers.values().forEach(pw -> { if (pw != null) pw.close(); });
    }
  }
  ...
  private static double evaluateCost(
      ...
      String readType, String writeType, int Q,
    ...
    for (int cellIdx : allCells) {
      ...
      double dEntryLeader = usesLeader
          ? gridDist(nodePos.get(nearestIdx), nodePos.get(leaderIdx)) * MS_PER_UNIT
          : 0.0;
 
      total += fRead  * cellCost(readType,  minDist, dEntryLeader, deltaSrc, W)
  ...
  private static double cellCost(String type, double dEntry, double dEntryLeader,
  ...
  /**
   * Converts a channel of the dense requestGrid into a sparse map.
   * channel=0 → reads, channel=1 → writes.
   */
  ...
  /**
   * Converts int[][] candidateNodes (grid row/col coordinates) to
   * Map&lt;String, Geolocation&gt; using the inverse of latToRow/lonToCol.
   */
