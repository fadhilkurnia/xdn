    ...
    public void testGetReplicaInfoSingleService() throws Exception {
        ...
        try (XdnTestCluster cluster = new XdnTestCluster()) {
            ...
            for (int i = 0; i < containers.length(); i++) {
                ...
                assertTrue(container.getString("createdAt").contains(" ago"),
        ...
        try (XdnTestCluster cluster = new XdnTestCluster()) {
            ...
            while (System.currentTimeMillis() < deadline && !hasLeaderAndFollowers) {
                ...
                for (int replicaIdx = 0; replicaIdx < replicaCount; replicaIdx++) {
                    HttpResponse<String> infoResponse =
                            cluster.sendGetRequest(serviceName, replicaIdx,
                                    "/api/v2/services/" + serviceName + "/replica/info");
                ...
                hasLeaderAndFollowers = roleByReplica.size() == replicaCount
                        && leaderCount == 1
            ...
            assertTrue(hasLeaderAndFollowers,
                    "Timed out waiting for leader and follower roles to stabilize");
            assertEquals(replicaCount, roleByReplica.size(),
                    "Each replica should report its role");
            ...
            assertEquals(replicaCount - 1, followerCount,
                    "Expected remaining replicas to be followers");
        }
    }
    ...
    public void testGetPrimaryBackupReplicaInfo() throws Exception {
        ...
        try (XdnTestCluster cluster = new XdnTestCluster()) {
            ...
            while (System.currentTimeMillis() < deadline && !hasPrimaryAndBackups) {
                ...
                hasPrimaryAndBackups = roleByReplica.size() == replicaCount
                        && primaryCount == 1
            ...
            assertTrue(hasPrimaryAndBackups,
                    "Timed out waiting for primary and backup roles to stabilize");
            assertEquals(replicaCount, roleByReplica.size(),
                    "Each replica should report its role");
            ...
            assertEquals(replicaCount - 1, backupCount,
                    "Expected remaining replicas to be backups");
        }
    }
