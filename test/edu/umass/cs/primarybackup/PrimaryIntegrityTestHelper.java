...
public class PrimaryIntegrityTestHelper {
  ...
  protected record CoordinatorNode(
  ...
  protected static Map<String, CoordinatorNode> startThreeNodesWithMonotonicApp(
    ...
    ReconfigurationPacketDemultiplexer pdAtNode1 =
        PrimaryIntegrityTestHelper.createDefaultPacketDemultiplexer(node1ID, config);
    JSONMessenger<String> messengerAtNode1 =
        PrimaryIntegrityTestHelper.createNodeDefaultMessenger(node1ID, config, pdAtNode1);
    PrimaryBackupReplicaCoordinator<String> node1 =
        new PrimaryBackupReplicaCoordinator<>(
            appAtNode1,
            node1ID,
            PrimaryIntegrityTestHelper.getDefaultNodeIdDeserializer(),
            messengerAtNode1);
    PrimaryIntegrityTestHelper.registerHandlerForDemultiplexer(pdAtNode1, node1);
    servers.put(
        node1ID,
        new PrimaryIntegrityTestHelper.CoordinatorNode(
            node1, node1.getPrimaryBackupManager(), appAtNode1, messengerAtNode1));

    // initializing app in node-2
    String node2ID = NODE_2_ID;
    MonotonicTestApp appAtNode2 = new MonotonicTestApp();
    ReconfigurationPacketDemultiplexer pdAtNode2 =
        PrimaryIntegrityTestHelper.createDefaultPacketDemultiplexer(node2ID, config);
    JSONMessenger<String> messengerAtNode2 =
        PrimaryIntegrityTestHelper.createNodeDefaultMessenger(node2ID, config, pdAtNode2);
    PrimaryBackupReplicaCoordinator<String> node2 =
        new PrimaryBackupReplicaCoordinator<>(
            ...
            PrimaryIntegrityTestHelper.getDefaultNodeIdDeserializer(),
            messengerAtNode2);
    PrimaryIntegrityTestHelper.registerHandlerForDemultiplexer(pdAtNode2, node2);
    servers.put(
        node2ID,
        new PrimaryIntegrityTestHelper.CoordinatorNode(
            node2, node2.getPrimaryBackupManager(), appAtNode2, messengerAtNode2));

    // initializing app in node-3
    String node3ID = NODE_3_ID;
    MonotonicTestApp appAtNode3 = new MonotonicTestApp();
    ReconfigurationPacketDemultiplexer pdAtNode3 =
        PrimaryIntegrityTestHelper.createDefaultPacketDemultiplexer(node3ID, config);
    JSONMessenger<String> messengerAtNode3 =
        PrimaryIntegrityTestHelper.createNodeDefaultMessenger(node3ID, config, pdAtNode3);
    PrimaryBackupReplicaCoordinator<String> node3 =
        new PrimaryBackupReplicaCoordinator<>(
            ...
            PrimaryIntegrityTestHelper.getDefaultNodeIdDeserializer(),
            messengerAtNode3);
    PrimaryIntegrityTestHelper.registerHandlerForDemultiplexer(pdAtNode3, node3);
    servers.put(
        node3ID,
        new PrimaryIntegrityTestHelper.CoordinatorNode(
            node3, node3.getPrimaryBackupManager(), appAtNode3, messengerAtNode3));

    return servers;
  }

  /**
   * cleanPreviousState removes all Gigapaxos state stored in the /tmp/gigapaxos directory. This
   * ensures that our tests run without any previous state.
   */
  ...
  protected static void printServers(ReconfigurableNodeConfig<String> config) {
    ...
    for (Map.Entry<String, InetSocketAddress> ar : config.getActiveReplicasReadOnly().entrySet()) {
      System.out.printf(" - %s %s\n", ar.getKey(), ar.getValue().toString());
    }
    System.out.print(">> List of reconfigurators:\n");
    for (Map.Entry<String, InetSocketAddress> rc : config.getReconfiguratorsReadOnly().entrySet()) {
  ...
  private static Stringifiable<String> getDefaultNodeIdDeserializer() {
    return strValue -> strValue;
  }

  private static ReconfigurationPacketDemultiplexer createDefaultPacketDemultiplexer(
      String nodeID, NodeConfig<String> config) {
    return new ReconfigurationPacketDemultiplexer(config).setThreadName(nodeID);
  }

  private static void registerHandlerForDemultiplexer(
      ReconfigurationPacketDemultiplexer pd, PrimaryBackupReplicaCoordinator<String> coordinator) {
  ...
  private static JSONMessenger<String> createNodeDefaultMessenger(
    ...
    try {
      ...
      System.out.printf(
          ">> initializing messenger for %s at %s\n",
