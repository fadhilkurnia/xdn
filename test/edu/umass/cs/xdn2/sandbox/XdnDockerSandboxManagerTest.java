 ...
 *
 * <p>Requires a running Docker daemon. Tests pull real images and start real containers. Each test
 * cleans up after itself via @AfterEach.
 *
 * <p>Images used: - michael2718/bookcatalog-nd:1 (SQLite single-component) -
...
class XdnDockerSandboxManagerTest {
  ...
  private static String containerName(int componentIdx, int epoch, String serviceName) {
    return String.format("c%d.e%d.%s.%s.xdn.io", componentIdx, epoch, serviceName, NODE_ID);
  }
  ...
  /** Builds a minimal single-component SQLite ServiceInstance. Mirrors sq_primary.yml. */
  ...
  private ServiceInstance buildSqliteInstance(int epoch) {
    ...
    return new ServiceInstance(property, SERVICE_SQLITE, networkName(SERVICE_SQLITE), port, names);
  }

  /** Builds a two-component MySQL ServiceInstance. Mirrors my_primary.yml. */
  ...
  private ServiceInstance buildMysqlInstance(int epoch) {
    ...
    return new ServiceInstance(property, SERVICE_MYSQL, networkName(SERVICE_MYSQL), port, names);
  }

  /** Builds a two-component PostgreSQL ServiceInstance. Mirrors pq_primary.yml. */
  ...
  private ServiceInstance buildPostgresInstance(int epoch) {
    ...
    List<String> names =
        List.of(
            containerName(0, epoch, SERVICE_POSTGRES), containerName(1, epoch, SERVICE_POSTGRES));

    containersToCleanup.addAll(names);
    networksToCleanup.add(networkName(SERVICE_POSTGRES));
    sandbox.prepareStateDirectory(SERVICE_POSTGRES, epoch);
    return new ServiceInstance(
        property, SERVICE_POSTGRES, networkName(SERVICE_POSTGRES), port, names);
  }
  ...
  void sqliteCreateNetwork() {
    assertTrue(sandbox.createNetwork(SERVICE_SQLITE), "createNetwork() should return true");
    String status =
        Shell.runCommandWithOutput(
                "docker network inspect " + networkName(SERVICE_SQLITE) + " --format='{{.Name}}'")
            .stdout
            .trim();
    assertTrue(status.contains("net::"), "Network should exist after createNetwork()");
  }
  ...
  void sqliteStartService() {
    ...
    assertTrue(sandbox.startService(instance, 0), "startService() should return true");
    String status = sandbox.getContainerStatus(instance.containerNames.get(0));
    assertEquals("running", status, "Container should be running after startService()");
  }
  ...
  void sqliteStopService() {
    ...
    assertTrue(sandbox.stopService(instance), "stopService() should return true");
    String status = sandbox.getContainerStatus(instance.containerNames.get(0));
    assertTrue(
        status == null || status.equals("exited"),
  ...
  void sqliteDeleteService() {
    ...
    sandbox.createNetwork(SERVICE_SQLITE);
    sandbox.startService(instance, 0);
    assertTrue(sandbox.deleteService(instance), "deleteService() should return true");
    String status = sandbox.getContainerStatus(instance.containerNames.get(0));
    assertNull(status, "Container should not exist after deleteService()");
  }
  ...
  void sqliteDeleteServiceIdempotent() {
    ...
    assertTrue(
        sandbox.deleteService(instance), "Second deleteService() call should still return true");
  }
  ...
  void mysqlStartService() {
    ...
    assertTrue(sandbox.startService(instance, 0), "startService() should return true for MySQL");
    for (String name : instance.containerNames) {
      String status = sandbox.getContainerStatus(name);
      assertEquals("running", status, "Container " + name + " should be running");
    }
  }
  ...
  void mysqlWaitUntilReady() {
    ...
    assertTrue(
        sandbox.waitUntilReady(dbContainer, healthcheckCmd),
  ...
  void mysqlStopService() {
    ...
    for (String name : instance.containerNames) {
      String status = sandbox.getContainerStatus(name);
      assertTrue(
          status == null || status.equals("exited"), "Container " + name + " should be stopped");
    }
  }
  ...
  void mysqlDeleteService() {
    ...
    for (String name : instance.containerNames) {
      assertNull(
          sandbox.getContainerStatus(name),
  ...
  void mysqlDeleteServiceIdempotent() {
    ...
    sandbox.startService(instance, 0);
    sandbox.deleteService(instance);
    assertTrue(sandbox.deleteService(instance), "Second deleteService() should still return true");
  }
  ...
  void postgresStartService() {
    ...
    assertTrue(
        sandbox.startService(instance, 0), "startService() should return true for PostgreSQL");
    for (String name : instance.containerNames) {
      String status = sandbox.getContainerStatus(name);
      assertEquals("running", status, "Container " + name + " should be running");
    }
  ...
  void allocatePortIsFree() throws IOException {
    int port = sandbox.allocatePort();
    assertTrue(port >= 50000 && port <= 65000, "Port should be within the expected range");
    // Verify the port is actually free by binding to it
    try (ServerSocket s = new ServerSocket(port)) {
      assertEquals(port, s.getLocalPort(), "Returned port should be bindable");
    }
  }
  ...
  void allocatePortAvoidsOccupied() throws IOException {
    ...
    try {
      ...
      assertFalse(
          occupied.contains(allocated), "allocatePort() should not return an occupied port");
  ...
  void startServiceBadImage() {
    ...
    try {
      property =
          ServiceProperty.createFromJsonString(
              """
            {
              "name": "test-bad-image",
    ...
    List<String> names = List.of(containerName(0, 0, "test-bad-image"));
    containersToCleanup.addAll(names);
    networksToCleanup.add(networkName("test-bad-image"));
    sandbox.prepareStateDirectory("test-bad-image", 0);
    ServiceInstance instance =
        new ServiceInstance(property, "test-bad-image", networkName("test-bad-image"), port, names);
  ...
  void getContainerStatusRunning() {
    ...
    sandbox.startService(instance, 0);
    String status = sandbox.getContainerStatus(instance.containerNames.get(0));
    assertEquals("running", status, "Running container should have status 'running'");
  }
  ...
  void createNetworkIdempotent() {
    networksToCleanup.add(networkName(SERVICE_SQLITE));
    assertTrue(sandbox.createNetwork(SERVICE_SQLITE), "First createNetwork() should return true");
    assertTrue(
        sandbox.createNetwork(SERVICE_SQLITE), "Second createNetwork() should also return true");
  }
  ...
  void deleteNetworkIdempotent() {
    sandbox.createNetwork(SERVICE_SQLITE);
    assertTrue(sandbox.deleteNetwork(SERVICE_SQLITE), "First deleteNetwork() should return true");
    assertTrue(
        sandbox.deleteNetwork(SERVICE_SQLITE), "Second deleteNetwork() should also return true");
  }
  ...
  void getContainerStatusNonexistent() {
    ...
    assertNull(status, "getContainerStatus() should return null for unknown container");
  }
  ...
  void getContainerIds() {
    ...
    assertEquals(1, ids.size(), "Should return one ID for single-component service");
    assertFalse(ids.get(0).isBlank(), "Container ID should not be blank");
  }
  ...
  void getContainerCreatedAt() {
    ...
    assertEquals(1, timestamps.size(), "Should return one timestamp for single-component service");
    assertFalse(timestamps.get(0).isBlank(), "Timestamp should not be blank");
  }
}
