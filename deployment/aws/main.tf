...
variable "rc_ami" {
  ...
  type        = string
  default     = "ami-004a7d599deff3295" # arm64, main: RESTful control API + GeoIP demand (geolocation_city_data.mmdb baked)
}
...
variable "ar_ami" {
  ...
  type        = string
  default     = "ami-0bb65e92559b91b3c" # arm64, main: RESTful control API + GeoIP demand (geolocation_city_data.mmdb baked)
}
...
resource "aws_instance" "rc" {
  ...
  user_data = templatefile("${path.module}/rc-userdata.tftpl", {
    ...
    # The per-replica `edge` sub-zone is a multi-region feature (see deployment/aws/multiregion);
