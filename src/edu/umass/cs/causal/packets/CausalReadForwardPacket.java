...
public class CausalReadForwardPacket extends CausalPacket implements Byteable {
  ...
  public CausalReadForwardPacket(
      String serviceName, String senderNodeId, ClientRequest clientReadRequest) {
    this(
  ...
  private CausalReadForwardPacket(
      long packetId, String serviceName, String senderNodeId, ClientRequest clientReadRequest) {
    ...
    assert (clientReadRequest instanceof BehavioralRequest br && br.isReadOnlyRequest())
        : "Client request carried by CausalReadForwardPacket must be read-only";
  ...
  public IntegerPacketType getRequestType() {
    return CausalPacketType.CAUSAL_READ_FORWARD_PACKET;
  }
  ...
  public byte[] toBytes() {
    ...
    int payloadSize =
        CodedOutputStream.computeInt64Size(1, this.packetId)
    ...
    try {
      ...
      throw new RuntimeException("Failed to serialize CausalReadForwardPacket", e);
    }
  ...
  public boolean equals(Object o) {
    ...
    if (!(o instanceof CausalReadForwardPacket that)) return false;
  ...
  public static CausalReadForwardPacket createFromBytes(
      byte[] encodedPacket, AppRequestParser appRequestParser) {
    ...
    if (packetType != CausalPacketType.CAUSAL_READ_FORWARD_PACKET.getInt()) {
      Logger.getGlobal()
          .log(
              Level.SEVERE,
              "Receiving an invalid encoded CausalReadForwardPacket: unexpected type "
                  + packetType);
      return null;
    }
    ...
    try {
      ...
      Logger.getGlobal()
          .log(
              Level.SEVERE,
              "Receiving an invalid encoded CausalReadForwardPacket: " + e.getMessage());
      return null;
    }
    ...
    if (serviceName == null || senderId == null || encodedRequest == null) {
      Logger.getGlobal()
          .log(
              Level.SEVERE, "Receiving an invalid encoded CausalReadForwardPacket: missing fields");
      return null;
    }
    ...
    assert (request instanceof ClientRequest)
        : "Payload inside CausalReadForwardPacket must be ClientRequest";
    assert (request instanceof BehavioralRequest br && br.isReadOnlyRequest())
        : "Client request inside CausalReadForwardPacket must be read-only";

    return new CausalReadForwardPacket(packetId, serviceName, senderId, (ClientRequest) request);
  }
}
