package edu.umass.cs.causal.examples;
...
public class KeyValueAppClient
    extends ReconfigurableAppClientAsync<edu.umass.cs.causal.examples.KeyValueAppRequest> {
  ...
  public static void main(String[] args) throws IOException, InterruptedException {
    ...
    for (int i = 0; i < 10; i++) {
      ...
      KeyValueAppWriteRequest appWriteRequest =
          new KeyValueAppWriteRequest(
              serviceName, myClientID, requestID, keys.get(i), values.get(i));
      appWriteRequest.dependencies = currentDeps;
      Request response =
          client.sendRequest(ReplicableClientRequest.wrap(appWriteRequest), replicaAddress);
      System.out.printf(
          ">> PUT %s response=%s version=%d deps=%s\n",
          appWriteRequest.getKey(),
          ((edu.umass.cs.causal.examples.KeyValueAppRequest) response).getResponseValue(),
          ((edu.umass.cs.causal.examples.KeyValueAppRequest) response).responseVersion,
          ((edu.umass.cs.causal.examples.KeyValueAppRequest) response).dependencies);
      currentDeps = ((edu.umass.cs.causal.examples.KeyValueAppRequest) response).dependencies;

      // send GET request
      edu.umass.cs.causal.examples.KeyValueAppRequest appRequest =
          new edu.umass.cs.causal.examples.KeyValueAppRequest(
              serviceName, myClientID, requestID, keys.get(i), null);
      appRequest.dependencies = currentDeps;
      response = client.sendRequest(ReplicableClientRequest.wrap(appRequest), replicaAddress);
      System.out.printf(
          ">> GET %s response=%s version=%d deps=%s\n",
          appRequest.getKey(),
          ((edu.umass.cs.causal.examples.KeyValueAppRequest) response).getResponseValue(),
          ((edu.umass.cs.causal.examples.KeyValueAppRequest) response).responseVersion,
          ((edu.umass.cs.causal.examples.KeyValueAppRequest) response).dependencies);
  ...
  public Request getRequest(String stringified) throws RequestParseException {
    assert stringified != null
            && stringified.startsWith(
                edu.umass.cs.causal.examples.KeyValueAppRequest.SERIALIZED_PREFIX)
