...
import edu.umass.cs.xdn.recorder.RecorderType;
import edu.umass.cs.xdn.sandbox.SandboxType;
import java.io.File;
...
/**
 * XdnConfig reads XDN-specific configuration from a properties file, independent of GigaPaxos's
 * configuration system.
 *
 * <p>Default location: conf/xdn.properties Override via: -DxdnConfig=path/to/xdn.properties
 *
 * <p>Required properties: SANDBOX_TYPE one of: docker (future: podman, gvisor, firecracker, qemu)
 * RECORDER_TYPE one of: rsync, fuselog, fuserust, zip
 *
 * <p>Optional properties (used by DockerSandboxManager.waitUntilReady()):
 * DEFAULT_HEALTHCHECK_INTERVAL_SECONDS default: 2 DEFAULT_HEALTHCHECK_TIMEOUT_SECONDS default: 5
 * DEFAULT_HEALTHCHECK_RETRIES default: 30
 *
 * <p>All fields are final — XdnConfig is immutable after construction. Required properties throw
 * immediately if absent. Optional properties fall back to hardcoded defaults if absent.
 */
public class XdnConfig {
  ...
  /**
   * Loads XDN configuration from the default location (conf/xdn.properties) or from the path
   * specified via -DxdnConfig=...
   */
  public XdnConfig() {
  ...
  public XdnConfig(String configPath) {
    ...
    this.healthcheckIntervalSeconds = parseOptionalInt(props, KEY_HC_INTERVAL, DEFAULT_HC_INTERVAL);
    this.healthcheckTimeoutSeconds = parseOptionalInt(props, KEY_HC_TIMEOUT, DEFAULT_HC_TIMEOUT);
    this.healthcheckRetries = parseOptionalInt(props, KEY_HC_RETRIES, DEFAULT_HC_RETRIES);
    this.healthcheckConsecutiveSuccesses =
        parseOptionalInt(props, KEY_HC_CONSECUTIVE, DEFAULT_HC_CONSECUTIVE);
    this.fuselogBaseDir = props.getProperty(KEY_FUSELOG_BASE_DIR, DEFAULT_FUSELOG_BASE_DIR);
  }
  ...
  private static Properties loadProperties(String configPath) {
    ...
    try (FileInputStream fis = new FileInputStream(file)) {
      ...
      throw new RuntimeException(
          "Failed to read XDN config file at '" + configPath + "': " + e.getMessage());
    }
  ...
  private static int parseOptionalInt(Properties props, String key, int defaultValue) {
    ...
    try {
      ...
      throw new RuntimeException(
          "Invalid value for optional property '"
              + key
              + "': '"
              + raw
