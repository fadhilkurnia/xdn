 ...
 *
 * <p>Implementations receive the sparse read/write demand grids (cell index →
 * request count), the available node geolocations, and the consistency model's
 * read/write access types, and return a {@link PlacementResult} identifying
 * which nodes to use and which should be the preferred Paxos coordinator.
 *
 * <p>Static grid-math helpers are provided here so all implementations share
 * a single source of truth for the lat/lon ↔ row/col conversion formulas.
 * Pass {@code numRows} and {@code numCols} from the profiler constants rather
 * than hard-coding them inside each algorithm.
 */
