 ...
 *
 * <p>At each of the K steps the algorithm picks the candidate node that minimises the total
 * weighted latency cost across all active demand cells, simultaneously searching for the best
 * leader within the tentative cluster. The cost model for each cell depends on the access type:
 *
 * <ul>
 *   <li><b>Closest</b> — {@code 2 · d(cell, entry)}
 *   <li><b>Source</b> — {@code 2 · d(cell, entry) + 2 · (d(entry, L) + Δ)} where {@code Δ = max_k
 *       d(L, replica_k)}
 *   <li><b>Majority</b> — {@code 2 · d(cell, entry) + 2 · (d(entry, L) + W)} where {@code W = d(L,
 *       Q-th nearest replica)}
 * </ul>
 *
 * <p>Node geolocations are converted to grid (row, col) coordinates before the algorithm runs;
 * distances are in grid-unit multiples of {@code msPerUnit}.
 */
public class Greedy implements PlacementAlgorithm {
  ...
  public PlacementResult selectReplicas(
    ...
    for (int step = 0; step < K; step++) {
      ...
      for (int candIdx = 0; candIdx < nJ; candIdx++) {
        ...
        int nCloser = 0;
        for (int ci = 0; ci < nActive; ci++) if (dIj[ci][candIdx] < dCellToEntry[ci]) nCloser++;
    ...
    String coordinator =
        (finalLeaderIdx >= 0) ? nodeIds.get(finalLeaderIdx) : nodeIds.get(clusterIdx[0]);
  ...
  /**
   * Try every placed replica as leader; return {@code [bestCost, bestLeaderCandIdx]}. If {@code
   * usesLeader} is false (all-Closest model), leaderIdx is -1.
   */
  ...
  private static double computeCost(
    ...
    for (int i = 0; i < nActive; i++) {
      ...
      total +=
          fRead[i] * cellCost(readType, dEntry, dEntryLeader, deltaSrc, W)
              + fWrite[i] * cellCost(writeType, dEntry, dEntryLeader, deltaSrc, W);
    }
    return total;
  }

  static double cellCost(
