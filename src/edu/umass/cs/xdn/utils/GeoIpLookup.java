...
public class GeoIpLookup {
  ...
  /**
   * Looks up the geolocation of the given IP string. Returns null if the MMDB is unavailable, the
   * IP is invalid, or no location record is found.
   */
  ...
  public static Geolocation lookup(String ip) {
    ...
    try {
      ...
      LOG.log(
          Level.INFO,
          "GeoIP lookup for {0} -> lat={1} lng={2}",
          new Object[] {ip, geo.latitude(), geo.longitude()});
      return geo;
    } catch (Exception e) {
      LOG.log(
          Level.WARNING,
          "GeoIP lookup exception for IP {0}: {1}",
