 ...
 * in XDN. Concrete implementations support different runtimes such as Docker, Podman, gVisor,
 * Firecracker, or QEMU.
 *
 * <p>A SandboxManager instance is created once per node via SandboxManager.create() and shared
 * between DeterministicService and NonDeterministicService.
 *
 * <p>Responsibilities: - Network lifecycle (create, delete) - Container lifecycle (start, stop,
 * delete) - State management (snapshot, restore, state directory path) - Readiness polling (wait
 * until container is healthy) - Observability (status, IDs, creation timestamps) - Port allocation
 */
public abstract class SandboxManager {
  ...
  public static SandboxManager create(XdnConfig config, String nodeId) {
    return switch (config.getSandboxType()) {
      ...
      default -> throw new RuntimeException("Unknown sandbox type: " + config.getSandboxType());
    };
  }
  ...
  /**
   * Starts all containers for the given service instance. For single-component services: runs one
   * docker run. For multi-component services: runs one docker run per component.
   *
  ...
  /**
   * Starts the service with an explicit volume mount path, overriding the default state directory.
   * Used by PrimaryBackupManager to mount primaryLive/, backupLive1/, or backupLive2/ depending on
   * the node's role.
   *
   * @param instance the service instance to start
  ...
  /**
   * Stops all containers for the given service instance without deleting the state directory. The
  ...
  /**
   * Stops and removes a single container by name without affecting other containers in the same
   * service.
   *
  ...
  /**
   * Stops and removes all containers for the given service instance, including the state directory
   * and network.
   *
  ...
  /**
   * Prepares a clean state directory for a fresh service creation. Wipes any existing content and
   * creates the directory. Must NOT be called during reconfiguration — restoreStateFromSnapshot()
  ...
  /**
   * Captures a full snapshot of the service's state directory as a tar archive. Used during
   * reconfiguration (getFinalState) and Paxos checkpointing (DeterministicService.checkpoint).
   *
   * @param serviceName name of the service
   * @param epoch current placement epoch
   ...
   */
  public abstract boolean restoreStateFromSnapshot(
      String serviceName, int epoch, String encodedState);

  /**
   * Returns the absolute path to the state directory for a service epoch. This directory is
   ...
   *
   * <p>If healthcheckCmd is non-null, polls via docker exec until the command exits 0. The polling
   * interval, timeout, and retry count are set at construction time from XdnConfig.
   *
   * <p>If healthcheckCmd is null, returns true immediately (no polling).
   *
  ...
  /**
   * Same as waitUntilReady(containerName, healthcheckCmd), but if requireConsecutiveHealthy is
   * true, requires multiple consecutive "healthy" reads (per XdnConfig's
   * DEFAULT_HEALTHCHECK_CONSECUTIVE_SUCCESSES) before returning true, rather than trusting the
   * first successful read. Used for images with a known startup false-positive window
   * (Postgres/MySQL).
   */
  ...
  /**
   * Polls an HTTP endpoint on the given host-published port until it responds with a 2xx status, or
   * returns true immediately if path is null. Used for entry (non-stateful) components, whose
   * readiness can't be inferred from a known image name the way stateful DB images can. Interval,
   * timeout, and retry count are set at construction time from XdnConfig, same as waitUntilReady().
   *
  ...
  /**
   * Allocates an available host port for a new service. The port is chosen randomly from the
   * ephemeral range and verified to be free before returning.
   *
