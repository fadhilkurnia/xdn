package edu.umass.cs.xdn.sandbox;
 ...
 *
 * <p>It manages the full lifecycle of Docker containers for XDN services: - Network
 * creation/deletion - Container start/stop/delete (single and multi-component via docker run) -
 * State snapshot capture and restore (tar-based, via shell tar) - Readiness polling via docker
 * inspect health status - Port allocation via ServerSocket binding check - Container observability
 * (status, IDs, creation timestamps)
 *
 * <p>Multi-component services run each component as a separate docker run call, joined to the same
 * Docker network. docker compose is not used.
 *
 * <p>Large state transfer (>500KB) uses GigaPaxos LargeCheckpointer, which runs an embedded HTTP
 * server on the old AR to serve the state file to the new AR during reconfiguration.
 */
public class DockerSandboxManager extends SandboxManager {

  // Base directory for all state directories
  // Structure: /tmp/xdn/state/<nodeId>/<serviceName>/e<epoch>/
  private static final String BASE_STATE_PATH = "/tmp/xdn/state/";

  // Base directory for final state archives
  // Structure: /tmp/xdn/final/<nodeId>/<serviceName>/<epoch>/final_state.tar
  private static final String BASE_FINAL_PATH = "/tmp/xdn/final/";
  ...
  public DockerSandboxManager(
    ...
    this.largeCheckpointer =
        new LargeCheckpointer(String.format("%s%s/", BASE_FINAL_PATH, nodeId), nodeId);
  }
  ...
  /**
   * Starts all components of the service at the given epoch. Each component is started via docker
   * run with appropriate flags. Each component joins the same Docker network. Components with a
   * healthcheck (client-specified or inferred) have --health-* flags added so Docker tracks
   * readiness internally. waitUntilReady() must be called separately after this method.
   */
  public boolean startService(
    ...
    long statefulCount = components.stream().filter(ServiceComponent::isStateful).count();
    if (statefulCount > 1) {
      logger.log(
          Level.SEVERE,
  ...
  public boolean stopService(ServiceInstance instance) {
    ...
    for (String containerName : instance.containerNames) {
      int exitCode = Shell.runCommand("docker container stop " + containerName, true);
  ...
  public boolean stopContainer(String containerName) {
    logger.log(
        ...
        new Object[] {nodeId, containerName});
    int code = Shell.runCommand(String.format("docker rm --force %s", containerName), true);
    if (code != 0) {
      logger.log(
          Level.WARNING,
  ...
  public boolean deleteService(ServiceInstance instance) {
    ...
    for (String containerName : instance.containerNames) {
      int exitCode = Shell.runCommand("docker container rm --force " + containerName, true);
      if (exitCode != 0 && exitCode != 1) {
        logger.log(
            Level.WARNING,
  ...
  /**
   * Captures the full state directory as a compressed tar archive. Uses shell `tar -czf` to avoid
   * ZipFiles dependency. Retries up to 10 times if rsync copy fails.
   *
  ...
  public String captureStateSnapshot(String serviceName, int epoch) {
    ...
    int exitCode = Shell.runCommand(String.format("tar -czf %s -C %s .", tarPath, stagingDir));
    if (exitCode != 0) {
      logger.log(
          Level.SEVERE,
  ...
  /**
   * Restores the state directory from a Base64-encoded tar or a URL. Handles both inline (≤500KB)
   * and large (url:) state formats.
   */
  @Override
  public boolean restoreStateFromSnapshot(String serviceName, int epoch, String encodedState) {
    String stateDir = getStateDirectory(serviceName, epoch);
    String tarDir = String.format("%s%s/%s/%d/", BASE_FINAL_PATH, nodeId, serviceName, epoch);
    ...
    if (encodedState.startsWith("url:")) {
      ...
      try {
        Files.write(Paths.get(tarPath), bytes, StandardOpenOption.CREATE, StandardOpenOption.DSYNC);
      } catch (IOException e) {
        logger.log(
            Level.SEVERE,
    ...
    int exitCode = Shell.runCommand(String.format("tar -xzf %s -C %s", tarPath, stateDir));
    if (exitCode != 0) {
      logger.log(
          Level.SEVERE,
  ...
  /**
   * Returns the absolute path to the state directory for a service epoch. Format:
   * /tmp/xdn/state/<nodeId>/<serviceName>/e<epoch>/
   */
  @Override
  ...
  /**
   * Produces the final state string for reconfiguration. Inline Base64 for small states, URL via
   * LargeCheckpointer for large ones.
   *
   ...
   */
  public String buildFinalStateString(
      String serviceName, int epoch, String tarPath, String servicePropertyJson) {
    File archive = new File(tarPath);
    String prefix =
        String.format(
            "%s%d:", edu.umass.cs.xdn.service.ServiceProperty.XDN_EPOCH_FINAL_STATE_PREFIX, epoch);

    if (archive.length() <= MAX_INLINE_STATE_BYTES) {
      try {
        byte[] bytes = Files.readAllBytes(Paths.get(tarPath));
        return String.format(
            "%s::%s::%s", prefix, servicePropertyJson, Base64.getEncoder().encodeToString(bytes));
      } catch (IOException e) {
        logger.log(
            Level.SEVERE,
  ...
  /**
   * Polls docker inspect health status until the container reports "healthy". Returns true
   * immediately if healthcheckCmd is null (no healthcheck defined). Uses healthcheck parameters set
   * at construction time from XdnConfig.
   */
  // new
  @Override
  public boolean waitUntilReady(
      String containerName, String healthcheckCmd, boolean requireConsecutiveHealthy) {
    if (healthcheckCmd == null || healthcheckCmd.isBlank()) {
      return true;
    }

    String inspectCmd =
        String.format("docker inspect --format='{{.State.Health.Status}}' %s", containerName);
  ...
  /**
   * Polls an HTTP endpoint on the given host-published port until it responds with a 2xx status.
   * Returns true immediately if path is null (no HTTP healthcheck configured). Uses the same
   * healthcheck interval/ timeout/retries as waitUntilReady().
   */
  @Override
  ...
  /**
   * Allocates an available host port using ServerSocket binding to verify the port is genuinely
   * free at the moment of check.
   */
  @Override
  public int allocatePort() {
    for (int attempt = 0; attempt < PORT_ALLOC_MAX_ATTEMPTS; attempt++) {
      int port = PORT_RANGE_MIN + (int) (Math.random() * (PORT_RANGE_MAX - PORT_RANGE_MIN));
  ...
  private boolean runDockerContainer(
    ...
    if (mountSource != null
        ...
        && !mountTarget.isBlank()) {
      cmd.add("--mount");
      cmd.add(String.format("type=bind,source=%s,target=%s", mountSource, mountTarget));
    }
  ...
  /**
   * Runs docker inspect with the given Go template format string. Returns the trimmed output, or
   * null if the command fails.
   */
  private String dockerInspect(String containerName, String format) {
    ProcessBuilder pb =
        new ProcessBuilder("docker", "container", "inspect", "--format", format, containerName);
  ...
  private static String formatTimestamp(String iso8601) {
    try {
      ...
      if (d.toDays() > 365) return (d.toDays() / 365) + " years ago";
      if (d.toDays() > 30) return (d.toDays() / 30) + " months ago";
      if (d.toDays() > 0) return d.toDays() + " days ago";
      if (d.toHours() > 0) return d.toHours() + " hours ago";
      if (d.toMinutes() > 0) return d.toMinutes() + " minutes ago";
