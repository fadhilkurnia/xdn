...
/**
 * This demand profiler splits the Earth into multiple square grid cells, based on the latitude
 * and longitude. The number of rows and columns are specified by the `NUM_GRID_ROWS` and
 * `NUM_GRID_COLUMNS` constant. This profiler aggregate the number of requests coming from
 * each grid cells, as the information for RC to chose the most appropriate replica placement.
 * <p>
 * TODO: improve the implementation to consider different type of request.
 *    - number of coordinated requests
 *    - number of local-execution requests
 *    - record the request rate instead pf the total number of request
 */
public class XdnGeoDemandProfiler extends AbstractDemandProfile {
     ...
     * (i.e., y-axis) while Longitude indicates the horizontal location (i.e., x-axis). One degree
     * in Latitude and Longitude is roughly 111000 meters. Thus, if we want to capture 1x1 km grid,
     * we need to split earth surfaces into 40030 x 19980 matrix. Note that this is mainly true near
     * the equator.
     */
    ...
    public XdnGeoDemandProfiler(JSONObject stats) throws JSONException {
        ...
        if (rawData instanceof JSONArray data) {
            ...
            assert data.getJSONArray(0).length() == NUM_GRID_COLUMNS
        ...
        throw new RuntimeException("Unexpected type for the statistic data of " +
                rawData.getClass().getSimpleName() + ". Expecting int[][] or JSONArray.");
    }
    ...
    public void combine(AbstractDemandProfile update) {
        ...
        assert incomingProfiler.gridTotalRequests.length == NUM_GRID_ROWS :
                "Invalid grid dimension";
