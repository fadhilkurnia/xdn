...
/**
 * Demand profiler that splits Earth into a {@code NUM_GRID_ROWS × NUM_GRID_COLUMNS} lat/lon grid
 * and tracks per-cell read and write request counts separately.
 *
 * <h3>Hot path</h3>
 *
 * {@link #shouldReportDemandStats} classifies each request as a read (GET/HEAD) or write
 * (POST/PUT/PATCH/DELETE), enqueues its {@link Geolocation} with the flag onto a bounded queue, and
 * returns. All lat/lon → cell conversion happens on a background worker thread so the request path
 * stays minimal.
 *
 * <h3>Reporting</h3>
 *
 * After {@code MIN_DEMAND_REPORT_PERIOD_MS} (10 s) the profiler signals the AR to push demand stats
 * to the RC. {@link #getDemandStats} snapshots and resets both sparse grids, trims each to {@code
 * MAX_GRID_ENTRIES_PER_REPORT} top-traffic cells, and serialises them as two base64-encoded binary
 * blobs inside a {@link JSONObject}, staying within GigaPaxos's 4 KB {@code
 * MAX_DEMAND_PROFILE_SIZE} cap.
 *
 * <h3>Placement</h3>
 *
 * On the RC side, {@link #getNewActivesPlacement} reads the service's registered {@link
 * ConsistencyModel} to obtain the readType/writeType access strings, then delegates to whichever
 * {@link PlacementAlgorithm} is selected by the {@link #ALGORITHM} constant. Switch between {@code
 * CENTROID} and {@code GREEDY_KNOWN} by changing that one constant and recompiling.
 *
 * <h3>Consistency model registration</h3>
 *
 * Call {@link #registerConsistencyModel(String, ConsistencyModel)} at startup, before any traffic
 * arrives, to associate a service with its consistency model. Unregistered services default to
 * {@link ConsistencyModel#EVENTUAL}.
 */
public class XdnGeoDemandProfiler extends AbstractDemandProfile {
  ...
  /**
   * Approximate milliseconds per grid-unit of Euclidean distance. Calibrated against a known
   * real-world benchmark: US (Utah, ~40°N 112°W) to Europe (Amsterdam, ~52°N 4°E) RTT ≈ 100 ms,
   * which corresponds to ~329 grid units → 0.30 ms/unit. Override via system property {@code
   * xdn.placement.msPerUnit}.
   */
  ...
  // ---------------------------------------------------------------------------
  public enum PlacementAlgorithmType {
    CENTROID,
    GREEDY
  }
  ...
  // ---------------------------------------------------------------------------
  private static final int QUORUM_SIZE = Integer.getInteger("xdn.placement.quorumSize", 2);
   ...
   *
   * <p>Should be called at application startup, before the service receives traffic. Calling again
   * with the same {@code serviceName} replaces the previous model. Services that are never
   * registered default to {@link ConsistencyModel#EVENTUAL}.
   *
  ...
  public static void registerConsistencyModel(
    ...
    LOGGER.log(
        Level.INFO,
        "XdnGeoDemandProfiler registering {0} with {1}",
  ...
  private static ConsistencyModel toPlacementConsistencyModel(
    ...
    return switch (model) {
      case LINEARIZABILITY, LINEARIZABLE -> ConsistencyModel.LINEARIZABLE;
      case SEQUENTIAL, PRAM, CAUSAL -> ConsistencyModel.SEQUENTIAL;
      case MONOTONIC_READS,
          MONOTONIC_WRITES,
          READ_YOUR_WRITES,
          WRITES_FOLLOW_READS -> ConsistencyModel.PRIMARY_BACKUP;
      case EVENTUAL -> ConsistencyModel.EVENTUAL;
      default -> ConsistencyModel.SEQUENTIAL;
    };
  }
  ...
  public static void registerReconfigurationPolicy(
    ...
    if (minRequests != null) SERVICE_MIN_RECONFIG_REQUESTS.put(serviceName, minRequests);

    LOGGER.log(
        Level.INFO,
        "XdnGeoDemandProfiler reconfiguration policy for ''{0}'': "
  ...
  public XdnGeoDemandProfiler(String name) {
    ...
    LOGGER.log(
        Level.INFO,
        "XdnGeoDemandProfiler initialized: grid={0}x{1} algorithm={2}",
  ...
  public XdnGeoDemandProfiler(JSONObject stats) throws JSONException {
    ...
    if (stats.has("consistency_model")) {
      this.consistencyModel = ConsistencyModel.valueOf(stats.getString("consistency_model"));
    }
  }
  ...
  public JSONObject getDemandStats() {
    ...
    LOGGER.log(
        Level.INFO,
        "XdnGeoDemandProfiler [AR] Sending demand report for {0}: reads={1} cells, writes={2}"
            + " cells, totalReqs={3}",
        new Object[] {this.name, readEntries.size(), writeEntries.size(), numReqs});
    ...
    try {
      ...
      stats.put(
          "consistency_model",
          SERVICE_CONSISTENCY_MODELS.getOrDefault(this.name, ConsistencyModel.EVENTUAL).name());
      stats.put(
          "min_reconfig_interval_ms", SERVICE_MIN_RECONFIG_INTERVAL_MS.getOrDefault(this.name, 0L));
      stats.put("min_reconfig_requests", SERVICE_MIN_RECONFIG_REQUESTS.getOrDefault(this.name, 0L));
  ...
  public void combine(AbstractDemandProfile update) {
    ...
    LOGGER.log(
        Level.INFO,
        "XdnGeoDemandProfiler [RC] Received demand report for '{0}': +{1} reqs (total now={2})",
        new Object[] {this.name, incoming.totalRequests, this.totalRequests});
  }
  ...
  public NodeIdsMetadataPair<String> getNewActivesPlacement(
    ...
    LOGGER.log(
        Level.INFO,
        "XdnGeoDemandProfiler [{0}] algorithm={1} model={2} actives={3} coordinator={4}",
        new Object[] {
          this.name, ALGORITHM, model, result.nodeIds(), result.preferredCoordinator()
        });
  ...
  public void justReconfigured() {
    ...
    mapLock.lock();
    try {
      requestsSinceLastReconfiguration = 0;
    } finally {
      mapLock.unlock();
    }
  }
  ...
  private boolean enqueueIfGeo(XdnHttpRequest req) {
    ...
    boolean isWrite =
        behaviors.contains(RequestBehaviorType.WRITE_ONLY)
            || behaviors.contains(RequestBehaviorType.READ_MODIFY_WRITE);

    if (!eventQueue.offer(new GeolocatedRequest(geo, isWrite))) {
      LOGGER.log(
          Level.FINE, "XdnGeoDemandProfiler event queue full, dropping sample for {0}", this.name);
      return false;
    }
    return true;
  }
  ...
  // ---------------------------------------------------------------------------
  private static final Set<String> FORCE_RECONFIGURATION_ONCE = ConcurrentHashMap.newKeySet();
  ...
  private boolean shouldTriggerReconfiguration() {
    ...
    LOGGER.log(
        Level.INFO,
        "XdnGeoDemandProfiler [{0}] shouldTriggerReconfiguration: "
    ...
    if (hasTimeThreshold
        && System.currentTimeMillis() - lastReconfigurationTimestamp
            >= minReconfigurationIntervalMs) return true;

    if (hasRequestThreshold && totalRequests >= minRequestsForReconfiguration) return true;
  ...
  private JSONObject cellToGeoJson(int idx, int count) {
    ...
    try {
      return new JSONObject().put("lat", lat).put("lng", lng).put("count", count);
    } catch (JSONException e) {
      throw new RuntimeException(e);
    }
  }
