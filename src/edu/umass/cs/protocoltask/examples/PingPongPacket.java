...
public class PingPongPacket
    extends ProtocolPacket<
        Integer,
        PingPongPacket
            .PacketType> { // BasicPacket implements ProtocolEvent<Packet.PacketType,Long> {
  ...
  public Integer flip(int rcvr) { // flip sender/reciever and ping/pong type
    if (this.getType() == PacketType.TEST_PING) this.setType(PacketType.TEST_PONG);
    else if (this.getType() == PacketType.TEST_PONG) this.setType(PacketType.TEST_PING);
