...
/**
 * @author V. Arun
 *     <p>This example waits for numPings ping-pongs form any node in a specified set of nodes. Note
 *     that it does not wait for numPings ping-pongs from *all* nodes in the set.
 */
public class PingPongProtocolTask extends PingPongServer {
      ...
      switch (PingPongPacket.PacketType.intToType.get(JSONPacket.getPacketType(msg))) {
        ...
        default:
          throw new RuntimeException(
              "Unrecognizable message type: " + JSONPacket.getPacketType(msg));
      }
    } catch (JSONException je) {
      je.printStackTrace();
    }
  ...
  public MessagingTask[] start() {
    ...
    log.info(
        "Node" + myID + " starting protocol task with nodeIDs " + Util.arrayOfIntToString(nodes));
  ...
  private MessagingTask handlePong(PingPongPacket pong) {
    ...
    if (pong.getCounter() >= (this.numPings - 1)) ProtocolExecutor.cancel(this); // throws exception
