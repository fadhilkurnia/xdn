 ...
 * @param <NodeIDType>
 * @param <EventType>
 * @param <KeyType> The purpose of this class is to store ProtocolTasks and activate them when a
...
public class ProtocolExecutor<NodeIDType, EventType, KeyType> {
  ...
  private final MultiArrayMap<KeyType, ProtocolTaskWrapper<NodeIDType, EventType, KeyType>>
      protocolTasks =
          new MultiArrayMap<KeyType, ProtocolTaskWrapper<NodeIDType, EventType, KeyType>>(
              MAX_TASKS);
  private final HashMap<EventType, ProtocolTaskWrapper<NodeIDType, EventType, KeyType>>
      defaultTasks = new HashMap<EventType, ProtocolTaskWrapper<NodeIDType, EventType, KeyType>>();

  private static final Logger log = Logger.getLogger(ProtocolExecutor.class.getName());
  ...
  public ProtocolExecutor(Messenger<NodeIDType, ?> messenger) {
    ...
    this.executor.scheduleWithFixedDelay(
        new TooManyTasksWarner(), 0, TOO_MANY_TASKS_CHECK_PERIOD, TimeUnit.SECONDS);
  }
  ...
  public void register(EventType event, ProtocolTask<NodeIDType, EventType, KeyType> task) {
    this.defaultTasks.put(event, new ProtocolTaskWrapper<NodeIDType, EventType, KeyType>(task));
  ...
  public synchronized boolean spawnIfNotRunning(
    ...
    if (this.isRunning(actualTask.getKey())) {
      log.log(
          Level.FINE,
          "{0} unable to re-spawn already running task",
  ...
  private synchronized void insert(ProtocolTaskWrapper<NodeIDType, EventType, KeyType> task) {
    if (task.getKey() == null || this.isRunning(task.getKey())) {
      String errorMsg =
          myID
              + " trying to insert "
              + (task.getKey() == null ? "null" : "duplicate")
              + " key "
              + task.getKey();
    ...
    log.log(
        Level.FINE,
        "{0} inserting key {1} for task {2}",
        new Object[] {this, task.getKey(), task.task.getClass()});
  ...
  public synchronized void schedule(
    ...
    log.log(
        Level.FINE, "{0} scheduling {1} for periodic restarts", new Object[] {this, task.getKey()});
    task.setFuture(
        this.executor.scheduleWithFixedDelay(restarter, period, period, TimeUnit.MILLISECONDS));
  }
  ...
  private synchronized ProtocolTaskWrapper<NodeIDType, EventType, KeyType> retrieve(KeyType key) {
    return this.protocolTasks.get(key);
  }

  private void handleException(
      Exception e, ProtocolTaskWrapper<NodeIDType, EventType, KeyType> task) {
    if (!(e instanceof CancelProtocolTaskException)) {
      log.severe("Exception in protocol task " + task.getClass() + ":" + task.getKey());
      e.printStackTrace();
    }
    remove(task);
  }
  ...
  private synchronized ProtocolTask<?, ?, ?> remove(
    ...
    if (task.getFuture() != null) {
      log.log(
          Level.FINE, "{0} canceling protocol task {1}", new Object[] {this, task.task.getKey()});
  ...
  public boolean handleEvent(ProtocolEvent<EventType, KeyType> event) {
    ...
    if ((task = this.defaultTasks.get(event.getType())) != null) {
      log.fine(myID + " handling default event " + event.getType());
    } else if (event.getKey() != null && (task = this.retrieve(event.getKey())) != null) {
      log.fine(myID + " handling protocol task for " + event.getType() + ":" + event.getKey());
    } else if (event.getKey() == null) {
      log.warning("No default handler and null key for event " + event.getType() + " " + event);
    }
    ...
    try {
      if (ptasks[0] != null) spawn((ptasks[0]));
   ...
   * @param key
   */
  public static synchronized void enqueueCancel(Object key) {
    canceledKeys.add(key);
  }

  private static synchronized boolean removable(Object key) {
  ...
  public static void cancel(ProtocolTask<?, ?, ?> task) {
    if (task != null)
      throw new CancelProtocolTaskException(
          "Canceling task " + task.getClass() + " with key " + task.getKey());
  }
  ...
  private GenericMessagingTask<NodeIDType, ?>[] setKey(
    ...
    if (GenericMessagingTask.isEmpty(mtasks)) return mtasks;
    for (GenericMessagingTask<NodeIDType, ?> mtask : mtasks) {
      if (!mtask.isEmpty()) {
  ...
  private class TooManyTasksWarner implements Runnable {
    public void run() {
      if (size() > MAX_TASKS) {
        log.severe("Too many tasks (" + size() + ") scheduled with ProtocolExecutor");
      }
    }
  }
