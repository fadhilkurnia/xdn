 ...
 * @author V. Arun
 * @param <NodeIDType>
 * @param <EventType> This class is concretized to use integer node IDs, long keys, and JSON
 *     messages, but the PacketType is left generic. The reason is so that implementors can define
 *     and use their own packet types. Note that it is not possible to extend the enum
 *     Packet.PacketType and it is not modular to have to put *all* packet types in the Packet
 *     class.
 */
@SuppressWarnings("javadoc")
public abstract class ProtocolPacket<NodeIDType, EventType>
    implements ThresholdProtocolEvent<NodeIDType, EventType, String> {
  ...
  public ProtocolPacket(JSONObject json, Stringifiable<NodeIDType> unstringer)
    ...
    try {
      ...
      assert (!json.has(Keys.SENDER.toString())
          || (json.get(Keys.SENDER.toString()) != null
              && unstringer.valueOf(json.getString(Keys.SENDER.toString())) != null));
    } catch (Error e) {
      e.printStackTrace();
    }
    if (json.has(Keys.SENDER.toString())) {
      assert (((json.get(Keys.SENDER.toString()).toString())) != null);
      assert (unstringer.valueOf((json.get(Keys.SENDER.toString()).toString())) != null) : json;
    }

    this.sender =
        (json.has(Keys.SENDER.toString())
            ? unstringer.valueOf((json.get(Keys.SENDER.toString())).toString())
            : null);
    this.initiator =
        (json.has(Keys.INITIATOR.toString())
            ? unstringer.valueOf((json.get(Keys.INITIATOR.toString())).toString())
            : null);
    this.key = (json.has(Keys.KEY.toString()) ? json.getString(Keys.KEY.toString()) : null);
  }
  ...
  protected ProtocolPacket(JSONObject json) throws JSONException {
    this.type = getPacketType(json);
    this.sender =
        (json.has(Keys.SENDER.toString()) ? (NodeIDType) (json.get(Keys.SENDER.toString())) : null);
    this.initiator =
        (json.has(Keys.INITIATOR.toString())
            ? (NodeIDType) (json.get(Keys.INITIATOR.toString()))
            : null);
    this.key = (json.has(Keys.KEY.toString()) ? json.getString(Keys.KEY.toString()) : null);
  }
  ...
  public abstract EventType getPacketType(JSONObject json) throws JSONException;

  public abstract void putPacketType(JSONObject json, EventType type) throws JSONException;
