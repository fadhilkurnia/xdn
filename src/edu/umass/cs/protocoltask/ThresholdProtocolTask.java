 ...
 * @param <NodeIDType>
 * @param <EventType>
 * @param <KeyType> This abstract class is concretized for Long keys, int node IDs, and
 *     ProtocolPackets. Its purpose is to receive responses from a threshold number of nodes in the
 *     specified set. To enable this, instantiators of this abstract class must implement (1) a
 *     "boolean handleEvent(event)" method that says whether or not the response was valid, which
 *     helps ThresholdProtocolTask automatically stop retrying with nodes that have already
 *     responded; (2) a handleThresholdEvent(.) method that returns a protocol task to be executed
 *     when the threshold is reached (returning null automatically cancels the task as it is
 *     considered complete).
 */
public abstract class ThresholdProtocolTask<NodeIDType, EventType, KeyType>
  ...
  /**
   * Set of nodes out of which we expect to hear back from {@code threshold} number of nodes.
   *
   * @param nodes
   * @param threshold
   */
   ...
   *
   * @param ptasks
   * @return Returns messaging tasks to be performed when the threshold is reached. Additional
   *     protocol task actions may also be spawned via ptasks[0].
   */
  ...
  /**
   * This method automatically keeps track of senders from which we have already received valid
   * responses in order to keep track of whether the threshold has been reached, and if so, it
   * invokes {@link #handleThresholdEvent(ProtocolTask[]) handleThresholdEvent}
   */
  ...
  public final GenericMessagingTask<NodeIDType, ?>[] handleEvent(
    ...
    if (this.waitfor.getHeardCount() >= Math.min(this.threshold, this.waitfor.getMembers().size())
  ...
  protected GenericMessagingTask<NodeIDType, ?>[] fix(
    ...
    if (mtasks == null
        ...
        || mtasks[0] == null
        || mtasks[0].msgs == null
        || mtasks[0].msgs.length == 0
        || mtasks[0].msgs[0] == null) return mtasks;
  ...
  private GenericMessagingTask<NodeIDType, ?> fix(GenericMessagingTask<NodeIDType, ?> mtask) {
    if (mtask == null || mtask.msgs == null || mtask.msgs.length == 0) return null;
    return new GenericMessagingTask<NodeIDType, Object>(
        fix(this.waitfor.getMembersHeardFrom(), mtask.recipients), mtask.msgs);
  }

  /**
   * @param restartPeriod The period after which restart messages will be sent (only to nodes from
   *     which valid responses have not yet been received).
   */
  ...
  private Object[] fix(Set<NodeIDType> filter, Object[] original) {
    if (filter == null || filter.size() == 0) return original;
    ...
    for (Object obji : original) {
      ...
      for (NodeIDType objj : filter) {
        if (i.equals(objj)) toFilter = true;
      }
      if (!toFilter) filtered.add(i);
    }
