...
/**
 * AwReplicaCoordinator implements a protocol providing sequential consistency inspired from the
 * paper "Sequential Consistency versus Linearizability" by Hagit Attiya and Jennifer L Welch. AW is
 * the initial of the authors' last name. Specifically, the protocol refers to the read-write object
 * that does local execution for read-only operations, and uses MultiPaxos as the Atomic Broadcast
 * (ABC) for write-only (and read-modify-write) operations.
 */
  ...
  public boolean coordinateRequest(Request request, ExecutedCallback callback)
    ...
    if (!(request instanceof ReplicableClientRequest rcr)) {
      throw new RuntimeException(
          "Unknown request/packet handled by AwReplicaCoordinator " + request.getRequestType());
    }
    ...
    if (logger.isLoggable(Level.FINE)) {
      ...
      logger.log(
          Level.FINE,
          "{0}:AwReplicaCoordinator -- coordinateRequest {1}",
    ...
    ExecutedCallback instrumentedCallback =
        (response, handled) -> {
