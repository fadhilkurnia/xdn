 ...
 * implemented based on <a href="https://www.cs.princeton.edu/techreports/1988/180.pdf">this
 * paper</a>.
 *
 * @param <NodeIDType>
 */
public class PramReplicaCoordinator<NodeIDType> extends AbstractReplicaCoordinator<NodeIDType> {
  ...
  public PramReplicaCoordinator(
    ...
    this.messenger.precedePacketDemultiplexer(new PramPacketDemultiplexer(this, app));
  }
    ...
    if (request instanceof ReplicableClientRequest rcr
      ...
      if (isReadOnly) {
        ...
        throw new RuntimeException(
            "PramReplicaCoordinator can only handle " + "WriteOnlyRequest or ReadOnlyRequest");
      }
  ...
  private boolean handlePramPacket(PramPacket packet, ExecutedCallback callback) {
    ...
    if (packet instanceof PramWritePacket p) {
      ...
      assert this.currentInstances.containsKey(serviceName) : "Unknown service name " + serviceName;
      ...
      try {
        if (logger.isLoggable(Level.FINER)) {
          logger.log(
              Level.FINER,
              String.format(
                  "%s:%s - sending WRITE_AFTER packet of %s",
                  myNodeID,
                  PramReplicaCoordinator.class.getSimpleName(),
                  writeRequest.getClass().getSimpleName()));
        }
    ...
    if (packet instanceof PramWriteAfterPacket p) {
      if (logger.isLoggable(Level.FINER)) {
        logger.log(
            Level.FINER,
            String.format(
                "%s:%s - handling write after packet %s name=%s sender=%s",
                myNodeID,
                PramReplicaCoordinator.class.getSimpleName(),
   ...
   *
   * <p>Only one executePool task runs per sender at a time (guarded by the AtomicBoolean flag). The
   * task polls and executes packets one by one, then releases the flag. A re-check after releasing
   * prevents lost packets from the add-then-CAS race window.
   */
  private void drainSenderQueue(
      ConcurrentLinkedQueue<PramWriteAfterPacket> queue, AtomicBoolean processing) {
    executePool.execute(
        () -> {
          try {
  ...
  public boolean createReplicaGroup(
    ...
    if (logger.isLoggable(Level.INFO)) {
      logger.log(
          Level.INFO,
          String.format(
              "%s:%s - creating replica group name=%s nodes=%s epoch=%d state=%s",
              myNodeID,
              PramReplicaCoordinator.class.getSimpleName(),
              serviceName,
              nodes,
    ...
    PramInstance<NodeIDType> pramInstance =
        new PramInstance<>(
            serviceName, epoch, state, nodes, new ConcurrentHashMap<>(), new ConcurrentHashMap<>());
