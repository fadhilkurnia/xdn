...
/**
 * PramPacketDemultiplexer requires AppRequestParser because the WriteAfterPacket must encapsulate
 * ClientRequest.
 *
 * <p>Deserialization order for each Demultiplexer: 1. processHeader(messageBytes, header) // given
 * bytes, quickly return null or Packet 2. getPacketType(message) 3. matchesType(message) 4.
 * handleMessage(message) Therefore,
 */
public class PramPacketDemultiplexer extends AbstractPacketDemultiplexer<PramPacket> {
  ...
  public PramPacketDemultiplexer(
      PramReplicaCoordinator<?> coordinator, AppRequestParser appRequestDeserializer) {
  ...
  protected PramPacket processHeader(byte[] message, NIOHeader header) {
    PramPacketType packetType = PramPacket.getQuickPacketTypeFromEncodedPacket(message);
    if (packetType == null) {
      return null;
    }
