...
public class PramWriteAfterPacket extends PramPacket implements Byteable {
  ...
  private PramWriteAfterPacket(
      long requestID, String senderID, ClientRequest clientWriteOnlyRequest) {
  ...
  public byte[] toBytes() {
    ...
    int payloadSize =
        CodedOutputStream.computeInt64Size(1, this.requestID)
  ...
  public static PramWriteAfterPacket createFromBytes(
    ...
    try {
      ...
      Logger.getGlobal()
          .log(
              Level.SEVERE, "Receiving an invalid encoded PramWriteAfterPacket: " + e.getMessage());
      return null;
    }

    if (senderId == null || encodedRequest == null) {
      Logger.getGlobal()
          .log(Level.SEVERE, "Receiving an invalid encoded PramWriteAfterPacket: missing fields");
      return null;
    }
