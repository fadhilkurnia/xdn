...
public class PramWritePacket extends PramPacket implements Byteable {
  ...
  private PramWritePacket(long requestID, ClientRequest writeOnlyRequest) {
    ...
    assert (writeOnlyRequest instanceof BehavioralRequest br && br.isWriteOnlyRequest())
        : "The provided request must be a WriteOnlyRequest, yet "
            + writeOnlyRequest.getClass().getSimpleName()
            + " is given.";
  ...
  public static PramWritePacket createFromBytes(
    ...
    try {
      ...
      Logger.getGlobal()
          .log(Level.SEVERE, "Receiving an invalid encoded PramWritePacket: " + e.getMessage());
      return null;
    }

    if (encodedRequest == null) {
      Logger.getGlobal()
          .log(Level.SEVERE, "Receiving an invalid encoded PramWritePacket: missing request");
      return null;
    }
