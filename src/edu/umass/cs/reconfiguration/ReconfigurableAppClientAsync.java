 ...
 * @param <V>
 */
public abstract class ReconfigurableAppClientAsync<V>
    implements GigaPaxosClient<V>, AppRequestParser {
  static {
  ...
  /**
   * The default timeout for {@link ClientRequest} as well as {@link ClientReconfigurationPacket}
   * requests.
   */
  ...
  final GCConcurrentHashMapCallback defaultGCCallback =
      new GCConcurrentHashMapCallback() {
        @Override
        public void callbackGC(Object key, Object value) {
          log.log(Level.INFO, "{0} timing out {1}:{2}", new Object[] {this, key, value});
        }
      };

  /**
   * Used to slowly forget replicas that cause a timeout. Using a moving average learner in {@link
   * #e2eRedirector} means that a single timeout will not necessarily immediately exclude that
   * replica. Furthermore, because of probing, some requests may still go to such "long latency"
   * replicas that have actually failed. Note that excluding such replicas for a limited time
   * doesn't prevent them from being probed again after that time. Unless we do active probing, we
   * have to either direct some requests to failed replicas or be reconciled to not ever using them
   * again even after they recover.
   */
  final GCConcurrentHashMapCallback appGCCallback =
      new GCConcurrentHashMapCallback() {
        ...
        public void callbackGC(Object key, Object value) {
          ...
          ReconfigurableAppClientAsync.this.e2eRedirector.learnSample(
              ...
              System.currentTimeMillis() - ((RequestAndCallback) value).sentTime);
          ReconfigurableAppClientAsync.this.mostRecentlyWrittenMap.remove(
              ((RequestAndCallback) value).request.getServiceName());
          log.log(
              Level.INFO,
              "{0} timing out {1}:{2}",
              new Object[] {this, key + "", ((RequestAndCallback) value).request.getSummary()});
        }
      };

  final GCConcurrentHashMapCallback crpGCCallback =
      new GCConcurrentHashMapCallback() {
        @Override
        public void callbackGC(Object key, Object value) {
          ...
          ReconfigurableAppClientAsync.this.mostRecentlyCreatedMap.remove(
              callback.request.getServiceName());
          // forget failed reconfigurators
          ReconfigurableAppClientAsync.this.e2eRedirector.learnSample(
              callback.serverSentTo, System.currentTimeMillis() - callback.sentTime);
          log.log(Level.INFO, "{0} timing out {1}:{2}", new Object[] {this, key, value});
        }
      };
  ...
  private final GCConcurrentHashMap<Long, Callback<Request, V>> callbacks =
      new GCConcurrentHashMap<Long, Callback<Request, V>>(
          appGCCallback, getAppRequestTimeout(DEFAULT_GC_TIMEOUT));
  private final GCConcurrentHashMap<Long, Callback<Request, V>> callbacksLongTimeout =
      new GCConcurrentHashMap<Long, Callback<Request, V>>(
          defaultGCCallback, getAppRequestLongTimeout(DEFAULT_GC_LONG_TIMEOUT));

  // client reconfiguration packet callbacks
  private final GCConcurrentHashMap<String, Callback<Request, Request>> callbacksCRP =
      new GCConcurrentHashMap<String, Callback<Request, Request>>(
          crpGCCallback, getCRPTimeout(DEFAULT_GC_TIMEOUT));
  private final GCConcurrentHashMap<String, Callback<Request, Request>> callbacksCRPLongTimeout =
      new GCConcurrentHashMap<String, Callback<Request, Request>>(
          crpGCCallback, getCRPTimeout(DEFAULT_GC_TIMEOUT));

  // server reconfiguration packet callbacks,
  private final GCConcurrentHashMap<String, RequestCallback> callbacksSRP =
      new GCConcurrentHashMap<String, RequestCallback>(crpGCCallback, SRP_GC_TIMEOUT);

  // name->actives map
  private final GCConcurrentHashMap<String, ActivesInfo> activeReplicas =
      new GCConcurrentHashMap<String, ActivesInfo>(defaultGCCallback, MIN_REQUEST_ACTIVES_INTERVAL);
  // name->unsent app requests for which active replicas are not yet known
  private final GCConcurrentHashMap<String, LinkedBlockingQueue<RequestAndCallback>>
      requestsPendingActives =
          new GCConcurrentHashMap<String, LinkedBlockingQueue<RequestAndCallback>>(
   ...
   *
   * <p>This map is used to implement {@link PaxosConfig.PC#READ_YOUR_WRITES} by sending a request
   * to the active replica from which a response to the most recent coordinated request was
   * received.
   *
   * <p>It is important to forget this entry when an app request times out, otherwise we may lose
   * liveness even if a single active replica crashes.
   */
  private final GCConcurrentHashMap<String, InetSocketAddress> mostRecentlyWrittenMap =
      new GCConcurrentHashMap<String, InetSocketAddress>(
          defaultGCCallback, MAX_COORDINATION_LATENCY);

  /**
   * name->last reconfigurator from which a create confirmation was received.
   *
   * <p>Sending ClientReconfigurationPackets to a reconfigurator that confirmed a creation implies
   * that a request for active replicas immediately after will necessarily succeed under graceful
   * conditions; sending to a random replica however may not satisfy this property.
   *
   * <p>Sending ClientReconfigurationPackets to the same reconfigurator repeatedly has the downside
   * of being less agile to reconfigurator failures, so it is important that this entry be removed
   * when a ClientReconfigurationPacket times out in callbacksCRP, otherwise we may lose liveness
   * even if a single reconfigurator crashes.
   */
  @SuppressWarnings("serial")
  // never serialized
  private final LinkedHashMap<String, InetSocketAddress> mostRecentlyCreatedMap =
      new LinkedHashMap<String, InetSocketAddress>() {
  ...
  private static final Logger log =
      Logger.getLogger(ReconfigurableAppClientAsync.class.getName()); // Reconfigurator.getLogger();
  ...
  /**
   * The constructor specifies the default set of reconfigurators. This set may change over time, so
   * it is the caller's responsibility to ensure that this set remains up-to-date. Some staleness
   * however can be tolerated as reconfigurators will by design forward a request to the responsible
   * reconfigurator if they are not responsible.
   *
   * @param reconfigurators
   * @param sslMode
   * @param clientPortOffset
   * @param checkConnectivity If true, a connectivity check with at least one reconfigurator will be
   *     enforced and an exception will be thrown if unsuccessful.
   * @throws IOException
   */
  public ReconfigurableAppClientAsync(
      Set<InetSocketAddress> reconfigurators,
    ...
    // set up transport
    (this.niot =
            (new MessageNIOTransport<String, Object>(
                null, null, (new ClientPacketDemultiplexer(getRequestTypes())), true, sslMode)))
        .setName(this.toString());
    if (this.sslMode != ReconfigurationConfig.getServerSSLMode()
        && (!this.getMutualAuthTypes().isEmpty()
      ...
      (this.niotMA =
          ...
          .setName(this.toString());
    else this.niotMA = null;

    log.log(
        Level.INFO,
        "{0} listening on {1}; ssl mode = {2}; client port offset = {3}",
        new Object[] {this, niot.getListeningSocketAddress(), this.sslMode, clientPortOffset});
    this.clientFacingReconfigurators =
        (PaxosConfig.offsetSocketAddresses(
            this.reconfigurators = reconfigurators, clientPortOffset));

    log.log(
        Level.FINE,
        "{0} reconfigurators={1}",
        new Object[] {this, (this.clientFacingReconfigurators)});
    this.e2eRedirector = new E2ELatencyAwareRedirector(this.niot.getListeningSocketAddress());

    if (checkConnectivity) this.checkConnectivity();
  }
   ...
   * @throws IOException
   */
  public ReconfigurableAppClientAsync(Set<InetSocketAddress> reconfigurators) throws IOException {
    this(
        reconfigurators,
   ...
   */
  public ReconfigurableAppClientAsync(
      Set<InetSocketAddress> reconfigurators, boolean checkConnectivity) throws IOException {
    this(
        reconfigurators,
        ReconfigurationConfig.getClientSSLMode(),
        (ReconfigurationConfig.getClientPortOffset()),
  ...
  class ClientPacketDemultiplexer extends AbstractPacketDemultiplexer<Request> {
    ...
    // TODO: unused, remove
    final BasicReconfigurationPacket<?> parseAsReconfigurationPacket(Object strMsg) {
      if (strMsg instanceof BasicReconfigurationPacket<?>)
        return (BasicReconfigurationPacket<?>) strMsg;

      if (strMsg instanceof ReplicableClientRequest) return null;

      if (!(strMsg instanceof JSONObject)) return null;

      BasicReconfigurationPacket<?> rcPacket =
          ReconfigurationPacket.getReconfigurationPacketSuppressExceptions(
              (JSONObject) strMsg, unstringer);

      if (rcPacket != null
          && (rcPacket =
                  (rcPacket instanceof ClientReconfigurationPacket
                          || rcPacket instanceof ServerReconfigurationPacket
                          || rcPacket instanceof EchoRequest
                      ? rcPacket
                      : null))
              == null)
        ReconfigurableAppClientAsync.log.log(
            Level.WARNING,
            "{0} dropping an undecodeable reconfiguration packet {1}",
            new Object[] {ReconfigurableAppClientAsync.this, strMsg});
    ...
    final Request parseAsAppRequest(Object msg) {
      ...
      try {
        request =
            ReconfigurableAppClientAsync.this.jsonPackets && msg instanceof JSONObject
                ? getRequestFromJSON((JSONObject) msg)
                : getRequest(
                    msg instanceof String
                        ...
                        : ((JSONObject) msg).getString(Keys.INCOMING_STRING.toString()));
      } catch (RequestParseException | JSONException e) {
        // should never happen unless app has bugs
        ReconfigurableAppClientAsync.log.log(
            Level.WARNING,
            "{0} received an unrecognizable message that can not be decoded as an application"
                + " packet: {1}",
            new Object[] {this, msg});
        e.printStackTrace();
      }
    ...
    /**
     * The main demultiplexing method for responses. It invokes the app request callback upon
     * getting app responses or internally processes ReconfigurationPacket packets. It includes
     * support for retransmitting active replicas requests upon a failure or retransmitting app
     * requests upon an active replica error. Such retransmission should make it unlikely but not
     * impossible for errors because of laggard replicas.
     *
     * @param request
     * @param header
    ...
    public boolean handleMessage(Request request, NIOHeader header) {
      ...
      if (response != null) {
        ...
        if ((response instanceof ClientRequest)
            && ((callback =
                        ReconfigurableAppClientAsync.this.callbacks.remove(
                            ((ClientRequest) response).getRequestID()))
                    != null
                || (callback =
                        ReconfigurableAppClientAsync.this.callbacksLongTimeout.remove(
                            ((ClientRequest) response).getRequestID()))
                    != null)) {

          if (!(response instanceof EchoRequest))
            AppInstrumenter.recvdResponse(((RequestAndCallback) callback).request);

          callback.processResponse(
              response instanceof ReplicableClientRequest
                  ? ((ReplicableClientRequest) response).getRequest()
          ...
          ReconfigurableAppClientAsync.log.log(
              ...
              new Object[] {ReconfigurableAppClientAsync.this, response.getSummary()});
        }
        // ActiveReplicaError has to be dealt with separately
        else if ((response instanceof ActiveReplicaError)
            && (callback = callbacks.get(((ActiveReplicaError) response).getRequestID())) != null
            && callback instanceof ReconfigurableAppClientAsync.RequestAndCallback) {
          ActivesInfo activesInfo =
              ReconfigurableAppClientAsync.this.activeReplicas.get(response.getServiceName());
          if (activesInfo != null
              && activesInfo.actives != null
              && ((RequestAndCallback) callback).incrActiveReplicaErrors()
                  < activesInfo.actives.size() / 2 + 1)
            try {
              InetSocketAddress untried =
                  ((RequestAndCallback) callback).getUntried(activesInfo.actives);
              ReconfigurableAppClientAsync.log.log(
                  Level.INFO,
                  ...
                  new Object[] {
                    ...
                    response.getSummary(),
                    untried,
                    ((RequestAndCallback) callback).activeReplicaErrors
                  });
              // retry with a random other active replica
              ReconfigurableAppClientAsync.this.sendRequest(
                  ((RequestAndCallback) callback).request,
                  untried,
                  ((RequestAndCallback) callback).callback);
            } catch (IOException e) {
              e.printStackTrace();
              ReconfigurableAppClientAsync.this.cleanupActiveReplicasInfo(
                  (RequestAndCallback) callback);
            }
          else {
            callbacks.remove(((ActiveReplicaError) response).getRequestID());
            ReconfigurableAppClientAsync.this.cleanupActiveReplicasInfo(
                (RequestAndCallback) callback);
          ...
          ReconfigurableAppClientAsync.log.log(
              ...
              new Object[] {
                this, response.getSummary(), ((ClientReconfigurationPacket) response).getSender()
              });
          // call create/delete app callback
          if ((callbackCRP =
                      ReconfigurableAppClientAsync.this.callbacksCRP.remove(
                          getKey((ClientReconfigurationPacket) response)))
                  != null
              || (callbackCRP =
                      ReconfigurableAppClientAsync.this.callbacksCRPLongTimeout.remove(
                          getKey((ClientReconfigurationPacket) response)))
                  != null) callbackCRP.processResponse(response);
          else
          ...
          if (response instanceof DeleteServiceName && !((DeleteServiceName) response).isFailed())
            ReconfigurableAppClientAsync.this.activeReplicas.remove(response.getServiceName());

          // if RequestActiveReplicas, send or unpend pending requests
          if (response instanceof RequestActiveReplicas)
            ReconfigurableAppClientAsync.this.sendRequestsPendingActives(
          ...
          if (response instanceof CreateServiceName && !((CreateServiceName) response).isFailed()) {
            ...
            ReconfigurableAppClientAsync.this.mostRecentlyCreatedMap.put(
                response.getServiceName(),
                isa =
                    ((ClientReconfigurationPacket) response).getForwader() == null
                            || !USE_FORWARDEE_INFO
                        ?
                        // initiating reconfigurator
                        ((ClientReconfigurationPacket) response).getSender()
                        :
                        // forwarder uses default port
                        Util.getOffsettedAddress(
                            ((CreateServiceName) response).getForwardee(), -getServerPortOffset()));
            ReconfigurableAppClientAsync.log.log(
                Level.FINER,
                "{0} inserted {1}:{2} into mostRecentlyCreatedMap",
                new Object[] {this, response.getServiceName(), isa});
          }
        } else if (response instanceof ServerReconfigurationPacket<?>) {
          if ((callbackCRP =
                  ReconfigurableAppClientAsync.this.callbacksSRP.remove(
                      getKey((ServerReconfigurationPacket<?>) response)))
              != null) {
            callbackCRP.processResponse(response);
          }
        } else if (response instanceof EchoRequest) {
          if ((callback =
                  ReconfigurableAppClientAsync.this.callbacks.remove((EchoRequest) response))
              != null) {
            callback.processResponse(response);
          }
        }
    ...
    /**
     * We simply return a constant integer corresponding to either a ClientReconfigurationPacket or
     * app request here as this method is only used to confirm the demultiplexer, which is needed
     * only in the case of multiple chained demultiplexers, but we only have one demultiplexer in
     * this simple client, so it suffices to return any registered packet type.
     *
     * @param strMsg
     * @return
     */
    ...
    protected Request processHeader(byte[] bytes, NIOHeader header) {
      ReconfigurableAppClientAsync.log.log(
          Level.FINEST, "{0} received message from {1}", new Object[] {this, header.sndr});
      String message = null;
      try {
        Integer type = null;
        ...
        if (BYTEIFICATION
            ...
            && ReconfigurationPacket.PacketType.intToType.containsKey(
                type = ByteBuffer.wrap(bytes, 0, 4).getInt())) {
          // typical reconfiguration protocol packet
          if (type != ReconfigurationPacket.PacketType.REPLICABLE_CLIENT_REQUEST.getInt())
            if (JSONPacket.couldBeJSON(
                message = MessageExtractor.decode(bytes, 4, bytes.length - 4)))
              return ReconfigurationPacket.getReconfigurationPacketSuppressExceptions(
                  MessageExtractor.stampAddressIntoJSONObject(
                      header.sndr, header.rcvr, new JSONObject(message)),
                  unstringer);
            // wrapped app packet, type == REPLICABLE_CLIENT_REQUEST
              ...
              return ReconfigurableAppClientAsync.this instanceof AppRequestParserBytes
                  ...
                  new ReplicableClientRequest(
                      bytes, header, (AppRequestParserBytes) ReconfigurableAppClientAsync.this)
                  // from string
                  : new ReplicableClientRequest(
                      bytes, (AppRequestParser) ReconfigurableAppClientAsync.this);

        }

        // old JSON option for reconfiguration packets
        else if (!BYTEIFICATION
            && JSONPacket.couldBeJSON(bytes)
        ...
        // byte-parseable app packet
          ...
          try {
            ...
            log.log(
                Level.SEVERE,
                "{0} received unparseable packet {1}:{2}",
                new Object[] {ReconfigurableAppClientAsync.this, header, new Stringer(bytes)});
            throw e;
          }
        }
        ...
        // JSON-stampable app packet
            ...
            && (message = MessageExtractor.decode(bytes)) != null)
          return getRequestFromJSON(
              MessageExtractor.stampAddressIntoJSONObject(
                  header.sndr, header.rcvr, new JSONObject(message)));

        // default stringified request
        else return getRequest(new String(bytes, MessageNIOTransport.NIO_CHARSET_ENCODING));
      } catch (Exception je) {
  ...
  private void cleanupActiveReplicasInfo(RequestAndCallback callback) {
    ReconfigurableAppClientAsync.this.activeReplicas.remove(callback.request.getServiceName());
    ReconfigurableAppClientAsync.this.mostRecentlyWrittenMap.remove(
        callback.request.getServiceName());
  }
  ...
  /**
   * This method will convert an app request to a {@link ReplicableClientRequest} and then send the
   * request.
   *
   * @param request
   * @param callback
  ...
  public RequestFuture<V> sendRequest(Request request, Callback<Request, V> callback)
    ...
    return (request instanceof ClientRequest
        ? this.sendRequest((ClientRequest) request, callback)
        : this.sendRequest(ReplicableClientRequest.wrap(request), callback));
  }

  // TODO: true hasn't been tested rigorously
  private static final boolean ENABLE_ID_TRANSFORM =
      Config.getGlobalBoolean(RC.ENABLE_ID_TRANSFORM);
   ...
   * @throws IOException
   */
  public Request sendRequest(ClientRequest request, InetSocketAddress server, long timeout)
      throws IOException {
    BlockingRequestCallback callback;
   ...
   * @throws IOException
   */
  public RequestFuture<V> sendRequest(
      ClientRequest request, InetSocketAddress server, Callback<Request, V> callback)
      throws IOException {
    ...
    if (this.sendToServerPort(request)) server = Util.offsetPort(server, getServerPortOffset());

    // use replica recently written to if any
    InetSocketAddress mostRecentlyWritten =
        READ_YOUR_WRITES ? this.mostRecentlyWrittenMap.get(request.getServiceName()) : null;
    ...
    if (!(request instanceof EchoRequest))
      AppInstrumenter.outstandingAppRequest(this.numOutstandingAppRequests(), request);

    RequestCallbackFuture<V> future =
        callback instanceof RequestCallbackFuture
            ? (RequestCallbackFuture<V>) callback
            : new RequestCallbackFuture<V>(request, callback);

    try {
      Map<Long, Callback<Request, V>> correctMap =
          (!hasLongTimeout(callback) ? this.callbacks : this.callbacksLongTimeout);
      RequestAndCallback requestAndCallback =
          new RequestAndCallback(request, future).setServerSentTo(server);
      // transforms all requests to a unique ID unless retransmission
      while ((prev = (correctMap).putIfAbsent(request.getRequestID(), requestAndCallback)) != null
          && !((RequestAndCallback) prev).request.equals(request))
        if (ENABLE_ID_TRANSFORM)
          request = ReplicableClientRequest.wrap(request, (long) (Math.random() * Long.MAX_VALUE));
        else
      ...
      if (prev != null
        ...
        (correctMap).put(request.getRequestID(), requestAndCallback);

      // special case for long timeout tasks
      if (hasLongTimeout(callback) && prev == null)
        this.spawnGCClientRequest(((TimeoutRequestCallback) callback).getTimeout(), request);

      sendFailed = this.getNIO(request).sendToAddress(server, request) <= 0;
      Level level = request instanceof EchoRequest ? Level.FINE : Level.FINE;
      log.log(
          level,
          ...
          new Object[] {
            ...
            ReconfigurationConfig.getSummary(request, log.isLoggable(level)),
            server
          });
      if (!sendFailed && !(request instanceof EchoRequest)) AppInstrumenter.sentRequest(request);
    } finally {
      if (sendFailed && prev == null) {
        this.callbacks.remove(request.getRequestID(), callback);
        this.callbacksLongTimeout.remove(request.getRequestID(), callback);
   ...
   * @param type
   * @param name
   * @param initialState Used only if type is {@link
   *     edu.umass.cs.reconfiguration.reconfigurationpackets.ReconfigurationPacket.PacketType#CREATE_SERVICE_NAME}
   *     .
   * @param callback
   * @throws IOException
   */
  ...
  /**
   * Server reconfiguration packets have to be privileged, so we authenticate them implicitly by
   * listening for them only on the server-server MUTUAL_AUTH port. So we subtract the offset below
   * accordingly.
   *
   * @param rcPacket
   * @param callback
   * @throws IOException
   */
  public void sendRequest(ServerReconfigurationPacket<?> rcPacket, RequestCallback callback)
      throws IOException {
    if (this.numOutStandingCRPs() > getMaxOutstandingCRPRequests())
      throw new IOException("Too many outstanding requests");

    InetSocketAddress isa =
        getRandom(this.clientFacingReconfigurators.toArray(new InetSocketAddress[0]));
    // overwrite the most recent callback
    this.callbacksSRP.put(getKey(rcPacket), callback);
    this.niot.sendToAddress(
        Util.offsetPort(
            isa,
  ...
  public RequestFuture<ClientReconfigurationPacket> sendRequest(ClientReconfigurationPacket request)
    ...
    return this.sendRequest(request, defaultCRPCallback);
  }
  ...
  private static RequestFuture<ClientReconfigurationPacket> toRequestFutureCRP(
    ...
    return new RequestFuture<ClientReconfigurationPacket>() {
      ...
      @Override
      public ClientReconfigurationPacket get() throws InterruptedException, ExecutionException {
  ...
  private static ClientReconfigurationPacket processCRPIfException(ClientReconfigurationPacket crp)
  ...
  private static ClientReconfigurationPacket processCRPResponse(ClientReconfigurationPacket crp)
      throws ReconfigurationException {
    if (crp.isFailed())
      throw new ReconfigurationException(crp.getResponseCode(), crp.getResponseMessage());
    return crp;
  }
   ...
   * @throws IOException
   */
  public ClientReconfigurationPacket sendRequest(ClientReconfigurationPacket request, long timeout)
      throws IOException, ReconfigurationException {
    RequestFuture<ClientReconfigurationPacket> future =
        this.sendRequest(request, defaultCRPCallback);
    try {
  ...
  public RequestFuture<ClientReconfigurationPacket> sendRequest(
      ClientReconfigurationPacket request, RequestCallback callback) throws IOException {
    InetSocketAddress server = null;
    return this.sendRequest(
        request,
        // to avoid lagging reconfigurators
        (server = this.mostRecentlyCreatedMap.get(request.getServiceName())) != null
            ? server
            // could also select random reconfigurator here
            : this.e2eRedirector.getNearest(
                this.sendToServerPort(request)
                    ? this.reconfigurators
                    : this.clientFacingReconfigurators),
        callback);
  }

  private void sendRequesNullCallback(ClientReconfigurationPacket request) throws IOException {
  ...
  private void initTimerIfNeeded() {
    synchronized (this) {
      if (this.timer == null) this.timer = new Timer(this.toString(), true);
    }
  }

  private void spawnGCClientReconfigurationPacket(
      long timeout, ClientReconfigurationPacket request) {
    initTimerIfNeeded();
    this.timer.schedule(
        new TimerTask() {
          ...
          @Override
          public void run() {
            ReconfigurableAppClientAsync.this.callbacksCRPLongTimeout.remove(request.getKey());
          }
        },
        timeout);
  }
  ...
  private void spawnGCClientRequest(long timeout, ClientRequest request) {
    ...
    this.timer.schedule(
        new TimerTask() {
          ...
          @Override
          public void run() {
            ReconfigurableAppClientAsync.this.callbacksLongTimeout.remove(request.getRequestID());
          }
        },
        timeout);
  }
  ...
  private RequestFuture<ClientReconfigurationPacket> sendRequest(
      ...
      throws IOException {
    if (callback == null) callback = defaultCRPCallback;
    RequestCallbackFuture<Request> future = new RequestCallbackFuture<Request>(request, callback);
    if ((callback = new CRPRequestCallback(request, future, reconfigurator)) != null)
      if (!hasLongTimeout(callback)) this.callbacksCRP.put(getKey(request), callback);
      else if (this.callbacksCRPLongTimeout.put(getKey(request), callback) == null)
        spawnGCClientReconfigurationPacket(
            ((TimeoutRequestCallback) callback).getTimeout(), request);

    log.log(
        Level.FINER,
        "{0} sending QQ request {1} to {2}",
        new Object[] {this, request.getSummary(), reconfigurator});
    this.sendRequest(
        request,
        reconfigurator != null
            ? reconfigurator
  ...
  private boolean sendRequest(ClientReconfigurationPacket request, InetSocketAddress reconfigurator)
      throws IOException {
    log.log(
        Level.FINER,
        "{0} sending request {1} to {2}",
        new Object[] {this, request.getSummary(), reconfigurator});
  ...
  private boolean isMutualAuth(Request request) {
    return (request instanceof ClientRequest
        && this.getMutualAuthTypes().contains(request.getRequestType()));
  }

  private InetSocketAddress getRandom(InetSocketAddress[] isas) {
    return isas != null && isas.length > 0 ? isas[(int) (Math.random() * isas.length)] : null;
  }
  ...
  private String getKey(ServerReconfigurationPacket<?> changeRCs) {
    return changeRCs.getRequestType()
        + ":"
        + (changeRCs.hasAddedNodes() ? changeRCs.newlyAddedNodes.keySet() : "")
        + ":"
  ...
  class RequestAndCallback extends RequestCallbackFuture<V> implements Callback<Request, V> {
    ...
    RequestAndCallback(
        ClientRequest request, Callback<Request, V> callback, NearestServerSelector redirector) {
    ...
    public String toString() {
      return this.request.getSummary()
          ...
          + this.serverSentTo
          + " "
          + (System.currentTimeMillis() - this.sentTime)
          + "ms back||"
          + this.callback;
    }
    ...
    private InetSocketAddress getUntried(Set<InetSocketAddress> actives) {
      ...
      for (InetSocketAddress isa : actives) if (!this.tried.contains(isa)) return isa;
      return null;
    }
  }
  ...
  private static final String ALL_ACTIVES =
      Config.getGlobalString(ReconfigurationConfig.RC.BROADCAST_NAME);
   ...
   * @throws IOException
   */
  public Request sendRequestAnycast(ClientRequest request, long timeout) throws IOException {
    BlockingRequestCallback callback;
    this.sendRequestAnycast(request, callback = new BlockingRequestCallback(timeout));
    return callback.getResponse(); // blocking
  }
   ...
   * @throws IOException
   */
  public RequestFuture<V> sendRequestAnycast(ClientRequest request, Callback<Request, V> callback)
      throws IOException {
    ...
    synchronized (this.activeReplicas) {
      if ((activesInfo = this.activeReplicas.get(ALL_ACTIVES)) != null
        ...
        return this.sendRequest(request, (InetSocketAddress) Util.selectRandom(actives), callback);
      // else
      RequestCallbackFuture<V> future;
  ...
  private boolean queriedActivesRecently(String name) {
    ...
    if ((activesInfo = this.activeReplicas.get(name)) != null
        ...
        && ((activesInfo.actives == null
                && System.currentTimeMillis() - lastQueriedTime
                    < getCRPTimeout(DEFAULT_GC_TIMEOUT) / 2)
  ...
  private boolean enqueueAndQueryForActives(RequestAndCallback rc, boolean force, boolean anycast)
      throws IOException {
  ...
  private boolean enqueue(RequestAndCallback rc, boolean anycast) {
    ...
    this.requestsPendingActives.putIfAbsent(name, new LinkedBlockingQueue<RequestAndCallback>());
    LinkedBlockingQueue<RequestAndCallback> pending = this.requestsPendingActives.get(name);
    assert (pending != null);
    return pending.add(rc);
  }

  private void queryForActives(Request request, boolean forceRefresh, boolean anycast)
      throws IOException {
    ...
    if (forceRefresh || !this.queriedActivesRecently(name)) {
      ...
      ReconfigurationConfig.getLogger()
          .log(Level.FINE, "{0} requesting active replicas for {1}", new Object[] {this, name});
      this.sendRequesNullCallback(new RequestActiveReplicas(name));
      // this.lastQueriedActives.put(name, System.currentTimeMillis());
      this.activeReplicas.put(name, new ActivesInfo(null, System.currentTimeMillis()));
    } else {
  ...
  private void sendRequestsPendingActives(RequestActiveReplicas response) {
    ...
    this.e2eRedirector.learnSample(
        response.getSender(), System.currentTimeMillis() - response.getCreateTime());
    ...
    if (actives != null && !actives.isEmpty()) {
      synchronized (this.activeReplicas) {
        this.activeReplicas.put(
            response.getServiceName(), new ActivesInfo(actives, response.getCreateTime()));
      }
      if (this.mostRecentlyWrittenMap.contains(response.getServiceName())
          && !actives.contains(this.mostRecentlyWrittenMap.get(response.getServiceName())))
    ...
    log.log(
        ...
        new Object[] {
          this,
          response.getSummary(),
          Util.truncatedLog(this.requestsPendingActives.get(response.getServiceName()), 8)
        });
    LinkedBlockingQueue<RequestAndCallback> pendingRequests =
        this.requestsPendingActives.get(response.getServiceName());
    ...
    synchronized (pendingRequests) {
      if (pendingRequests.isEmpty()) this.requestsPendingActives.remove(response.getServiceName());
      else
        for (Iterator<RequestAndCallback> reqIter =
                this.requestsPendingActives.get(response.getServiceName()).iterator();
            reqIter.hasNext(); ) {
          ...
          if (actives != null && !actives.isEmpty()) {
            ...
            try {
              this.sendRequest(
                  rc.request,
                  rc.redirector != null
                      ? rc.redirector.getNearest(actives)
                      : (InetSocketAddress) (Util.selectRandom(actives)),
                  rc.callback);
            } catch (IOException e) {
              log.log(
                  Level.WARNING,
                  "{0} encountered IOException while trying " + "to release requests pending {1}",
                  new Object[] {this, response.getSummary()});
              e.printStackTrace();
            }
          } else {
            ...
            for (RequestAndCallback pendingRequest : pendingRequests)
              ...
              if (response.getHashRCs() != null
                ...
                response.getHashRCs().remove(response.getSender());
                log.log(
                    Level.FINE,
                    "{0} received no actives from {1} for name {2}; trying other reconfigurators"
                        + " {3}",
                    new Object[] {
                      this, response.getSender(), response.getServiceName(), response.getHashRCs()
                    });
                try {
                  for (InetSocketAddress reconfigurator : response.getHashRCs())
                    this.sendRequest(
                        new RequestActiveReplicas(response.getServiceName()),
                ...
                pendingRequest.callback.processResponse(
                    new ActiveReplicaError(
                            response.getServiceName(),
                            pendingRequest.request.getRequestID(),
                            response)
                        .setResponseMessage(
                            ClientReconfigurationPacket.ResponseCodes.ACTIVE_REPLICA_EXCEPTION
  ...
  private void updateE2ERedirector(EchoRequest response) {
    if (response.isRequest()) return;
    for (InetSocketAddress isa : this.actives)
      if (isa.getAddress().equals(response.getSender()))
        this.e2eRedirector.learnSample(isa, System.currentTimeMillis() - response.sentTime);
  }
  ...
  private void updateClosest(EchoRequest response) {
    ...
    synchronized (this.closest) {
      ...
      if (closest.size() < CLOSEST_K && !closest.containsKey(response.getSender()))
        closest.put(response.getSender().getAddress(), delay);
      InetAddress maxDelayAddr = null;
      for (InetAddress addr : closest.keySet())
        if ((closest.get(addr)) > delay) maxDelayAddr = addr;
      if (maxDelayAddr != null) {
        this.closest.remove(maxDelayAddr);
        this.closest.put(response.getSender().getAddress(), delay);
        log.log(Level.INFO, "{0} updated closest server to {1}", new Object[] {this, this.closest});
      }
    }
    if (this.heardFrom.size() == this.actives.size() / 2 + 1) {

      Set<InetSocketAddress> servers = new HashSet<InetSocketAddress>(actives);
      if (SEND_CLOSEST_TO_RECONFIGURATORS) servers.addAll(clientFacingReconfigurators);
      log.log(
          Level.INFO,
          "{0} sending closest-{1} map {2} to {3}",
          new Object[] {this, this.closest.size(), this.closest, servers});
      for (InetSocketAddress address : servers) {
        try {
          this.sendRequest(
              new EchoRequest((InetSocketAddress) null, this.closest),
              address,
              new Callback<Request, V>() {
                @Override
                public V processResponse(Request response) {
                  // do nothing
                  return null;
  ...
  private static final boolean SEND_CLOSEST_TO_RECONFIGURATORS =
      Config.getGlobalBoolean(RC.SEND_CLOSEST_TO_RECONFIGURATORS);

  private static final boolean ORIENT_CLIENT = Config.getGlobalBoolean(RC.ORIENT_CLIENT);

  private void orient() {
    if (!ORIENT_CLIENT) return;
    assert (!actives.isEmpty());
    log.log(Level.INFO, "{0} orienting itself against {1}", new Object[] {this, actives});
    this.heardFrom.clear();
    for (InetSocketAddress address : actives) {
      try {
        this.sendRequest(
            new EchoRequest((InetSocketAddress) null),
            address,
            new Callback<Request, V>() {
              @Override
              public V processResponse(Request response) {
  ...
  /**
   * Connectivity check with reconfigurator {@code address} that will block for up to {@code
   * timeout} duration. If {@code address} is null, the check will be performed against a random
   * reconfigurator.
   *
   * @param timeout
   * @param address
  ...
  protected boolean checkConnectivity(long timeout, InetSocketAddress address) {
    ...
    try {
      this.sendRequest(
          new RequestActiveReplicas(Config.getGlobalString(RC.BROADCAST_NAME)),
          address != null
              ? address
              : this.getRandom(this.clientFacingReconfigurators.toArray(new InetSocketAddress[0])),
          new RequestCallback() {
            @Override
            public void handleResponse(Request response) {
              ...
              success[0] = true;
              if (response instanceof RequestActiveReplicas)
                ReconfigurableAppClientAsync.this.actives =
                    ((RequestActiveReplicas) response).getActives();
      ...
      log.log(
          ...
          new Object[] {this, Config.getGlobalString(RC.BROADCAST_NAME)});
  ...
  /**
   * Connectivity check that will be attempted up to {@code numAttempts} times each with a timeout
   * of {@code attemptTimeout} with the reconfigurator {@code address}. If {@code address} is null,
   * a random reconfigurator will be chosen for each attempt.
   *
  ...
  protected boolean checkConnectivity(
      long attemptTimeout, int numAttempts, InetSocketAddress address) {
    int attempts = 0;
    while (attempts++ < numAttempts)
      if (this.checkConnectivity(attemptTimeout, address)) {
        ...
        System.out.print(
            (attempts == 1 ? "Retrying connectivity check to " + address + "..." : "")
                + attempts
                + " ");
  ...
  /**
   * A connectivity check with every known reconfigurator each with a default timeout of {@link
  ...
  public void checkConnectivity() throws IOException {
    for (InetSocketAddress address : this.clientFacingReconfigurators)
      if (this.checkConnectivity(
          ...
          2 + 1 / this.clientFacingReconfigurators.size(),
          address)) return;

    throw new IOException(CONNECTION_CHECK_ERROR + " : " + this.clientFacingReconfigurators);
  }
  ...
  public String toString() {
    return this.getClass().getSimpleName()
        + this.getLabel()
        + (this.niot != null ? this.niot.getListeningSocketAddress().getPort() : "");
  }
  ...
  /**
   * If true, the app will only be handed JSON formatted packets via getJSONRequest(), not via
   * {@link #getRequest(String)}. If this method is invoked, the protected method {@link
   * #getJSONRequest(JSONObject)} must be overridden.
   *
   * @return {@code this}.
   */
  ...
  private Set<IntegerPacketType> getMutualAuthTypes() {
    return cachedMutualAuthTypes != null
        ? cachedMutualAuthTypes
        : (cachedMutualAuthTypes = getMutualAuthRequestTypes()) != null
            ? cachedMutualAuthTypes
  ...
  public static class ReconfigurationException extends Exception {
     ...
     * @param message
     */
    public ReconfigurationException(ResponseCodes code, String message, String GUID) {
      super(message);
      this.code = code;
    }
     ...
     * @param cause
     */
    public ReconfigurationException(ResponseCodes code, String message, Throwable cause) {
      super(message, cause);
      this.code = code;
    }
