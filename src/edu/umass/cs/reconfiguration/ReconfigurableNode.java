...
public abstract class ReconfigurableNode<NodeIDType> {
     ...
     * @param nodeConfig      Maps node IDs of active replicas and reconfigurators to their
     *                        socket addresses.
     * @param args
     * @param startCleanSlate Used to join newly added nodes.
     * @throws IOException Thrown if networking functions can not be successfully
    ...
    public static Set<ReconfigurableNode> main1(String[] args) throws IOException {
        ...
        for (String node : servers) {
            logServerIDsStr.append(String.format(" %s:%s:%s ",
                    node, nodeConfig.getNodeAddress(node), nodeConfig.getNodePort(node)));
        }
        String logStr = String.format(">> Initializing gigapaxos %s [%s] ...",
                logServerStr, logServerIDsStr);
        ...
        ReconfigurationConfig.getLogger().log(Level.INFO, "{0} server{1} ready (total number of servers={2})",
                new Object[]{logServerIDsStr, (numServers > 1 ? "s" : ""), PaxosConfig.getActives().size() +
