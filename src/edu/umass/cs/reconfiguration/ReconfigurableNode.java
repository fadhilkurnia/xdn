...
/**
 * @param <NodeIDType> A generic type for representing node identifiers. It must support an
 *     explicitly overridden toString() method that converts NodeIDType to String, and the
 *     NodeConfig object supplied to this class' constructor must support a valueOf(String) method
 *     that returns back the original NodeIDType. Thus, even though NodeIDType is generic, a
 *     one-to-one mapping between NodeIDType and String is necessary.
 * @author V. Arun
 */
public abstract class ReconfigurableNode<NodeIDType> {
  ...
  private final Set<ActiveReplica<NodeIDType>> activeReplicas =
      new HashSet<ActiveReplica<NodeIDType>>();
  private final Set<Reconfigurator<NodeIDType>> reconfigurators =
      new HashSet<Reconfigurator<NodeIDType>>();
   ...
   * @throws IOException
   */
  public ReconfigurableNode(NodeIDType id, ReconfigurableNodeConfig<NodeIDType> nodeConfig)
      throws IOException {
  ...
  public void close() {
    ...
    this.messenger.stop();
    ReconfigurationConfig.getLogger().info("----------" + this + " closed----->||||");
  }

  /**
   * For testing. Only a server that has been started from within this JVM can be closed and
   * force-cleared using this method.
   *
   * @param server
   */
  ...
  private AbstractReplicaCoordinator<NodeIDType> createApp(
    ...
    // else
    {
      ...
      if (app instanceof ClientMessenger) ((ClientMessenger) app).setClientMessenger(messenger);
      else
      ...
      appCoordinator = this.getAppCoordinator(app, myID, nodeConfig, messenger);
      ...
      ReconfigurationConfig.getLogger()
          .log(
              ...
              new Object[] {
                this, PaxosConfig.getDefaultServiceName(), nodeConfig.getActiveReplicas()
              });
      appCoordinator.createReplicaGroup(
          PaxosConfig.getDefaultServiceName(),
          0,
          Config.getGlobalString(PaxosConfig.PC.DEFAULT_NAME_INITIAL_STATE),
  ...
  private AbstractReplicaCoordinator<NodeIDType> getAppCoordinator(
    ...
    String coordinatorClassName =
        Config.getGlobalString(ReconfigurationConfig.RC.REPLICA_COORDINATOR_CLASS);
    ...
    switch (coordinatorClassName) {
      case "edu.umass.cs.reconfiguration.PaxosReplicaCoordinator" -> {
        return new PaxosReplicaCoordinator<NodeIDType>(app, myID, nodeIDStringifier, messenger)
            .setOutOfOrderLimit(Config.getGlobalInt(ReconfigurationConfig.RC.OUT_OF_ORDER_LIMIT));
      }
      case "edu.umass.cs.reconfiguration.ChainAndPaxosReplicaCoordinator" -> {
        ReconfigurationConfig.getLogger()
            .log(Level.SEVERE, "Unimplemented ChainAndPaxosReplicaCoordinator");
      }
      case "edu.umass.cs.reconfiguration.ChainReplicaCoordinator" -> {
        return new ChainReplicaCoordinator<NodeIDType>(app, myID, nodeIDStringifier, messenger);
      }
      ...
      case "edu.umass.cs.xdn.XdnReplicaCoordinator" -> {
        return new XdnReplicaCoordinator<NodeIDType>(app, myID, nodeIDStringifier, messenger);
      }
      case "edu.umass.cs.causal.CausalReplicaCoordinator" -> {
        return new CausalReplicaCoordinator<NodeIDType>(app, myID, nodeIDStringifier, messenger);
      }
    ...
    ReconfigurationConfig.getLogger()
        .log(Level.SEVERE, "Unknown replica coordinator class of " + coordinatorClassName);
    System.exit(-1);
    return null;
  ...
  /**
   * @param id Node ID of this ReconfigurableNode being created.
   * @param nodeConfig Maps node IDs of active replicas and reconfigurators to their socket
   *     addresses.
   * @param args
   * @param startCleanSlate Used to join newly added nodes.
   * @throws IOException Thrown if networking functions can not be successfully initialized. A
   *     common reason for this exception is that the socket addresses corresponding to the supplied
   *     'id' argument are not local, i.e., the node with this id should not be created on this
   *     machine in the first place, or if the id is not present at all in the supplied 'nodeConfig'
   *     argument.
   */
  public ReconfigurableNode(
      NodeIDType id,
      ReconfigurableNodeConfig<NodeIDType> nodeConfig,
      String[] args,
    ...
    messenger =
        (new JSONMessenger<NodeIDType>(
            (niot =
                new MessageNIOTransport<NodeIDType, JSONObject>(
                    ReconfigurableNode.this.myID,
                    nodeConfig,
                    (pd =
                        new ReconfigurationPacketDemultiplexer(nodeConfig)
                            .setThreadName(ReconfigurableNode.this.myID.toString())),
                    true,
    ...
    if (!niot.getListeningSocketAddress().equals(isa)
      ...
      ReconfigurationConfig.getLogger()
          .severe(err = this + " unable to start ReconfigurableNode at socket address " + isa);
      throw new IOException(err);
    }
    ...
    if (nodeConfig.getActiveReplicas().contains(id)) {
      ...
      // create active
      ActiveReplica<NodeIDType> activeReplica =
          new ActiveReplica<NodeIDType>(
      ...
      Reconfigurator<NodeIDType> reconfigurator =
          new Reconfigurator<NodeIDType>(nodeConfig, messenger, startCleanSlate);
      pd.register(
          reconfigurator.getPacketTypes().toArray(new IntegerPacketType[0]), reconfigurator);
      this.reconfigurators.add(reconfigurator);

      // wrap reconfigurator in active to make it reconfigurable
      ActiveReplica<NodeIDType> activeReplica =
          reconfigurator.getReconfigurableReconfiguratorAsActiveReplica();
  ...
  private static ConcurrentMap<String, ReconfigurableNode<?>> allInstances =
      new ConcurrentHashMap<String, ReconfigurableNode<?>>();

  // because ReconfigurableNode is abstract for backwards compatibility

  /** */
  public static final class DefaultReconfigurableNode extends ReconfigurableNode<String> {
     ...
     * @throws IOException
     */
    public DefaultReconfigurableNode(
        String id,
  ...
  private static final Set<String> getAllNodes(String[] args) {
    ...
    if (args[args.length - 1].equals(ReconfigurationConfig.CommandArgs.START_ALL.toString())
        // look for "start all" or "clear all" at the end
        || (args.length >= 2
            && args[args.length - 1].equals(ReconfigurationConfig.CommandArgs.all)
            && (args[args.length - 2].equals(ReconfigurationConfig.CommandArgs.start)
                || args[args.length - 2].equals(ReconfigurationConfig.CommandArgs.clear)))) {
      ...
      for (int i = args.length - 1; i >= 0; i--)
        if ((ReconfigurationConfig.getReconfiguratorIDs().contains(args[i])
                || PaxosConfig.getActives().keySet().contains(args[i]))
            && !args[i].equals(ReconfigurationConfig.CommandArgs.start.toString())
            && !args[i].equals(ReconfigurationConfig.CommandArgs.clear.toString()))
          nodeIDs.add(args[i]);
        else {
          clear = clear || args[i].equals(ReconfigurationConfig.CommandArgs.clear.toString());
          break;
        }
    return nodeIDs;
  }
  ...
  private static final String getAppArgs(String[] args) {
    String argsAsString = "";
    for (String arg : args) argsAsString += " " + arg;
    argsAsString =
        argsAsString
            .replaceAll(ReconfigurationConfig.CommandArgs.START_ALL.toString(), "")
            .replaceAll(" " + ReconfigurationConfig.CommandArgs.start.toString() + " .*", "")
            .trim();
    return argsAsString;
  }
   ...
   * node IDs at the end. The string "start all" is accepted as a proxy for the list of all nodes if
   * the socket addresses of all nodes are on the local machine.
   *
   * @param args
   * @throws IOException
   */
  public static Set<ReconfigurableNode> main1(String[] args) throws IOException {
    ...
    if (Config.getGlobalBoolean(PC.EMULATE_DELAYS)) AbstractPacketDemultiplexer.emulateDelays();
    ...
    String sysPropAppArgsAsString =
        System.getProperty(ReconfigurationConfig.CommandArgs.appArgs.toString());
    // append cmdline args to system property based args
    String cmdlineAppArgs = getAppArgs(args);
    String[] appArgs =
        ((sysPropAppArgsAsString != null ? sysPropAppArgsAsString : "") + " " + cmdlineAppArgs)
            .trim()
            .split("(\\s)+");
    int numServers = servers.size();
    if (numServers == 0) throw new RuntimeException("No valid server names supplied");

    // printout all the nodes' information
    String logServerStr = numServers > 1 ? "servers" : "server";
    StringBuilder logServerIDsStr = new StringBuilder();
    for (String node : servers) {
      logServerIDsStr.append(
          String.format(
              " %s:%s:%s ", node, nodeConfig.getNodeAddress(node), nodeConfig.getNodePort(node)));
    }
    String logStr =
        String.format(">> Initializing gigapaxos %s [%s] ...", logServerStr, logServerIDsStr);
    ...
    ReconfigurationConfig.getLogger()
        .log(
            Level.INFO,
            "{0} server{1} ready (total number of servers={2})",
            new Object[] {
              logServerIDsStr,
              (numServers > 1 ? "s" : ""),
              PaxosConfig.getActives().size() + ReconfigurationConfig.getReconfigurators().size()
            });
