...
/**
 * HttpRequest is the type of request used by {@link HttpActiveReplica}. All the applications that
 * need to use {@link HttpActiveReplica} must follow the spec of this class (HttpRequest).
 *
 * <p>HttpRequest requires the content of a HTTP POST request (to {@link HttpActiveReplica} ) in
 * JSON format. It must contains the following keys: NAME, VALUE, and SYNC
 *
 * <p>The key PACKET_TYPE of {@link JSONPacket}.
 *
 * @author gaozy
 */
public class HttpActiveReplicaRequest extends JSONPacket
    implements ReplicableRequest, ReconfigurableRequest, ClientRequest, Byteable {
  ...
  /**
   * HttpRequest is request type agnostic, the underlying application needs to define their own
   * types if necessary.
   *
   * @param t
   * @param name
   ...
   * @param epoch
   */
  public HttpActiveReplicaRequest(
      IntegerPacketType t,
      String name,
      long id,
      String value,
   ...
   */
  public HttpActiveReplicaRequest(
      IntegerPacketType t, String name, long id, String value, boolean stop, int epoch) {
  ...
  public HttpActiveReplicaRequest(String value, HttpActiveReplicaRequest req) {
    this(
        HttpActiveReplicaPacketType.getPacketType(req.type),
        req.name,
        req.id,
        value,
        req.coord,
  ...
  public ClientRequest getResponse() {
    if (this.response != null) return new HttpActiveReplicaRequest(this.response, this);
    return null;
  }
  ...
  private static int estimateSize() {
    // type (int) | name (String) | id (long) | value (String) | coord (bool) | stop (bool) | epoch
    // (int)

    int length =
        Integer.BYTES * 2 // type + epoch
            + Long.BYTES // id
            + 2 // coord + stop
        ;
  ...
  private int lengthEstimate() {
    int len =
        this.value.length()
            + this.name.length()
            + (this.response == null ? 0 : this.response.length())
            + SIZE_ESTIMATE;
    return len;
  }
  ...
  public byte[] toBytes() {
    ...
    try {
      ...
      byte[] valueBytes = this.value != null ? this.value.getBytes(CHARSET) : new byte[0];
      ...
      byte[] responseBytes = this.response != null ? this.response.getBytes(CHARSET) : new byte[0];
   ...
   */
  public HttpActiveReplicaRequest(byte[] bytes)
   ...
   * @throws UnknownHostException
   */
  public HttpActiveReplicaRequest(ByteBuffer bbuf)
      throws UnsupportedEncodingException, UnknownHostException {
    ...
    bbuf.get(nameBytes);
    this.name = nameBytes.length > 0 ? new String(nameBytes, CHARSET) : null;
    ...
    bbuf.get(valBytes);
    this.value = valBytes.length > 0 ? new String(valBytes, CHARSET) : null;
    ...
    bbuf.get(respBytes);
    this.response = respBytes.length > 0 ? new String(respBytes, CHARSET) : null;
  }
  ...
  public static void main(String[] args) {
    ...
    HttpActiveReplicaRequest req1 =
        new HttpActiveReplicaRequest(HttpActiveReplicaPacketType.EXECUTE, serviceName, value);
    ...
    // Consistent: if x.equals(y)==true and no value is modified, then it's always true for every
    // call.
