...
public class ReconfigurationPacketDemultiplexer extends AbstractPacketDemultiplexer<Request> {
  ...
  protected Request processHeader(byte[] message, NIOHeader header) {
    ...
    if ((BYTEIFICATION
            && (rcType = ReconfigurationPacket.PacketType.intToType.get(bbuf.getInt())) != null
            && rcType != ReconfigurationPacket.PacketType.REPLICABLE_CLIENT_REQUEST
            && (json =
                    AbstractJSONPacketDemultiplexer.processHeaderStatic(
                        message, Integer.BYTES, header, true))
                != null)
        || (!BYTEIFICATION
            && JSONPacket.couldBeJSON(message, Integer.BYTES)
            && (json =
                    AbstractJSONPacketDemultiplexer.processHeaderStatic(message, 0, header, true))
                != null)) {
      // decode json as reconfiguration packet
      return ReconfigurationPacket.getReconfigurationPacketSuppressExceptions(
          json, this.unstringer);
    }
    // else must be app packet
    if (this.appRequestparser != null) {
      try {
        if (this.appRequestparser instanceof AppRequestParserBytes)
          return ((AppRequestParserBytes) this.appRequestparser).getRequest(message, header);
        else
          return this.appRequestparser.getRequest(
              new String(message, MessageNIOTransport.NIO_CHARSET_ENCODING));
  ...
  @Override
  public boolean handleMessage(Request message, edu.umass.cs.nio.nioutils.NIOHeader header) {
  ...
  boolean handleMessage(JSONObject json) {
    throw new RuntimeException(
        "This method should never be called unless we have \"forgotten\" to register or handle some"
            + " packet types.");
  }
