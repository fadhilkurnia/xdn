...
/**
 * @author arun
 * @param <NodeIDType> The point of this class is to retain the supplied nodeConfig only for actives
 *     so that the supplier can change them at will. We make a separate copy of the reconfigurators
 *     as they have to be changed carefully.
 *     <p>As the purpose of this class is also to serve as the "underlying" node config to
 *     ConsistentNodeConfig, we allow getNodeIDs() to be called here.
 */
public class SimpleReconfiguratorNodeConfig<NodeIDType>
    implements ModifiableReconfigurableNodeConfig<NodeIDType> {

  private final ReconfigurableNodeConfig<NodeIDType> nodeConfig;
  private int version = 0;
  private final HashMap<NodeIDType, InetSocketAddress> rcMap =
      new HashMap<NodeIDType, InetSocketAddress>();
  ...
  public SimpleReconfiguratorNodeConfig(ReconfigurableNodeConfig<NodeIDType> nc) {
    ...
    for (NodeIDType node : nc.getReconfigurators()) {
      this.rcMap.put(node, new InetSocketAddress(nc.getNodeAddress(node), nc.getNodePort(node)));
  ...
  public boolean nodeExists(NodeIDType id) {
    return this.rcMap.containsKey(id) || this.nodeConfig.getActiveReplicas().contains(id);
  }

  @Override
  public InetAddress getNodeAddress(NodeIDType id) {
    return this.rcMap.containsKey(id)
        ? this.rcMap.get(id).getAddress()
  ...
  public InetAddress getBindAddress(NodeIDType id) {
    return this.rcMap.containsKey(id)
        ? this.rcBindMap.get(id)
        : (this.nodeConfig.getActiveReplicas().contains(id)
            ? this.nodeConfig.getBindAddress(id)
            : null);
  }
  ...
  public int getNodePort(NodeIDType id) {
    return this.rcMap.containsKey(id)
        ? this.rcMap.get(id).getPort()
        : (this.nodeConfig.getActiveReplicas().contains(id) ? this.nodeConfig.getNodePort(id) : -1);
  }
  ...
  @Override
  public synchronized InetSocketAddress addReconfigurator(
      NodeIDType id, InetSocketAddress sockAddr) {
    ...
    if (this.nodeConfig instanceof ModifiableRCConfig)
      ((ModifiableRCConfig<NodeIDType>) this.nodeConfig).addReconfigurator(id, sockAddr);
    return prevSockAddr;
  }
  ...
  public synchronized InetSocketAddress removeReconfigurator(NodeIDType id) {
    ...
    if (this.nodeConfig instanceof ModifiableRCConfig)
      ((ModifiableRCConfig<NodeIDType>) this.nodeConfig).removeReconfigurator(id);

    return removed;
  }

  @Override
  public synchronized InetSocketAddress addActiveReplica(
      NodeIDType id, InetSocketAddress sockAddr) {
    InetSocketAddress old =
        (this.nodeConfig.getActiveReplicas().contains(id)
            ? new InetSocketAddress(
                this.nodeConfig.getNodeAddress(id), this.nodeConfig.getNodePort(id))
            : null);
    if (this.nodeConfig instanceof ModifiableActiveConfig)
      ((ModifiableActiveConfig<NodeIDType>) this.nodeConfig).addActiveReplica(id, sockAddr);
    else
  ...
  public synchronized InetSocketAddress removeActiveReplica(NodeIDType id) {
    InetSocketAddress old =
        (this.nodeConfig.getActiveReplicas().contains(id)
            ...
            : null);
    if (this.nodeConfig instanceof ModifiableActiveConfig)
      ((ModifiableActiveConfig<NodeIDType>) this.nodeConfig).removeActiveReplica(id);
    else
      throw new RuntimeException(
          "Underlying nodeConfig does not implement InterfaceModifiableActiveConfig");
    return old;
