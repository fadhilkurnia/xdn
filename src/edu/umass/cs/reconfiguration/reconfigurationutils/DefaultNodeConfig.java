 ...
 * @param <NodeIDType>
 */
public class DefaultNodeConfig<NodeIDType>
    implements ModifiableReconfigurableNodeConfig<NodeIDType> {

  final ConcurrentHashMap<NodeIDType, InetSocketAddress> actives =
      new ConcurrentHashMap<NodeIDType, InetSocketAddress>();
  final ConcurrentHashMap<NodeIDType, InetSocketAddress> reconfigurators =
      new ConcurrentHashMap<NodeIDType, InetSocketAddress>();
  final ConcurrentHashMap<NodeIDType, Geolocation> geolocations =
      new ConcurrentHashMap<NodeIDType, Geolocation>();
  ...
  public DefaultNodeConfig(
      Map<NodeIDType, InetSocketAddress> actives,
   ...
   * @param actives
   * @param reconfigurators
   * @param geolocations Per-node geolocation; nodes with no entry will return {@code null} from
   *     {@link #getNodeGeolocation}.
   */
  public DefaultNodeConfig(
      Map<NodeIDType, InetSocketAddress> actives,
      Map<NodeIDType, InetSocketAddress> reconfigurators,
  ...
  public boolean nodeExists(NodeIDType id) {
    return this.actives.containsKey(id) || this.reconfigurators.containsKey(id);
  }

  @Override
  public InetAddress getNodeAddress(NodeIDType id) {
    return this.actives.containsKey(id)
        ? this.actives.get(id).getAddress()
        : this.reconfigurators.containsKey(id) ? this.reconfigurators.get(id).getAddress() : null;
  }
  ...
  public int getNodePort(NodeIDType id) {
    int port =
        this.actives.containsKey(id)
            ? this.actives.get(id).getPort()
            : this.reconfigurators.containsKey(id) ? this.reconfigurators.get(id).getPort() : -1;
    return port;
  }
