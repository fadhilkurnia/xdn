...
/**
 * @author V. Arun
 *     <p>This sample implementation of {@link AbstractDemandProfile} maintains the demand profile
 *     for a single name and returns it as a JSONObject via its getStats() method. The specification
 *     of its methods is in the documentation of {@link AbstractDemandProfile}.
 */
public class DemandProfile extends AbstractDemandProfile {
  ...
  /**
   * The minimum amount of time (ms) that must elapse since the previous reconfiguration before the
   * next reconfiguration can happen.
   */
  ...
  /**
   * The string argument {@code name} is the service name for which this demand profile is being
   * maintained.
   *
   * @param name
   */
  ...
  /**
   * Deep copy constructor. This constructor should create a copy of the supplied DemandProfile
   * argument {@code dp} such that the newly constructed DemandProfile instance dpCopy != dp but
   * dpCopy.equals(dp).
   *
   * @param dp
   */
  ...
  /**
   * All {@link AbstractDemandProfile} instances must be constructible from a JSONObject.
   *
   * @param json
  ...
  /**
   * @param name
   * @return All {@link AbstractDemandProfile} instances must support a single String argument
   *     constructor that is the underlying service name.
   */
  ...
  public boolean shouldReportDemandStats(
    ...
    // incorporate request
    if (!request.getServiceName().equals(this.name)) return false;
    this.numRequests++;
    ...
    } else lastRequestTime = System.nanoTime(); // initialization
  ...
  public void combine(AbstractDemandProfile dp) {
    ...
    this.interArrivalTime =
        Util.movingAverage(update.interArrivalTime, this.interArrivalTime, update.getNumRequests());
  ...
  public Set<String> reconfigure(Set<String> curActives, ReconfigurableAppInfo nodeConfig) {
    ...
    /**
     * This example simply returns curActives as this policy trivially reconfigures to the same set
     * of locations after every call. In general, AbstractDemandProfile implementations should
     * return a new list different from curActives.
     */
    return curActives;
  }
