...
/**
 * @author V. Arun
 *     <p>A utility class for maintaining demand profiles, specifically geo-distribution of demand
 *     in order to send it to reconfigurators.
 */
public class AggregateDemandProfiler {
  ...
  private final HashMap<String, AbstractDemandProfile> map =
      new HashMap<String, AbstractDemandProfile>();
   ...
   */
  public synchronized boolean shouldSendDemandReport(Request request, InetAddress sender) {
    String name = request.getServiceName();
    AbstractDemandProfile demand = this.getDemandProfile(name);
    if (demand == null) demand = AbstractDemandProfile.createDemandProfile(name); // reflection
  ...
  public synchronized void putIfEmpty(AbstractDemandProfile profile) {
    if (!this.map.containsKey(profile.getName())) this.map.put(profile.getName(), profile);
  }

  /**
   * @param name
   * @param curActives
   * @return True if Reconfigurator should initiate a reconfiguration to the IP addresses returned.
   */
  public synchronized Set<String> reconfigure(String name, Set<String> curActives) {
    AbstractDemandProfile demand = this.getDemandProfile(name);
    if (demand == null) return null;
   ...
   * @param curActives
   * @param appInfo
   * @return Set of IP addresses to which the replicas have been reconfigured. The testAndSet
   *     ensures atomicity.
   */
  public synchronized Set<String> testAndSetReconfigured(
    ...
    if (demand == null || (newActives = demand.reconfigure(curActives, appInfo)) == null)
      return curActives;
  ...
  /**
   * Similar as {@link #testAndSetReconfigured(String, Set, ReconfigurableAppInfo)} method, but this
   * variant also return optional metadata, which is useful for replica placement.
   *
   * @param name The service name.
   * @param curActives The current ID of Nodes that host the replica.
   * @param appInfo Interface that contains the application information, including method to get
   *     possible Node for placing the replica.
  ...
  public synchronized NodeIdsMetadataPair<String> testAndSetReconfigured2(
    ...
    NodeIdsMetadataPair<String> newActives = demand.getNewActivesPlacement(curActives, appInfo);
    if (newActives == null) {
      return new NodeIdsMetadataPair<>(curActives, null);
    }
  ...
  /**
   * If aggregate size becomes more than DEFAULT_MAX_SIZE, this method will pluck DEFAULT_PLUCK_SIZE
   * out so that an active replica can report it to reconfigurators. This design allows an active
   * replica to simply use transmission to reconfigurators as an alternative to persistently storing
  ...
  public synchronized Set<AbstractDemandProfile> trim() {
    ...
    if (this.map.size() >= DEFAULT_MAX_SIZE) {
      int count = 0;
      for (Iterator<Entry<String, AbstractDemandProfile>> iter = this.map.entrySet().iterator();
          iter.hasNext(); /* nothing */ ) {
        Entry<String, AbstractDemandProfile> entry = iter.next();
        plucked.add(entry.getValue());
        iter.remove();
        if (count++ >= DEFAULT_PLUCK_SIZE) break;
      }
    }
    return plucked;
  }
  ...
  public synchronized AbstractDemandProfile pluckDemandProfile(String name) {
    ...
    AbstractDemandProfile copy =
        AbstractDemandProfile.createDemandProfile(demand.getDemandStats()); // demand.clone();
    // demand.reset();
   ...
   */
  public synchronized AbstractDemandProfile combine(AbstractDemandProfile update) {
    AbstractDemandProfile existing = this.map.get(update.getName());
    if (existing != null) existing.combine(update);
