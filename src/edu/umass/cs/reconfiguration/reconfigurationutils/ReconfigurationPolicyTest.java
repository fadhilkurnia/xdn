...
/**
 * @author arun
 *     <p>A utility class to sanity check the reconfiguration policy implementation.
 */
public class ReconfigurationPolicyTest extends DefaultTest {
  ...
  public void beforeMethod() {
    if (!C.getName()
        .equals(ReconfigurationConfig.RC.DEMAND_PROFILE_TYPE.getDefaultValue().toString())) {
  ...
  public void testStringConstructor() {
    AbstractDemandProfile profile1 = AbstractDemandProfile.createDemandProfile(C, name1);
    Assert.assertEquals(name1, profile1.getName());
    AbstractDemandProfile profile2 = AbstractDemandProfile.createDemandProfile(C, name2);
  ...
  public void testJSONConstructor() throws UnknownHostException {
    AbstractDemandProfile profile = AbstractDemandProfile.createDemandProfile(C, name1);
    for (int i = 0; i < numRequests; i++)
      profile.shouldReportDemandStats(
          getRequest(name1), InetAddress.getByName("64.46.77." + i), getAppInfo());
    Assert.assertEquals(
        profile.getDemandStats().toString(),
        AbstractDemandProfile.createDemandProfile(C, profile.getDemandStats())
            .getDemandStats()
            .toString());
  }
  ...
  /**
   * Sanity checks the implementation of a reconfiguration policy using a trivial default Request
   * implementation.
   *
   * @param clazz
   */
  public static void testPolicyImplementation(Class<?> clazz) {
    ...
    for (Failure failure : result.getFailures()) failure.getException().printStackTrace();
  }
}
