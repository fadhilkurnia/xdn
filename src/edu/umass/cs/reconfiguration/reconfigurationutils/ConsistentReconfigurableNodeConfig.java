...
/**
 * @author arun
 * @param <NodeIDType> * This class is a wrapper around NodeConfig to ensure that it is consistent,
 *     i.e., it returns consistent results even if it changes midway. In particular, it does not
 *     allow the use of a method like getNodeIDs().
 *     <p>It also has consistent hashing utility methods.
 */
public class ConsistentReconfigurableNodeConfig<NodeIDType> extends ConsistentNodeConfig<NodeIDType>
    implements ModifiableActiveConfig<NodeIDType>, ModifiableRCConfig<NodeIDType> {
  ...
  public ConsistentReconfigurableNodeConfig(ReconfigurableNodeConfig<NodeIDType> nc) {
    ...
    this.CH_AR =
        new ConsistentHashing<NodeIDType>(
            this.activeReplicas, Config.getGlobalBoolean(RC.REPLICATE_ALL));
  }
  ...
  /**
   * @return Socket addresses of active replicas. Used by reconfigurator to respond to broadcast
   *     queries.
   */
  ...
  /**
   * This method tries to return a limited number of active replica socket addresses that to the
   * extent possible have distinct IPs. The socket addresses are a randomly chosen subset of size up
   * to limit from the set of all active replicas.
   *
   * @param limit
   * @return Socket addresses of active replicas. Used by reconfigurator to respond to broadcast
   *     queries.
  ...
  public Set<InetSocketAddress> getActiveReplicaSocketAddresses(int limit) {
    ...
    for (NodeIDType node : this.getActiveReplicas()) {
      ...
      } else duplicateIPs.add(sockAddr);
  ...
  /**
   * @param name
   * @return Set of consecutive reconfigurators to which {@code name} hashes on the consistent hash
   *     ring.
   */
  ...
  public Set<InetSocketAddress> getReconfiguratorsAsAddresses(String name) {
    ...
    for (NodeIDType node : nodes) addrs.add(this.getNodeSocketAddress(node));
    return addrs;
  }

  /**
   * @param name
   * @return First reconfigurator to which {@code name} hashes on the consistent hash ring.
   */
  // refresh before returning
  ...
  /**
   * @param name
   * @return Set of active replica nodes to which {@code name} hashes on the consistent hash ring of
   *     all active replica nodes.
   */
  ...
  /**
   * Variant of {@link #getReplicatedActives(String)} that lets the caller override the
   * replica-group size for this specific service, without mutating the shared {@link
   * ConsistentHashing} instance.
   *
  ...
  public Set<NodeIDType> getReplicatedActives(String name, int numReplicas) {
    this.refreshActives();
    return new HashSet<NodeIDType>(this.CH_AR.getReplicatedServersArray(name, numReplicas));
  }

  /**
   * @param name
   * @return Set of active replica IPs to which {@code name} hashes on the consistent hash ring of
   *     all active replica nodes.
   */
   ...
   */
  private Set<InetSocketAddress> getReplicatedActivesAddresses(String name, int limit) {
  ...
  /**
   * This method maps a set of addresses, newAddresses, to a set of nodes such that there is maximal
   * overlap with the specified set of nodes, oldNodes. It is somewhat nontrivial only because there
   * is a many-to-one mapping from nodes to addresses, so a simple reverse lookup is not meaningful.
   *
   * @param newAddresses
   * @param oldNodes
   * @return Set of active replica IPs corresponding to {@code newAddresses} that have high overlap
   *     with the set of old active replica nodes {@code oldNodes}.
   */
  public Set<NodeIDType> getIPToActiveReplicaIDs(
    ...
    for (InetAddress address : newAddresses) unassigned.add(address);
    ...
    for (NodeIDType node : this.nodeConfig.getActiveReplicas()) {
      if (this.activesSlatedForRemoval.contains(node)) continue;
  ...
  /**
   * This method should not be used. Reconfigurator uses this at exactly one place, namely, to
   * encapsulate itself inside ActiveReplica so as to be itself reconfigurable.
   *
  ...
  private synchronized boolean refreshActives() {
    ...
    if (curActives.equals(this.getLastActives())) return false;
  ...
  private synchronized boolean refreshReconfigurators() {
    ...
    if (curReconfigurators.equals(this.getLastReconfigurators())) return false;
  ...
  private synchronized Set<NodeIDType> setLastActives(Set<NodeIDType> curActives) {
    return this.activeReplicas = curActives;
  }

  private synchronized Set<NodeIDType> setLastReconfigurators(Set<NodeIDType> curReconfigurators) {
  ...
  public InetSocketAddress getNodeSocketAddress(NodeIDType id) {
    ...
    if (port == -1 && !this.isSlatedForRemoval(id))
      ReconfigurationConfig.getLogger().warning("No port found for nodeID " + id);
    return (ip != null ? new InetSocketAddress(ip, port) : null);
  }

  /**
   * Returns the bindable socket address of the public host corresponding to this id. This code be a
   * private address in the case of where we are inside a NAT.
   *
   * @param id
  ...
  /**
   * A utility method to split a collection of names into batches wherein names in each batch map to
   * the same reconfigurator group. The set of reconfigurators can be specified either as a
   * NodeIDType or a String set but not as an InetSocketAddress set (unless NodeIDType is
   * InetSocketAddress).
   *
   * @param names
   * @param reconfigurators
   * @return A set of batches of names wherein names in each batch map to the same reconfigurator
   *     group.
   */
  public static Collection<Set<String>> splitIntoRCGroups(
      Set<String> names, Set<?> reconfigurators) {
    if (reconfigurators.isEmpty())
      throw new RuntimeException("A nonempty set of reconfigurators must be specified.");
    ...
    for (String name : names) {
      ...
      if (!batches.containsKey(rc)) batches.put(rc, new HashSet<String>());
  ...
  public boolean checkSameGroup(Set<String> names) {
    NodeIDType rc = null;
    for (String name : names)
      if (rc == null) rc = this.getFirstReconfigurator(name);
      else if (!rc.equals(this.getFirstReconfigurator(name))) return false;
    return true;
  }
  ...
  /**
   * @return True if all nodes slated for removal have been successfully removed.
   */
  public boolean removeReconfiguratorsSlatedForRemoval() {
    boolean allRemoved = true;
    for (Iterator<NodeIDType> nodeIter = this.reconfiguratorsSlatedForRemoval.iterator();
        nodeIter.hasNext(); ) {
      NodeIDType slated = nodeIter.next();
      if (this.removeReconfigurator(slated) != null) nodeIter.remove();
  ...
  public boolean removeActivesSlatedForRemoval() {
    boolean allRemoved = true;
    for (Iterator<NodeIDType> nodeIter = this.activesSlatedForRemoval.iterator();
        nodeIter.hasNext(); ) {
      NodeIDType slated = nodeIter.next();
  ...
  public Set<NodeIDType> getNodeIDs(Set<String> newActives) {
    ...
    for (String newActive : newActives) nodes.add(this.valueOf(newActive));
    return nodes;
  }
}
