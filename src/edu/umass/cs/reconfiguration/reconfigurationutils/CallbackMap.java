...
/**
 * @author V. Arun
 * @param <NodeIDType> FIXME: May need to add a self-destruct property to entries so that they are
 *     automatically removed after some idle time.
 */
public class CallbackMap<NodeIDType> {
  private final ConcurrentHashMap<String, List<StopEpoch<NodeIDType>>> listMap =
      new ConcurrentHashMap<String, List<StopEpoch<NodeIDType>>>();
  ...
  public synchronized StopEpoch<NodeIDType> notifyStop(String name, int epoch) {
    if (!this.listMap.containsKey(name)) return null;
    StopEpoch<NodeIDType> notifiee = null, retval = null;
    for (Iterator<StopEpoch<NodeIDType>> notifieeIter = this.listMap.get(name).iterator();
        notifieeIter.hasNext(); ) {
      notifiee = notifieeIter.next();
      if (notifiee.getEpochNumber() - epoch > 0) continue;
      // else
      retval = notifiee;
      notifieeIter.remove();
      break;
    }
    if (this.listMap.get(name).isEmpty()) this.listMap.remove(name);
    return retval;
  }
}
