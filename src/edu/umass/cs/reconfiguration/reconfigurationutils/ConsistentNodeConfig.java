...
/**
 * This class isn't really used for anything other than as a parent for
 * ConsistentReconfigurableNodeConfig, so it has been declared abstract (even though it has no
 * abstract methods).
 *
 * @param <NodeIDType>
 */
public abstract class ConsistentNodeConfig<NodeIDType> implements NodeConfig<NodeIDType> {
  ...
  private synchronized boolean refresh() {
    ...
    if (curActives.equals(this.nodes)) return false;
