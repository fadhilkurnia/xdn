...
public class ReconfigurationRecord<NodeIDType> extends JSONObject {
  ...
  public static enum RCStates {
    /**
     * The default state and one of only two states from which reconfiguration can be initiated, the
     * other being READY_READY.
     */
    READY,

    /**
     * Waiting for AckStopEpoch, typically for the epoch that was READY just before this state
     * transition.
     */
    WAIT_ACK_STOP,

    /**
     * Waiting for AckStartEpoch, for the incremented, current epoch that is at least one higher
     * than the epoch just before the transition. Typically, an RC record transitions from
     * WAIT_ACK_STOP(n) to WAIT_ACK_START(n+1), where n and n+1 are epoch numbers. This state is
     * currently not used as we directly skip from WAIT_ACK_STOP to READY.
     */
    WAIT_ACK_START,

    /**
     * Waiting for AckDropEpoch for the previous epoch. This state is not currently used explicitly
     * because we transition directly from WAIT_ACK_START to READY because reconfiguration is
     * technically complete when a majority of AckStartEpochs have been received, so we simply
     * transition to READY while issuing DropEpoch requests for the previous epoch lazily. Dropping
     * state for the previous epoch is needed only for garbage collection and does not impact
     * safety.
     */
    READY_READY,

    /**
     * This is a pending delete state, i.e., we expect a deletion to occur in the near future, but
     * we can not delete it yet because WaitAckDropEpoch may not have yet completed.
     */
    WAIT_DELETE
  };
  ...
  private ReconfigurationConfig.ReconfigureUponActivesChange policy =
   ...
   * @param newActives
   */
  public ReconfigurationRecord(String name, int epoch, Set<NodeIDType> newActives) {
    this(
        name,
        epoch,
        null,
  ...
  public ReconfigurationRecord(
    ...
    if (name.equals(AbstractReconfiguratorDB.RecordNames.RC_NODES.toString()))
      for (NodeIDType node : newActives) this.rcEpochs.put(node, 0);
    this.policy = policy;
  }
  ...
  public JSONObject toJSONObject() throws JSONException {
    ...
    json.put(Keys.NUM_UNCLEAN.toString(), this.numPossiblyUncleanReconfigurations);
   ...
   * @throws JSONException
   */
  public ReconfigurationRecord(JSONObject json, Stringifiable<NodeIDType> unstringer)
      throws JSONException {
    ...
    this.actives =
        json.has(Keys.ACTIVES.toString())
            ? toSet(json.getJSONArray(Keys.ACTIVES.toString()))
            : null;
    this.newActives = toSet(json.getJSONArray(Keys.NEW_ACTIVES.toString()));
    this.merged =
        json.has(Keys.MERGED.toString())
            ? toStringSet(json.getJSONArray(Keys.MERGED.toString()))
            : null;
    this.toMerge =
        json.has(Keys.MERGE_TASKS.toString())
            ? toStringSet(json.getJSONArray(Keys.MERGE_TASKS.toString()))
            : null;
    this.rcEpochs =
        this.jsonArrayToMap(json.getJSONArray(Keys.RC_EPOCH_MAP.toString()), unstringer);
    this.deleteTime =
        (json.has(Keys.DELETE_TIME.toString()) ? json.getLong(Keys.DELETE_TIME.toString()) : null);
    this.numPossiblyUncleanReconfigurations = json.getInt(Keys.NUM_UNCLEAN.toString());
    this.policy =
        ReconfigurationConfig.ReconfigureUponActivesChange.valueOf(
            json.optString(
  ...
  public Set<NodeIDType> getActiveReplicas(String name, int epoch) {
    if ((this.name.equals(name) && this.epoch == epoch)) return this.getActiveReplicas();
    else
      // should never get called otherwise
      assert (false) : name + ":" + epoch + " != " + this.name + ":" + this.epoch;
    return null;
  }

  /**
   * @return Set of current active replicas. Will return null if this record is pending a delete,
   *     will return newActives if it is awaiting the start of the next epoch, and the current
   *     actives otherwise.
   */
  public Set<NodeIDType> getActiveReplicas() {
    Set<NodeIDType> activeReplicas =
        !this.getState().equals(RCStates.WAIT_DELETE) ? this.actives : null;
   ...
   */
  public ReconfigurationRecord<NodeIDType> putActiveReplicas(
      String name, int epoch, Set<NodeIDType> arSet) {
  ...
  /**
   * @return True if ready for the next epoch (but may not have dropped previous epoch final state
   *     yet).
   */
  public boolean isReady() {
    return this.state.equals(RCStates.READY) || this.state.equals(RCStates.READY_READY);
  }

  /**
   * @return True if the new active replica group is ready and aggressive reconfigurations are
   *     allowed or the previous epoch final state has been completely dropped (a stronger
   *     condition), and in either case, merges are all done.
   */
  public boolean isReconfigurationReady() {
    return
    // unclean ready
    ((this.state.equals(RCStates.READY)
                && ReconfigurationConfig.aggressiveReconfigurationsAllowed())
            // clean ready
  ...
  public boolean addToMerge(String s) {
    if (this.toMerge == null) this.toMerge = new HashSet<String>();
  ...
  public void clearMerged() {
    ...
    if (this.toMerge != null) this.toMerge.clear();
  }
  ...
  public boolean areMergesAllDone() {
    if (this.toMerge == null) return true;
  ...
  public ReconfigurationRecord<NodeIDType> setState(String name, int epoch, RCStates state) {
    ...
    // READY to READY_READY => clean
        ...
        ((epoch == this.epoch && !this.state.equals(RCStates.READY_READY))
            ||
            // FIXME: needed if !TWO_PAXOS_RC?
            (epoch - this.epoch == 1 && this.state.equals(RCStates.READY_READY))))
    ...
    if (this.numPossiblyUncleanReconfigurations < 0) this.numPossiblyUncleanReconfigurations = 0;

    this.epoch = epoch;
    this.state = state;

    if (state.equals(RCStates.WAIT_DELETE)) this.deleteTime = System.currentTimeMillis();
   ...
   * @param mergees
   */
  public void setStateMerge(String name, int epoch, RCStates state, Set<String> mergees) {
    this.setState(name, epoch, state);
    if (mergees != null) for (String mergee : mergees) this.addToMerge(mergee);
  }
  ...
  public boolean isDeletable() {
    ...
    return this.state.equals(RCStates.WAIT_DELETE)
        ...
        this.getUnclean() == 0
            ...
            || System.currentTimeMillis() - this.deleteTime
  ...
  /**
   * To get the RC epoch upon a node config change completion. Used only when reconfiguring
   * reconfigurators.
   *
   * @param rcGroupName
  ...
  public Integer getRCEpoch(String rcGroupName) {
    ...
    throw new RuntimeException(
        "Unable to obtain RC epoch for " + rcGroupName + "; rcEpochs = " + this.rcEpochs);
  }

  /**
   * @param rcGroupName
   * @return The epoch number for the reconfigurator group name. Inserts default of 0 if no entry
   *     found, which can only happen if the inserted value before a crash was not checkpointed.
   */
  public Integer getRCEpoch(NodeIDType rcGroupName) {
    for (NodeIDType node : this.rcEpochs.keySet()) {
      if (node.equals(rcGroupName)) return this.rcEpochs.get(node);
    }
   ...
   * @param deleteNodes
   */
  public void setRCEpochs(
      Set<NodeIDType> affectedRCs, Set<NodeIDType> addNodes, Set<NodeIDType> deleteNodes) {
    for (NodeIDType node : affectedRCs) {
      if (this.rcEpochs.containsKey(node)) {
        if (!deleteNodes.contains(node)) this.rcEpochs.put(node, this.rcEpochs.get(node) + 1);
  ...
  public void trimRCEpochs() {
    if (!this.getName().equals(AbstractReconfiguratorDB.RecordNames.RC_NODES.toString())) return;
    for (Iterator<NodeIDType> iterator = this.rcEpochs.keySet().iterator(); iterator.hasNext(); ) {
      if (!this.newActives.contains(iterator.next())) iterator.remove();
    }
  }

  private JSONArray mapToJSONArray(Map<NodeIDType, Integer> map) throws JSONException {
  ...
  private HashMap<NodeIDType, Integer> jsonArrayToMap(
      JSONArray jArray, Stringifiable<NodeIDType> unstringer) throws JSONException {
    HashMap<NodeIDType, Integer> map = new HashMap<NodeIDType, Integer>();
    for (int i = 0; i < jArray.length(); i++) {
      JSONObject json = jArray.getJSONObject(i);
      map.put(
          unstringer.valueOf(json.getString(Keys.RC_NODE.toString())),
  ...
  public String getSummary() {
    return this.getName()
        ...
        + ":"
        + this.getState()
        + (this.actives != null && !this.actives.equals(this.newActives)
            ? ":" + this.actives + "->" + this.newActives
            : this.actives)
        + (!this.areMergesAllDone()
            ? ":!merged[" + this.merged + "!=" + this.toMerge + "]"
            : !this.isToMergeNull() && !this.toMerge.isEmpty()
                ? " merged[" + this.merged + "==" + this.toMerge + "]"
                : "");
  }
   ...
   */
  public ReconfigurationConfig.ReconfigureUponActivesChange
  ...
  static void main(String[] args) {
    try {
      ...
      ReconfigurationRecord<Integer> rr2 =
          new ReconfigurationRecord<Integer>(
      ...
      System.out.println(
          rr1.jsonArrayToMap(rr1.mapToJSONArray(map), new StringifiableDefault<Integer>(0)));
    } catch (JSONException je) {
      je.printStackTrace();
    }
  }
