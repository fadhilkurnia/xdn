...
/**
 * @author V. Arun
 * @param <NodeIDType> This class is the main wrapper around active replicas of reconfigurable app
 *     instances. It processes requests both from the Reconfigurator as well as the underlying app's
 *     clients. This class is also use to wrap the Reconfigurator itself in order to reconfigure
 *     Reconfigurators (to correctly perform reconfigurator add/remove operations).
 *     <p>This class handles the following ReconfigurationPackets: {@code STOP_EPOCH, START_EPOCH,
 *     REQUEST_EPOCH_FINAL_STATE, DROP_EPOCH} as also listed in {@link ActiveReplicaProtocolTask
 *     ActiveReplicaProtocolTask}. It relies upon the app's implementation of AbstractDemandProfile
 *     in order to determine how frequenltly to report demand statistics to Reconfigurators and
 *     whether and how to reconfigure the current active replica placement.
 */
public class ActiveReplica<NodeIDType>
    implements ReconfiguratorCallback, PacketDemultiplexer<Request>, ActiveReplicaFunctions {
  /**
   * Offset for client facing port that may in general be different from server-to-server
   * communication as we may need different transport-layer security schemes for server-server
   * compared to client-server communication.
   *
   * <p>The commented DEFAULT_CLIENT_PORT_OFFSET field below has been moved to
  ...
  private ActiveReplica(
      AbstractReplicaCoordinator<NodeIDType> appC,
    ...
    this.protocolTask = new ActiveReplicaProtocolTask<NodeIDType>(getMyID(), this.nodeConfig, this);
    ...
    if (ReconfigurationConfig.getClientSSLMode() != SSL_MODES.CLEAR) initClientMessenger(true);
  ...
  protected static AbstractReplicaCoordinator<?> wrapCoordinator(
    ...
    Class<?> clazz = null;
    try {
      clazz = Class.forName(Config.getGlobalString(RC.COORDINATOR_WRAPPER));
    } catch (ClassNotFoundException e) {
      // eat up exception, normal case
    }
    if (clazz == null) return coordinator;
    // reflectively instantiate
    try {
      return (AbstractReplicaCoordinator<?>)
          clazz.getConstructor(AbstractReplicaCoordinator.class).newInstance(coordinator);
  ...
  public String preCheckpoint(String name) {
    return name.equals(AbstractReconfiguratorDB.RecordNames.AR_AR_NODES.toString())
        ? this.nodeConfig.getActiveReplicasReadOnly().toString()
        : name.equals(AbstractReconfiguratorDB.RecordNames.AR_RC_NODES.toString())
            ? this.nodeConfig.getReconfiguratorsReadOnly().toString()
  ...
  private static Map<String, InetSocketAddress> sockAddrMapFromStringified(String state) {
    ...
    for (String entry : state.replaceAll("\\{", "").replaceAll("\\}", "").split(",")) {
      String[] pieces = entry.split("=");
      assert (pieces.length == 2);
      map.put(pieces[0].trim(), Util.getInetSocketAddressFromString(pieces[1].trim()));
    }
    return map;
  }
  ...
  private void addMissingToNodeConfig(Map<String, InetSocketAddress> map, boolean isReconfigurator)
      throws IOException {
    for (String strNode : map.keySet())
      if (!this.nodeConfig.nodeExists(this.nodeConfig.valueOf(strNode))) {
        if (isReconfigurator)
          this.nodeConfig.addReconfigurator(this.nodeConfig.valueOf(strNode), map.get(strNode));
        else this.nodeConfig.addActiveReplica(this.nodeConfig.valueOf(strNode), map.get(strNode));
        ...
        if (propertiesFile != null)
          UtilServer.writeProperty(
              ...
              propertiesFile,
              isReconfigurator
                  ? ReconfigurationConfig.DEFAULT_RECONFIGURATOR_PREFIX.toString()
                  : PaxosConfig.DEFAULT_SERVER_PREFIX.toString());
      }
  }

  private void deleteFromNodeConfig(Map<String, InetSocketAddress> map) throws IOException {
    this.deleteFromNodeConfig(map, false);
  }

  private void deleteFromNodeConfig(Map<String, InetSocketAddress> map, boolean isReconfigurator)
      throws IOException {
    for (NodeIDType node :
        (isReconfigurator
            ? this.nodeConfig.getReconfigurators()
            : this.nodeConfig.getActiveReplicas()))
      if (!map.containsKey(node.toString())) {
        if (isReconfigurator) this.nodeConfig.removeReconfigurator(node);
        else this.nodeConfig.removeActiveReplica(node);
        log.log(
            Level.INFO,
            "{0} removing {1} {2}",
            new Object[] {this, isReconfigurator ? "reconfigurator" : "active replica", node});
        String propertiesFile = PaxosConfig.getPropertiesFile();
        if (propertiesFile != null)
          UtilServer.writeProperty(
              node.toString(),
  ...
  private ReconfigurableAppInfo getReconfigurableAppInfo() {
    return new ReconfigurableAppInfo() {
      ...
      public Set<String> getReplicaGroup(String serviceName) {
        ...
        for (NodeIDType node : ActiveReplica.this.appCoordinator.getReplicaGroup(serviceName))
  ...
  class SenderAndRequest implements ExecutedCallback {
    ...
    SenderAndRequest(
        Request request, InetSocketAddress isa, InetSocketAddress mysa, long recvTime) {
      this.csa = isa;
  ...
  /**
   * The outstanding queue is no longer used as ENQUEUE_REQUEST is false by default. So this timeout
   * value doesn't really matter for anything.
   */
  private static final long REQUEST_TIMEOUT = 5000;

  private final GCConcurrentHashMap<Long, SenderAndRequest> outstanding =
  ...
  private SenderAndRequest enqueue(SenderAndRequest senderAndRequest) {
    return this.outstanding.put(
        ((RequestIdentifier) (senderAndRequest.request)).getRequestID(), senderAndRequest);
  }
  ...
  private SenderAndRequest dequeueAndSendResponse(RequestIdentifier request) {
    return this.sendResponse((Request) request, this.outstanding.remove(request.getRequestID()));
  }

  private SenderAndRequest sendResponse(Request request, SenderAndRequest senderAndRequest) {
    return this.sendResponseAndDemandStats(
        request,
        senderAndRequest,
        senderAndRequest != null ? senderAndRequest.isCoordinated : true);
  }

  private ClientRequest makeClientResponse(Request response, long requestID) {
    return response == null
            || response instanceof ClientRequest
                && ((ClientRequest) response).getRequestID() == requestID
        ? (ClientRequest) response
        : ReplicableClientRequest.wrap(response, requestID);
  }

  // used for both local and coordinated requests
  private SenderAndRequest sendResponseAndDemandStats(
      Request request, SenderAndRequest senderAndRequest, boolean isCoordinated) {
    if (senderAndRequest == null) return null;
    // else
    // send demand report
    this.updateDemandStats(request, senderAndRequest.csa.getAddress());
    instrumentNanoApp(
        isCoordinated ? Instrument.replicable : Instrument.local, senderAndRequest.recvTime);
    ...
    if (request instanceof ClientRequest
      ...
      try {
        ...
        if (written > 0)
          if (isCoordinated) AppInstrumenter.sentResponseCoordinated(request);
          else AppInstrumenter.sentResponseLocal(request);
  ...
  /**
   * The interval to send {@link HelloRequest} to update the NIO socket address on the other
   * replicas.
   */
  ...
  private class HelloRunnable implements Runnable {
    ...
    public void run() {
        ...
        if (!id.toString().equals(myID.toString())) {
          ...
          try {
            this.messenger.sendToAddress(
                (InetSocketAddress) nodeConfig.getActiveReplicasMap().get(id.toString()),
                request.toBytes());
          } catch (IOException e) {
      ...
      for (Object id : nodeConfig.getReconfiguratorsReadOnly().keySet()) {
        ...
        HelloRequest<NodeIDType> request = new HelloRequest(this.messenger.getMyID().toString());
        try {
          this.messenger.sendToAddress(addr, request.toBytes());
        } catch (IOException e) {
          // TODO Auto-generated catch block
          e.printStackTrace();
        }
  ...
  protected void initInstrumenter() {
    if (Config.getGlobalBoolean(RC.ENABLE_INSTRUMENTATION))
      this.protocolExecutor.scheduleWithFixedDelay(
          new Runnable() {
            public void run() {
              System.out.println(DelayProfiler.getStats() + AppInstrumenter.getStats());
            }
          },
          0,
          5,
  ...
  public boolean handleMessage(Request incoming, NIOHeader header) {
    ...
    log.log(
        ...
        new Object[] {this, incoming.getSummary(log.isLoggable(debug))});
    ...
    try {
      ...
      if (rcPacket != null) {
        ...
        if (!this.protocolExecutor.handleEvent(rcPacket))
          ...
          log.log(
              ...
              new Object[] {this, incoming.getSummary(log.isLoggable(unhandled))});
      }
      // else must be app request
        ...
        if (ENQUEUE_REQUEST) if (isCoordinatedRequest) enqueue(senderAndRequest);

        // app doesn't understand ReplicableClientRequest
        if (!isCoordinatedRequest && request instanceof ReplicableClientRequest)
          request = this.appCoordinator.getRequest((ReplicableClientRequest) request, header);

        // send to app via its coordinator
        boolean handled =
            this.handRequestToApp(
        ...
        if (handled) {
          ...
          if (isCoordinatedRequest) this.dequeue(((ReplicableRequest) request));
          ...
          ((JSONMessenger<?>) this.messenger)
              .sendClient(
                  ...
                  new ActiveReplicaError(
                      this.nodeConfig.getNodeSocketAddress(getMyID()),
        ...
        if (instrument(Instrument.getStats)) {
          ...
          log.log(Level.INFO, "{0} {1}", new Object[] {this, DelayProfiler.getStats()});
        }
      }
  ...
  private Request getRequest(JSONObject jsonObject) throws RequestParseException, JSONException {
    ...
    if (jsonObject instanceof JSONMessenger.JSONObjectWrapper)
      try {
        ...
        Request request =
            this.appCoordinator.getRequest(
                Arrays.copyOfRange(bytes, NIOHeader.BYTES, bytes.length),
    ...
    // else slow path
    String stringified =
        STAMP_SENDER_ADDRESS_JSON || !jsonObject.has(MessageExtractor.STRINGIFIED)
            ? jsonObject.toString()
  ...
  private static final boolean STAMP_SENDER_ADDRESS_JSON =
      Config.getGlobalBoolean(RC.STAMP_SENDER_ADDRESS_JSON);
  ...
  private void executedInternal(
      Request request, boolean handled, SenderAndRequest senderAndRequest) {
    if (this.isRecovering()) return;
  ...
  class AckStopNotifier implements Runnable {
    ...
    public void run() {
      ...
      while ((stopEpoch = callbackMap.notifyStop(request.getServiceName(), epoch)) != null) {
        appCoordinator.deleteReplicaGroup(request.getServiceName(), epoch);
  ...
  /**
   * @return Set of packet types processed by ActiveReplica on the server port. These include active
   *     replica types plus app or app coordinator types allowed on the server port.
   */
  protected Set<IntegerPacketType> getPacketTypes(boolean allow) {
    Set<IntegerPacketType> types =
        allow
            ? this.appCoordinator.getRequestTypes()
            : this.appCoordinator.getCoordinatorRequestTypes();
  ...
  protected Set<IntegerPacketType> getPacketTypes() {
    return getPacketTypes(Config.getGlobalBoolean(RC.ALLOW_APP_TYPES_ON_SERVER_PORT));
  }
  ...
  /**
   * Will spawn FetchEpochFinalState to fetch the final state of the previous epoch if one existed,
   * else will locally create the current epoch with an empty initial state.
   *
   * @param event
   * @param ptasks
   * @return Messaging task, typically null as we spawn a protocol task to fetch the previous
   *     epoch's final state.
   */
  public GenericMessagingTask<NodeIDType, ?>[] handleStartEpoch(
    ...
    AckStartEpoch<NodeIDType> ackStart =
        new AckStartEpoch<NodeIDType>(
            startEpoch.getSender(), startEpoch.getServiceName(),
            startEpoch.getEpochNumber(), getMyID());
    GenericMessagingTask<NodeIDType, ?>[] mtasks =
        (new GenericMessagingTask<NodeIDType, AckStartEpoch<NodeIDType>>(
                startEpoch.getSender(), ackStart))
            .toArray();
    ...
    // if no previous group, create replica group with empty state
        ...
        || AbstractReconfiguratorDB.RecordNames.AR_AR_NODES
            .toString()
            .equals(startEpoch.getServiceName())
      ...
      assert (!created
              || startEpoch
                  .getCurEpochGroup()
                  .equals(this.appCoordinator.getReplicaGroup(startEpoch.getServiceName())))
          : this
  ...
  private synchronized void spawnWaitEpochFinalState(StartEpoch<NodeIDType> startEpoch) {
    WaitEpochFinalState<NodeIDType> waitFinal =
        new WaitEpochFinalState<NodeIDType>(getMyID(), startEpoch, this.appCoordinator);
    if (!this.protocolExecutor.isRunning(waitFinal.getKey()))
      this.protocolExecutor.spawn(waitFinal);
    else {
      WaitEpochFinalState<NodeIDType> running =
          (WaitEpochFinalState<NodeIDType>) this.protocolExecutor.getTask(waitFinal.getKey());
      if (running != null) running.addNotifiee(startEpoch.getInitiator(), startEpoch.getKey());
    }
  }
   ...
   * @param stopEpoch
   * @param ptasks
   * @return Messaging task, typically null as we coordinate the stop request and use a callback to
   *     notify the reconfigurator that issued the {@link StopEpoch}.
   */
  public GenericMessagingTask<NodeIDType, ?>[] handleStopEpoch(
    ...
    if (!stopEpoch.shouldExecuteStop()) return null;
    // else coordinate stop with callback
    this.callbackMap.addStopNotifiee(stopEpoch);
    ReconfigurableRequest appStop =
        this.getAppStopRequest(stopEpoch.getServiceName(), stopEpoch.getEpochNumber());
    log.log(
        Level.INFO,
    ...
    this.appCoordinator.handleIncoming(
        makeClientRequest(appStop, this.nodeConfig.getNodeSocketAddress(stopEpoch.getInitiator())),
        this);
    return null; // need to wait until callback
  }

  private static final Request makeClientRequest(Request request, InetSocketAddress csa) {

    if (request instanceof ReplicableClientRequest)
      return ((ReplicableClientRequest) request).setClientAddress(csa);
    else if (request instanceof RequestPacket) return request;

    // else
    if (!(request instanceof ClientRequest)
        || !(request instanceof ReplicableRequest
            && ((ReplicableRequest) request).needsCoordination())
        || request instanceof ReconfigurationPacket) return request;
   ...
   * @param event
   * @param ptasks
   * @return Messaging task to send AckDropEpochFinalState to reconfigurator that issued the
   *     corresponding DropEpochFinalState.
   */
  public GenericMessagingTask<NodeIDType, ?>[] handleDropEpochFinalState(
      DropEpochFinalState<NodeIDType> event,
      ProtocolTask<NodeIDType, ReconfigurationPacket.PacketType, String>[] ptasks) {
    Level level =
        event.getServiceName().equals(AbstractReconfiguratorDB.RecordNames.RC_NODES.toString())
                || event
                    .getServiceName()
    ...
    boolean deleted =
        this.appCoordinator.deleteFinalState(
            dropEpoch.getServiceName(), dropEpoch.getEpochNumber());
    AckDropEpochFinalState<NodeIDType> ackDrop =
        new AckDropEpochFinalState<NodeIDType>(getMyID(), dropEpoch);
    GenericMessagingTask<NodeIDType, AckDropEpochFinalState<NodeIDType>> mtask =
        new GenericMessagingTask<NodeIDType, AckDropEpochFinalState<NodeIDType>>(
            dropEpoch.getInitiator(), ackDrop);
    assert (ackDrop.getInitiator().equals(dropEpoch.getInitiator()));
    log.log(
        deleted ? level : Level.FINE,
        "{0} {1} sending {2} to {3}",
        new Object[] {this, deleted ? "" : "*NOT*", ackDrop.getSummary(), ackDrop.getInitiator()});
  ...
  public GenericMessagingTask<NodeIDType, ?>[] handleHelloRequest(
    ...
    ReconfigurationConfig.log.log(
        Level.FINE, "NodeConfig gets updated to {0}", new Object[] {nodeConfig});
    return null;
  }
      ...
      try {
        ((JSONMessenger<?>) this.messenger)
            .sendClient(
                ...
                echo.makeResponse(this.nodeConfig.getNodeSocketAddress(getMyID()))
                // .toJSONObject()
                ,
                echo.myReceiver);
  ...
  public GenericMessagingTask<NodeIDType, ?>[] handleSetCoordinatorNodeRequest(
    ...
    try {
      this.appCoordinator.coordinateRequest(
          request,
          (requestWithResponse, handled) -> {
            ...
            SetCoordinatorNodeResponse<NodeIDType> response =
                new SetCoordinatorNodeResponse<>(
                    ...
                    getMyID(),
                    request.getServiceName(),
                    request.getEpochNumber(),
                    crp.isFailed() ? 500 : 200,
  ...
  private void garbageCollectPendingTasks(DropEpochFinalState<NodeIDType> dropEpoch) {
     ...
     * epoch after all. */
    boolean removed =
        (this.protocolExecutor.remove(
                Reconfigurator.getTaskKeyPrev(
                    WaitEpochFinalState.class, dropEpoch, this.getMyID().toString()))
            != null);
    if (removed)
      log.log(
          Level.FINEST,
          "{0} removed WaitEpochFinalState {1}:{2}",
          new Object[] {this, dropEpoch.getServiceName(), (dropEpoch.getEpochNumber() - 1)});
  }

  /**
   * @param event
   * @param ptasks
   * @return Messaging task returning the requested epoch final state to the requesting
   *     ActiveReplica.
   */
  public GenericMessagingTask<NodeIDType, ?>[] handleRequestEpochFinalState(
    ...
    this.logEvent(event);
    StringContainer stateContainer =
        this.getFinalStateContainer(request.getServiceName(), request.getEpochNumber());
    ...
    EpochFinalState<NodeIDType> epochState =
        new EpochFinalState<NodeIDType>(
    ...
    log.log(
        Level.INFO,
        "{0} returning epoch final state to {1} {2}",
        new Object[] {this, event.getKey(), epochState.getSummary()});
    mtask =
        new GenericMessagingTask<NodeIDType, EpochFinalState<NodeIDType>>(
    ...
    if (retval)
      ...
      log.log(
          ...
          new Object[] {
            ...
            epoch,
            this.appCoordinator.getReplicaGroup(packet.getServiceName()),
            packet.getServiceName(),
            packet.getEpochNumber()
          });
    return retval;
  }
  ...
  private boolean alreadyMovedOn(BasicReconfigurationPacket<?> packet) {
    Integer epoch = this.appCoordinator.getEpoch(packet.getServiceName());
    if (epoch != null)
      if (epoch - packet.getEpochNumber() > 0) return true;
       ...
       * final state that must be necessarily outdated at this point. */
        ...
        // active epoch
        return (this.appCoordinator.getReplicaGroup(packet.getServiceName()) != null);
    return false;
  }
  ...
  private Set<IntegerPacketType> getAppRequestTypes() {
    return cachedTypes != null
        ? cachedTypes
        : ((cachedTypes = this.appCoordinator.getAppRequestTypes()) != null
            ? cachedTypes
  ...
  private boolean isAppRequest(JSONObject jsonObject) throws JSONException {
    ...
    Integer type =
        jsonObject instanceof JSONMessenger.JSONObjectWrapper
            ...
            (JSONPacket.couldBeJSON(
                    bytes = (byte[]) (((JSONMessenger.JSONObjectWrapper) jsonObject).getObj()),
                    NIOHeader.BYTES)
                ? 0
                :
    ...
    if (appTypesAsIntegers == null) appTypesAsIntegers = toIntegerSet(this.getAppRequestTypes());
  ...
  private static final Set<Integer> toIntegerSet(Set<IntegerPacketType> types) {
    ...
    for (IntegerPacketType type : types) integers.add(type.getInt());
    return integers;
  }
  ...
  private static final boolean instrument(Instrument param) {
    if (!ENABLE_INSTRUMENTATION) return false;
    else if (param == Instrument.getStats)
      if (System.currentTimeMillis() - lastDumpedTime < MIN_INTER_DUMP_TIME) return false;
      else {
        lastDumpedTime = System.currentTimeMillis();
        return true;
      }
    else return instrument(param.val);
  }

  private static final void instrumentNano(Instrument param, long t) {
    if (instrument(param.val)) DelayProfiler.updateDelayNano(param.toString(), t);
  }

  private static final void instrumentNanoApp(Instrument param, long t) {
    if (instrument(param.val)) DelayProfiler.updateDelayNano(appName + "." + param.toString(), t);
  }

  @SuppressWarnings("unused")
  private static final void instrumentApp(Instrument param, long t) {
    if (instrument(param.val)) DelayProfiler.updateDelay(appName + "." + param.toString(), t);
  }
  ...
  private void report(AbstractDemandProfile demand) {
    try {
      ...
      GenericMessagingTask<NodeIDType, ?> mtask =
          new GenericMessagingTask<NodeIDType, Object>(
              reportee,
              (new DemandReport<NodeIDType>(
                  getMyID(), demand.getName(), (epoch == null ? 0 : epoch), demand)));
      this.send(mtask);
    } catch (Exception je) {
      je.printStackTrace();
    }
  ...
  private void send(InetSocketAddress isa, JSONObject msg) {
    try {
      ((AddressMessenger<JSONObject>) this.messenger).sendToAddress(isa, msg);
    } catch (IOException ioe) {
      ioe.printStackTrace();
    }
  }
  ...
  private GenericMessagingTask<NodeIDType, ?> sendAckStopEpoch(StopEpoch<NodeIDType> stopEpoch) {
    ...
    AckStopEpoch<NodeIDType> ackStopEpoch =
        new AckStopEpoch<NodeIDType>(
            this.getMyID(),
            stopEpoch,
            (stopEpoch.shouldGetFinalState()
                ? this.appCoordinator.getFinalState(
                    stopEpoch.getServiceName(), stopEpoch.getEpochNumber())
                : null));
    GenericMessagingTask<NodeIDType, ?> mtask =
        new GenericMessagingTask<NodeIDType, Object>((stopEpoch.getInitiator()), ackStopEpoch);
    log.log(
        Level.FINE,
        "{0} sending {1} to {2}",
        new Object[] {this, ackStopEpoch.getSummary(), stopEpoch.getInitiator()});
    this.send(mtask);
    return mtask;
  }

  private ReconfigurableRequest getAppStopRequest(String name, int epoch) {
    ReconfigurableRequest appStop = this.appCoordinator.getStopRequest(name, epoch);
    return appStop == null ? new DefaultAppRequest(name, epoch, true) : appStop;
  }
  ...
  private AddressMessenger<?> initClientMessenger(boolean ssl) {
    ...
    try {
      ...
      if ((ssl ? getClientFacingSSLPort(myPort) : getClientFacingClearPort(myPort)) != myPort) {
          ...
          cMsgr =
              new JSONMessenger<InetSocketAddress>(
                  (niot =
                      ...
                      .setName(
                          this.appCoordinator.app.toString()
                              + ":"
                              + (ssl ? "SSL" : "")
                              + "ClientMessenger"));
          ...
          if (this.messenger.getClientMessenger() instanceof Messenger)
            ((Messenger<NodeIDType, ?>) existing)
                .addPacketDemultiplexer(
                    pd =
                        new ReconfigurationPacketDemultiplexer(this.nodeConfig, this.appCoordinator)
        ...
        appTypes.removeAll(this.appCoordinator.getMutualAuthAppRequestTypes());
  ...
  private void logEvent(BasicReconfigurationPacket<NodeIDType> event, Level level) {
    log.log(
        level,
        "{0} received {1} from {2}",
        new Object[] {this, event.getSummary(), event.getSender()});
  ...
  public boolean preExecuted(Request request) {
    return request
                .getServiceName()
                .equals(AbstractReconfiguratorDB.RecordNames.AR_AR_NODES.toString())
            || request
                .getServiceName()
                .equals(AbstractReconfiguratorDB.RecordNames.AR_RC_NODES.toString())
