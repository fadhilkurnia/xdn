...
/**
 * @author V. Arun
 *     <p>A class like this is necessary if the app wants to use its own packet types to send
 *     requests and responses (as opposed to just using {@link RequestPacket}. The main requirements
 *     of application requests is that they must implement at least InterfaceRequest, and must
 *     implement InterfaceReplicable, InterfaceReconfigurable, and InterfceClientRequest in order to
 *     respectively support replication, reconfiguration, and the ability to delegate client
 *     response messaging to gigapaxos.
 */
public class AppRequest extends JSONPacket
    implements ReconfigurableRequest, ReplicableRequest, ClientRequest {
  ...
  public AppRequest(String name, String value, IntegerPacketType type, boolean stop) {
    this(name, 0, (int) (Math.random() * Integer.MAX_VALUE), value, type, stop);
  }
   ...
   */
  public AppRequest(
      String name, int epoch, long id, String value, IntegerPacketType type, boolean stop) {
    super(type);
    this.name = name;
    this.epoch = epoch;
    this.id = id;
   ...
   * @param stop
   */
  public AppRequest(String name, long id, String value, IntegerPacketType type, boolean stop) {
  ...
  public AppRequest(String value, AppRequest req) {
    this(req.name, req.epoch, req.id, value, PacketType.getPacketType(req.type), req.stop);
  ...
  public AppRequest(JSONObject json) throws JSONException {
    ...
    this.coordType =
        (json.has(Keys.COORD.toString()) ? json.getBoolean(Keys.COORD.toString()) : false);
    ...
    this.clientAddress =
        json.has(Keys.CSA.toString())
            ? Util.getInetSocketAddressFromString(json.getString(Keys.CSA.toString()))
            : isa;

    this.response = json.has(Keys.RVAL.toString()) ? json.getString(Keys.RVAL.toString()) : null;
  }
  ...
  public JSONObject toJSONObjectImpl() throws JSONException {
    ...
    if (this.clientAddress != null) json.put(Keys.CSA.toString(), this.clientAddress.toString());
  ...
  public boolean needsCoordination() {
    return (this.type != AppRequest.PacketType.META_REQUEST_PACKET.getInt()) && this.coordType;
  }
  ...
  /**
   * An oblivious stop request is a no-op stop request that is not actually passed to {@link
   * Replicable#execute(Request)}. It is generally not a good idea to use an oblivious stop request
   * as the app won't know that the current epoch has been stopped and will not be in a position to
   * do any garbage collection if needed right away.
   *
   * <p>This method is deprecated. gigapaxos currently generates its own oblivous stop request and
   * subsequently invokes {@link Replicable#restore(String, String)} with a null state argument to
   * indicate to the app that the current epoch is over. For coordination protocols other than
   * paxos, {@link Reconfigurable#getStopRequest(String, int)} suffices.
   *
   * @param name
   * @param epoch
  ...
  public static void main(String[] args) {
    AppRequest request =
        new AppRequest("name1", 0, 0, "request1", AppRequest.PacketType.DEFAULT_APP_REQUEST, false);
