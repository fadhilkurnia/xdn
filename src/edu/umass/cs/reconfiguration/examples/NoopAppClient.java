 ...
 * @author arun
 */
public class NoopAppClient extends ReconfigurableAppClientAsync<Request>
  ...
  private void testSendBunchOfRequests(String name, int numRequests) throws IOException {
    System.out.println("Created " + name + " and beginning to send test requests");
    new Thread(
            new Runnable() {
              public void run() {
                for (int i = 0; i < numRequests; i++) {
                  ...
                  try {
                    NoopAppClient.this.sendRequest(
                        ReplicableClientRequest.wrap(
                            new AppRequest(
                                name,
                                "request_value" + i,
                        ...
                        new RequestCallback() {
                          ...
                          public void handleResponse(Request response) {
                            if (response instanceof ActiveReplicaError) return;
                            // else
                            System.out.println(
                                "Received response: "
                                    + response
   ...
   * Refer to the parent class {@link ReconfigurableAppClientAsync} for other utility methods
   * available to this method or to know how to write your own client.
   *
   * @param args
   * @throws IOException
   */
  public static void main(String[] args) throws IOException {
    ...
    for (int i = 0; i < numNames; i++) {
      final String name = namePrefix + ((int) (Math.random() * Integer.MAX_VALUE));
      System.out.println("Creating name " + name);
      client.sendRequest(
          new CreateServiceName(name, initialState),
          new RequestCallback() {
            ...
            public void handleResponse(Request response) {
              try {
                ...
                else System.out.println(this + " failed to create name " + name);
              } catch (IOException e) {
                e.printStackTrace();
              }
            }
    ...
    synchronized (client) {
      long maxWaitTime = numReqs * INTER_REQUEST_TIME + 4000,
          waitInitTime = System.currentTimeMillis();
