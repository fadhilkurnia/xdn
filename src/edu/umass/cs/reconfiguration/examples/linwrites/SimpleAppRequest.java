...
/**
 * @author arun
 *     <p>A class like this is needed only if the app wants to use request types other than {@link
 *     RequestPacket}, which is generally useful only if the app wants to coordinate only some
 *     request types using consensus but process other requests locally at replicas or coordinate
 *     them using custom replica coordination protocols. For using just gigapaxos to implement
 *     linearizability, i.e., coordinating all requests via consensus-based RSM, this class is
 *     unnecessary as applications can simply encapsulate requests as {@link RequestPacket}.
 */
public class SimpleAppRequest extends JSONPacket implements ReplicableRequest, ClientRequest {
   ...
   * @param stop
   */
  public SimpleAppRequest(String name, String value, IntegerPacketType type, boolean stop) {
   ...
   */
  public SimpleAppRequest(
      String name, long id, String value, IntegerPacketType type, boolean stop) {
  ...
  public static void main(String[] args) {
    SimpleAppRequest request =
        new SimpleAppRequest(
            "name1", 0, 0, "request1", SimpleAppRequest.PacketType.COORDINATED_WRITE, false);
    System.out.println(request);
    try {
      SimpleAppRequest request2 = (new SimpleAppRequest(request.toJSONObject()));
