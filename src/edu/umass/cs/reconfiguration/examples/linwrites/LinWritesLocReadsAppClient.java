...
/**
 * @author arun
 *     <p>A simple client for {@link LinWritesLocReadsApp}.
 */
public class LinWritesLocReadsAppClient extends ReconfigurableAppClientAsync<SimpleAppRequest> {
  ...
  /**
   * A simple example of asynchronously sending a few requests with a callback method that is
   * invoked when the request has been executed or is known to have failed.
   *
   * @param args
   * @throws IOException
  ...
  public static void main(String[] args) throws IOException, InterruptedException {
    LinWritesLocReadsAppClient asyncClient = new LinWritesLocReadsAppClient();
    ...
    for (int i = 0; i < numRequests; i++) {
      ...
      asyncClient.sendRequest(
          ...
          request = i % 2 == 0 ? makeReadRequest() : makeWriteRequest(),
          ...
          new Callback<Request, SimpleAppRequest>() {
            ...
            public SimpleAppRequest processResponse(Request response) {
              assert (response instanceof SimpleAppRequest) : response.getSummary();
              System.out.println(
                  j
