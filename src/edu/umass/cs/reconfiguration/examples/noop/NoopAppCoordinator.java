...
/**
 * @author V. Arun
 *     <p>An explicit coordinator class like this is optional and is needed only of the application
 *     wants to implement its own replica coordination scheme. For applications simply wanting to
 *     use gigapaxos as their replica coordination protocol, a class like this is unnecessary. The
 *     code below shows how to use a replica coordination scheme based on lazy propagation as an
 *     alternative to paxos. Note that the lazy propagation scheme below is not even eventually
...
public class NoopAppCoordinator extends PaxosReplicaCoordinator<String> {
  ...
  NoopAppCoordinator(
    ...
    if (app instanceof NoopApp) ((NoopApp) app).setClientMessenger(msgr);
  }
  ...
  public boolean coordinateRequest(Request request, ExecutedCallback callback)
    ...
    try {
      ...
      if (this.coordType.equals(CoordType.LAZY)) this.sendAllLazy(request);
  ...
  public boolean createReplicaGroup(
      String serviceName, int epoch, String state, Set<String> nodes) {
  ...
  public Set<String> getReplicaGroup(String serviceName) {
    ...
    } else assert (this.coordType.equals(CoordType.PAXOS));
