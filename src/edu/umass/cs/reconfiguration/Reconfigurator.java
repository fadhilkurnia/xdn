...
/**
 * @param <NodeIDType> This class is the main Reconfigurator module. It issues reconfiguration
 *     commands to ActiveReplicas and also responds to client requests to create or delete names or
 *     request the list of active replicas for a name.
 *     <p>It relies on the following helper protocol tasks: {@link WaitAckStopEpoch
 *     WaitAckStopEpoch}, {@link WaitAckStartEpoch}, {@link WaitAckDropEpoch}, {@link CommitWorker},
 *     {@link WaitPrimaryExecution}. The last one is to enable exactly one primary Reconfigurator in
 *     the common case to conduct reconfigurations but ensure that others safely complete the
 *     reconfiguration if the primary fails to do so. CommitWorker is a worker that is needed to
 *     ensure that a paxos-coordinated request eventually gets committed; we need this property to
 *     ensure that a reconfiguration operation terminates, but paxos itself provides us no such
 *     liveness guarantee.
 *     <p>This class now supports add/remove operations for the set of Reconfigurator nodes. This is
 *     somewhat tricky, but works correctly. A detailed, formal description of the protocol is TBD.
 *     The documentation further below in this class explains the main ideas.
 * @author V. Arun
 */
public class Reconfigurator<NodeIDType>
    implements PacketDemultiplexer<Request>, ReconfiguratorCallback, ReconfiguratorFunctions {

  private final SSLMessenger<NodeIDType, JSONObject> messenger;
  private final ProtocolExecutor<NodeIDType, ReconfigurationPacket.PacketType, String>
      protocolExecutor;
  ...
  protected Reconfigurator(
    ...
    this.DB =
        new RepliconfigurableReconfiguratorDB<NodeIDType>(
    ...
    this.protocolExecutor =
        new ProtocolExecutor<NodeIDType, ReconfigurationPacket.PacketType, String>(messenger);
    ...
    if (ReconfigurationConfig.getClientSSLMode() != SSL_MODES.CLEAR) this.initClientMessenger(true);
    ...
    // Initialize the InitialStateValidator, if configured correctly.
    String stateValidatorClassName = Config.getGlobalString(RC.INITIAL_STATE_VALIDATOR_CLASS);
    if (stateValidatorClassName != null && !stateValidatorClassName.isEmpty()) {
      this.configureInitialStateValidator(stateValidatorClassName);
    }
    if (this.initialStateValidator != null) {
      ReconfigurationConfig.log.log(
          Level.INFO,
          "Using initial state validator of {0}",
  ...
  private void initHTTPServer(boolean ssl) {
    if (!Config.getGlobalBoolean(RC.ENABLE_RECONFIGURATOR_HTTP)) return;
    ...
    try {
      ...
      new HttpReconfigurator(
          this,
          new InetSocketAddress(me.getAddress(), ReconfigurationConfig.getHTTPPort(me.getPort())),
          ssl);
  ...
  private void initDnsServer() {
    if (!Config.getGlobalBoolean(RC.ENABLE_RECONFIGURATOR_DNS)) return;
    new DnsReconfigurator(this);
  }

  /**
   * Sets the validator in {@link Reconfigurator#initialStateValidator} variable given the class
   * name. If unsuccessful (e.g., due to invalid class name or non-existent default constructor),
   * the validator stays null and warning messages are logged.
   *
   * @param validatorClassName the class name for InitialStateValidator
   */
  private void configureInitialStateValidator(String validatorClassName) {
    Class<?> clazz;

    // get the class given the name
    try {
      clazz = Class.forName(validatorClassName);
    } catch (ClassNotFoundException e) {
      ReconfigurationConfig.log.log(
          Level.WARNING,
          "Unknown InitialStateValidator with class name of {0}, "
              + "fallback to the default behavior with no initial state validation.",
          validatorClassName);
      return;
    }

    // check if the specified class implement InitialStateValidator
    if (!InitialStateValidator.class.isAssignableFrom(clazz)) {
      ReconfigurationConfig.log.log(
          Level.WARNING,
          "The specified InitialStateValidator with class name of {0}, "
              + "does not implement InitialStateValidator interface, so we fallback "
              + "to the default behavior with no initial state validation.",
          validatorClassName);
      return;
    }

    // check if class has empty constructor
    ...
    try {
      ...
      ReconfigurationConfig.log.log(
          Level.WARNING,
          "The specified InitialStateValidator with class name of {0}, "
              + "does not implement default constructor, so we fallback "
              + "to the default behavior with no initial state validation.",
          validatorClassName);
      return;
    }
    ...
    try {
      this.initialStateValidator = (InitialStateValidator) constructor.newInstance();
  ...
  /**
   * Sets the extractor in {@link Reconfigurator#numReplicasExtractor} given the class name. If
   * unsuccessful, the extractor stays null and warnings are logged; creation falls back to the
   * global DEFAULT_NUM_REPLICAS for every service.
   *
  ...
  private void configureNumReplicasExtractor(String extractorClassName) {
    ...
    try {
      this.numReplicasExtractor = (InitialStateNumReplicasExtractor) constructor.newInstance();
    } catch (InstantiationException | IllegalAccessException | InvocationTargetException e) {
      ReconfigurationConfig.log.log(
          Level.WARNING,
          "The specified InitialStateNumReplicasExtractor with class name of {0}, "
  ...
  /**
   * This treats the reconfigurator itself as an "active replica" in order to be able to reconfigure
   * reconfigurator groups.
   */
  protected ActiveReplica<NodeIDType> getReconfigurableReconfiguratorAsActiveReplica() {
    return new ActiveReplica<NodeIDType>(
        this.DB, this.consistentNodeConfig.getUnderlyingNodeConfig(), this.messenger);
  }

  private static final Level debug = Level.FINE;

  private ConcurrentHashMap<String, NIOHeader> headerMap =
      new ConcurrentHashMap<String, NIOHeader>();

  @Override
  public boolean handleMessage(Request incoming, edu.umass.cs.nio.nioutils.NIOHeader header) {
    try {
      // otherwise demultiplexer wouldn't come here
      assert (incoming instanceof BasicReconfigurationPacket) : incoming.getSummary();
      @SuppressWarnings("unchecked")
      ReconfigurationPacket.PacketType rcType =
          ((BasicReconfigurationPacket<NodeIDType>) incoming).getReconfigurationPacketType();
      ReconfigurationConfig.log.log(
          debug,
          "{0} received {1} {2}",
          new Object[] {
            this, rcType, incoming.getSummary(ReconfigurationConfig.log.isLoggable(debug))
          });
      ...
      ReconfigurationConfig.log.severe(
          this
              + " incurred exception "
              + e.getMessage()
  ...
  /**
   * Incorporates demand reports (possibly but not necessarily with replica coordination), checks
   * for reconfiguration triggers, and initiates reconfiguration if needed.
   *
   * @param report
   * @param ptasks
  ...
  public GenericMessagingTask<NodeIDType, ?>[] handleDemandReport(
    ...
    ReconfigurationConfig.log.log(
        Level.FINEST, "{0} received {1} {2}", new Object[] {this, report.getType(), report});
     ...
     * of reconfigurators. */
    if (!this.consistentNodeConfig
        .getReplicatedReconfigurators(report.getServiceName())
        .contains(this.getMyID())) {
      return new GenericMessagingTask<NodeIDType, DemandReport<NodeIDType>>(
              (NodeIDType)
                  (Util.selectRandom(
                      this.consistentNodeConfig.getReplicatedReconfigurators(
                          report.getServiceName()))),
              report)
          .toArray();
    }
    if (report.needsCoordination()) this.DB.handleIncoming(report, null); // coordinated
    else this.updateDemandProfile(report); // no coordination
    ReconfigurationRecord<NodeIDType> record =
        this.DB.getReconfigurationRecord(report.getServiceName());
    if (record != null) {
      NodeIdsMetadataPair<NodeIDType> result = this.shouldReconfigure2(report.getServiceName());
      if (result != null) {
        this.initiateReconfiguration(
            ...
            /* policy= */ ReconfigurationConfig.ReconfigureUponActivesChange.DEFAULT);
      }
    }
  ...
  private boolean isLegitimateCreateRequest(CreateServiceName create) {
    if (!create.isBatched()) return true;
  ...
  private CreateServiceName[] makeCreateServiceNameBatches(CreateServiceName batchedCreate) {
    CreateServiceName[] creates =
        ReconfigurationConfig.makeCreateNameRequest(
            batchedCreate.nameStates,
  ...
  class BatchCreateJobs {
    ...
    int totalNames() {
      int total = 0;
      for (CreateServiceName part : parts) total += part.size();
      return total;
    }
  }
  ...
  class PendingBatchCreates {
    /**
     * Splits a batch creation jobs into smaller batch creates each of which is consistent. The set
     * {@link BatchCreateJobs#parts} holds those parts.
     */
    private ConcurrentHashMap<String, BatchCreateJobs> consistentBatchCreateJobs =
        new ConcurrentHashMap<String, BatchCreateJobs>();

    /**
     * Maps the headname of a part to the overall head name of the original client-issued batch
     * create request.
     */
    private ConcurrentHashMap<String, String> headnameToOverallHeadname =
        new ConcurrentHashMap<String, String>();
  }

  private void failLongPendingBatchCreate(
      String name, long timeout, Callback<Request, ReconfiguratorRequest> callback) {
    for (Iterator<String> strIter =
            this.pendingBatchCreations.consistentBatchCreateJobs.keySet().iterator();
        strIter.hasNext(); ) {
      ...
      if (System.currentTimeMillis() - jobs.creationTime > timeout) {
        ...
        callback.processResponse(
            new CreateServiceName(nameStates, failedCreates, jobs.parts.iterator().next())
                .setFailed()
                .setResponseMessage(
  ...
  private CreateServiceName updateAndCheckComplete(String batchCreateHeadName) {
    String headName =
        this.pendingBatchCreations.headnameToOverallHeadname.remove(batchCreateHeadName);
    ...
    if (jobs.remaining.isEmpty()) {
      ReconfigurationConfig.log.log(
          Level.INFO,
          "{0} returning completed batch create with head name {1} with {2} parts and {3} total"
              + " constituent names",
  ...
  private String updateAndCheckComplete(StartEpoch<NodeIDType> startEpoch) {
    boolean hasBatchCreatePartJobs =
        this.pendingBatchCreations.headnameToOverallHeadname.containsKey(
            startEpoch.getServiceName());
    CreateServiceName original = this.updateAndCheckComplete(startEpoch.getServiceName());
    if (original != null) return original.getServiceName();
    // remaining batch job parts
    else if (hasBatchCreatePartJobs) return null;
    // no batch job parts to begin with
    else return startEpoch.getServiceName();
  }

  /**
   * Create service name is a special case of reconfiguration where the previous group is
   * non-existent.
   *
   * @param create
   * @param ptasks
  ...
  public GenericMessagingTask<NodeIDType, ?>[] handleCreateServiceName(
    ...
    if (create.isBatched()
        ...
        && !this.pendingBatchCreations.consistentBatchCreateJobs.containsKey(
      ...
      this.pendingBatchCreations.consistentBatchCreateJobs.put(
          ...
          jobs =
              new BatchCreateJobs(
                  create.getServiceName(), this.makeCreateServiceNameBatches(create), create));
      ReconfigurationConfig.log.log(
          Level.INFO,
      ...
      this.protocolExecutor.scheduleSimple(
          new Runnable() {
            @Override
            public void run() {
              Reconfigurator.this.failLongPendingBatchCreate(
                  create.getServiceName(), BATCH_CREATE_TIMEOUT, callback);
            }
          },
    ...
    else if (this.pendingBatchCreations.headnameToOverallHeadname.containsKey(
            create.getServiceName())
        && create.isForwarded()
    ...
    if (!this.isLegitimateCreateRequest(create)) {
      callback.processResponse(
          create
              .setFailed()
              .setResponseMessage(
                  "The names in this create request do not all map to the same "
                      + "reconfigurator group"));
    }

    if (this.processRedirection(create)) {
      if (callback != this.defaultCallback) this.callbacksCRP.put(getCRPKey(create), callback);
      return null;
    }
    ...
    ReconfigurationConfig.log.log(
        Level.FINE,
        "{0} processing {1} from creator {2} {3}",
        new Object[] {
          this,
          create.getSummary(),
          create.getCreator(),
          create.getForwader() != null ? " forwarded by " + create.getForwader() : ""
        });

    // Validate the initial state for this create request, if the InitialStateValidator is set.
    if (this.initialStateValidator != null) {
      try {
        this.initialStateValidator.validateInitialState(create.getInitialState());
      } catch (InitialStateValidator.InvalidInitialStateException e) {
        // the given initial state is invalid, return error message to client
        callback.processResponse(
            create
                .setFailed(ClientReconfigurationPacket.ResponseCodes.MALFORMED_REQUEST)
                .setResponseMessage("Invalid initial state provided. Error: " + e.getMessage()));
        return null;
      }
    }
    ...
    if (this.numReplicasExtractor != null && create.getInitGroup() == null) {
      numReplicasOverride = this.numReplicasExtractor.extract(create.getInitialState());
     ...
     * creation fail later if that is not the case. */
    if ((record = this.DB.getReconfigurationRecord(create.getServiceName())) == null) {
      ...
      if (create.getInitGroup() != null) {
        ...
        initialActives = this.consistentNodeConfig.getReplicatedActives(create.getServiceName());
      }
      ...
      this.initiateReconfiguration(
          ...
          create.getForwader(),
          create.getInitialState(),
          create.getNameStates(),
          null,
  ...
  private static final boolean TWO_PAXOS_RC =
      Config.getGlobalBoolean(ReconfigurationConfig.RC.TWO_PAXOS_RC);

  /**
   * Simply hand over DB request to DB. The only type of RC record that can come here is one
   * announcing reconfiguration completion. Reconfiguration initiation messages are derived locally
   * and coordinated through paxos, not received from outside.
   *
   * @param rcRecReq
   * @param ptasks
  ...
  public GenericMessagingTask<NodeIDType, ?>[] handleRCRecordRequest(
      ...
      ProtocolTask<NodeIDType, ReconfigurationPacket.PacketType, String>[] ptasks) {
    ReconfigurationConfig.log.log(
        Level.FINE, "{0} receievd {1}", new Object[] {this, rcRecReq.getSummary()});
    ...
    if (rcRecReq.isReconfigurationPrevDropComplete()
        && rcRecReq
            .getServiceName()
            .equals(AbstractReconfiguratorDB.RecordNames.RC_NODES.toString())
      ...
      this.sendReconfigureRCNodeConfigConfirmationToInitiator(rcRecReq);
        ...
        && rcRecReq
            .getServiceName()
            .equals(AbstractReconfiguratorDB.RecordNames.AR_NODES.toString()))
    ...
    // single paxos reconfiguration allowed only for non-RC-group names
      ...
      if (rcRecReq.isReconfigurationComplete() || rcRecReq.isReconfigurationPrevDropComplete()) {
        if (rcRecReq.getInitiator().equals(getMyID()))
          mtask =
              new GenericMessagingTask<NodeIDType, RCRecordRequest<NodeIDType>>(
                  getOthers(
                      this.consistentNodeConfig.getReplicatedReconfigurators(
  ...
  Object[] getOthers(Set<NodeIDType> nodes) {
    ...
    for (NodeIDType node : nodes) if (!node.equals(getMyID())) others.add(node);
    return others.toArray();
  }

  /**
   * We need to ensure that both the stop/drop at actives happens atomically with the removal of the
   * record at reconfigurators. To accomplish this, we first mark the record as stopped at
   * reconfigurators, then wait for the stop/drop tasks to finish, and finally coordinate the
   * completion notification so that reconfigurators can completely remove the record from their DB.
   *
   * @param delete
   * @param ptasks
   * @return Messaging task, typically null. No protocol tasks spawned.
  ...
  public GenericMessagingTask<NodeIDType, ?>[] handleDeleteServiceName(
    ...
    ReconfigurationConfig.log.log(
        Level.INFO,
        "{0} received {1} from creator {2}",
        new Object[] {this, delete.getSummary(), delete.getCreator()});

    if (this.processRedirection(delete)) {
      if (callback != defaultCallback) this.callbacksCRP.put(getCRPKey(delete), callback);
      return null;
    }
    ReconfigurationConfig.log.log(
        Level.FINE,
    ...
    ReconfigurationRecord<NodeIDType> record =
        this.DB.getReconfigurationRecord(delete.getServiceName());
  ...
  /**
   * Client reconfiguration packet callbacks. We need callbacks primarily to support other front-end
   * APIs like HTTP and create/delete-like asynchronous requests that return only after
   * coordination.
   */
  private final GCConcurrentHashMap<String, Callback<Request, ReconfiguratorRequest>> callbacksCRP =
      new GCConcurrentHashMap<String, Callback<Request, ReconfiguratorRequest>>(
          new GCConcurrentHashMapCallback() {
            @Override
            public void callbackGC(Object key, Object value) {
              assert (value instanceof RequestCallback);
              ReconfigurationConfig.log.log(
                  Level.INFO, "{0} timing out {1}:{2}", new Object[] {this, key, value});
            }
          },
          CRP_GC_TIMEOUT);

  /**
   * Default response is to simply invoke {@link
   * #sendClientReconfigurationPacket(ClientReconfigurationPacket)}.
   */
  private final Callback<Request, ReconfiguratorRequest> defaultCallback =
      new Callback<Request, ReconfiguratorRequest>() {

        @Override
        public ReconfiguratorRequest processResponse(Request response) {
          if (response != null && response instanceof ClientReconfigurationPacket) {
            Reconfigurator.this.sendClientReconfigurationPacket(
  ...
  /**
   * @return Default callback for auto-invoked methods. Meant only for internal use.
   */
  ...
  private static final String getCRPKey(ClientReconfigurationPacket crp) {
    return crp.getRequestType() + ":" + crp.getServiceName() + ":" + crp.getCreator();
  }

  /**
   * This method simply looks up and returns the current set of active replicas. Maintaining this
   * state consistently is the primary and only existential purpose of reconfigurators.
   *
   * @param request
   * @param ptasks
   * @return Messaging task returning the set of active replicas to the requestor. No protocol tasks
   *     spawned.
   */
  public GenericMessagingTask<NodeIDType, ?>[] handleRequestActiveReplicas(
      RequestActiveReplicas request,
      ProtocolTask<NodeIDType, ReconfigurationPacket.PacketType, String>[] ptasks) {
    return this.handleRequestActiveReplicas(request, ptasks, defaultCallback);
  }
  ...
  public GenericMessagingTask<NodeIDType, ?>[] handleRequestActiveReplicas(
    ...
    if (this.processRedirection(request)) {
      /**
       * Asynchronous step here, but we enqueue only if the callback is non-default because the
       * default callback action of sending responses back to the client will be done anyway if no
       * explicit callback is found. This way, we avoid unnecessary enqueue/dequeue/GC operations in
       * the common case.
       */
      if (callback != this.defaultCallback) this.callbacksCRP.put(getCRPKey(request), callback);
      return null;
    }

    if (request.needsCoordination() && this.DB.handleIncoming(request, this)) return null;

    // else at an appropriate replica
    ReconfigurationRecord<NodeIDType> record =
        this.DB.getReconfigurationRecord(request.getServiceName());
  ...
  public GenericMessagingTask<NodeIDType, ?>[] handleSetReplicaPlacementRequest(
    ...
    Map<String, InetSocketAddress> allActives = this.consistentNodeConfig.getAllActiveReplicas();
    ...
    if (preferredCoordinatorId != null) {
      if (!newActivesStringSet.contains(preferredCoordinatorId)) {
        request.setFailed(ClientReconfigurationPacket.ResponseCodes.GENERIC_EXCEPTION);
        request.setResponseMessage("Invalid coordinator node id of " + preferredCoordinatorId);
        callback.processResponse(request.makeResponse());
        return null;
      }
    }
    ...
    if (preferredCoordinatorId != null) {
      ...
      try {
        metadataJson.put(
            AbstractDemandProfile.Keys.PREFERRED_COORDINATOR.toString(), preferredCoordinatorId);
  ...
  public GenericMessagingTask<NodeIDType, ?>[] handleGetReplicaPlacementRequest(
    ...
    for (NodeIDType node : nodeIds) {
      ...
      httpAddresses.add(new InetSocketAddress(address.getAddress(), httpPort).toString());
    }
  ...
  public GenericMessagingTask<NodeIDType, ?>[] handleSetCoordinatorNodeRequest(
      ...
      ProtocolTask<NodeIDType, ReconfigurationPacket.PacketType, String>[] ptasks) {
    throw new RuntimeException("Unexpected: receiving SetCoordinatorNodeRequest without callback");
  }
  ...
  public GenericMessagingTask<NodeIDType, ?>[] handleSetCoordinatorNodeRequest(
    ...
    try {
      ...
      request.setFailed(ClientReconfigurationPacket.ResponseCodes.MALFORMED_REQUEST);
      request.setResponseMessage(
          "Invalid NodeID format for coordinator: " + newCoordinatorNodeIdStr);
      callback.processResponse(request.makeResponse());
      return null;
    }
    ...
    if (activeReplicas == null || !activeReplicas.contains(newCoordinatorNodeId)) {
      request.setFailed(ClientReconfigurationPacket.ResponseCodes.GENERIC_EXCEPTION);
      request.setResponseMessage(
          "The to-be coordinator "
              + newCoordinatorNodeIdStr
              + " is not an active replica for "
              + serviceName);
      callback.processResponse(request.makeResponse());
      return null;
    }
  ...
  public GenericMessagingTask<NodeIDType, ?>[] handleSetCoordinatorNodeResponse(
    ...
    if (clientCallback != null) {
      ...
      RequestCallbackFuture<ReconfiguratorRequest> rcf =
          (RequestCallbackFuture<ReconfiguratorRequest>) clientCallback;
  ...
  /**
   * Handles a request to add or delete a reconfigurator from the set of all reconfigurators in
   * NodeConfig. The reconfiguration record corresponding to NodeConfig is stored in the RC records
   * table and the "active replica state" or the NodeConfig info itself is stored in a separate
   * NodeConfig table in the DB.
   *
   * @param changeRC
   * @param ptasks
   * @return Messaging task typically null. No protocol tasks spawned.
  ...
  public GenericMessagingTask<?, ?>[] handleReconfigureRCNodeConfig(
    ...
    if (!this.isPermitted(changeRC)) {
      ...
      return (new GenericMessagingTask<InetSocketAddress, ServerReconfigurationPacket<NodeIDType>>(
              changeRC.getIssuer(), changeRC.setFailed().setResponseMessage(errorMessage)))
          .toArray();
    }
    // check first if NC is ready for reconfiguration
    ReconfigurationRecord<NodeIDType> ncRecord =
        this.DB.getReconfigurationRecord(changeRC.getServiceName());
    if (ncRecord == null) return null; // possible if startCleanSlate
    ...
    if (changeRC.newlyAddedNodes != null || changeRC.deletedNodes != null)
      this.initiateReconfiguration(
          ...
          (changeRC.getIssuer()),
          changeRC.getMyReceiver(),
          null,
          null,
          null,
          changeRC.newlyAddedNodes,
          ReconfigurationConfig.ReconfigureUponActivesChange.DEFAULT);
    return null;
  }
  ...
  public GenericMessagingTask<?, ?>[] handleReconfigureActiveNodeConfig(
    ...
    ReconfigurationConfig.log.log(
        ...
        new Object[] {
          separator,
          this,
          changeActives.getType(),
          changeActives.getSummary(),
    ...
    if (!this.isPermitted(changeActives)) {
      ...
      return (new GenericMessagingTask<InetSocketAddress, ServerReconfigurationPacket<NodeIDType>>(
              changeActives.getIssuer(),
              changeActives.setFailed().setResponseMessage(errorMessage)))
          .toArray();
    } else if (this.nothingToDo(changeActives)) {
      String errorMessage = " Requested node additions or deletions already in place";
      ReconfigurationConfig.log.log(
          Level.INFO, "{0} {1} : {2}", new Object[] {this, errorMessage, changeActives});
      // do not setFailed() in this case
      return (new GenericMessagingTask<InetSocketAddress, ServerReconfigurationPacket<NodeIDType>>(
              changeActives.getIssuer(), changeActives.setResponseMessage(errorMessage)))
          .toArray();
    }
    // check first if NC is ready for reconfiguration
    ReconfigurationRecord<NodeIDType> activeNCRecord =
        this.DB.getReconfigurationRecord(changeActives.getServiceName());
    if (activeNCRecord == null) return null; // possible if startCleanSlate

    if (!activeNCRecord.isReady()) {
      String errorMessage = " Trying to conduct concurrent node config changes";
      ReconfigurationConfig.log.warning(
          this
              + errorMessage
              + ": "
              + changeActives.getSummary()
  ...
  private boolean nothingToDo(ReconfigureActiveNodeConfig<NodeIDType> changeActives) {
    ...
    nothing =
        nothing
            && (changeActives.hasAddedNodes()
                ? this.consistentNodeConfig
                    .getActiveReplicas()
                    .containsAll(changeActives.newlyAddedNodes.keySet())
                : true);
  ...
  /**
   * Reconfiguration is initiated using a callback because the intent to conduct a reconfiguration
   * must be persistently committed before initiating the reconfiguration. Otherwise, the failure of
   * say the initiating reconfigurator can leave an active replica group stopped indefinitely.
   * Exactly one reconfigurator, the one that proposes the request initiating reconfiguration
   * registers the callback. This initiating reconfigurator will spawn a WaitAckStopEpoch task when
   * the initiating request is locally executed. The other replicas only spawn a
   * WaitPrimaryExecution task as a double check that the initiating reconfigurator does complete
   * the reconfiguration; if it does not, they will follow up with their own attempt after a
   * timeout. This works because all three steps: WaitAckStopEpoch, WaitAckStartEpoch, and
   * WaitAckDropEpoch are idempotent operations.
   *
   * <p>A reconfiguration attempt can still get stuck if all reconfigurators crash or the only
   * reconfigurators that committed the intent crash. So, a replica recovery procedure should ensure
   * that replicas eventually execute committed but unexecuted requests. This naturally happens with
   * paxos.
   */
  @Override
  public void executed(Request request, boolean handled) {
    if (this.isRecovering()
        && !((request instanceof RCRecordRequest<?>)
    ...
    try {
      ...
      if (!request.toString().equals(Request.NO_OP)) e.printStackTrace();
    }
    if (rcPacket == null
        || (!rcPacket.getType().equals(ReconfigurationPacket.PacketType.RC_RECORD_REQUEST)
            && !rcPacket
                .getType()
                .equals(ReconfigurationPacket.PacketType.REQUEST_ACTIVE_REPLICAS))) return;

    if (request instanceof RequestActiveReplicas) {
      this.handleRequestActiveReplicas(
          ((RequestActiveReplicas) request).unsetNeedsCoordination(), null);
      return;
    }
    ...
    if (handled
        && rcRecReq.isReconfigurationIntent()
        && !rcRecReq.isNodeConfigChange()
      ...
      if (!handled) {
        ...
        ReconfigurationConfig.log.log(
            Level.INFO,
            "{0} restarting failed merge {1}",
            new Object[] {this, rcRecReq.getSummary()});
        this.protocolExecutor.spawnIfNotRunning(
            new WaitAckStopEpoch<NodeIDType>(rcRecReq.startEpoch, this.DB));
  ...
  private void ncAssert(RCRecordRequest<NodeIDType> rcRecReq, boolean handled) {
    ReconfigurationRecord<NodeIDType> ncRecord =
        this.DB.getReconfigurationRecord(AbstractReconfiguratorDB.RecordNames.RC_NODES.toString());
    assert (!ncRecord.getActiveReplicas().equals(ncRecord.getNewActives()))
        : this + " : handled=" + handled + "; " + ncRecord + "\n  upon \n" + rcRecReq;
  }
  ...
  public boolean preExecuted(Request request) {
    ...
    if (rcRecReq == null
        || !this.DB.isNCRecord(rcRecReq.getServiceName())
        || !rcRecReq.isReconfigurationComplete()) return false;
  ...
  private RCRecordRequest<NodeIDType> requestToRCRecordRequest(Request request) {
    if (request instanceof RCRecordRequest<?>) return (RCRecordRequest<NodeIDType>) request;
    BasicReconfigurationPacket<?> rcPacket = null;
    try {
      rcPacket = ReconfigurationPacket.getReconfigurationPacket(request, getUnstringer());
    } catch (JSONException e) {
  ...
  private void spawnPrimaryReconfiguratorTask(RCRecordRequest<NodeIDType> rcRecReq) {
    ...
    if (!rcRecReq.isSplitIntent())
      ...
      this.protocolExecutor.spawnIfNotRunning(
          new WaitAckStartEpoch<NodeIDType>(rcRecReq.startEpoch, this.DB));
  }
  ...
  private Boolean receivedOnSSLPort(ClientReconfigurationPacket request) {
    return request.getMyReceiver().getPort()
            == ReconfigurationConfig.getClientFacingSSLPort(
                this.consistentNodeConfig.getNodePort(getMyID()))
        ? (Boolean) true
        : (request.getMyReceiver().getPort()
                == ReconfigurationConfig.getClientFacingClearPort(
                    this.consistentNodeConfig.getNodePort(getMyID()))
            ? (Boolean) false
  ...
  private void spawnSecondaryReconfiguratorTask(RCRecordRequest<NodeIDType> rcRecReq) {
    ...
    this.protocolExecutor.schedule(
        new WaitPrimaryExecution<NodeIDType>(
            getMyID(),
            rcRecReq.startEpoch,
            this.DB,
  ...
  /**
   * These are the only request types which {@link Reconfigurator} will accept on the client facing
   * port.
   */
  ...
  private void instrument(Level level) {
    ReconfigurationConfig.log.log(
        ...
        new Object[] {
          this,
          this.protocolExecutor.getActiveCount(),
  ...
  /**
   * Returns true if this request's handling is complete, i.e., it has either been forwarded to
   * another server or it was a response to a request forwarded earlier and the response has now
   * been forwarded to the end-client.
   *
  ...
  private boolean processRedirection(ClientReconfigurationPacket clientRCPacket) {
    ...
    else return (this.redirectableRequest(clientRCPacket));
  }

  private ClientReconfigurationPacket modifyPortsForSSLIfNeeded(
      ClientReconfigurationPacket clientRCPacket) {
    return clientRCPacket.getRequestType()
                == ReconfigurationPacket.PacketType.REQUEST_ACTIVE_REPLICAS
            && this.amReceiver(((RequestActiveReplicas) clientRCPacket))
        ? (((RequestActiveReplicas) clientRCPacket)
                .setActives(
                    modifyPortsForSSL(
                        ((RequestActiveReplicas) clientRCPacket).getActives(),
                        receivedOnSSLPort(clientRCPacket)))
                .isFailed()
            ? clientRCPacket.setHashRCs(
                modifyPortsForSSL(
                    this.getSocketAddresses(
                        this.consistentNodeConfig.getReplicatedReconfigurators(
                            clientRCPacket.getServiceName())),
                    receivedOnSSLPort(clientRCPacket)))
            : clientRCPacket)
        : clientRCPacket;
  }
  ...
  private static final Set<InetSocketAddress> modifyPortsForSSL(
    ...
    if (ssl == null
        || ReconfigurationConfig.getClientPortOffset() == 0
        || replicas == null
        || replicas.isEmpty()) return replicas;
    ...
    for (InetSocketAddress sockAddr : replicas)
      modified.add(
          new InetSocketAddress(
              sockAddr.getAddress(),
              ssl
                  ? ReconfigurationConfig.getClientFacingSSLPort(sockAddr.getPort())
                  : ReconfigurationConfig.getClientFacingClearPort(sockAddr.getPort())));
    return modified;
  }

  private boolean clientFacingPortIsMyPort() {
    return getClientFacingClearPort(this.consistentNodeConfig.getNodePort(getMyID()))
        == this.consistentNodeConfig.getNodePort(getMyID());
  }
  ...
  private static Set<String> getStringSet(Set<?> nodes) {
    ...
    for (Object node : nodes) strNodes.add(node.toString());
    return strNodes;
  }
  ...
  private AddressMessenger<JSONObject> initClientMessenger(boolean ssl) {
    ...
    try {
      ...
      if ((ssl ? getClientFacingSSLPort(myPort) : getClientFacingClearPort(myPort)) != myPort) {
        ...
        AddressMessenger<?> existing =
            (ssl ? this.messenger.getSSLClientMessenger() : this.messenger.getClientMessenger());
              ...
              new JSONMessenger<InetSocketAddress>(
                  niot =
                      new MessageNIOTransport<InetSocketAddress, JSONObject>(
                          ...
                          isa.getPort(),
                          (pd =
                              new ReconfigurationPacketDemultiplexer(
          ...
          if (this.messenger.getClientMessenger() instanceof Messenger)
            ((Messenger<NodeIDType, ?>) this.messenger.getClientMessenger())
                .addPacketDemultiplexer(
                    pd =
                        new ReconfigurationPacketDemultiplexer(this.getUnstringer(), this.DB)
                            .setThreadName(this.getMyID().toString() + "-clientFacing"));
        } else {
          ReconfigurationConfig.log.log(
              Level.INFO,
              "{0} adding self as demultiplexer to existing {1} client messenger",
          ...
          if (this.messenger.getSSLClientMessenger() instanceof Messenger)
            ((Messenger<NodeIDType, ?>) this.messenger.getSSLClientMessenger())
                .addPacketDemultiplexer(
                    pd =
                        new ReconfigurationPacketDemultiplexer(this.getUnstringer(), this.DB)
                            .setThreadName(this.getMyID().toString() + "-clientFacingSSL"));
        }
        assert (pd != null);
        pd.register(
            Config.getGlobalBoolean(RC.ALLOW_CLIENT_TO_CREATE_DELETE)
                ? clientRequestTypes
                : clientRequestTypesNoCreateDelete,
            this);
        if (!Config.getGlobalBoolean(RC.ALLOW_CLIENT_TO_CREATE_DELETE)
            && ReconfigurationConfig.getServerSSLMode() != SSL_MODES.MUTUAL_AUTH)
          Util.suicide(
              "Can not prevent clients from creating and deleting names "
                  + "(ALLOW_CLIENT_TO_CREATE_DELETE=false) unless SERVER_SSL_MODE=MUTUAL_AUTH");
      }
    } catch (IOException e) {
      e.printStackTrace();
      ReconfigurationConfig.log.severe(
          this + " failed to initialize client messenger: " + e.getMessage());
      System.exit(1);
    }
  ...
  private Set<NodeIDType> shouldReconfigure(String name) {
    ...
    Set<String> newActiveIPs =
        this.demandProfiler.testAndSetReconfigured(
    ...
    Set<NodeIDType> newActives = this.consistentNodeConfig.getNodeIDs(newActiveIPs);
    return (!newActives.equals(oldActives) || ReconfigurationConfig.shouldReconfigureInPlace())
        ? newActives
        : null;
  }

  /**
   * Indicates whether reconfiguration is needed. Exactly the same as {@link
   * #shouldReconfigure(String)} with additional metadata in the return value, which is useful for
   * conveying reconfiguration/placement metadata (e.g., which node is preferable as the
   * coordinator).
   *
   * <p>TODO: handle configuration if the replica group does not change, but the preferred node for
   * coordinator does change.
   *
   * @param name The service name
   * @return pair of Node ID set and optional metadata if reconfiguration is needed, otherwise null
   *     is returned.
   */
  private NodeIdsMetadataPair<NodeIDType> shouldReconfigure2(String name) {
    ...
    if (isSameActives && !ReconfigurationConfig.shouldReconfigureInPlace()) return null;

    return new NodeIdsMetadataPair<>(
        newActiveNodeIds, newActiveNodeIdsAndMetadata.placementMetadata());
  }
  ...
  private ReconfigurableAppInfo getReconfigurableAppInfo() {
    return new ReconfigurableAppInfo() {
      ...
      public Set<String> getReplicaGroup(String serviceName) {
        ReconfigurationRecord<NodeIDType> record =
            Reconfigurator.this.DB.getReconfigurationRecord(serviceName);
        return record != null ? getStringSet(record.getActiveReplicas()) : null;
      }
      ...
      public Map<String, InetSocketAddress> getAllActiveReplicas() {
        return Reconfigurator.this.consistentNodeConfig.getAllActiveReplicas();
      }

      @Override
      public Map<String, Geolocation> getActiveReplicaGeolocations() {
        Map<String, Geolocation> result = new HashMap<>();
        for (NodeIDType id : Reconfigurator.this.consistentNodeConfig.getActiveReplicas()) {
          Geolocation geo = Reconfigurator.this.consistentNodeConfig.getNodeGeolocation(id);
          if (geo != null) {
  ...
  private void updateDemandProfile(DemandReport<NodeIDType> report) {
    ...
    if (!this.demandProfiler.contains(report.getServiceName())) {
      ...
      try {
        if (statsStr != null) statsJSON = new JSONObject(statsStr);
      } catch (JSONException e) {
        e.printStackTrace();
      }
      if (statsJSON != null)
  ...
  private void trimAggregateDemandProfile() {
    ...
    for (AbstractDemandProfile profile : profiles) {
      ...
      DemandReport<NodeIDType> report =
          new DemandReport<NodeIDType>(this.getMyID(), profile.getName(), 0, profile);
  ...
  private boolean isReadyForReconfiguration(
    ...
    ReconfigurationRecord<NodeIDType> recordGroupName =
        this.DB.getReconfigurationRecord(this.DB.getRCGroupName(rcPacket.getServiceName()));
  ...
  private ReconfigureUponActivesChange getReconfigureUponActivesChangePolicy(String name) {
    return this.DB.isRCGroupName(name)
            || AbstractReconfiguratorDB.RecordNames.RC_NODES.toString().equals(name)
            || AbstractReconfiguratorDB.RecordNames.AR_NODES.toString().equals(name)
        ? ReconfigurationConfig.ReconfigureUponActivesChange.DEFAULT
        : ReconfigurationConfig.getDefaultReconfigureUponActivesChangePolicy();
  }

  private StartEpoch<NodeIDType> formStartEpoch(
      String name,
      ReconfigurationRecord<NodeIDType> record,
      Set<NodeIDType> newActives,
      InetSocketAddress sender,
   ...
   * @return The task key.
   */
  public static final String getTaskKey(
      Class<?> C, BasicReconfigurationPacket<?> rcPacket, String myID) {
    return getTaskKey(C, myID, rcPacket.getServiceName(), rcPacket.getEpochNumber());
  }

  private static final String getTaskKey(Class<?> C, String myID, String name, int epoch) {
  ...
  protected static String getTaskKeyPrev(
      Class<?> C, BasicReconfigurationPacket<?> rcPacket, String myID) {
    return getTaskKeyPrev(C, rcPacket, myID, 1);
  }

  private static final String getTaskKeyPrev(
      Class<?> C, BasicReconfigurationPacket<?> rcPacket, String myID, int prev) {
  ...
  private void initFinishPendingReconfigurations() throws IOException {
    ...
    this.protocolExecutor.scheduleWithFixedDelay(
        ...
        0,
        ReconfigurationConfig.getMaxFinalStateAge() / 10,
        TimeUnit.MILLISECONDS);
  ...
  private void finishPendingReconfigurations() throws IOException {
    ...
    for (String name : pending) {
        ...
        if (!SKIP_ACTIVE_DELETIONS_UPON_RECOVERY)
          this.executeActiveNodeConfigChange(
              new RCRecordRequest<NodeIDType>(
                  this.getMyID(),
                  this.formStartEpoch(
                      name, record, record.getNewActives(), null, null, null, null, null, null),
      ...
      ReconfigurationConfig.log.log(
          Level.FINE, "{0} initiating pending reconfiguration for {1}", new Object[] {this, name});
      RCRecordRequest<NodeIDType> rcRecReq =
          new RCRecordRequest<NodeIDType>(
              this.getMyID(),
              this.formStartEpoch(
  ...
  private boolean forwardClientReconfigurationPacket(ClientReconfigurationPacket request) {
    try {
      ...
      @SuppressWarnings("unchecked")
      NodeIDType randomResponsibleRC =
          (NodeIDType) (responsibleRCs.toArray()[(int) (Math.random() * responsibleRCs.size())]);
      ...
      ReconfigurationConfig.log.severe(
          this + " incurred " + e.getClass().getSimpleName() + e.getMessage());
      e.printStackTrace();
    }
    return true;
  }
    ...
    try {
      ...
      if (querier.equals(response.getCreator())) {
        ...
        ReconfigurationConfig.log.log(
            ...
            new Object[] {
              this, response.isRequest() ? "request" : "RESPONSE", response.getSummary(), querier
            });
  ...
  private AddressMessenger<JSONObject> getMessenger(InetSocketAddress receiver) {
    if (receiver.equals(this.consistentNodeConfig.getBindSocketAddress(this.getMyID()))) {
      ReconfigurationConfig.log.log(
          ...
          new Object[] {
            this, receiver, this.consistentNodeConfig.getBindSocketAddress(this.getMyID())
          });
      ...
      CreateServiceName response =
          (CreateServiceName)
              (new CreateServiceName(
                      ...
                      rcRecReq.getEpochNumber(),
                      null,
                      rcRecReq.startEpoch.getMyReceiver()))
                  .setForwader(rcRecReq.startEpoch.getForwarder())
                  .makeResponse();

      Callback<Request, ReconfiguratorRequest> callback =
          this.callbacksCRP.remove(getCRPKey(response));
      if (callback != null) callback.processResponse(response);
  ...
  private void sendDeleteConfirmationToClient(RCRecordRequest<NodeIDType> rcRecReq) {
    if (rcRecReq.startEpoch.creator == null || !rcRecReq.getInitiator().equals(getMyID())) return;
    try {
      InetSocketAddress querier = this.getQuerier(rcRecReq);
      // copy forwarder from startEpoch and mark as response
      DeleteServiceName response =
          (DeleteServiceName)
              new DeleteServiceName(
                      rcRecReq.startEpoch.creator,
                      rcRecReq.getServiceName(),
                      rcRecReq.getEpochNumber() - 1,
                      rcRecReq.startEpoch.getMyReceiver())
  ...
  private InetSocketAddress getQuerier(RCRecordRequest<NodeIDType> rcRecReq) {
    ...
    if (forwarder != null && !forwarder.equals(me)) return forwarder;
    else
  ...
  private void sendReconfigureRCNodeConfigConfirmationToInitiator(
      RCRecordRequest<NodeIDType> rcRecReq) {
    try {
      ReconfigureRCNodeConfig<NodeIDType> response =
          new ReconfigureRCNodeConfig<NodeIDType>(
              this.DB.getMyID(),
  ...
  private void sendReconfigureActiveNodeConfigConfirmationToInitiator(
      RCRecordRequest<NodeIDType> rcRecReq) {
    try {
      ReconfigureActiveNodeConfig<NodeIDType> response =
          new ReconfigureActiveNodeConfig<NodeIDType>(
              this.DB.getMyID(),
              rcRecReq.startEpoch.newlyAddedNodes,
              this.diff(rcRecReq.startEpoch.prevEpochGroup, rcRecReq.startEpoch.curEpochGroup));
      ReconfigurationConfig.log.log(
      ...
      ReconfigurationConfig.log.log(
          ...
          "\n\n"
              + "{0}\n"
              + "{1} finished required reconfigurations to change active replica(s) {2}; sending"
              + " response to {3}\n"
              + "{4}\n",
          new Object[] {
            separator, this, response.getSummary(), rcRecReq.startEpoch.creator, separator
          });
      (this.messenger)
          .sendToAddress(
              rcRecReq.startEpoch.creator,
  ...
  private Set<NodeIDType> diff(Set<NodeIDType> s1, Set<NodeIDType> s2) {
    ...
    for (NodeIDType node : s1) if (!s2.contains(node)) diff.add(node);
    return diff;
  }

  // all nodes are primaries for NC change.
  private boolean reconfigureNodeConfigRecord(RCRecordRequest<NodeIDType> rcRecReq) {
    if (rcRecReq.getInitiator().equals(getMyID())) this.spawnPrimaryReconfiguratorTask(rcRecReq);
    else this.spawnSecondaryReconfiguratorTask(rcRecReq);
    return true;
  }
  ...
  public GenericMessagingTask<NodeIDType, ?>[] handleEchoRequest(
      ...
      ProtocolTask<NodeIDType, ReconfigurationPacket.PacketType, String>[] ptasks) {
    ReconfigurationConfig.log.log(
        Level.FINE, "{0} received echo request {1}", new Object[] {this, echo.getSummary()});
    if (echo.isRequest()) {
  ...
  public GenericMessagingTask<NodeIDType, ?>[] handleHelloRequest(
      ...
      ProtocolTask<NodeIDType, ReconfigurationPacket.PacketType, String>[] ptasks) {
    ReconfigurationConfig.log.log(
        Level.FINE, "{0} received hello request {1}", new Object[] {this, hello.getSummary()});

    NodeIDType nodeID = hello.getInitiator();
    if (nodeID == null) {
      ReconfigurationConfig.log.log(
          Level.FINE, "{0} has no initiator in it.", new Object[] {hello.getSummary()});
      return null;
    }
    ...
    ReconfigurationConfig.log.log(
        Level.FINE, "NodeConfig gets updated to {0}", new Object[] {consistentNodeConfig});
    return null;
  }
  ...
  private boolean executeNodeConfigChange(RCRecordRequest<NodeIDType> rcRecReq) {
    ...
    if (!ncChanged)
      throw new RuntimeException(
          this.getMyID() + " unable to change node config: " + rcRecReq.getSummary());
  ...
  private boolean executeActiveNodeConfigChange(RCRecordRequest<NodeIDType> rcRecReq) {
    ...
    ReconfigurationRecord<NodeIDType> record =
        this.DB.getReconfigurationRecord(AbstractReconfiguratorDB.RecordNames.AR_NODES.toString());

    // change soft copy
    if (rcRecReq.startEpoch.hasNewlyAddedNodes())
      for (NodeIDType node : rcRecReq.startEpoch.newlyAddedNodes.keySet())
        this.consistentNodeConfig.addActiveReplica(
            node, rcRecReq.startEpoch.newlyAddedNodes.get(node));
    ...
    for (NodeIDType active : this.diff(record.getNewActives(), record.getActiveReplicas()))
      this.consistentNodeConfig.addActiveReplica(
          active, rcRecReq.startEpoch.newlyAddedNodes.get(active));
    ...
    if (rcRecReq.startEpoch.newlyAddedNodes != null) {
      Set<NodeIDType> addedNodes = rcRecReq.startEpoch.newlyAddedNodes.keySet();
    ...
    if (rcRecReq.getInitiator().equals(this.getMyID()))
      this.protocolExecutor.spawnIfNotRunning(
          new WaitAckDropEpoch<NodeIDType>(
              rcRecReq.startEpoch, this.consistentNodeConfig.getReconfigurators(), this.DB));
    ...
    if (startEpoch.hasNewlyAddedNodes())
      for (Map.Entry<NodeIDType, InetSocketAddress> entry : startEpoch.newlyAddedNodes.entrySet()) {
        ...
        ReconfigurationConfig.log.log(
            ...
            new Object[] {
              this, entry.getKey(), this.consistentNodeConfig.getNodeSocketAddress(entry.getKey())
            });
      }
  ...
  private boolean isPermitted(ReconfigureRCNodeConfig<NodeIDType> changeRC) {
    ...
    if (changeRC.getAddedNodeIDs() != null)
      for (NodeIDType addNode : changeRC.getAddedNodeIDs()) {
        ReconfigurationRecord<NodeIDType> rcRecord =
            this.DB.getReconfigurationRecord(this.DB.getRCGroupName(addNode));
        {
          if (rcRecord != null && rcRecord.isDeletePending()) {
            changeRC.setResponseMessage(
                "Can not add reconfigurator named "
                    + addNode
    ...
    return changeRC.getDeletedNodeIDs().size() > permittedSize
        ? (changeRC.setResponseMessage(
                "Deleting more than "
                    + (permittedSize - 1)
                    + " reconfigurators simultaneously is not permitted")
            != null)
        : true;
  }
  ...
  private boolean amAffected(Set<NodeIDType> addNodes, Set<NodeIDType> deleteNodes) {
    ...
    for (NodeIDType node : addNodes) if (this.DB.amAffected(node)) affected = true;
    for (NodeIDType node : deleteNodes) if (this.DB.amAffected(node)) affected = true;
    return affected;
  }

  private boolean changeSplitMergeGroups(
      Set<NodeIDType> affectedNodes, Set<NodeIDType> addNodes, Set<NodeIDType> deleteNodes) {
    if (!amAffected(addNodes, deleteNodes)) return false;
    ...
    if (ncRecord == null) return false;

    // adjustCurWithNewRCGroups(curRCGroups, newRCGroups, ncRecord);
    String changedSplitMerged =
        this.changeExistingGroups(curRCGroups, newRCGroups, ncRecord, affectedNodes);
  ...
  private boolean isPresent(String rcGroupName, Set<NodeIDType> affectedNodes) {
    for (NodeIDType node : affectedNodes) {
      if (this.DB.getRCGroupName(node).equals(rcGroupName)) return true;
    }
    return false;
  }
  ...
  private void repeatUntilObviated(RCRecordRequest<NodeIDType> rcRecReq) {
    if (this.DB.isNCRecord(rcRecReq.getServiceName()))
      ...
      if (rcRecReq.isReconfigurationMerge())
        ReconfigurationConfig.log.log(
            Level.INFO, "{0} coordinating merge {1}", new Object[] {this, rcRecReq.getSummary()});
      this.commitWorker.enqueueForExecution(rcRecReq);
    }
  }

  /**
   * This method reconfigures groups that exist locally both in the old and new rings, i.e., this
   * node just has to do a standard reconfiguration operation because the membership of the paxos
   * group is changing.
   *
   * <p>With {@link ReconfigurationConfig#isAggregatedMergeSplit()}, this method suffices for all
   * necessary reconfigurations; otherwise, we have to also invoke splitExistingGroups and
   * mergeExistingGroups.
   */
  private String changeExistingGroups(
    ...
    for (String newRCGroup : newRCGroups.keySet()) {
      if (!isPresent(newRCGroup, affectedNodes)) continue; // don't trivial-reconfigure
      else
        ReconfigurationConfig.log.log(
            Level.FINE,
            "{0} finds {1} present in affected RC groups {2}",
            new Object[] {this, newRCGroup, affectedNodes});
      final Map<String, Set<String>> mergees =
          this.isMergerGroup(newRCGroup, curRCGroups, newRCGroups, ncRecord);
      Map<String, Set<NodeIDType>> splitParentGroup =
          this.isSplitGroup(newRCGroup, curRCGroups, newRCGroups, ncRecord);
      int ncEpoch = ncRecord.getRCEpoch(newRCGroup);

      boolean invokeAggregatedMergeSplit =
          ReconfigurationConfig.isAggregatedMergeSplit()
      ...
      if (curRCGroups.keySet().contains(newRCGroup) && !invokeAggregatedMergeSplit) {
        ...
        debug +=
            (this
                + (curRCGroups.keySet().contains(newRCGroup)
                    ? " changing local group "
                    : " creating ")
                + "{"
                + newRCGroup
                ...
                + "}");
        this.aggregatedSplitMerge(
            newRCGroup, curRCGroups, newRCGroups, ncRecord, mergees, splitParentGroup);
  ...
  private void aggregatedSplitMerge(
      String newRCGroup,
      Map<String, Set<NodeIDType>> curRCGroups,
      Map<String, Set<NodeIDType>> newRCGroups,
      ReconfigurationRecord<NodeIDType> ncRecord,
    ...
    this.protocolExecutor.submit(
        new Runnable() {
          ...
          public void run() {
            ...
            String mergedStateFilename =
                Reconfigurator.this.spawnMergeeStopAndFetchStateTasks(
                    Reconfigurator.this.getStopTasks(
                        newRCGroup,
                        mergees.keySet(),
                        splitParentGroup,
                        monitor,
            ...
            boolean created =
                Reconfigurator.this.DB.createReplicaGroup(
                    newRCGroup,
                    ncRecord.getRCEpoch(newRCGroup),
                    LargeCheckpointer.createCheckpointHandle(mergedStateFilename),
                    newRCGroups.get(newRCGroup));
            assert (created);

            // will issue the complete upon majority
            Reconfigurator.this.protocolExecutor.spawnIfNotRunning(
                new WaitAckStartEpoch<NodeIDType>(
                    // passive start epoch
                    new StartEpoch<NodeIDType>(
                        Reconfigurator.this.getMyID(),
                        newRCGroup,
                        ncRecord.getRCEpoch(newRCGroup),
                        newRCGroups.get(newRCGroup),
                        true),
  ...
  private Map<String, Set<NodeIDType>> isSplitGroup(
      String rcGroup,
      Map<String, Set<NodeIDType>> curRCGroups,
      Map<String, Set<NodeIDType>> newRCGroups,
      ReconfigurationRecord<NodeIDType> ncRecord) {
   ...
   * @param newRCGroups
   * @param ncRecord
   * @return True if {@code rcGroup} is a group into which other groups are being merged. This is
   *     true when a group is being deleted and it consistent-hashes on to {@code rcGroup} in the
   *     new ring.
   */
  private Map<String, Set<String>> isMergerGroup(
      String rcGroup,
      Map<String, Set<NodeIDType>> curRCGroups,
      Map<String, Set<NodeIDType>> newRCGroups,
      ReconfigurationRecord<NodeIDType> ncRecord) {
    ...
    if (!mergees.isEmpty())
      mergees.put(
          rcGroup, Util.setToStringSet(this.DB.getOldGroup(rcGroup).values().iterator().next()));
    return mergees;
  }
   ...
   * @param newRCGroups
   * @param ncRecord
   * @return All of the {@link WaitAckStopEpoch} tasks needed to stop groups being merged or split
   *     in order to create {@code newRCGroup}
   */
  private Set<WaitAckStopEpoch<NodeIDType>> getStopTasks(
      String newRCGroup,
      Set<String> mergees,
      Map<String, Set<NodeIDType>> splitParentGroup,
      Object monitor,
    ...
    for (String mergee : mergees) {
      ...
      stopTasks.add(
          new WaitAckStopEpoch<NodeIDType>(
              new StartEpoch<NodeIDType>(
                  this.getMyID(),
                  newRCGroup,
                  ncRecord.getRCEpoch(newRCGroup),
                  ...
                  mergee,
                  true,
                  newRCGroups.containsKey(mergee)
                      ? ncRecord.getRCEpoch(mergee) - 1
      ...
      stopTasks.add(
          new WaitAckStopEpoch<NodeIDType>(
              new StartEpoch<NodeIDType>(
                  ...
                  ncRecord.getRCEpoch(newRCGroup),
                  newRCGroups.get(newRCGroup),
                  splitParentGroup.values().iterator().next(),
                  splitParent,
                  false,
                  ncRecord.getRCEpoch(splitParent) - 1),
              this.DB,
              monitor));
    }
    return stopTasks;
  ...
  private String spawnMergeeStopAndFetchStateTasks(
      Set<WaitAckStopEpoch<NodeIDType>> stopTasks,
      Object monitor,
      String mergerGroup,
    ...
    ReconfigurationConfig.log.log(
        Level.INFO, "{0} starting wait on stop task monitors {1}", new Object[] {this, stopTasks});
    ...
    synchronized (monitor) {
      while (!stopTasks.isEmpty())
        try {
          monitor.wait();
          for (Iterator<WaitAckStopEpoch<NodeIDType>> iter = stopTasks.iterator();
              iter.hasNext(); ) {
            ...
            if (finalState != null) {
              ...
              finalStates.put(
                  stopTask.startEpoch.getPrevGroupName()
                      + ":"
                      + stopTask.startEpoch.getPrevEpochNumber(),
                  finalState);
            }
          }
        } catch (InterruptedException e) {
  ...
  /**
   * This method "reconfigures" groups that will exist locally in the new ring but do not currently
   * exist in the old ring. This "reconfiguration" is actually a group split operation, wherein an
   * existing group is stopped and two new groups are created by splitting the final state of the
   * stopped group, one with membership identical to the stopped group and the other corresponding
   * to the new but currently non-existent group. A detailed example is described below.
   *
   * <p>Note: This method is unused with {@link ReconfigurationConfig#isAggregatedMergeSplit()}.
   *
   * @param curRCGroups
   * @param newRCGroups
   * @param ncRecord
      ...
      if (!curRCGroups.keySet().contains(newRCGroup)) {
        ...
        debug +=
            this
                ...
                + "}\n";
        if (newRCGroup.equals(oldGroupName)) continue; // no trivial splits
  ...
  /**
   * This method "reconfigures" groups that will not exist locally in the new ring but do currently
   * exist locally in the old ring. This "reconfiguration" is actually a group merge operation,
   * wherein the old "mergee" group is stopped, the group which with the old group is supposed to
   * merge (and will continue to exist locally in the new ring) is stopped, and the mergee group's
   * final state is merged into the latter group simply through a paxos update operation. A detailed
   * example and a discussion of relevant concerns is described below.
   *
   * <p>Note: This method is unused with {@link ReconfigurationConfig#isAggregatedMergeSplit()}.
   *
   * @param curRCGroups
        ...
        this.protocolExecutor.spawnIfNotRunning(
            new WaitAckStopEpoch<NodeIDType>(
                new StartEpoch<NodeIDType>(
                    ...
                    curRCGroup,
                    true,
                    ncRecord.getRCEpoch(curRCGroup)),
                this.DB));
      } else if (!newRCGroups.containsKey(curRCGroup) && !this.DB.isBeingDeleted(curRCGroup)) {
        // delete current group and merge into a new "mergeGroup"
        debug +=
            (this
                    + " expecting others to delete current group {"
                    + curRCGroup
                    + ":"
                    + (ncRecord.getRCEpoch(curRCGroup) - 1)
                    + "="
                    + this.DB.getReplicaGroup(curRCGroup))
                + "}\n";
      }
  ...
  /**
   * This method issues reconfigurations for records replicated on active in a manner that limits
   * the number of outstanding reconfigurations using the {@link #outstandingReconfigurations}
   * queue.
   */
  @SuppressWarnings({"unchecked"})
  private boolean deleteActiveReplica(NodeIDType active, InetSocketAddress creator) {
    ...
    while ((record = this.DB.app.readNextActiveRecord(false)) != null) {
      ...
      // else
      NodeIDType newActive =
          (NodeIDType)
              Util.getRandomOtherThan(this.consistentNodeConfig.getActiveReplicas(), newActives);
      if (newActive != null) newActives.add(newActive);
      newActives.remove(active);
      // Note: any REPLICATE_ALL record will be reconfigured
      if (this.initiateReconfiguration(
          record.getName(),
          record,
          newActives,
          creator,
  ...
  private boolean addActiveReplica(NodeIDType active, InetSocketAddress creator) {
    ReconfigurationRecord<NodeIDType> record = null;
    ...
    {
      record =
          this.DB.getReconfigurationRecord(
              AbstractReconfiguratorDB.RecordNames.AR_AR_NODES.toString());
      if (record != null
          && (newActives = this.consistentNodeConfig.getActiveReplicas()) != null
          && record.getActiveReplicas() != null
          && !record.getActiveReplicas().contains(active)
        ...
        try {
          ReconfigurationConfig.log.log(
              ...
              new Object[] {this, AbstractReconfiguratorDB.RecordNames.AR_AR_NODES});
          this.DB.addToOutstanding(AbstractReconfiguratorDB.RecordNames.AR_AR_NODES.toString());
          record =
              this.DB.getReconfigurationRecord(
                  AbstractReconfiguratorDB.RecordNames.AR_AR_NODES.toString());
          if (record != null
        ...
        ReconfigurationConfig.log.log(
            ...
            new Object[] {this, AbstractReconfiguratorDB.RecordNames.AR_AR_NODES, record});
      }
    }
    ...
    while ((record = this.DB.app.readNextActiveRecord(true)) != null) {
      ...
      if (record.getReconfigureUponActivesChangePolicy()
        ...
        if (record.getActiveReplicas().contains(active)
            || (record.getNewActives().contains(active))) continue;
      } else if (record.getReconfigureUponActivesChangePolicy()
          == ReconfigurationConfig.ReconfigureUponActivesChange.CUSTOM) {
  ...
  /**
   * This method reconfigures (in place) the AR_RC_NODES record at actives that contains the map of
   * current reconfigurators. The reconfiguration is not strictly necessary but is the easiest way
   * to update state at actives without introducing new packet types and code.
   *
   * <p>Note: The change to the comparable AR_AR_NODES is simply done along with all REPLICATE_ALL
   * records when an active replica is added or deleted.
   *
   * @param rcRecReq
   * @return True if successfully changed reconfigurator map at actives.
   */
  private boolean changeRCNodeConfigAtActives(RCRecordRequest<NodeIDType> rcRecReq) {
    ReconfigurationRecord<NodeIDType> record =
        this.DB.getReconfigurationRecord(
            AbstractReconfiguratorDB.RecordNames.AR_RC_NODES.toString());
    if (record == null
        || !this.consistentNodeConfig
            .getReplicatedReconfigurators(record.getName())
            .contains(this.getMyID())) return true;
    ReconfigurationConfig.log.log(
        Level.INFO,
        "{0} reconfiguring RC map at actives upon adds/deletes {1}/{2}",
        new Object[] {
          this, rcRecReq.startEpoch.getNewlyAddedNodes(), rcRecReq.startEpoch.getDeletedNodes()
        });
    // else
    this.DB.addToOutstanding(AbstractReconfiguratorDB.RecordNames.AR_RC_NODES.toString());
    boolean initiated =
        this.initiateReconfiguration(
            AbstractReconfiguratorDB.RecordNames.AR_RC_NODES.toString(),
            record,
            ...
            null,
            null,
            this.consistentNodeConfig.getReconfiguratorsReadOnly().toString(),
            null,
            null,
            ReconfigurationConfig.ReconfigureUponActivesChange.REPLICATE_ALL);
    if (!initiated)
      ReconfigurationConfig.log.log(
          ...
          new Object[] {this, AbstractReconfiguratorDB.RecordNames.AR_RC_NODES});
    try {
      this.DB.waitOutstanding(1);
  ...
  public ReconfigurationTriggerResult triggerDemandBasedReconfiguration(String serviceName) {
    ReconfigurationRecord<NodeIDType> record = this.DB.getReconfigurationRecord(serviceName);
    if (record == null) return ReconfigurationTriggerResult.SERVICE_NOT_FOUND;
  ...
  public ReconfiguratorRequest sendRequest(ReconfiguratorRequest request) {
    ...
    BasicReconfigurationPacket<?> packet =
        request instanceof ClientReconfigurationPacket
            ? (ClientReconfigurationPacket) request
            : request instanceof ServerReconfigurationPacket
                ? (ServerReconfigurationPacket<?>) request
                : null;
    if (packet == null)
      throw new RuntimeException(
          "The "
    ...
    this.protocolTask.handleEvent(
        packet,
        null,
        callbackFuture = new RequestCallbackFuture<ReconfiguratorRequest>(packet, null));
    try {
  ...
  public RequestCallbackFuture<ReconfiguratorRequest> sendRequest(
      ReconfiguratorRequest request, Callback<Request, ReconfiguratorRequest> callback) {
    RequestCallbackFuture<ReconfiguratorRequest> callbackFuture;
    BasicReconfigurationPacket<?> packet =
        request instanceof ClientReconfigurationPacket
            ? (ClientReconfigurationPacket) request
    ...
    this.protocolTask.handleEvent(
        packet,
        null,
        callbackFuture = new RequestCallbackFuture<ReconfiguratorRequest>(packet, callback));
  ...
  public JSONObject getDemandSnapshot(String serviceName) {
    ...
    try {
      ...
      if (actives != null) actives.forEach(n -> activeIds.add(n.toString()));
  ...
  /**
   * If only a subset of reconfigurators get a node config change intent, they could end up never
   * executing the intent and therefore never doing the split/change/merge sequence. If only a
   * single node is in this situation, there shouldn't be a problem as reconfiguration of
   * reconfigurator groups is done redundantly by all relevant reconfigurators. However, if multiple
   * nodes miss the NC change intent, say, only a single node executes it in the worst case, it will
   * go ahead and create the next NC epoch but the other reconfigurators will not do anything for
   * groups other than those at that reconfigurator. To address this problem, there must be enough
   * information in the NC change complete for reconfigurators that missed the corresponding intent
   * to go ahead and initiate the change/split/merge sequence anyway. There is of course enough
   * information as all that is really needed is the old and new set of reconfigurators.
   *
   * <p>This fix has now been implemented using the {@link #executed(Request, boolean)} callback in
   * AbstractReconfiguratorDB that gets it via {@link RepliconfigurableReconfiguratorDB} that in
   * turn gets it from {@link AbstractReplicaCoordinator}.
   */
}
