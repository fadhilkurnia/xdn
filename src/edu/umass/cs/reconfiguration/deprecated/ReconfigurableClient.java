...
/**
 * @author V. Arun
 *     <p>FIXME: This is an example client for testing purposes. A cleaner
 *     "AbstractReconfigurationClient" is TBD.
 *     <p>This class starts with a set of reconfigurator socket addresses known a priori and shows
 *     the following operations for a name (1) request active replicas, (2) creation, and (3)
 *     deletion. It also shows how to add and remove reconfigurator nodes, but these latter
 *     operations are normally for use only by administrators, not clients.
 */
@Deprecated
public class ReconfigurableClient {
  ...
  private final ConcurrentHashMap<String, Long> sentRequests =
      new ConcurrentHashMap<String, Long>();
  private final ConcurrentHashMap<String, BasicReconfigurationPacket<?>> rcvdResponses =
      new ConcurrentHashMap<String, BasicReconfigurationPacket<?>>();
  private Set<InetSocketAddress> activeReplicas = null;

  private Logger log = Logger.getLogger(getClass().getName());

  ReconfigurableClient(Set<InetSocketAddress> reconfigurators, JSONMessenger<?> messenger) {
  ...
  private InetSocketAddress getRandomActiveReplica() {
    return (InetSocketAddress)
        (this.getActiveReplicas()
            .toArray()[(int) (this.getActiveReplicas().size() * Math.random())]);
  }
  ...
  private InetSocketAddress getRandomRCReplica(boolean offset) {
    ...
    return offset
        ? new InetSocketAddress(
            address.getAddress(), ActiveReplica.getClientFacingPort(address.getPort()))
        : address;
  }
  ...
  private void sendRequest(AppRequest req)
      throws JSONException, IOException, RequestParseException {
    InetSocketAddress sockAddr =
        (!RANDOM_SERVER ? this.getFirstActiveReplica() : this.getRandomActiveReplica());
    log.log(
        Level.INFO,
        MyLogger.FORMAT[7].replace(" ", ""),
        new Object[] {
          "Sending ", req.getRequestType(), " to ", sockAddr, ":", (sockAddr), ": ", req
        });
  ...
  private void sendRequest(BasicReconfigurationPacket<?> req) throws JSONException, IOException {
    InetSocketAddress sockAddr =
        (!RANDOM_SERVER
            ? this.getFirstRCReplica(req instanceof ClientReconfigurationPacket)
            : this.getRandomRCReplica(req instanceof ClientReconfigurationPacket));
    log.log(
        Level.INFO,
        MyLogger.FORMAT[7].replace(" ", ""),
        new Object[] {"Sending ", req.getSummary(), " to ", sockAddr, ":", (sockAddr), ": ", req});
    this.sentRequests.put(req.getServiceName(), System.currentTimeMillis());
    this.sendRequest(sockAddr, req.toJSONObject());
  }
  ...
  private class ClientPacketDemultiplexer extends AbstractJSONPacketDemultiplexer {
    ...
    public boolean handleMessage(JSONObject json, edu.umass.cs.nio.nioutils.NIOHeader header) {
      log.log(Level.INFO, "Client received {0}", new Object[] {json});
      try {
          ...
          switch (ReconfigurationPacket.getReconfigurationPacketType(json)) {
            ...
            case DELETE_SERVICE_NAME:
              ...
              log.log(
                  ...
                  new Object[] {
                    delete.isFailed() ? " ERROR " : " CONFIRMATION ",
            ...
            case RECONFIGURE_RC_NODE_CONFIG:
              ReconfigureRCNodeConfig<String> rcnc =
                  new ReconfigureRCNodeConfig<String>(json, new StringifiableDefault<String>(""));
              log.log(
                  Level.INFO,
                  "Received node config change {0} {1}{2}",
                  new Object[] {
                    rcnc.isFailed() ? "ERROR: " + rcnc.getMessage() : "confirmation",
        ...
        AppRequest.PacketType type =
            AppRequest.PacketType.getPacketType(JSONPacket.getPacketType(json));
        if (type != null) {
          switch (AppRequest.PacketType.getPacketType(JSONPacket.getPacketType(json))) {
            case DEFAULT_APP_REQUEST:
              ...
              log.log(
                  ...
                  MyLogger.FORMAT[1],
                  new Object[] {
                    "App executed request", request.getRequestID() + ":" + request.getValue()
                  });
  ...
  synchronized BasicReconfigurationPacket<?> waitForReply(
      String name, long timeout, boolean retransmission) {
    while (sentRequests.containsKey(name)
        && (!retransmission || (System.currentTimeMillis() - sentRequests.get(name) < timeout)))
      try {
        wait(timeout);
  ...
  synchronized boolean waitForReconfigureRCSuccess(String name) {
    BasicReconfigurationPacket<?> reply = this.waitForReply(name, RC_RECONFIGURE_TIMEOUT, false);
    return reply != null
        && reply instanceof ReconfigureRCNodeConfig<?>
  ...
  /**
   * Simple test client for the reconfiguration package. Clients only know the set of all
   * reconfigurators, not active replicas for any name. All information about active replicas for a
   * name is obtained from reconfigurators. Any request can be sent to any reconfigurator and it
   * will forward to the appropriate reconfigurator if necessary and relay back the response.
   *
   * @param args
   */
  public static void main(String[] args) {
    ...
    try {
      ...
      client =
          new ReconfigurableClient(ReconfigurationConfig.getReconfiguratorAddresses(), messenger);
      int numRequests = 2;
      ...
      for (int j = 0; j < numIterations; j++) {
        ...
        do client.sendRequest(client.makeRequestActiveReplicas(namePrefix));
        ...
        // ////////////////////create name/////////////////////////
        t0 = System.currentTimeMillis();
        do client.sendRequest(client.makeCreateNameRequest(namePrefix, initValue));
        ...
        for (int i = 0; i < numRequests; i++) {
          t0 = System.currentTimeMillis();
          do client.sendRequest(client.makeRequest(namePrefix, requestValuePrefix + i));
        ...
        t0 = System.currentTimeMillis();
        do client.sendRequest(client.makeDeleteNameRequest(namePrefix));
        while (!client.waitForSuccess(namePrefix));
        ...
        // do
        client.sendRequest(
            new ReconfigureRCNodeConfig<String>(
                null,
                reconfiguratorID,
                new InetSocketAddress(InetAddress.getByName("localhost"), TEST_PORT)));
        while (!client.waitForReconfigureRCSuccess(
