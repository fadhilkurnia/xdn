...
public class WaitAckStopEpoch<NodeIDType>
  ...
  /**
   * Retransmission timeout. The restart periods for most other reconfiguration protocol tasks are
   * multiples of this, so it should be changed with care.
   */
  public static final long RESTART_PERIOD =
      Config.getGlobalLong(RC.STOP_TASK_RESTART_PERIOD); // 2000;
  ...
  public WaitAckStopEpoch(
    ...
    this.stopEpoch =
        new StopEpoch<NodeIDType>(
            ...
            startEpoch.isMerge() || (this.isAggregatedMergeSplit() && startEpoch.isSplit()),
  ...
  public GenericMessagingTask<NodeIDType, ?>[] restart() {
    ...
    if (++restartCount > WARN_NUM_RESTARTS)
      log.log(
          Level.WARNING,
          MyLogger.FORMAT[2],
          new Object[] {this.refreshKey(), " resending ", this.stopEpoch.getSummary()});
    return start();
  }
  ...
  protected boolean amObviated() {
    ReconfigurationRecord<NodeIDType> record =
        this.DB.getReconfigurationRecord(this.stopEpoch.getServiceName());
    if ((record == null // nothing to delete
            // moved on
            || (record.getEpoch() - this.stopEpoch.getEpochNumber() > 0)
            // moved beyond WAIT_ACK_STOP
            || (record.getEpoch() == this.stopEpoch.getEpochNumber()
                && record.getState().equals(RCStates.WAIT_DELETE)))
        // need the splittee state if isAggregatedMergeSplit
        && (!this.startEpoch.isSplitOrMerge() || !this.isAggregatedMergeSplit())) {
      log.log(
          Level.INFO,
          "{0} obviated because record = ",
          new Object[] {this.getKey(), record != null ? record.getSummary() : record});
      return true;
    }
    return false;
  }
  ...
  public GenericMessagingTask<NodeIDType, ?>[] start() {
    ...
    log.log(
        Level.INFO,
        "{0} sending {1} to {2}",
        new Object[] {this, this.stopEpoch.getSummary(), nextNode});
    // else send stopEpoch sequentially to old actives and await a response
    return this.startEpoch.hasPrevEpochGroup()
        ? new GenericMessagingTask<NodeIDType, StopEpoch<NodeIDType>>(nextNode, this.stopEpoch)
            .toArray()
        : null;
  }
  ...
  /**
   * Note: Trying to start this task when one is already running will cause the executor to get
   * stuck.
   *
   * @return The refreshed key.
   */
  public String refreshKey() {
    return Reconfigurator.getTaskKey(getClass(), stopEpoch, this.DB.getMyID().toString())
        + (this.startEpoch.isSplit()
            ? ":" + this.startEpoch.getServiceName() + ":" + this.startEpoch.getEpochNumber()
            : "");
  }
  ...
  public boolean handleEvent(ProtocolEvent<PacketType, String> event) {
    ...
    log.log(Level.FINE, "{0} received {1} ", new Object[] {this, ackStopEpoch.getSummary()});
    ...
    if (this.stopEpoch.shouldGetFinalState()) {
      if (ackStopEpoch.getFinalState() != null) this.setFinalState(ackStopEpoch.getFinalState());
      else return false;
    }
    return true;
  }
  ...
  public GenericMessagingTask<NodeIDType, ?>[] handleThresholdEvent(
    ...
    log.log(
        ...
        "{0} received ACK_{1} {2}",
        new Object[] {
          this, this.stopEpoch.getSummary(), this.finalState != null ? "(with finalState)" : ""
        });
    ...
    if (this.startEpoch.noCurEpochGroup()) {
      ...
      if (!this.isAggregatedMergeSplit()) {
        ...
        RCRecordRequest<NodeIDType> merge =
            new RCRecordRequest<NodeIDType>(
                this.DB.getMyID(),
                new StartEpoch<NodeIDType>(this.startEpoch, this.finalState),
                RCRecordRequest.RequestTypes.RECONFIGURATION_MERGE);
        mtasks = (new GenericMessagingTask<NodeIDType, Object>(this.DB.getMyID(), merge)).toArray();
        log.log(Level.INFO, "{0} sending to self {1}", new Object[] {this, merge.getSummary()});
      }
    } else if (!this.startEpoch.isSplitOrMerge() || !this.isAggregatedMergeSplit())
  ...
  private GenericMessagingTask<NodeIDType, ?>[] prepareDeleteIntent() {
    ...
    return (new GenericMessagingTask<NodeIDType, Object>(this.DB.getMyID(), rcRecReq)).toArray();
  }
