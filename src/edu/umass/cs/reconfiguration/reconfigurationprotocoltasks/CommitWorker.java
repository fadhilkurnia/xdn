...
/**
 * @author V. Arun
 * @param <NodeIDType> This class is not a protocol task, just a usual runnable worker. It is
 *     initiated at a reconfigurator in order to commit the coordinated RCRecordRequest packets. We
 *     need a task for this because simply invoking handleIncoming (that in turn calls paxos
 *     propose) does not suffice to ensure that the command will be eventually committed.
 */
public class CommitWorker<NodeIDType> implements Runnable {

  /**
   * Map, not Set, only because otherwise we will get concurrent modification exception in the case
   * of a single reconfigurator replica group.
   */
  ConcurrentHashMap<RCRecordRequest<NodeIDType>, Boolean> pending =
      new ConcurrentHashMap<RCRecordRequest<NodeIDType>, Boolean>();

  Map<RCRecordRequest<NodeIDType>, Long> lastAttempts =
      new ConcurrentHashMap<RCRecordRequest<NodeIDType>, Long>();
   ...
   */
  public synchronized boolean enqueueForExecution(RCRecordRequest<NodeIDType> request) {
    boolean enqueued = false;
    if (!this.checkIfObviated(request)) {
      log.log(
          Level.FINEST, "{0} enqueueing request {1}", new Object[] {this, request.getSummary()});
      assert (request != null);
      enqueued = this.pending.putIfAbsent(request, true) != null;
    }
    log.log(
        Util.oneIn(10) ? Level.INFO : Level.FINE,
  ...
  public synchronized boolean enqueueForExecution(
    ...
    this.nonDefaultRestartPeriods.put(request.getServiceName(), restartPeriod);
  ...
  public synchronized boolean executedCallback(RCRecordRequest<NodeIDType> request) {
    log.log(
        Level.FINEST,
        "{0} executed request {1}",
    ...
    if (shouldEnqueueEarlyExecutedNotification(request)) this.executed.add(request);
    return equalRemovedFromPending;
  }

  /**
   * We process some failed (handled = false) execution notifications to solve the early notify
   * issue with complete requests for service names, i.e., the notify arrives before the pending
   * request is enqueued but the pending request has been obviated anyway.
   *
   * @param request
   * @param handled
  ...
  public synchronized boolean executedCallback(
    ...
    if (request.getInitiator().equals(this.coordinator.getMyID())
        && (request.isReconfigurationComplete() || request.isReconfigurationPrevDropComplete())
        && !this.isRCGroupName(request)
        && !this.executed.contains(request)) return this.executedCallback(request);
  ...
  private boolean checkIfObviated(RCRecordRequest<NodeIDType> request) {
    if (this.executed.remove(request)) return true;
    for (RCRecordRequest<NodeIDType> executedReq : this.executed)
      if (request.lessThan(executedReq)) return true;
    return false;
  }

  // knocks off elements in set strictly lower than request
  private boolean knockOffLower(
    ...
    for (Iterator<RCRecordRequest<NodeIDType>> reqIter = set.iterator(); reqIter.hasNext(); )
      if ((lower = reqIter.next()).lessThan(request) && (lowerRemoved = true)) {
        log.log(
            Level.FINEST,
            "{0} knocked off lower request {1}",
            new Object[] {this, lower.getSummary(Level.FINEST)});
        reqIter.remove();
        if (isPending) this.lastAttempts.remove(request);
      }
    return lowerRemoved;
  }
  ...
  private boolean isRCGroupName(RCRecordRequest<NodeIDType> request) {
    return (this.coordinator instanceof RepliconfigurableReconfiguratorDB)
        ? (((RepliconfigurableReconfiguratorDB<NodeIDType>) this.coordinator)
            .isRCGroupName(request.getServiceName()))
        : false;
  }

  private boolean shouldEnqueueEarlyExecutedNotification(RCRecordRequest<NodeIDType> request) {
  ...
  private synchronized long coordinate() {
    ...
    for (Iterator<RCRecordRequest<NodeIDType>> reqIter = this.pending.keySet().iterator();
        reqIter.hasNext(); ) {
      ...
      if (repeatable(request)) {
        Level level = request.isReconfigurationMerge() ? Level.FINEST : Level.FINEST;
        log.log(
            level, "{0} coordinating request {1}", new Object[] {this, request.getSummary(level)});

        this.coordinate(request);
        this.lastAttempts.put(request, System.currentTimeMillis());
        oldestAttempt = Math.min(oldestAttempt, this.getLastAttempt(request));
      } else if (this.removable(request)) reqIter.remove();
    }
  ...
  private static final long MAX_PREV_DROP_COMMIT_ATTEMPT_TIME =
      32 * WaitAckStopEpoch.RESTART_PERIOD;

  private boolean repeatable(RCRecordRequest<NodeIDType> request) {
    // else
    return System.currentTimeMillis() - this.getLastAttempt(request)
        > (this.nonDefaultRestartPeriods.containsKey(request.getServiceName())
            ? this.nonDefaultRestartPeriods.get(request.getServiceName())
            : RESTART_PERIOD);
  }

  private boolean removable(RCRecordRequest<NodeIDType> request) {
    if (request.isReconfigurationPrevDropComplete()
        && System.currentTimeMillis() - request.startEpoch.getInitTime()
            < MAX_PREV_DROP_COMMIT_ATTEMPT_TIME) {
      return true;
    }
    return false;
  }
  ...
  private long getWaitTime(long oldestAttempt) {
    return Math.max(RESTART_PERIOD - (System.currentTimeMillis() - oldestAttempt), 0);
  }
  ...
  private String getSetSummary(Set<RCRecordRequest<NodeIDType>> set) {
    String s = "[";
    for (RCRecordRequest<NodeIDType> request : set) s += request.getSummary() + " ";
    return s + "]";
  }

  // for testing
  private void addAssert(
      RCRecordRequest<NodeIDType> request, Set<RCRecordRequest<NodeIDType>> set) {
    boolean contains = set.contains(request);
    if (!contains)
      for (RCRecordRequest<NodeIDType> member : set)
        assert (!member.equals(request))
            : request
  ...
  private final long RESTART_PERIOD =
      Config.getGlobalLong(ReconfigurationConfig.RC.COMMIT_WORKER_RESTART_PERIOD);
  ...
  public CommitWorker(AbstractReplicaCoordinator<?> coordinator, ReconfiguratorCallback callback) {
    ...
    me.setName(CommitWorker.class.getSimpleName() + ":" + coordinator.getMyID());
  }
  ...
  public static void main(String[] args) {
    ...
    curNodes.add(96);
    StartEpoch<Integer> se1 =
        new StartEpoch<Integer>(23, "name", (int) 3, prevNodes, curNodes, null);
    StartEpoch<Integer> se2 =
        new StartEpoch<Integer>(27, "name", (int) 3, curNodes, curNodes, null);
    @SuppressWarnings("deprecation")
    RCRecordRequest<Integer> rc1 =
        new RCRecordRequest<Integer>(23, se1, RCRecordRequest.RequestTypes.RECONFIGURATION_MERGE);
    @SuppressWarnings("deprecation")
    RCRecordRequest<Integer> rc2 =
        new RCRecordRequest<Integer>(92, se2, RCRecordRequest.RequestTypes.RECONFIGURATION_MERGE);
