...
public class WaitEpochFinalState<NodeIDType>
  ...
  public WaitEpochFinalState(
    ...
    this.reqState =
        new RequestEpochFinalState<NodeIDType>(
            myID, startEpoch.getPrevGroupName(), (startEpoch.getPrevEpochNumber()));
    this.key = this.refreshKey();
    this.setPeriod(RESTART_PERIOD);
    this.notifiees.put(this.startEpoch.getInitiator(), this.startEpoch.getKey());
  }
  ...
  public GenericMessagingTask<NodeIDType, ?>[] restart() {
    ...
    if (mtasks != null && ++numRestarts > WARN_NUM_RESTARTS)
      log.log(
          Level.WARNING,
          MyLogger.FORMAT[2],
          new Object[] {getKey(), " resending request to ", mtasks[0].recipients[0]});
    return mtasks;
  }
  ...
  public GenericMessagingTask<NodeIDType, ?>[] start() {
    ...
    log.log(
        Level.INFO,
        "{0} requesting final epoch state {1}",
    ...
    GenericMessagingTask<NodeIDType, ?> mtask =
        new GenericMessagingTask<NodeIDType, Object>(target, this.reqState);
  ...
  private NodeIDType positionIterator() {
    ...
    NodeIDType firstTry =
        this.startEpoch.getFirstPrevEpochCandidate() != null
            ? this.startEpoch.getFirstPrevEpochCandidate()
    ...
    if (!this.startEpoch.getPrevEpochGroup().contains(firstTry)
        || !this.first
        || (this.first = false)) return this.prevGroupIterator.next();
    // else contains firstTry and first time
    while (this.prevGroupIterator.hasNext() && !this.prevGroupIterator.next().equals(firstTry))
      ;
    return firstTry; // leave iterator at self
  }

  private boolean amObviated() {
    Integer curEpoch = this.appCoordinator.getEpoch(this.startEpoch.getServiceName());
    if (curEpoch != null && curEpoch - this.startEpoch.getEpochNumber() >= 0) return true;
    return false;
  }
  ...
  private void sleepOptimization() {
    try {
      ...
      if (this.startEpoch.getFirstPrevEpochCandidate() == null || this.startEpoch.isSplitOrMerge())
        Thread.sleep(200);
  ...
  public String refreshKey() {
    return (Reconfigurator.getTaskKey(
            getClass(), reqState, this.appCoordinator.getMyID().toString())
        +
  ...
  public boolean handleEvent(ProtocolEvent<PacketType, String> event) {
    ...
    switch (type) {
      case EPOCH_FINAL_STATE:
        ...
        if (!checkEpochFinalState(event)) break;
        log.log(
            Level.INFO,
            "{0} received {1}",
   ...
   * @param node
   * @param key
   * @return Node to be notified with AckStartEpoch when epoch final state becomes ready.
   */
  ...
  public GenericMessagingTask<NodeIDType, ?>[] getAckStarts() {
    Set<GenericMessagingTask<NodeIDType, AckStartEpoch<NodeIDType>>> mtasks =
        new HashSet<GenericMessagingTask<NodeIDType, AckStartEpoch<NodeIDType>>>();
    for (NodeIDType node : new HashSet<NodeIDType>(this.notifiees.keySet())) {

      AckStartEpoch<NodeIDType> ackStartEpoch =
      ...
      mtasks.add(
          new GenericMessagingTask<NodeIDType, AckStartEpoch<NodeIDType>>(node, ackStartEpoch));
      log.log(
          Level.INFO,
          "{0} sending {1} to RC {2} with key {3}",
          new Object[] {this, ackStartEpoch.getSummary(), node, this.notifiees.get(node)});
    }
