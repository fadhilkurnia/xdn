 ...
 * @param <Node>
 */
public interface ModifiableActiveConfig<Node> extends ReconfigurableNodeConfig<Node> {

  /**
   * @param id
   * @param sockAddr
   * @return Socket address previously mapped to this id. But we really should not allow mappings to
   *     be changed via an add method.
   */
  ...
  default Map<String, InetSocketAddress> getActiveReplicasMap() {
    return new Map<String, InetSocketAddress>() {
      ...
      public boolean containsKey(Object key) {
        return getActiveReplicasReadOnly().containsKey(valueOf(key.toString()));
      }
      ...
      @Override
      public void putAll(Map<? extends String, ? extends InetSocketAddress> m) {
        throw new UnsupportedOperationException();
      }
      ...
      public Set<String> keySet() {
        ...
        for (Node node : getActiveReplicasReadOnly().keySet()) keys.add(node.toString());
        return keys;
      }
      ...
      public Set<java.util.Map.Entry<String, InetSocketAddress>> entrySet() {
        Set<java.util.Map.Entry<String, InetSocketAddress>> set =
            new HashSet<java.util.Map.Entry<String, InetSocketAddress>>();
        for (Node node : getActiveReplicasReadOnly().keySet())
          set.add(
              new java.util.Map.Entry<String, InetSocketAddress>() {
                ...
                @Override
                public InetSocketAddress setValue(InetSocketAddress value) {
                  throw new UnsupportedOperationException();
                }
              });
        return set;
