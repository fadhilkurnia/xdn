...
public interface ReconfiguratorDB<NodeIDType> {
  ...
  public boolean createReconfigurationRecords(
      ...
      ReconfigureUponActivesChange reconfigureUponActivesChange);
   ...
   * @param name
   * @param epoch
   * @return True if record existed and was successfully marked as pending a deletion.
   */
   ...
   * @param epoch
   * @param state
   * @return True if successfully set; false if the state transition is not legitimate.
   */
  ...
  public boolean setStateInitReconfiguration(
      String name, int epoch, ReconfigurationRecord.RCStates state, Set<NodeIDType> newActives);
   ...
   * @param state
   * @param newActives
   * @return True if successfully set; false if any of the constituent state transitions are not
   *     legitimate.
   */
  public boolean setStateInitReconfiguration(
      Map<String, String> nameStates,
  ...
  /**
   * Merge state, i.e., append instead of replacing state, exactly once. The tricky part is to
   * ensure exactly-once semantics, which we ensure by maintaining a "merged" set in
   ...
   */
  public boolean mergeState(String name, int epoch, String mergee, int mergeeEpoch, String state);
  ...
  /**
   * Clear all merged state in RC record. Invoked when a stop request is executed just before
  ...
  public String fetchAndAggregateMergeeStates(
      Map<String, String> finalStates, String mergerGroup, int epoch);
