...
public interface InitialStateValidator {
  ...
  /**
   * validateInitialState validates the given initialState provided to create a Replica Group. The
   * method must throw InvalidInitialStateException when the provided initialState is invalid.
   * Otherwise, the initialState is assumed to be correct and will be used to create a Replica
   * Group. Doing the validation before Replica Group creation is preferred so all Replicas will be
   * successfully instantiated.
   *
   * <p>Note that the validation *must* be deterministic since it will be executed independently in
   * all replicas and must yield the same result.
   *
