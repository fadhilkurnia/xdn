...
/**
 * @author V. Arun
 *     <p>This abstract class is the parent class for all packets that go back and forth between
 *     clients and reconfigurators. The current instantiations of this class include {@link
 *     CreateServiceName}, {@link DeleteServiceName}, and {@link RequestActiveReplicas}. The same
 *     implementation of ClientReconfigurationPacket is used for both the request and the
 *     corresponding response. The method isRequest() says whether the packet is an incoming request
 *     or an outgoing response.
 */
public abstract class ClientReconfigurationPacket
    extends BasicReconfigurationPacket<InetSocketAddress> implements ReconfiguratorRequest {
  ...
  public static enum ResponseCodes {
    ...
    /** If an active replica receives a request for a name that is not replicated at there. */
    ACTIVE_REPLICA_EXCEPTION,

    /** If a reconfigurator receives a request for a name that does not exist. */
    NONEXISTENT_NAME_ERROR,

    /**
     * Only needed for clients using other protocols (like HTTP) or languages. With the default Java
     * client, malformed requests won't happen, and if they do because of a buggy implementation,
     * such requests will be silently dropped.
     */
    MALFORMED_REQUEST,
  ...
  public static final Stringifiable<InetSocketAddress> unstringer =
   ...
   * @param epochNumber
   */
  public ClientReconfigurationPacket(
   ...
   * @param crp
   */
  public ClientReconfigurationPacket(String name, ClientReconfigurationPacket crp) {
   ...
   */
  public ClientReconfigurationPacket(JSONObject json, Stringifiable<?> unstringer)
      throws JSONException {
    ...
    this.myReceiver =
        json.has(Keys.MY_RECEIVER.toString())
            ? Util.getInetSocketAddressFromString(json.getString(Keys.MY_RECEIVER.toString()))
            : (JSONNIOTransport.getReceiverAddress(json));

    this.failed = json.optBoolean(Keys.FAILED.toString());
    this.recursiveRedirect = json.optBoolean(Keys.RECURSIVE_REDIRECT.toString());
    this.responseMessage =
        json.has(Keys.RESPONSE_MESSAGE.toString())
            ? json.getString(Keys.RESPONSE_MESSAGE.toString())
            : null;

    JSONArray jsonArray =
        json.has(Keys.RECONFIGURATORS.toString())
            ? json.getJSONArray(Keys.RECONFIGURATORS.toString())
            : null;
    if (jsonArray != null) {
      this.hashRCs = new HashSet<InetSocketAddress>();
      for (int i = 0; jsonArray != null && i < jsonArray.length(); i++)
        this.hashRCs.add(RequestActiveReplicas.unstringer.valueOf(jsonArray.get(i).toString()));
    }
    this.forwarder =
        json.has(Keys.FORWARDER.toString())
            ...
            : null;
    this.forwardee =
        json.has(Keys.FORWARDEE.toString())
            ? Util.getInetSocketAddressFromString(json.getString(Keys.FORWARDEE.toString()))
            : null;
    ...
    if (this.creator == null) this.creator = this.getSender();

    this.isRequest = json.getBoolean(Keys.IS_QUERY.toString());

    this.createTime =
        json.has(Keys.CREATE_TIME.toString())
            ? json.getLong(Keys.CREATE_TIME.toString())
            : System.currentTimeMillis();

    this.responseCode =
        json.has(Keys.RESPONSE_CODE.toString())
            ? ResponseCodes.valueOf(json.getString(Keys.RESPONSE_CODE.toString()))
            : null;

    this.state =
        json.has(Keys.INITIAL_STATE.toString())
            ? json.getString(Keys.INITIAL_STATE.toString())
            : null;
  }
  ...
  public JSONObject toJSONObjectImpl() throws JSONException {
    ...
    if (this.hashRCs != null)
      json.put(Keys.RECONFIGURATORS.toString(), new JSONArray(this.hashRCs));
    if (failed) json.put(Keys.FAILED.toString(), this.failed);
    ...
    if (this.forwarder != null) json.put(Keys.FORWARDER.toString(), this.forwarder.toString());
    if (this.forwardee != null) json.put(Keys.FORWARDEE.toString(), this.forwardee.toString());
    if (this.myReceiver != null) json.put(Keys.MY_RECEIVER.toString(), this.myReceiver.toString());
    if (this.creator != null) json.put(Keys.CREATOR.toString(), this.creator.toString());
    json.put(Keys.IS_QUERY.toString(), this.isRequest);
    json.put(Keys.CREATE_TIME.toString(), this.createTime);
    if (this.responseCode != null) json.put(Keys.RESPONSE_CODE.toString(), this.responseCode);
    return json;
  }
  ...
  public ClientReconfigurationPacket setFailed(ResponseCodes code) {
    ...
    if (code != null) this.responseCode = code;
    return this;
  }
  ...
  /**
   * @param RCs Set of consistently hashed reconfigurators for this name.
   * @return {@code this}
   */
  ...
  /**
   * @return True if it has not already been forwarded and recursive redirect is enabled.
   */
  ...
  /**
   * The sender will be different from the forwarder. The initiator is the end client that initiated
   * this request.
   *
   * @return True if this is a response from a node that received a forwarded client request.
   */
