...
/**
 * @author arun
 *     <p>The primary purpose of this class is just to report to an app client that no active
 *     replica was found at the node that received the request using the response code {@link
 *     edu.umass.cs.reconfiguration.reconfigurationpackets.ClientReconfigurationPacket.ResponseCodes#ACTIVE_REPLICA_EXCEPTION}
 *     .
 *     <p>This class is also (ab-)used to report to the app client that the name itself does not
 *     exist; if so, the corresponding respose code is {@link
 *     edu.umass.cs.reconfiguration.reconfigurationpackets.ClientReconfigurationPacket.ResponseCodes#NONEXISTENT_NAME_ERROR}
 *     .
 */
public class ActiveReplicaError extends ClientReconfigurationPacket {
  ...
  public ActiveReplicaError(InetSocketAddress initiator, String name, long requestID) {
    super(initiator, ReconfigurationPacket.PacketType.ACTIVE_REPLICA_ERROR, name, 0);
  ...
  public ActiveReplicaError(JSONObject json) throws JSONException {
    ...
    this.code = ResponseCodes.valueOf(json.getString(Keys.ERROR_CODE.toString()));
  }
