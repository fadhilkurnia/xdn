 ...
 * @author arun
 */
public class ReplicableClientRequest extends JSONPacket
    implements ReplicableRequest, ClientRequest, ReconfigurableRequest, Byteable {
  ...
  /**
   * @param request
   * @param coord If true, the request will be coordinated, else it will not be coordinated. If the
   *     {@code coord} flag is specified, it determines the coordination mode even if the underlying
   *     {@code request} is a {@link ReplicableRequest} and its {@link
   *     ReplicableRequest#needsCoordination()} method returns a conflicting value.
  ...
  ReplicableClientRequest(ByteBuffer bbuf, NIOHeader header, Object parser) {
    ...
    this.requestBytes = Arrays.copyOfRange(bbuf.array(), REQUEST_BYTES_OFFSET, bbuf.array().length);
    this.request = this.parse(this.requestBytes, header, parser);
  }

  private Request parse(byte[] buf, NIOHeader header, Object parser) {
    return parser instanceof AppRequestParserBytes
        ? this.parse(buf, header, (AppRequestParserBytes) parser)
        : this.parse(buf, header, (AppRequestParser) parser);
  }

  private Request parse(byte[] buf, NIOHeader header, AppRequestParserBytes parser) {
    try {
  ...
  ReplicableClientRequest(JSONObject json) throws JSONException, UnsupportedEncodingException {
    this(json, null);
  }
  ...
  public ReplicableClientRequest(JSONObject json, Stringifiable<?> unstringer)
    ...
    this.request = null;
    this.needsCoordination =
        json.has(Keys.COORD.toString()) ? json.getBoolean(Keys.COORD.toString()) : false;
    this.requestBytes = json.getString(Keys.QV.toString()).getBytes(CHARSET);
  }
  ...
  public Request getRequest(AppRequestParser parser)
    ...
    return this.request != null
        ? this.request
        : parser.getRequest(new String(this.requestBytes, CHARSET));
  }
  ...
  public Request getRequest(AppRequestParserBytes parser, NIOHeader header)
      throws UnsupportedEncodingException, RequestParseException {
    return this.request != null ? this.request : parser.getRequest(this.requestBytes, header);
  }
  ...
  public int getEpochNumber() {
    return request instanceof ReconfigurableRequest
        ? ((ReconfigurableRequest) this.request).getEpochNumber()
        : 0;
  }
  ...
  public boolean isStop() {
    return request instanceof ReconfigurableRequest
        ? ((ReconfigurableRequest) this.request).isStop()
        : false;
  }

  @Override
  public ClientRequest getResponse() {
    return request instanceof ClientRequest ? ((ClientRequest) this.request).getResponse() : null;
  }
  ...
  public byte[] toBytes() {
    ...
    try {
      reqBytes =
          this.request instanceof Byteable
              ? ((Byteable) this.request).toBytes()
              : this.request.toString().getBytes(CHARSET);
  ...
  public String getRequestAsString() {
    try {
      ...
      return this.request instanceof Byteable || this.requestBytes == null
  ...
  /**
   * Note: This method returns the stringified form of the underlying request. To get the
   * stringified form of this {@link ReplicableClientRequest}, use {@link #toJSONObject()}.{@link
   * #toString()}.
   */
  @Override
  ...
  public boolean equals(Object o) {
    ...
    return this.requestID == ((ClientRequest) o).getRequestID()
        && (this.needsCoordination
            == (o instanceof ReplicableRequest && ((ReplicableRequest) o).needsCoordination()))
        && (this.request.equals(o)
            || (o instanceof ReplicableClientRequest
                && (this.request.equals(((ReplicableClientRequest) o).request)
                    || Arrays.equals(
