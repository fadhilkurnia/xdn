...
public class RCRecordRequest<NodeIDType> extends BasicReconfigurationPacket<NodeIDType>
    implements ReplicableRequest, ReconfigurableRequest, SummarizableRequest {
  ...
  public static enum RequestTypes {
    /** Step 1 of reconfiguration. Takes RC record from READY(n) to WAIT_ACK_STOP(n). */
    RECONFIGURATION_INTENT,

    /**
     * Step 2 of reconfiguration. We have two steps so that just a single primary actually does the
     * reconfiguration, and the other secondary nodes simply record the corresponding state change.
     * Takes RC record from WAIT_ACK_STOP(n) to READY(n+1). Typically, after
     * RECONFIGURATION_COMPLETE, the new epoch is considered operational.
     */
    RECONFIGURATION_COMPLETE,

    /**
     * Step 3 (optional) of reconfiguration that says that the previous epoch's final state has been
     * completely dropped. At this point, the reconfiguration is really complete. Takes RC record
     * from READY(n) to READY_READY(n) provided all prior reconfigurations have been "clean", i.e.,
     * they involved transitions to READY_READY after a transition to READY. The reason we track
     * this is that clean records can be quickly deleted but unclean records have to wait for a
     * MAX_FINAL_STATE_AGE timeout.
     */
    RECONFIGURATION_PREV_DROPPED,

    /** Merges one RC group with another upon RC node deletes. This type is now deprecated. */
    @Deprecated
   ...
   * @param reqType
   */
  public RCRecordRequest(
      NodeIDType initiator, StartEpoch<NodeIDType> startEpoch, RequestTypes reqType) {
    super(
        initiator,
  ...
  public RCRecordRequest(JSONObject json, Stringifiable<NodeIDType> unstringer)
    ...
    this.reqType = RequestTypes.valueOf(json.get(Keys.REQUEST_TYPE.toString()).toString());
    this.coordType = false;
    this.startEpoch =
        json.has(Keys.START_EPOCH.toString())
  ...
  public JSONObject toJSONObjectImpl() throws JSONException {
    ...
    json.put(BasicReconfigurationPacket.Keys.COORDINATED.toString(), this.coordType);
    json.put(Keys.REQUEST_TYPE.toString(), this.reqType);
    if (this.startEpoch != null)
      json.put(Keys.START_EPOCH.toString(), this.startEpoch.toJSONObject());
  ...
  public boolean isDeleteIntentOrPrevDropComplete() {
    return this.isDeleteIntent() || this.reqType.equals(RequestTypes.RECONFIGURATION_PREV_DROPPED);
  }
  ...
  public String getSummary() {
    return this.getServiceName()
        ...
        + (this.startEpoch.isSplitOrMerge()
            ? ":" + this.startEpoch.getPrevGroupName() + ":" + this.startEpoch.getPrevEpochNumber()
            : "");
  }

  public String getSummary(Level level) {
    if (ReconfigurationConfig.getLogger().isLoggable(level)) return this.getSummary();
    else return null;
  }
  ...
  public boolean equals(Object o) {
    if (!(o instanceof RCRecordRequest)) return false;
    ...
    return this.getServiceName().equals(req2.getServiceName())
        ...
        && (this.startEpoch.getPrevGroupName().equals(req2.startEpoch.getPrevGroupName())
            && this.startEpoch.getPrevEpochNumber() == req2.startEpoch.getPrevEpochNumber());
  }
  ...
  public boolean lessThan(RCRecordRequest<NodeIDType> req2) {
    return this.getServiceName().equals(req2.getServiceName())
        && (this.getEpochNumber() - req2.getEpochNumber() < 0
            || (this.getEpochNumber() == req2.getEpochNumber()
                && this.getRCRequestType().equals(RequestTypes.RECONFIGURATION_INTENT)
                && !req2.getRCRequestType().equals(RequestTypes.RECONFIGURATION_INTENT)));
  }
