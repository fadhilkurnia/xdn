...
/**
 * @author V. Arun
 *     <p>This class has an additional field for returning the set of active replicas in addition to
 *     the fields in the generic ClientReconfigurationPacket. This class is used to both request for
 *     and return the set of active replicas for a name. If the field actives is null, it is
 *     implicitly interpreted as a request, else as a response.
 */
public class RequestActiveReplicas extends ClientReconfigurationPacket
    implements ReplicableRequest {
   ...
   * @param epochNumber
   */
  public RequestActiveReplicas(InetSocketAddress initiator, String name, int epochNumber) {
    super(initiator, ReconfigurationPacket.PacketType.REQUEST_ACTIVE_REPLICAS, name, epochNumber);
    this.actives = null;
   ...
   */
  public RequestActiveReplicas(JSONObject json, Stringifiable<?> unstringer) throws JSONException {
    ...
    JSONArray jsonArray =
        json.has(Keys.ACTIVE_REPLICAS.toString())
            ? json.getJSONArray(Keys.ACTIVE_REPLICAS.toString())
            : null;
    if (jsonArray != null) {
      this.actives = new HashSet<InetSocketAddress>();
      for (int i = 0; jsonArray != null && i < jsonArray.length(); i++)
        this.actives.add(RequestActiveReplicas.unstringer.valueOf(jsonArray.get(i).toString()));
    }
  ...
  public JSONObject toJSONObjectImpl() throws JSONException {
    ...
    if (this.actives != null)
      json.put(Keys.ACTIVE_REPLICAS.toString(), new JSONArray(this.actives));
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      //			assert(req1.toString().equals(new RequestActiveReplicas(req1.toJSONObject(),
      // null).toJSONObject().toString()));
