...
public class EchoRequest extends BasicReconfigurationPacket<InetSocketAddress>
  ...
  /** True if this is a request, false otherwise. Requests beget responses; responses don't. */
   ...
   * @param closest
   */
  public EchoRequest(InetSocketAddress initiator, Map<InetAddress, Long> closest) {
    super(
        initiator,
  ...
  public EchoRequest(JSONObject json, Stringifiable<?> unstringer)
    ...
    this.isRequest =
        json.has(Keys.IS_REQUEST.toString()) ? json.getBoolean(Keys.IS_REQUEST.toString()) : false;
    this.closest =
        json.has(Keys.CLOSEST.toString())
            ? getClosest(json.getJSONArray(Keys.CLOSEST.toString()))
            : null;
   ...
   * @throws UnknownHostException
   */
  public EchoRequest(JSONObject json) throws JSONException, UnknownHostException {
  ...
  private Map<InetAddress, Long> getClosest(JSONArray jarray)
    ...
    if (jarray.length() > 0) {
      ...
      for (int i = 0; i < jarray.length(); i++) {
        nearest.put(
            Util.getInetAddressFromString(jarray.getJSONArray(i).getString(0)),
            jarray.getJSONArray(i).getLong(1));
  ...
  /**
   * @return True if this is a request from an external client to an active replica.
   */
  ...
  public String getSummary() {
    return EchoRequest.this.serviceName
        + ":"
        + EchoRequest.this.requestID
        + ":"
