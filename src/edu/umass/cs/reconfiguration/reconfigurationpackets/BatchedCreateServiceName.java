...
public class BatchedCreateServiceName extends CreateServiceName {
   ...
   * @param name
   * @param epochNumber
   * @param state @TODO Change access to private after moving test to test source
   */
  public BatchedCreateServiceName(
   ...
   * @param names
   * @param epochNumber
   * @param states @TODO Change access to private after moving test to test source
   */
  public BatchedCreateServiceName(
      InetSocketAddress initiator, String[] names, int epochNumber, String[] states) {
  ...
  public BatchedCreateServiceName(JSONObject json, Stringifiable<?> unstringer)
    ...
    JSONArray nameStateArray =
        json.has(BatchKeys.NAME_STATE_ARRAY.toString())
            ? json.getJSONArray(BatchKeys.NAME_STATE_ARRAY.toString())
            : new JSONArray();
    ...
    for (int i = 0; i < nameStateArray.length(); i++) {
      ...
      this.names[i] = nameState.getString(BasicReconfigurationPacket.Keys.NAME.toString());
      this.initialStates[i] = nameState.getString(Keys.STATE.toString());
    }
  }
  ...
  public JSONObject toJSONObjectImpl() throws JSONException {
    ...
    for (int i = 0; i < names.length; i++) {
      ...
      nameState.put(BasicReconfigurationPacket.Keys.NAME.toString(), this.names[i]);
