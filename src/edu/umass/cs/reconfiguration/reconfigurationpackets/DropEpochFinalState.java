...
public class DropEpochFinalState<NodeIDType> extends BasicReconfigurationPacket<NodeIDType> {
   ...
   * @param deleteName
   */
  public DropEpochFinalState(
      NodeIDType initiator, String name, int epochNumber, boolean deleteName) {
   ...
   */
  public DropEpochFinalState(JSONObject json, Stringifiable<NodeIDType> unstringer)
      throws JSONException {
    super(json, unstringer);
    this.deleteName =
        (json.has(Keys.DELETE_NAME.toString())
            ? json.getBoolean(Keys.DELETE_NAME.toString())
            : false);
  }

  /**
   * @return Whether this name is being deleted (as opposed to being reconfigured).
   */
