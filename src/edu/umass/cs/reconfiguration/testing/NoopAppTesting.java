...
/**
 * @author V. Arun
 *     <p>A simple no-op app for testing.
 */
public class NoopAppTesting extends AbstractReconfigurablePaxosApp<String>
  ...
  /**
   * Constructor used to create app replica via reflection. A reconfigurable app must support a
   * constructor with a single String[] as an argument.
   *
   * @param args
   */
  ...
  public boolean execute(Request request, boolean doNotReplyToClient) {
    assert (request instanceof AppRequest || request instanceof RequestPacket)
        : request.getSummary();
  ...
  private boolean processRequest(AppRequest request, boolean doNotReplyToClient) {
    if (request.getServiceName() == null) return true; // no-op
    if (request.isStop()) return processStopRequest(request);
    AppData data = this.appData.get(request.getServiceName());
    if (data == null) {
      System.out.println(
          "App-" + myID + " has no record for " + request.getServiceName() + " for " + request);
    ...
    if (verbose)
      System.out.println(
          "App-" + myID + " wrote to " + data.name + " with state " + data.getState());
    if (DELEGATE_RESPONSE_MESSAGING) this.sendResponse(request);
    else sendResponse(request, doNotReplyToClient);
    return true;
  }

  /**
   * This method exemplifies one way of sending responses back to the client. A cleaner way of
   * sending a simple, single-message response back to the client is to delegate it to the replica
   * coordinator, as exemplified below in {@link #sendResponse(AppRequest)} and supported by
   * gigapaxos.
   *
   * @param request
  ...
  private void sendResponse(AppRequest request, boolean doNotReplyToClient) {
    ...
    if (this.messenger == null || doNotReplyToClient) return;

    InetSocketAddress sockAddr = request.getSenderAddress();
    try {
      this.messenger.getClientMessenger().sendToAddress(sockAddr, request.toJSONObject());
  ...
  private void sendResponse(AppRequest request) {
    ...
    request.setResponse(ResponseCodes.ACK.toString() + " " + numReconfigurationsSinceRecovery);
  }
  ...
  public Request getRequest(String stringified) throws RequestParseException {
    try {
      ...
      ReconfigurationConfig.getLogger()
          .fine("App-" + myID + " unable to parse request " + stringified);
  ...
  public static Request staticGetRequest(String stringified)
    ...
    if (JSONPacket.getPacketType(json) == AppRequest.PacketType.META_REQUEST_PACKET.getInt())
  ...
  public static Request staticGetRequest(byte[] bytes, NIOHeader header)
    ...
    } catch (UnsupportedEncodingException | JSONException | UnknownHostException e) {
  ...
  private static Request getNoopRequest() {
    return new AppRequest(
        null, 0, 0, Request.NO_OP, AppRequest.PacketType.DEFAULT_APP_REQUEST, false);
  }
  ...
  public boolean restore(String name, String state) {
    ...
    if (data == null && state != null) {
      ...
      if (verbose)
        System.out.println(">>>App-" + myID + " creating " + name + " with state " + state);
    ...
    // if state==null => end of epoch
      ...
      if (data != null)
        if (verbose)
          System.out.println(
              "App-" + myID + " deleting " + name + " with final state " + data.state);
    ...
    // typical reconfiguration or epoch change
      ...
      System.out.println("App-" + myID + " updating " + name + " with state " + state);
      data.state = state;
    } else
      // do nothing when data==null && state==null
      ;
    if (state != null) this.appData.put(name, data);
