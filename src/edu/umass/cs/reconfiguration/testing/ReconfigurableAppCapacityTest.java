...
/**
 * @author arun
 *     <p>Tests the capacity of the GNS using a configurable number of async clients.
 */
...
public class ReconfigurableAppCapacityTest extends DefaultTest {
  ...
  static class RCClient extends ReconfigurableAppClientAsync<Request>
  ...
  private static void initStaticParams() {
    ...
    TEST_GUID_PREFIX =
        ReconfigurationConfig.application
            .getSimpleName(); // Config.getGlobalString(TC.TEST_GUID_PREFIX);
    NUM_GROUPS_CLIENT = Config.getGlobalInt(TC.NUM_GROUPS_CLIENT);
    GIBBERISH =
        new String(Util.getRandomAlphanumericBytes(Config.getGlobalInt(TC.REQUEST_BAGGAGE_SIZE)));
  ...
  private static void setupClientsAndGuids() throws Exception {
    ...
    executor = (ScheduledThreadPoolExecutor) Executors.newScheduledThreadPool(NUM_CLIENTS);
    for (int i = 0; i < NUM_CLIENTS; i++) clients[i] = new RCClient();
    ...
    for (int i = 0; i < NUM_GROUPS_CLIENT; i++) {
      ...
      clients[0].sendRequest(
          new CreateServiceName(names[i], "somestate"),
          new RequestCallback() {
            @Override
            public void handleResponse(Request response) {
              ...
              assert (!((ClientReconfigurationPacket) response).isFailed()
                      || ((ClientReconfigurationPacket) response)
                          .getResponseCode()
                          .equals(ClientReconfigurationPacket.ResponseCodes.DUPLICATE_ERROR))
                  : response;
    ...
    System.out.println("Created " + NUM_GROUPS_CLIENT + " names with prefix " + TEST_GUID_PREFIX);
  }
  ...
  private void blockingRead(String name, RCClient client, boolean block) throws IOException {
    Runnable task =
        (new Runnable() {
          public void run() {
            ...
            try {
              client.sendRequest(
                  new MetaRequestPacket(name, GIBBERISH),
                  new RequestCallback() {
  ...
  public void test_02_SequentialReadCapacity() throws Exception {
    ...
    for (int i = 0; i < numReads; i++) this.blockingRead(names[0], clients[0]);
    System.out.print(
        "sequential_read_rate="
  ...
  static synchronized void incrFinishedReads() {
    numFinishedReads++;
  ...
  public void test_04_ParallelReadCapacity() throws Exception {
    for (int k = 0; k < 5; k++) {
      ...
      System.out.print("] ");
      System.out.print(
          "parallel_read_rate=" + Util.df(numReads * 1.0 / (lastReadFinishedTime - t)) + "K/s");
    }
  }
  ...
  public static void cleanup() throws Exception {
    ...
    for (int i = 0; i < NUM_GROUPS_CLIENT; i++) {
      final int j = i;
      clients[0].sendRequest(
          new DeleteServiceName(TEST_GUID_PREFIX + i),
          new RequestCallback() {
    ...
    executor.shutdown();
    for (RCClient client : clients) client.close();
  ...
  public static void main(String[] args) throws IOException {
    ...
    processArgs(args);
    Result result = JUnitCore.runClasses(ReconfigurableAppCapacityTest.class);
