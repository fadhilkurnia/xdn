...
/**
 * @author V. Arun
 *     <p>FIXME: Near-duplication of TESTPaxosClient. Remove redundant code by extending from
 *     TESTPaxosClient.
 */
public class TESTPaxosClient {
  ...
  private static synchronized double getTimeout() {
    return Math.max(
        movingAvgLatency + 4 * movingAvgDeviation, TESTPaxosConfig.CLIENT_REQ_RTX_TIMEOUT);
  }
  ...
  protected static synchronized void resetLatencyComputation(TESTPaxosClient[] clients) {
    runDone = false;
    totalLatency = 0;
    numResponses = 0;
    for (TESTPaxosClient client : clients) client.runReplyCount = 0;
  }
  ...
  private final ConcurrentHashMap<Long, RequestAndCreateTime> requests =
      new ConcurrentHashMap<Long, RequestAndCreateTime>();
  ...
  private static synchronized int incrTotalNoopCount() {
    return totalNoopCount++;
  }

  protected static synchronized int getTotalNoopCount() {
    return totalNoopCount;
  }
  ...
  private class ClientPacketDemultiplexer extends AbstractPacketDemultiplexer<Object> {
    ...
    public boolean handleMessage(RequestPacket request) {
      ...
      try {
        ...
        if (sentRequest != null) {
          ...
          if (!PROBE_CAPACITY) DelayProfiler.updateInterArrivalTime("response_rate", 1, 100);
        ...
        if (request.isNoop()) client.incrNoopCount();
    ...
    protected Object processHeader(byte[] message, NIOHeader header) {
      if (ByteBuffer.wrap(message).getInt() == AppRequest.PacketType.META_REQUEST_PACKET.getInt()) {
        try {
          return MetaRequestPacket.getMetaRequestPacket(message);
        } catch (UnsupportedEncodingException | UnknownHostException | JSONException e) {
          e.printStackTrace();
        }
      }
      ...
      try {
        return new MetaRequestPacket(new JSONObject(MessageExtractor.decode(message)));
      ...
      try {
        ...
        if (requests.containsKey(req.requestID)) {
          ...
          log.log(
              ...
              "{0}{1}{2}{3}{4}{5}",
              new Object[] {
                "Retransmitting request ", "" + req.requestID, " to node ", id, ": ", req
              });
  ...
  protected TESTPaxosClient(int id, NodeConfig<Integer> nc) throws IOException {
    ...
    niot =
        (new MessageNIOTransport<Integer, Object>(
            id,
            this.nc,
            (new ClientPacketDemultiplexer(this)),
            true,
      ...
      if (executor == null || executor.isShutdown())
        ...
        executor =
            (ScheduledThreadPoolExecutor)
                Executors.newScheduledThreadPool(Config.getGlobalInt(TC.NUM_CLIENTS) + 1);
    }
  }
  ...
  private boolean sendRequest(RequestPacket req) throws IOException, JSONException {
    ...
    int index =
        !PIN_CLIENT ? (int) (req.requestID % group.length) : (int) ((myID + 0) % group.length);
  ...
  static synchronized void urc(int id) {
  ...
  static synchronized boolean testAndSetLastWarningTime() {
  ...
  static String gibberish =
      Config.getGlobalBoolean(TC.COMPRESSIBLE_REQUEST)
          ? createGibberishCompressible()
          : createGibberish();

  private static final String createGibberishCompressible() {
    gibberish = GIBBERISH;
    int baggageSize = Config.getGlobalInt(TC.REQUEST_BAGGAGE_SIZE);
    if (gibberish.length() > baggageSize) gibberish = gibberish.substring(0, baggageSize);
    else
      while (gibberish.length() < baggageSize)
        gibberish +=
            (baggageSize > 2 * gibberish.length()
                ? gibberish
  ...
  private static final String createGibberish() {
    ...
    if (gibberish.length() > baggageSize) gibberish = gibberish.substring(0, baggageSize);
    else gibberish += gibberish.substring(0, baggageSize - gibberish.length());
    Util.assertAssertionsEnabled();
    assert (gibberish.length() == baggageSize);
    return gibberish;
  }
  ...
  protected static void sendTestRequests(int numReqs, TESTPaxosClient[] clients, double load)
    ...
    System.out.print(
        "\nTesting "
            ...
            + ", group_size="
            + Config.getGlobalInt(TC.GROUP_SIZE)
            + ", load="
            + Util.df(load)
            + "/s"
            + "]"
    ...
    if (!Config.getGlobalBoolean(TC.PROBE_CAPACITY)) initResponseTracker(clients, numReqs);
    ...
    // if (TOTAL_LOAD > LOAD_THRESHOLD)
    {
      ...
      } else sendTestRequests(numReqs, clients, false, load);
    }
  ...
  private static void sendTestRequests(
      int numReqs, TESTPaxosClient[] clients, boolean warmup, double rate)
      throws JSONException, IOException, InterruptedException, ExecutionException {
    if (warmup)
      System.out.print(
          (warmup ? "\nWarming up " : "\nTesting ")
    ...
    for (int i = 0; i < numReqs; i++) {
      while (!clients[i % NUM_CLIENTS].makeAndSendRequest(
          TEST_GUID_PREFIX
    ...
    if (warmup) System.out.println("done");
  }

  protected static void waitForResponses(
      TESTPaxosClient[] clients, long startTime, int numRequests) {
    for (int i = 0; i < Config.getGlobalInt(TC.NUM_CLIENTS); i++) {
      while ((numResponses < numRequests) || (clients[i].requests.size() > 0)) {
        ...
        if (System.currentTimeMillis() - startTime > MAX_WAIT_TIME) {
          String err =
              ("Timing out after having received "
                  + numResponses
                  + " responses for "
                  + numRequests
                  + " requests");
    ...
    assert (numResponses >= numRequests && noOutstanding(clients))
        : numResponses + " responses received for " + numRequests + " requests";
    synchronized (TESTPaxosClient.class) {
  ...
  private static void clearOutstanding(TESTPaxosClient[] clients) {
    for (TESTPaxosClient client : clients) client.requests.clear();
  }
  ...
  protected static Set<RequestPacket> getMissingRequests(TESTPaxosClient[] clients) {
    ...
    for (int i = 0; i < clients.length; i++) {
      for (RequestAndCreateTime reqCT : clients[i].requests.values()) missing.add(reqCT.request);
    }
    return missing;
  }
  ...
  /**
   * This method probes for the capacity by multiplicatively increasing the load until the response
   * rate is at least a threshold fraction of the injected load and the average response time is
   * within a threshold. These thresholds are picked up from {@link TESTPaxosConfig}.
   *
   * @param load
   * @param clients
   * @return
  ...
  private static double probeCapacity(double load, TESTPaxosClient[] clients)
    ...
    do {
      if (runs++ > 0)
        ...
        if (consecutiveFailures == 0) load *= loadIncreaseFactor;
        else
      ...
      if (consecutiveFailures == 2) loadIncreaseFactor = (1 + (loadIncreaseFactor - 1) / 2);

      // we are within roughly 0.1% of capacity
      if (loadIncreaseFactor < minLoadIncreaseFactor) break;
      ...
      while (numResponses < threshold * numRunRequests) Thread.sleep(500);
    ...
    System.out.println(
        "capacity <= "
            ...
            + (capacity < threshold * load
                ? " response_rate was less than 95% of injected load" + Util.df(load) + "/s; "
                : "")
            + (latency > Config.getGlobalLong(TC.PROBE_LATENCY_THRESHOLD)
                ? " average_response_time="
                    + Util.df(latency)
  ...
  public static void main(String[] args) {
    try {
      ...
      TESTPaxosClient[] clients =
          TESTPaxosClient.setupClients(TESTPaxosConfig.getFromPaxosConfig(true));
      ...
      else TESTPaxosClient.twoPhaseTest(numReqs, clients);
  ...
  private static void initStaticParams() {
    ...
    MAX_WAIT_TIME =
        (long) (Config.getGlobalInt(TC.NUM_REQUESTS) / Config.getGlobalDouble(TC.TOTAL_LOAD))
