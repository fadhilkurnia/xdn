...
/**
 * @author arun
 *     <p>The tests in TESTReconfigurationClient invoked herein test basic operations like creation,
 *     request active replicas, app requests, and deletions with reconfigurations enabled.
 *     <p>TODOS: (1) Add tests with different numbers of reconfigurators and actives.
 */
public class TESTReconfigurationMain {

  static ReconfigurableNodeConfig<String> dnc = null;

  private static Set<ReconfigurableNode<?>> startReconfigurators(String[] args) throws IOException {
  ...
  private static Set<ReconfigurableNode<?>> startActives(String[] args) throws IOException {
    Set<ReconfigurableNode<?>> createdNodes = new HashSet<ReconfigurableNode<?>>();
    ...
    System.out.println(
        "] with application [" + ReconfigurationConfig.application.getCanonicalName() + "]");
    return createdNodes;
  }

  private static Set<ReconfigurableNode<?>> reconfigurators = null, actives = null;
   ...
   */
  // @BeforeClass
  public static void startLocalServers() throws IOException, InterruptedException {
  ...
  public static void closeServers() throws IOException, InterruptedException {
    ...
    if (reconfigurators != null) for (ReconfigurableNode<?> node : reconfigurators) node.close();
    if (actives != null) for (ReconfigurableNode<?> node : actives) node.close();
  }

  protected static void wipeoutServers() throws IOException, InterruptedException {
    for (ReconfigurableNode<?> node : reconfigurators) wipeout(node);
    for (ReconfigurableNode<?> node : actives) wipeout(node);
  }
