...
/**
 * @author arun
 *     <p>A request packet for testing in a manner very similar to TESTPaxosClient and RequestPacket
 *     but using {@link TESTPaxosClient}.
 */
public class MetaRequestPacket extends RequestPacket {
  ...
  public static MetaRequestPacket getMetaRequestPacket(byte[] bytes)
      throws UnsupportedEncodingException, UnknownHostException, JSONException {
    MetaRequestPacket meta = null;
    if (BYTEIFICATION) {
      meta = new MetaRequestPacket(bytes);
    } else {
      meta = new MetaRequestPacket(new JSONObject(new String(bytes, NIOHeader.CHARSET)));
    }
  ...
  public byte[] toBytes() {
    if (BYTEIFICATION)
      return ByteBuffer.wrap(super.toBytes())
          .putInt(
              USE_REQUEST_PACKET
    ...
    try {
      ...
      if (USE_REQUEST_PACKET)
        JSONPacket.putPacketType(json, AppRequest.PacketType.META_REQUEST_PACKET.getInt());
