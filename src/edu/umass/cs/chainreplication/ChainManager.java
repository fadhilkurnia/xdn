...
/**
 * @author Z Gao
 *     <p>ChainManager is the primary class to manage and use chain replication.
 *     <p>ChainManager manages all chains at a node. There is supposed to be one chain manager per
 *     machine.
 *     <p>ChainManager does not support hot swap as described in GigaPaxos paper.
 */
public class ChainManager<NodeIDType> {
  ...
  public ChainManager(
    ...
    this.largeCheckpointer = new LargeCheckpointer(logFolder, id.toString());
  ...
  public int getVersion(String chainID) {
    ...
    if (crsm != null) return (int) crsm.getVersion();
  ...
  /**
   * handleChainPacket method abandoned the design in {@link PaxosManager} handlePaxosPacket, which
   * creates MessageTasks to process the current Request packet. We
   *
   * @param cp
   * @param rcsm
   */
  private void handleChainPacket(ChainPacket cp, ReplicatedChainStateMachine rcsm) {
    ...
    ChainPacket.ChainPacketType packetType =
        cp != null ? cp.getType() : ChainPacket.ChainPacketType.NO_TYPE;

    // forward to the next node in the chain
    log.log(
        level,
        "ChainManager.handleChainPacket: members: {0}, + myID(NodeIDType):{1}, myID:{2},"
            + " request:{3}",
        new Object[] {
          this.integerMap.getIntArrayAsNodeSet(rcsm.getMembers()),
          this.myID,
  ...
  /**
   * Check whether myID is the same as the head node ID. If so, call handleForward to forward the
   * replication request. Otherwise, forward the request to the head node.
   */
  private void handleChainRequest(ChainPacket cp, ReplicatedChainStateMachine rcsm) {
    if (((ChainRequestPacket) cp).getEntryReplica() == IntegerMap.NULL_INT_NODE)
      ((ChainRequestPacket) cp).setEntryReplica(this.myID);

    log.log(
        level,
        "ChainManager.handleChainRequest handles " + "ChainPacket {0} for state machine {1}",
        new Object[] {cp, rcsm});
   ...
   * @param rcsm
   */
  private void handleForwardRequest(ChainPacket cp, ReplicatedChainStateMachine rcsm) {

    log.log(
        level,
        "ChainManager.handleForward handles " + "ChainPacket {0} for state machine {1}",
        new Object[] {cp, rcsm});
  ...
  private void handleAckRequest(ChainPacket cp, ReplicatedChainStateMachine rcsm) {
    ...
    if (rcsm.getChainHead() == this.myID) {
      ...
      log.log(
          level,
          "ChainManager.handleAck received an ACK request {0}," + " for state machine {1}",
          new Object[] {cp, rcsm});
  ...
  private void handleResponse(ChainPacket cp, ReplicatedChainStateMachine rcsm) {

    // About to send response back to client
    long requestID = ((ChainRequestPacket) cp).requestID;
    ChainRequestAndCallback requestAndCallback = outstanding.dequeue((ChainRequestPacket) cp);
    log.log(
        level,
    ...
    if (requestAndCallback != null && requestAndCallback.callback != null) {
      ...
      Request request =
          getInterfaceRequest(this.myApp, requestAndCallback.chainRequestPacket.requestValue);
      ...
      requestAndCallback.callback.executed(request, true);

    } else {
      // can't find the request being queued in outstanding
      log.log(
          Level.WARNING,
          "ChainManager.handleResponse received "
   ...
   * @param rcsm
   */
  private void handleRead(ChainPacket cp, ReplicatedChainStateMachine rcsm) {
    ...
    if (rcsm.getChainTail() == this.myID && crp.getEntryReplica() == this.myID) {
      ...
      ChainRequestAndCallback requestAndCallback = outstanding.dequeue(crp);
      //
      if (requestAndCallback != null && requestAndCallback.callback != null) {
  ...
  /**
   * Once a failure is detected, we can send a request through this method. TODO: need a new demand
   * profile for chain replication reconfiguration
   */
    ...
    GenericMessagingTask<NodeIDType, ?> mtask =
        new GenericMessagingTask<NodeIDType, Object>(
            ...
            (new DemandReport<NodeIDType>(
                this.messenger.getMyID(), demand.getName(), (epoch == null ? 0 : epoch), demand)));
  ...
  public String propose(String chainID, Request request, ExecutedCallback callback) {

    ChainRequestPacket chainRequestPacket = this.getChainRequestPacket(request, false);

    log.log(
        level,
        "ChainManager.propsoe request " + "{0} for service name {1}",
    ...
    log.log(
        level,
        "ChainManager.propsoe retrieves ReplicatedChainStateMachine {0}",
        new Object[] {rcsm});
    if (rcsm != null) {
      matched = true;
      // put chainID and version into request
      chainRequestPacket.putChainIDAndVersion(chainID, rcsm.getVersion());
      this.outstanding.enqueue(new ChainRequestAndCallback(chainRequestPacket, callback));

      log.log(
          level,
          "ChainManager.propsoe starts handling ChainRequestPacket:{0}",
      ...
      log.log(
          Level.INFO,
          "{0} could not find paxos instance {1} for "
              + "request {2} with body {3}; last known version was [{4}]",
          new Object[] {
            this,
            chainID,
            chainRequestPacket.getSummary(),
            Util.truncate(chainRequestPacket.getRequestValues()[0], 64),
            this.getVersion(chainID)
          });
    }
  ...
  public boolean createReplicatedChainForcibly(
      String chainID, int version, Set<NodeIDType> nodes, Replicable app, String state) {
  ...
  private synchronized ReplicatedChainStateMachine createReplicatedChainFinal(
      String chainID, int version, Set<NodeIDType> nodes, Replicable app, String initialState) {
    if (this.isClosed()) return null;

    if (!nodes.contains(this.getNodeID()))
      throw new ReplicatedChainException(
          this.getNodeID()
              + " can not create a replicated chain for group "
              + nodes
    ...
    ReplicatedChainStateMachine rcsm = this.getInstance(chainID);
    if (rcsm != null) return rcsm;

    try {
      // else try to create (could still run into exception)
      rcsm =
          new ReplicatedChainStateMachine(
              chainID,
              version,
              myID,
      ...
      log.log(
          level,
          "Create replicated chain state machine for " + "chainID {0}: {1} on {2}:{3}",
  ...
  private ChainRequestPacket getChainRequestPacket(Request request, boolean stop) {
    ...
    if (request instanceof ChainRequestPacket
      ...
      return (stop == ((ChainRequestPacket) request).stop)
          ? (ChainRequestPacket) request
          : new ChainRequestPacket(
              ((ChainRequestPacket) request).requestID,
              ((ChainRequestPacket) request).requestValue,
              stop,
              (ChainRequestPacket) request);
  ...
  /** The class is used to keep track of outstanding requests that have not been acknowledged. */
      ...
      synchronized (this.requests) {
        ...
        if (prev != null) {
          ...
          log.log(
              ...
              new Object[] {
                requestAndCallback.chainRequestPacket.getRequestID(),
                requestAndCallback.chainRequestPacket
              });
        }
      }
    }
   ...
   * processHeader: convert a byte[] to ChainPacket getPacketType: get packet type of a ChainPacket
   */
  class ChainDemultiplexer extends AbstractPacketDemultiplexer<Object> {

    final ChainManager manager;

    public ChainDemultiplexer(int numThreads, ChainManager manager) {
      super(numThreads);
    ...
    protected Integer getPacketType(Object message) {
      ...
      return (message instanceof byte[])
          ? ByteBuffer.wrap((byte[]) message, 0, 4).getInt()
    ...
    public boolean handleMessage(Object message, NIOHeader header) {
      ...
      ChainManager.log.log(
          level,
          "ChainDemultiplexer.handleMessage: receive message {0}" + " with NIOHeader {1}",
          new Object[] {message.toString(), header});
      ...
      if (message instanceof byte[]) {
        ...
        try {
          packet = new ChainRequestPacket(bbuf);
          this.manager.handleChainPacket(packet, this.manager.getInstance(packet.getChainID()));
          return true;
        } catch (UnsupportedEncodingException | UnknownHostException e) {
          e.printStackTrace();
        }
