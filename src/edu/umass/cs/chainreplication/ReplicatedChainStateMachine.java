...
/**
 * This class keeps the state of a replicated chain used by chain replication algorithm. The
 * information kept by this class is as follows: (1) members of the chain (from head to tail, tail
 * is the primary) (2) id of the chain (3) current version of the chain (4) a pointer to the
 * ChainManager of this node
 *
 * <p>A typical process to coordinate a request is as this: A write request is first sent to the
 * head, which then passes the update along the chain to the tail (primary). All read requests are
 * sent to the tail (primary).
 *
 * <p>TODO: reconfiguration, fault-tolerance
 *
 * @author Z Gao
 */
public class ReplicatedChainStateMachine {
  ...
  public ReplicatedChainStateMachine(
      String chainID,
      int version,
      int id,
      Set<Integer> chainMembers,
      Replicable app,
      String initialState,
      ChainManager<?> cm) {
    ...
    if (found && idx < this.chainMembers.length - 1) this.next = this.chainMembers[idx + 1];
    else {
  ...
  */
  ...
  protected String getChainID() {
    return (chainID instanceof String ? (String) chainID : new String((byte[]) chainID));
  }
