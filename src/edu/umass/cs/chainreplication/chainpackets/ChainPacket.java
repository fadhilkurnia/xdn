...
public abstract class ChainPacket extends JSONPacket {
  ...
  protected ChainPacket(JSONObject json) throws JSONException {
    ...
    if (json != null) {
      if (json.has(ChainPacket.Keys.PT.toString()))
        this.packetType =
            ChainPacketType.getChainPacketType(json.getInt(ChainPacket.Keys.PT.toString()));
  ...
  protected ChainPacket(ByteBuffer bbuf) throws UnsupportedEncodingException {
    ...
    this.chainID = chainIDBytes.length > 0 ? new String(chainIDBytes, CHARSET) : null;
  ...
  protected ByteBuffer toBytes(ByteBuffer bbuf) throws UnsupportedEncodingException {
    ...
    byte[] chainIDBytes = this.chainID != null ? this.chainID.getBytes(CHARSET) : new byte[0];
  ...
  /**
   * To avoid potential conflict with existing {@link PaxosPacket} PaxosPacketType, ChainPacketType
   * starts from 1000 to 1100
   */
  public enum ChainPacketType implements IntegerPacketType {
    ...
    private static HashMap<Integer, ChainPacketType> numbers =
        new HashMap<Integer, ChainPacketType>();
