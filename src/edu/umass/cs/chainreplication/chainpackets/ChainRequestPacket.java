...
public class ChainRequestPacket extends ChainPacket implements Request, ClientRequest, Byteable {
  ...
  public ChainRequestPacket(long reqID, String value, boolean stop, ChainRequestPacket req) {
  ...
  public ChainRequestPacket(long reqID, String value, boolean stop, InetSocketAddress csa) {
    this(reqID, value, stop, (ChainRequestPacket) null);
    this.clientSocketAddress = csa;
  }

  public ChainRequestPacket(ByteBuffer bbuf)
    ...
    this.clientSocketAddress =
        cport != 0
            ? new InetSocketAddress(InetAddress.getByAddress(clientAddressBytes), cport)
            : null;
    ...
    this.listenSocketAddress =
        lport != 0
            ? new InetSocketAddress(InetAddress.getByAddress(listenAddressBytes), lport)
            : null;
    ...
    try {
      ...
      bbuf.put(
          this.clientSocketAddress != null
              ? this.clientSocketAddress.getAddress().getAddress()
              : new byte[4]);
      // 0 (not -1) means invalid port
      bbuf.putShort(
          this.clientSocketAddress != null ? (short) this.clientSocketAddress.getPort() : 0);
      ...
      bbuf.put(
          this.listenSocketAddress != null
              ? this.listenSocketAddress.getAddress().getAddress()
              : new byte[4]);
      // 0 (not -1) means invalid port
      bbuf.putShort(
          this.listenSocketAddress != null ? (short) this.listenSocketAddress.getPort() : 0);
  ...
  private static final int EXPECTED_LENGTH_WITH_FIXED_FIELD =
      Long.BYTES // request ID: long
          + 1 // stop: boolean
  ...
  public ClientRequest getResponse() {
    ...
    ChainRequestPacket reply =
        new ChainRequestPacket(
            this.requestID, RequestPacket.ResponseCodes.ACK.toString(), this.stop, this);
    System.out.println(
  ...
  protected JSONObject toJSONObjectImpl() throws JSONException {
    ...
    if (this.clientSocketAddress != null) json.put(Keys.CA.toString(), this.clientSocketAddress);
    if (this.listenSocketAddress != null) json.put(Keys.LA.toString(), this.listenSocketAddress);
