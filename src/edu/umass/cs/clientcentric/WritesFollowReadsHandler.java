...
public class WritesFollowReadsHandler {
  ...
  private static final Logger logger =
      Logger.getLogger(WritesFollowReadsHandler.class.getSimpleName());
  ...
  private static <NodeIDType> boolean handleClientRequest(
    ...
    if (logger.isLoggable(Level.FINE)) {
      logger.log(
          ...
          String.format(
              ...
              messenger.getMyID(),
              WritesFollowReadsHandler.class.getSimpleName(),
    ...
    if (behavioralRequest.isWriteOnlyRequest() || behavioralRequest.isReadModifyWriteRequest()) {
      // Dispatch immediately to executePool — timestamp check runs inside
      // the lambda to free the caller thread.
      final VectorTimestamp finalReadTs2 = requestLastReadTimestamp;
      executePool.execute(
          () -> {
              ...
              try {
                if (logger.isLoggable(Level.INFO)) {
                  logger.log(
                      Level.INFO,
                      String.format(
                          "%s:%s - sending WriteAfterPacket for reqId=%d",
                          myNodeID,
                          WritesFollowReadsHandler.class.getSimpleName(),
            ...
            serviceInstance
                .pendingRequests()
                .put(requestID, new RequestAndCallback(clientRequest, finalReadTs2, callback));
            ...
            for (String nodeIdRaw : serviceLastTimestamp.getNodeIds()) {
              ...
              long clientTs = finalReadTs2.getNodeTimestamp(nodeIdRaw);
  ...
  private static <NodeIDType> void handleCoordinationPacket(
    ...
    if (logger.isLoggable(Level.FINE)) {
      logger.log(
          Level.FINE,
          String.format(
              "%s:%s - handling coordination packet %s for name=%s",
              myNodeID,
              WritesFollowReadsHandler.class.getSimpleName(),
  ...
  private static <NodeIDType> void processBufferedWriteRequests(
    ...
    while (it.hasNext()) {
      ...
      if (!isExecSuccess) {
        logger.log(Level.WARNING, "Failed to execute write request: " + clientWriteRequest);
        continue;
      }
      ...
      if (logger.isLoggable(Level.INFO)) {
        logger.log(
            Level.INFO,
            String.format(
                "%s:%s - just executed a pending write request with id=%d",
                myNodeId,
                WritesFollowReadsHandler.class.getSimpleName(),
      ...
      ((TimestampedResponse) clientWriteRequest).setLastTimestamp("W", serviceLastTimestamp);
