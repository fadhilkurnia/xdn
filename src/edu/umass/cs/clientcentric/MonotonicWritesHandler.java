...
public class MonotonicWritesHandler {
  ...
  private static final Logger logger =
      Logger.getLogger(MonotonicWritesHandler.class.getSimpleName());
  ...
  private static <NodeIDType> boolean handleClientRequest(
    ...
    if (logger.isLoggable(Level.FINER)) {
      logger.log(
          Level.FINER,
          String.format(
              "%s:%s - handling client request %s id=%d",
              messenger.getMyID(),
    ...
    // Handle read request — async dispatch to avoid blocking the caller thread.
    if (behavioralRequest.isReadOnlyRequest()) {
    ...
    // Handle write request — dispatch immediately to executePool, with the
    // timestamp check inside the lambda to free the caller thread.
            ...
            if (finalWriteTs.isLessThanOrEqualTo(serviceLastTimestamp)) {
              ...
              try {
                if (logger.isLoggable(Level.FINER)) {
                  logger.log(
                      Level.FINER,
                      "Sending ClientCentricWriteAfterPacket: "
  ...
  private static <NodeIDType> void handleCoordinationPacket(
    ...
    if (logger.isLoggable(Level.FINER)) {
      logger.log(
          Level.FINER,
          String.format(
              "%s:%s - handling coordination packet %s id=%d",
              messenger.getMyID(),
              MonotonicWritesHandler.class.getSimpleName(),
      ...
      if (ourTs >= theirTs) {
        ...
        if (logger.isLoggable(Level.FINER)) {
          logger.log(
              Level.FINER,
              String.format(
                  "%s:%s - ignoring stale write request ourTs=%s theirTs=%s",
                  messenger.getMyID(),
                  MonotonicWritesHandler.class.getSimpleName(),
                  ourTs,
                  theirTs));
      ...
      if (ourTs == theirTs - 1) {
        ...
        if (logger.isLoggable(Level.FINER)) {
          logger.log(
              Level.FINER,
              String.format(
                  "%s:%s - executed the write after request, updating ts to %s",
                  messenger.getMyID(),
                  MonotonicWritesHandler.class.getSimpleName(),
        ...
        try {
          if (logger.isLoggable(Level.INFO)) {
            logger.log(
                Level.INFO,
                String.format(
                    "%s:%s - missing updates, thus need to send ClientCentricSyncRequestPacket to"
                        + " %s",
                    messenger.getMyID(),
                    MonotonicWritesHandler.class.getSimpleName(),
                    rawSenderID));
  ...
  private static <NodeIDType> void processBufferedWriteRequests(
    ...
    VectorTimestamp serviceLastTimestamp = serviceInstance.currTimestamp();
    Map<Long, RequestAndCallback> pendingWriteRequests = serviceInstance.pendingRequests();

    // Iterate through all the pending write requests and execute them if our current
    // timestamp is already "recent" enough.
    Iterator<Map.Entry<Long, RequestAndCallback>> it = pendingWriteRequests.entrySet().iterator();
    while (it.hasNext()) {
      ...
      if (!isExecSuccess) {
        logger.log(Level.WARNING, "Failed to execute write request: " + clientReadRequest);
        continue;
      }

      // send response back to client
      assert clientReadRequest instanceof TimestampedResponse;
      ((TimestampedResponse) clientReadRequest).setLastTimestamp("W", serviceLastTimestamp);
