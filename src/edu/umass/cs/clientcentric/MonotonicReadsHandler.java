...
public class MonotonicReadsHandler {
  ...
  private static final Logger logger =
      Logger.getLogger(MonotonicReadsHandler.class.getSimpleName());
  ...
  protected static <NodeIDType> boolean coordinateRequest(
    ...
    if (request instanceof ReplicableClientRequest rcr) {
      return handleClientRequest(
          rcr, callback, serviceInstance, app, nodeIdDeserializer, messenger, executePool);
    }

    if (request instanceof ClientCentricPacket packet) {
      handleCoordinationPacket(packet, serviceInstance, app, nodeIdDeserializer, messenger);
      return true;
    }

    throw new RuntimeException(
        "Unknown request/packet handled by "
  ...
  private static <NodeIDType> boolean handleClientRequest(
    ...
    if (!(clientRequest instanceof ClientRequest)
      ...
      throw new RuntimeException(
          "ClientCentricReplicaCoordinator can only handle "
    ...
    if (!(clientRequest instanceof BehavioralRequest behavioralRequest)) {
      throw new RuntimeException(
          "ClientCentricReplicaCoordinator can only handle " + "BehavioralRequest");
    }
    ...
    if (logger.isLoggable(Level.FINE)) {
      logger.log(
          Level.FINE,
          String.format(
              "%s:%s - handling client request %s id=%d clientReadTs=%s ourTs=%s",
              messenger.getMyID(),
              MonotonicReadsHandler.class.getSimpleName(),
    ...
    // handling read request — dispatch immediately to the execute pool so the
    // caller (writePool) thread returns in microseconds. The timestamp check and
    // potential sync logic run on the executePool thread instead.
    if (behavioralRequest.isReadOnlyRequest()) {
      final VectorTimestamp finalReadTs = requestLastReadTimestamp;
      executePool.execute(
          () -> {
            // Case-1: handle read locally if the request's read timestamp is
            // less than or equal to the service's current timestamp.
            if (finalReadTs.isLessThanOrEqualTo(serviceLastTimestamp)) {
              ...
              if (finalReadTs.isLessThan(serviceLastTimestamp)) {
                ((TimestampedResponse) clientRequest).setLastTimestamp("R", serviceLastTimestamp);
              }
            ...
            serviceInstance
                .pendingRequests()
                .put(requestID, new RequestAndCallback(clientRequest, finalReadTs, callback));
            ...
            for (String nodeIdRaw : serviceLastTimestamp.getNodeIds()) {
              ...
              long clientTs = finalReadTs.getNodeTimestamp(nodeIdRaw);
              ...
              if (clientTs > replicaTs) {
                ...
                if (logger.isLoggable(Level.INFO)) {
                  logger.log(
                      Level.INFO,
                      String.format(
                          "%s:%s - preparing SyncRequestPacket to %s, clientTs=%d ourTs=%d",
                          messenger.getMyID(),
                          MonotonicReadsHandler.class.getSimpleName(),
                          nodeIdRaw,
                          clientTs,
    ...
    // handling write request — async dispatch to avoid blocking the caller thread
    // on the container HTTP round-trip inside app.execute().
    if (behavioralRequest.isWriteOnlyRequest() || behavioralRequest.isReadModifyWriteRequest()) {
      executePool.execute(
          () -> {
            // execute the write request (blocking HTTP call to container)
            boolean isExecSuccess = app.execute(clientRequest, false);
            if (!isExecSuccess) {
              logger.log(Level.WARNING, "Failed to execute request: " + clientRequest);
              callback.executed(clientRequest, false);
            ...
            VectorTimestamp updatedTimestamp =
                serviceInstance.currTimestamp().increaseNodeTimestamp(myNodeID.toString());

            // send response back to client, along with the service's latest timestamp
            ((TimestampedResponse) clientRequest).setLastTimestamp("W", updatedTimestamp);
            callback.executed(clientRequest, true);
            ...
            try {
              if (logger.isLoggable(Level.FINER)) {
                logger.log(
                    Level.FINER,
                    "Sending ClientCentricWriteAfterPacket: " + writeAfterPacket.getServiceName());
              }
              messenger.send(m);
  ...
  private static <NodeIDType> void handleCoordinationPacket(
    ...
    if (logger.isLoggable(Level.FINE)) {
      logger.log(
          Level.FINE,
          String.format(
              "%s:%s - handling coordination packet %s",
    ...
    if (packet.getRequestType() == ClientCentricPacketType.CLIENT_CENTRIC_WRITE_AFTER_PACKET) {
      ...
      //  Thus, we need to send sync packet.
      {
        ...
        Long prevPeerRequestedFromSeqNum =
            serviceInstance
                .peerLastSyncRequestSeqNum()
                .get(nodeIdDeserializer.valueOf(rawSenderID));
        ...
        GenericMessagingTask<NodeIDType, ClientCentricPacket> m =
            new GenericMessagingTask<>(nodeIdDeserializer.valueOf(rawSenderID), syncRequestPacket);

        // cache the last requested seqNum to prevent storming the same peer with same requests
        serviceInstance
            .peerLastSyncRequestSeqNum()
            .put(nodeIdDeserializer.valueOf(rawSenderID), peerFromSeqNum);

        try {
          logger.log(
              Level.INFO,
              String.format(
                  "%s:%s - missing writes (theirTs=%d), thus requesting them from %s with"
                      + " startSeqNum=%d",
                  myNodeId,
                  MonotonicReadsHandler.class.getSimpleName(),
                  theirTs,
    ...
    if (packet.getRequestType() == ClientCentricPacketType.CLIENT_CENTRIC_SYNC_REQ_PACKET) {
      ...
      synchronized (serviceInstance.executedRequests()) {
        ...
        List<byte[]> subList =
            serviceInstance.executedRequests().subList((int) theirReqSeq - 1, (int) size);
      ...
      try {
        logger.log(
            Level.FINER,
            "Sending ClientCentricSyncResponsePacket: " + responsePacket.getServiceName());
        messenger.send(m);
      } catch (IOException | JSONException e) {
        throw new RuntimeException(e);
      }
    ...
    if (packet.getRequestType() == ClientCentricPacketType.CLIENT_CENTRIC_SYNC_RES_PACKET) {
      ...
      logger.log(
          Level.INFO,
          String.format(
              "%s:%s - handling SyncResponsePacket from %s, theirSeqNum=%d ourSeqNum=%d",
              messenger.getMyID(),
              MonotonicReadsHandler.class.getSimpleName(),
              senderId,
              respStartingSeqNum,
      ...
      if (respStartingSeqNum > ourLatestSeqNum + 1) {
        logger.log(
            Level.WARNING,
            String.format(
                "%s:%s - detecting missing operations from %s, theirSeqNum=%d ourSeqNum=%d",
                messenger.getMyID(),
                ReadYourWritesHandler.class.getSimpleName(),
                senderId,
                respStartingSeqNum,
                ourLatestSeqNum));
        return;
      ...
      if (respStartingSeqNum <= ourLatestSeqNum + 1) {
        ...
        for (long currSeqNum = respStartingSeqNum; currSeqNum <= lastSeqNum; ++currSeqNum) {
          ...
          try {
            ...
            Request appReq = app.getRequest(new String(encodedReq, StandardCharsets.ISO_8859_1));
    ...
    throw new IllegalStateException("Unexpected ClientCentricPacket: " + packet.getRequestType());
  }

  private static <NodeIDType> void processBufferedReadRequests(
      BayouReplicaCoordinator.ReplicaInstance<NodeIDType> serviceInstance, Replicable app) {
    ...
    VectorTimestamp serviceLastTimestamp = serviceInstance.currTimestamp();
    Map<Long, RequestAndCallback> pendingReadRequests = serviceInstance.pendingRequests();

    // Iterate through all the pending requests and execute them if our current
    // timestamp is already "recent" enough.
    Iterator<Map.Entry<Long, RequestAndCallback>> it = pendingReadRequests.entrySet().iterator();
    while (it.hasNext()) {
      ...
      if (!isExecSuccess) {
        logger.log(Level.WARNING, "Failed to execute read request: " + clientReadRequest);
        continue;
      }

      // send response back to client
      assert clientReadRequest instanceof TimestampedResponse;
      ((TimestampedResponse) clientReadRequest).setLastTimestamp("R", serviceLastTimestamp);
