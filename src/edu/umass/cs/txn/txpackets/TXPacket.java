...
/**
 * @author arun
 *     <p>All transaction processing related packets inherit from this class.
 */
public abstract class TXPacket extends JSONPacket implements ReplicableRequest, ClientRequest {
  ...
  /**
   * @return The ID of the transaction to which this packet corresponds. The initiator is set only
   *     after a transaction request has been received by the entry server, so until then, the ID
   *     only has the transaction number.
   */
