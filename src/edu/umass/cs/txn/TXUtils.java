...
/**
 * @author arun
 *     <p>A class with static utility methods related to transactions.
 */
public class TXUtils implements Transactor {
  ...
  /**
   * Blocks and retries in parallel until all of a set of transaction steps have successfully
   * completed. The steps have to be idempotent, i.e., executing them multiple times should have the
   * same effect as executing them exactly once.
   *
   * @param gpClient
   * @param txRequests The list of operations comprising the transaction.
   * @param numAttempts Maximum number of times each task will be retransmitted before timing out.
   * @return An array of responses where the i'th element of the array is the response for the i'th
   *     element in {@code txRequests}.
   */
  protected static Request[] tryFinishAsyncTasks(
      GigaPaxosClient<Request> gpClient, ArrayList<Request> txRequests, int numAttempts) {
      ...
      callbacks[i] =
          new RequestCallback() {
            ...
            public void handleResponse(Request response) {
              if (response instanceof TXPacket && !((TXPacket) response).isFailed()
    ...
    synchronized (monitor) {
      for (int j = 0; j < numAttempts; j++) {
        ...
        for (int i = 0; i < monitor.length; i++) {
          if (!monitor[i])
            try {
              gpClient.sendRequest(txRequests.get(i), callbacks[i]);
            } catch (IOException e) {
              e.printStackTrace();
              // just continue
            }
  ...
  /**
   * This method fetches the union set of the set of active replica addresses for all participant
   * groups in a transaction by issuing an async task for the lock list of the transaction.
   *
   * @param gpClient
   * @param tx
   * @return
  ...
  protected static Set<InetSocketAddress> requestActiveReplicas(
    ...
    for (String name : tx.getLockList()) rars.add(new RequestActiveReplicas(name));
    ...
    for (Request response : responses)
      ...
      if (response != null && !((RequestActiveReplicas) response).isFailed())
        addresses.addAll(((RequestActiveReplicas) response).getActives());
    addresses.add(tx.getEntryServer());
    return addresses;
  }

  protected TxStateRequest.State getTxState(GigaPaxosClient<Request> gpClient, Transaction tx)
      throws IOException {
    Request response = gpClient.sendRequest(new TxStateRequest(tx.getTxGroupName()));
    return (((TxStateRequest) response).getState());
  }

  /**
   * Creates an async gigapaxos client without checking connectivity (because the reconfigurators
   ...
   * @throws IOException
   */
  protected static GigaPaxosClient<Request> getGPClient(AppRequestParser app) throws IOException {
