...
public class LazyWriteAfterPacket extends LazyPacket implements Byteable {
  ...
  private LazyWriteAfterPacket(
      long packetId, String senderId, ClientRequest clientWriteOnlyRequest) {
  ...
  public static LazyWriteAfterPacket createFromBytes(
    ...
    if (encodedPacket.length < Integer.BYTES) {
      Logger.getGlobal()
          .log(Level.SEVERE, "Receiving an invalid encoded LazyWriteAfterPacket: header too small");
      return null;
    }
    ...
    if (packetType != LazyPacketType.LAZY_WRITE_AFTER.getInt()) {
      Logger.getGlobal()
          .log(
              Level.SEVERE,
              "Receiving an invalid encoded LazyWriteAfterPacket: unexpected type " + packetType);
      return null;
    }
    ...
    try {
      ...
      Logger.getGlobal()
          .log(
              Level.SEVERE, "Receiving an invalid encoded LazyWriteAfterPacket: " + e.getMessage());
      return null;
    }

    if (senderId == null || encodedClientRequest == null) {
      Logger.getGlobal()
          .log(Level.SEVERE, "Receiving an invalid encoded LazyWriteAfterPacket: missing fields");
      return null;
    }
    ...
    if (serviceName != null && !serviceName.equals(concreteRequest.getServiceName())) {
      Logger.getGlobal()
          .log(Level.WARNING, "Service name mismatch when decoding LazyWriteAfterPacket");
    }
