...
public class XdnReplicaCoordinator<NodeIDType> extends AbstractReplicaCoordinator<NodeIDType> {
  ...
  public XdnReplicaCoordinator(
    ...
    assert app.getClass().getSimpleName().equals(XdnApp.class.getSimpleName())
            : "XdnReplicaCoordinator must be used with XdnApp";
  ...
  public boolean coordinateRequest(Request request, ExecutedCallback callback)
    ...
    xdnApp.cacheRequest(gpRequest.getRequest());
  ...
  private void handleXdnGetProtocolRoleRequest(
    ...
    if (xdnApp != null && !xdnApp.hostsService(serviceName)) {
      request.setHttpErrorCode(404);
    ...
    if (xdnApp != null) {
      List<String> containerIds = xdnApp.getContainerIds(serviceName);
      List<String> createdAtInfo = xdnApp.getContainerCreatedAtInfo(serviceName);
      List<String> containerStatus = xdnApp.getContainerStatus(serviceName);

      ServiceInstance instance = xdnApp.getServiceInstance(serviceName);
      ...
      Integer epoch = xdnApp.getEpoch(serviceName);
