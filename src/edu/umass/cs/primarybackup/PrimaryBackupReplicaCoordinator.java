 ...
 * request execution in the primary, leaving other replicas as backups. Generally, primary-backup is
 * suitable to provide fault-tolerance for non-deterministic services.
 *
...
public class PrimaryBackupReplicaCoordinator<NodeIDType>
  ...
  public PrimaryBackupReplicaCoordinator(
    ...
    if (paxosManager == null) {
      ...
      this.pbManager =
          new PrimaryBackupManager<>(myID, unstringer, middlewarePaxosApp, paxosManager, messenger);
    }
  ...
  /**
   * The default constructor for Primary Backup without providing paxos manager and paxos middleware
   * application. This constructor should be used when primary-backup is *the only* coordinator used
   * in the deployment.
   *
  ...
  public PrimaryBackupReplicaCoordinator(
      ...
      Messenger<NodeIDType, JSONObject> messenger) {
    this(app, myNodeId, nodeIdTypeDeserializer, messenger, null);
  }
  ...
  public boolean coordinateRequest(Request request, ExecutedCallback callback)
    ...
    logger.log(
        Level.FINE,
        String.format(
            "%s:%s - coordinating request %s %s %s",
            getMyID(),
            PrimaryBackupReplicaCoordinator.class.getSimpleName(),
            request.getClass().getSimpleName(),
            request instanceof ReplicableClientRequest rcr
                ? rcr.getRequest().getClass().getSimpleName()
                : "null",
