...
public class ApplyStateDiffPacket extends PrimaryBackupPacket implements Byteable {
  ...
  @Deprecated public static final String SERIALIZED_PREFIX = "pb:sd:";
  ...
  public ApplyStateDiffPacket(String serviceName, PrimaryEpoch<?> primaryEpoch, byte[] stateDiff) {
    this(
  ...
  private ApplyStateDiffPacket(
      long packetId, String serviceName, PrimaryEpoch<?> primaryEpoch, byte[] stateDiff) {
    assert packetId > 0;
    assert serviceName != null;
    assert primaryEpoch != null;
    assert stateDiff != null;
  ...
  public byte[] toBytes() {
    ...
    PrimaryBackupPacketProto.ApplyStateDiffPacket.Builder protoBuilder =
        PrimaryBackupPacketProto.ApplyStateDiffPacket.newBuilder()
            ...
            .setEncodedStateDiff(ByteString.copyFrom(this.stateDiff));
  ...
  public static ApplyStateDiffPacket createFromBytes(byte[] encodedPacket) {
    ...
    return new ApplyStateDiffPacket(
        ...
        new PrimaryEpoch<>(decodedProto.getPrimaryNodeId(), decodedProto.getPrimaryEpoch()),
