...
public class GCConcurrentHashMap<K, V> extends ConcurrentHashMap<K, V> {
  ...
  public synchronized void putAll(Map<? extends K, ? extends V> map) {
    for (K key : map.keySet()) this.putGC(key);
    super.putAll(map);
  }
  ...
  private synchronized void putGC(K key) {
    ...
    if (this.size() > gcThresholdSize || Util.oneIn(1000)) GC();
  }
  ...
  private synchronized void GC(long timeout) {
    if (System.currentTimeMillis() - this.lastGCTime < this.minGCInterval) return;
    else this.lastGCTime = System.currentTimeMillis();
    boolean removed = false;
    numGCAttempts++;
    for (Iterator<K> iterK = this.putTimes.keySet().iterator(); iterK.hasNext(); ) {
      K key = iterK.next();
      Long time = this.putTimes.get(key);
      if (time != null && (System.currentTimeMillis() - time > timeout)) {
        iterK.remove();
        V value = this.remove(key);
        if (value != null && this.callback != null) this.callback.callbackGC(key, value);
        removed = true;
      } else break;
    }
    if (removed) numGC++;
  ...
  public static class GCConcurrentHashMapTest extends DefaultTest {
    ...
    public void testMain() throws InterruptedException {
      ...
      GCConcurrentHashMap<String, Integer> map1 =
          new GCConcurrentHashMap<String, Integer>(
      ...
      for (int i = 0; i < n; i++) {
        ...
        if (i >= sizeThreshold) map.remove(prefix + (i - sizeThreshold));
      }
      ...
      Thread.sleep(1500);
      (map != null ? map : hmap).put(prefix + (n + 1), (int) (Math.random() * Integer.MAX_VALUE));
      for (int i = 0; i < n; i++)
        assert (!map1.containsKey(prefix + i)
                || !map1.putTimes.containsKey(prefix + i)
                || (t2 - map1.putTimes.get(prefix + i) < map1.gcTimeout))
            : prefix + i;
