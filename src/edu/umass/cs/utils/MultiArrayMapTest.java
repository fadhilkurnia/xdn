...
public class MultiArrayMapTest extends DefaultTest {
  ...
  private static MultiArrayMap<String, StringValue<Integer>> createRandomMAM(int size) {
    System.out.print("Inserting " + size / (1000 * 1000) + " million values");
    MultiArrayMap<String, StringValue<Integer>> map =
        new MultiArrayMap<String, StringValue<Integer>>((int) (size), 6);
  ...
  private static void printProgressBar(int i) {
    if (i % 200000 == 0) System.out.print(".");
  }
  ...
  public void testMain() {
    ...
    if (simpleArray) createSimpleArray(size);
    map = createRandomMAM(size);
    System.out.println(
        "succeeded (cop-out hashmap size = "
            + map.hashmapSize()
            + "); time = "
    ...
    count = sum = 0;
    for (Iterator<StringValue<Integer>> iter = map.concurrentIterator(); iter.hasNext(); ) {
      printProgressBar(count);
      sum += iter.next().value;
      count++;
    }
    assert (count == map.size() && sum == testSum) : count + " != " + map.size();
    System.out.println("succeeded; time = " + (System.currentTimeMillis() - t1) + "ms");

    System.out.print("Iterating(2)");
    t1 = System.currentTimeMillis();
    count = sum = 0;
    for (Iterator<StringValue<Integer>> iter = map.iterator(); iter.hasNext(); ) {
      printProgressBar(count);
      sum += iter.next().value;
      count++;
    }
