...
public class DiskMapTest extends DefaultTest {
  private static class KeyableString extends LogIndex implements Keyable<String> {
    /** */
  ...
  private static DiskMap<String, KeyableString> makeTestMap() {
    ...
    ConcurrentHashMap<String, KeyableString> db =
        new ConcurrentHashMap<String, KeyableString>(dbSize);
    boolean sleep = false;
    int capacity = 1000000;
    DiskMap<String, KeyableString> dmap =
        new DiskMap<String, KeyableString>(capacity) {

          @Override
          public Set<String> commit(Map<String, KeyableString> toCommit) throws IOException {
            try {
              if (sleep) Thread.sleep(toCommit.size());
  ...
  public void test99_Main() {
    ...
    while (count < numRequests) {
      ...
      if (count % freq == 0) {
        System.out.println(
            "after "
                + count
                + ": put/get rate = "
                + Util.df(resetCount * 1000.0 / (System.currentTimeMillis() - t1))
      ...
      if (Math.random() < 0.5) {
        ...
        assert ((retrieved == null && real == null)
                || (retrieved != null && real != null && retrieved.equals(real)))
            : retrieved + " != " + real + " at count=" + count;
      }
