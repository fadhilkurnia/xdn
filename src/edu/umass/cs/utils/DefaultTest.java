...
public class DefaultTest {
  ...
  public TestWatcher watcher =
      new TestWatcher() {
        ...
        protected void failed(Throwable e, Description description) {
          System.out.println(
              testName.getMethodName() + ":" + (repeatIndex) + " FAILED!!!!!!!!!!!!! " + e);
          e.printStackTrace();
  ...
  public void beforeMethod() {
    if (repeatRule.times() > 1)
      System.out.print(
          repeatIndex++ == 0
              ? testName.getMethodName() + " " + repeatIndex + " "
              : (repeatIndex == lastPrinted * 2 && (lastPrinted *= 2) > 0)
                  ? (repeatIndex + " ")
                  : "");
    else if (verbose == 0) System.out.print(testName.getMethodName() + " ");
  }
   ...
   */
  public static void testAll(List<Class<?>> classes) throws ClassNotFoundException {
  ...
  public static void testAll(Class<?>... classes) {
    for (Class<?> clazz : classes) {
      if (!clazz.getName().endsWith("Test")) continue;
      System.out.println("\n\n" + MARKER + "Testing " + clazz.getSimpleName() + MARKER);
