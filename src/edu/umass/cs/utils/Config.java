...
public class Config extends Properties {
  ...
  public static interface ConfigurableEnum extends DefaultValueEnum {
    ...
    /**
     * @return The JVM property key used to specify a non-default config file using
     *     -Dkey=/path/to/config/file.
     */
    public String getConfigFileKey();
  }

  /**
   * Used to disallow some parameters from being configured during production runs but allow them to
   * be configured for testing or instrumentation purposes.
   */
  ...
  /**
   * An interface that makes it convenient to use commons-CLI style option while defining all fields
   * as an enum at one place.
   */
  ...
  /**
   * Returns a {@link Config} object with properties read from {@code configFile}
   *
   * @param configFile
   * @throws IOException
   */
  ...
  private static final HashMap<Class<?>, Boolean> disableCommandLine =
  ...
  private static boolean isDefaultValueEnum(Class<?> clazz) {
    for (Class<?> iface : clazz.getInterfaces())
      if (iface.equals(DefaultValueEnum.class)) return true;
    return false;
  }
  ...
  public static Config register(
      Class<?> type, String systemPropertyKey, String defaultConfigFile, Level level)
      throws IOException {
    if (!type.isEnum() && !isDefaultValueEnum(type)) return null;
    // one time
    if (Config.getConfig(type) != null) return Config.getConfig(type);
    // else
    String configFile =
    ...
    try {
      ...
      log.warning(
          Config.class.getSimpleName()
              + " unable to find file "
              + configFile
              + "; using default values for type "
              + type);
      throw ioe;
    }
  }

  public static Config register(Class<?> type, String systemPropertyKey, String defaultConfigFile)
      throws IOException {
    return register(type, systemPropertyKey, defaultConfigFile, Level.WARNING);
  }
  ...
  public static HashMap<?, ?> register(String[] args) {
    for (String arg : args) {
      ...
      if (arg.startsWith("-")) arg = arg.replaceFirst("-", "");
      ...
      if (tokens.length == 2) {
        ...
        for (Object key : cacheMap.keySet().toArray())
          if (key.toString().equals(tokens[0])) cacheMap.remove(key);
      }
    }
    return cmdLine;
  }
  ...
  public static Properties getProperties(
      Class<?> type, String systemPropertyKey, String defaultConfigFile) throws IOException {
  ...
  public Config setSystemProperties() {
    ...
    for (String prop : this.stringPropertyNames())
      if (prop.startsWith("-D") && System.getProperty(key = prop.replaceFirst("-D", "")) == null)
  ...
  /**
   * @param field
   * @return The configuration value of field if its type was previously registered. If it was not
   *     previously registered, an exception will be thrown.
   */
  public static Object getGlobal(Enum<?> field) {
    ...
    if ((retval = cmdLine.get(strField)) != null
        && (!disableCommandLine.containsKey(clazz) || disableCommandLine.get(clazz))) {
      return retval;
    }
    ...
    // auto-register and recursively invoke
      ...
      return putObject(field, ((DefaultValueEnum) field).getDefaultValue());
  ...
  private static Object putObject(Enum<?> field, Class<?> clazz, Object obj) {
    if (clazz.isInstance(obj)) return putObject(field, obj);
    return obj;
  }

  private static Object putObject(Enum<?> field, Object obj) {
    if (!cacheMap.containsKey(field)) cacheMap.put(field, obj);
    return obj;
  }
  ...
  public static boolean getGlobalBoolean(Enum<?> field) {
    ...
    return (boolean)
        (obj instanceof Boolean
            ? obj
            : putObject(field, Boolean.class, (Boolean.valueOf(obj.toString().trim()))));
  }
  ...
  public static int getGlobalInt(Enum<?> field) {
    ...
    return (int)
        (obj instanceof Integer
            ? obj
            : putObject(field, Integer.class, Integer.valueOf(getGlobal(field).toString().trim())));
  }
  ...
  public static long getGlobalLong(Enum<?> field) {
    ...
    return (long)
        (obj instanceof Long
            ? obj
            : putObject(field, Long.class, Long.valueOf(getGlobal(field).toString().trim())));
  }
  ...
  public static double getGlobalDouble(Enum<?> field) {
    ...
    return (double)
        (obj instanceof Double
            ? obj
            : putObject(field, Double.class, Double.valueOf(getGlobal(field).toString().trim())));
  }
  ...
  public static short getGlobalShort(Enum<?> field) {
    ...
    return (short)
        (obj instanceof Short
            ? obj
            : putObject(field, Short.class, Short.valueOf(getGlobal(field).toString().trim())));
  }
  ...
  public Object get(Enum<?> field) {
    String strField = field.toString();
    Object retval;
    if ((retval = this.get(strField)) != null) return retval;
    else return ((Config.DefaultValueEnum) field).getDefaultValue();
  }
  ...
  private void readConfigFile(String configFile) throws IOException {
    ...
    for (Object prop : this.keySet()) {
      log.log(
          Level.FINE,
          "Set property {0}={1}",
          new Object[] {prop, this.getProperty(prop.toString())});
    }
  }
  ...
  private static void testMethod2() throws IOException {
    ...
    Config config = new Config("/Users/arun/GNS/src/edu/umass/cs/utils/config.properties");
    System.out.println(Fields.class);

    // asserting global config matches specific config
    for (Fields field : Fields.values()) assert (Config.getGlobal(field).equals(config.get(field)));

    // specific field value assertions
    assert (Fields.FIRST.getDefaultValue().toString().equals(Config.getGlobal(Fields.FIRST)));
    assert (Fields.SECOND
        .getDefaultValue()
        .toString()
        .toUpperCase()
        .equals(config.get(Fields.SECOND)));
    assert (!Fields.THIRD.getDefaultValue().toString().equals(config.get(Fields.THIRD)));
  }

  /** Example of defining an enum that can be used to define commons-cli options. */
  static enum OptionFields implements CLIOption {
    ...
    NINTH("9", "ninthOption", false, "ninth option used for all else", "file", true);
    ...
    OptionFields(Object defaultValue, String optionName, boolean hasArg, String description) {
  ...
  private Object toLowerCase(Object key) {
    return caseSensitive || !(key instanceof String) || ((String) key).startsWith("-D")
        ? key
        : ((String) key).toLowerCase();
  }

  /**
   * The methods {@link #put(Object, Object)}, {@link #getProperty(String)}, and {@link
   * #getProperty(String, String)} below make this Config case-insensitive.
   */
