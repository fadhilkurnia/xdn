...
public class DelayProfiler {
  private static ConcurrentHashMap<String, Double> averageMillis =
      new ConcurrentHashMap<String, Double>();
  private static ConcurrentHashMap<String, Double> averageNanos =
      new ConcurrentHashMap<String, Double>();
  private static ConcurrentHashMap<String, Double> averages =
      new ConcurrentHashMap<String, Double>();
  private static ConcurrentHashMap<String, Double> stdDevs =
      new ConcurrentHashMap<String, Double>();
  private static ConcurrentHashMap<String, Double> counters =
      new ConcurrentHashMap<String, Double>();
  private static ConcurrentHashMap<String, Double> instarates =
      new ConcurrentHashMap<String, Double>();

  private static ConcurrentHashMap<String, Double> lastArrivalNanos =
      new ConcurrentHashMap<String, Double>();

  private static ConcurrentHashMap<String, Double> lastRecordedNanos =
      new ConcurrentHashMap<String, Double>();
  private static ConcurrentHashMap<String, Double> lastCount =
      new ConcurrentHashMap<String, Double>();
   ...
   */
  public static boolean register(String field, ConcurrentMap<String, Double> map) {
    if (!enabled) return false;
    synchronized (map) {
      if (map.containsKey(field)) return true;
  ...
  public static void updateDelayNano(String field, long time, int n) {
    if (!enabled) return;
    for (int i = 0; i < n; i++)
      updateDelayNano(field, System.nanoTime() - (System.nanoTime() - time) * 1.0 / n);
  }
  ...
  public static void updateDelay(String field, long time, int n) {
    ...
    for (int i = 0; i < n; i++)
      updateDelay(
          field, System.currentTimeMillis() - (System.currentTimeMillis() - time) * 1.0 / n);
  }
  ...
  public static double get(String field) {
    synchronized (averageMillis) {
      return averageMillis.containsKey(field)
          ? averageMillis.get(field)
          : averageNanos.containsKey(field)
              ? averageNanos.get(field)
              : averages.containsKey(field)
                  ? averages.get(field)
                  : counters.containsKey(field)
                      ? counters.get(field)
                      : instarates.containsKey(field) ? instarates.get(field) : 0.0;
    }
  }
   ...
   * @param alpha
   */
  public static void updateInterArrivalTime(
      String field, int numArrivals, int samplingFactor, double alpha) {
    if (!enabled) return;
    ...
    synchronized (lastArrivalNanos) {
      ...
      double value = lastArrivalNanos.containsKey(field) ? lastArrivalNanos.get(field) : curTime;
      if (value == 0) value = curTime;
      DelayProfiler.updateMovAvg(field, (curTime - (long) value) / (numArrivals * samplingFactor));
  ...
  public static void updateRate(String field, int numArrivals, int samplingFactor) {
    ...
    synchronized (lastCount) {
      ...
      } else lastCount.put(field, count);
    }
  }
  ...
  public static double getThroughput(String field) {
    return averages.containsKey(field) && averages.get(field) > 0
        ? 1000 * 1000 * 1000.0 / (averages.get(field))
        : 0;
  }
  ...
  private static String statsHelper(
      ConcurrentMap<String, Double> map, String units, Set<String> fields) {
    String s = "";
      ...
      for (String field : treeMap.keySet()) {
        ...
        s +=
            (field
                + ":"
                + (!rateParam
                    ? Util.df(units.equals("ns") ? treeMap.get(field) / 1000.0 : treeMap.get(field))
                    : Util.df(getThroughput(field)))
                + "/"
                + (stdDevs.get(field) >= 0 ? "+" : "")
                + (!rateParam
                    ? Util.df(units.equals("ns") ? stdDevs.get(field) / 1000.0 : treeMap.get(field))
                    : Util.df(1000 * 1000 * 1000.0 / stdDevs.get(field)))
                + (!rateParam ? units.equals("ns") ? "us" : units : "/s")
                + " | ");
      }
    }
    return s;
