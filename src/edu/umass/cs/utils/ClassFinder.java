...
/**
 * @author arun
 *     <p>Borrowed this code from stackoverflow: http://stackoverflow.com/questions/
...
public class ClassFinder {
  ...
  public static List<Class<?>> findRegex(String regex) {
    String classpath =
        DefaultTest.class.getClassLoader().getResource(".").toString().replace("file:", "");
    ArrayList<String> packages = Util.recursiveFind(classpath, regex);
    ArrayList<Class<?>> classes = new ArrayList<Class<?>>();
    for (String pkg : packages)
      if (new File(pkg).isDirectory())
        classes.addAll(find(pkg.replace(classpath, "").replace(DIR_SEPARATOR, PKG_SEPARATOR)));
    return classes;
  }
  ...
  public static List<Class<?>> findRest(Class<?> clazz) {
    List<Class<?>> classes =
        find(clazz.getCanonicalName().replace("." + clazz.getSimpleName(), ""));
  ...
  public static void main(String[] args) {
    System.out.println(ClassFinder.find("edu.umass.cs.gigapaxos.paxosutil"));
  }
}
