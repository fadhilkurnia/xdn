 ...
 * @author arun
 * @param <K>
 * @param <V> DiskMap implements the ConcurrentMap<K,V> interface and allows applications to
 *     maintain very large maps limited only by the available disk space by automatically stowing
 *     away infrequently used map entries to disk. DiskMap is not a database and does not try to
 *     guarantee durability under crashes. However, if the map is closed gracefully, it will ensure
 *     durability by committing entries to disk before closing.
 *     <p>DiskMap itself does not implement any disk I/O but relies on the application that must
 *     implement {@link #commit(Map)} and {@link #restore(Object)} methods that respectively write
 *     to and read from the disk.
 *     <p>DiskMap is nearly indistinguishable in performance from ConcurrentHashMap when all map
 *     entries fit in memory. If not, its performance depends on how frequently entries are reused.
 *     If entries are rarely reused, e.g., a sweep over a very large number of map entries, then
 *     every access will on average force a disk read (to unpause the entry being accessed) and a
 *     write (to pause long idle entries). Normally, only entries that have been idle for at least
 *     {@code idleThreshold} paused to disk, so the caller must use {@link #setIdleThreshold(long)}
 *     so as to ensure that either no more than {@code sizeThreshold} entries are accessed in that
 *     interval or there is sufficient memory to hold all entries accessed in the last {@code
 *     idleThreshold} time. If not, a random fraction of entries, including possibly recently used
 *     entries, will be paused to disk in order to make room in memory.
 *     <p>DiskMap can use MultiArrayMap as its underlying in-memory map if that is supplied in the
 *     constructor. Although not necessary, the use of MultiArrayMap makes the map more compact
 *     because of its cuckoo hashing design, but it requires values to implement the Keyable<K>
 *     interface. If values additionally also implement {@link Pausable}, DiskMap incurs just ~6B
 *     overhead per value as it does not have to maintain its own stats for last active times for
 *     map entries.
 *     <p>Note: There is no way to ensure that the value pointed to in the map is not modified after
 *     it has been paused to disk. It does not help to verify that the value pointed to is the same
 *     as the serialized value paused to disk as the value pointed to in the map could be modified
 *     by a caller even after its serialized form has been paused to disk. As a result, we can not
 *     support the "expectation" that modifications to the value object by the caller will be
 *     reflected in the map. So, the caller is forced to reckon with the possibility that any
 *     modifications to the value object may be lost unless the caller explicitly invokes a put
 *     subsequently.
 */
public abstract class DiskMap<K, V> implements ConcurrentMap<K, V>, Diskable<K, V> {
  ...
  /**
   * This map could either be a MultiArrayMap or LinkedHashMap depending on whether {@link
   * #enablePriorityQueue()} is enabled.
   */
  ...
  /**
   * True means that the in-memory map will always be consistent with the underlying database, which
   * is achieved by always committing an entry upon a put.
   */
  ...
  /**
   * Set the cleanCache option to true, which means that puts will be immediately committed, i.e.,
   * the in-memory cache is always consistent with the disk.
   *
   * @param set
   */
  ...
  public synchronized void enablePriorityQueue() {
    if (isSingleLinkedHashMap()) return;
    if (this.stats instanceof LinkedHashMap) return;
    LinkedHashMap<Object, LastActive> tmpStats =
        new LinkedHashMap<Object, LastActive>(16, (float) 0.75, true);
    if (this.stats != null)
      for (Iterator<LastActive> iterLA = ((MultiArrayMap) this.stats).concurrentIterator();
          iterLA.hasNext(); ) {
  ...
  private void initPeriodicGC() {
    this.GC.scheduleWithFixedDelay(
        new TimerTask() {
          @Override
          public void run() {
  ...
  /**
   * @param inMemoryCapacity Capacity for in-memory map. If the in-memory map size reaches this
   *     value, GC forcibly kicks in.
   */
  public DiskMap(long inMemoryCapacity) {
    this.map =
        USE_LINKED_HASH_MAP
            ? Collections.synchronizedMap(new LinkedHashMap<K, V>(16, (float) 0.75, true))
  ...
  /**
   * The supplied {@code map} will be used as the underlying in-memory map. From this point onwards,
   * making direct modifications to this underlying map is unsafe.
   *
   * @param map
   */
  public DiskMap(ConcurrentMap<K, V> map) {
    ...
    this.capacityEstimate =
        map instanceof MultiArrayMap ? ((MultiArrayMap<?, ?>) map).capacity() : DEFAULT_CAPACITY;
    this.initPeriodicGC();
  }
  ...
  /**
   * This method should return only after successfully persisting the key,value pair, otherwise it
   * should throw an exception.
   *
   * @throws IOException
   */
  public abstract Set<K> commit(Map<K, V> toCommit) throws IOException;
   ...
   * @throws IOException
   */
  public abstract V restore(K key) throws IOException;
  ...
  public boolean containsValue(Object value) {
    ...
    return (this.map.containsValue(value) || this.get(((Keyable<?>) value).getKey()).equals(value));
  }

  @Override
  public V get(Object key) {
    V value = this.map.get(key);
    if (value == null) value = this.getOrRestore(key);
    if (!(value instanceof Pausable)) this.markActive(key);
    if (this.shouldGC(true)) this.initOnetimeGC();
    return value;
  }
  ...
  private void markActive(Object key) {
    if (isSingleLinkedHashMap()) return;
    if (this.stats == null) this.initStats();
  ...
  public V put(K key, V value) {
    ...
    synchronized (this) {
      ...
      recordPut(key);
      if (this.cleanCache) this.putCommit(key, value);
    }
    return prev;
  }
  ...
  private void putCommit(K key, V value) {
    ...
    try {
      ...
      if (committed == null || !committed.contains(key) || committed.size() != 1)
        throw new RuntimeException("Failed to commit <" + key + ", " + value + ">" + "to disk");
    }
  }
  ...
  public synchronized V remove(Object key) {
    ...
    if (this.stats != null) this.stats.remove(key);
    ...
    if (entryAsMap != null && !entryAsMap.isEmpty()) {
      ...
      if (!this.isGCEnabled()) return entry.getValue();
  ...
  private Map<K, V> removeEntry(Object key) {
    if (this.map.containsKey(key)) {
      ...
      if (this.map instanceof MultiArrayMap) {
        ...
        if (value != null) entryAsMap.put(((Keyable<K>) value).getKey(), value);
        return entryAsMap;
      } else {
        K removedKey = null;
        V value = null;
        try {
          ...
          for (Iterator<Map.Entry<K, V>> entryIter = this.map.entrySet().iterator();
              entryIter.hasNext(); ) {
        ...
        if (removedKey != null) entryAsMap.put(removedKey, value);
        return entryAsMap;
      }
    }
    return null;
  ...
  public synchronized void putAll(Map<? extends K, ? extends V> m) {
    for (K key : m.keySet()) this.map.put(key, m.get(key));
  }

  @Override
  public void clear() {
    if (this.stats != null) this.stats.clear();
    this.map.clear();
  }

  /**
   * This method only returns the in-memory key set. There is currently no method to get the on-disk
   * keys.
   */
  @Override
  public Set<K> keySet() {
  ...
  /**
   * This method only returns the in-memory value set. There is currently no method to get the
   * on-disk values.
   */
  @Override
  ...
  /**
   * This method only returns the in-memory entry set. There is currently no method to get the
   * on-disk entries.
   */
  @Override
  ...
  protected synchronized void hintRestore(K key, V value) {
    if (!this.map.containsKey(key) && !this.pauseQ.containsKey(key)) this.map.put(key, value);
  }
  ...
  private V getOrRestore(Object key) {
    V value = null;
    if ((value = this.map.get(key)) != null) return value;
      ...
      try {
        synchronized (this) {
          if ((value = this.restore((K) key)) != null) this.map.put((K) key, value);
        }
      } catch (IOException e) {
        e.printStackTrace();
      } catch (ClassCastException e) {
  ...
  private boolean enqueuePause(K key, V value, LastActive la, Iterator<?> iterator) {
    assert (value == null || iterator != null);
    synchronized (this) {
      // came here via stats
      if (value == null) value = this.map.get(key);
      if (((value instanceof Pausable) && ((Pausable) value).isPausable())
          // if value is not Pausable, stats must be non-null
          || ((la != null || (this.stats != null && (la = this.stats.get(key)) != null))
              && longIdle(la))
          // pause at least one entry
          || this.pauseQ.size() < Math.max(this.capacityEstimate / FORCE_PAUSE_FACTOR, 1)) {
        if (iterator != null) iterator.remove();
        else value = this.map.remove(key);
  ...
  private boolean shouldGC(boolean strict) {
    ...
    synchronized (this) {
      if (strict) return this.map.size() >= this.capacityEstimate;
      if (this.map.size() > this.capacityEstimate
          && System.currentTimeMillis() - lastGCAttempt < this.pauseThreadPeriod) return true;
      return false;
    }
  }
  ...
  private void GC(boolean strict) {
    ...
    if (this.stats != null && this.stats instanceof LinkedHashMap) {
      while (this.map.size() >= this.capacityEstimate && this.pauseQ.isEmpty()) {
        // need to check for concurrent modification
        try {
          for (Iterator<Map.Entry<Object, LastActive>> iterLA = this.stats.entrySet().iterator();
              iterLA.hasNext(); ) {
            Map.Entry<Object, LastActive> entry = iterLA.next();
            if (!this.enqueuePause((K) (entry.getValue().key), null, entry.getValue(), null)) break;
          }
     ...
     * cleanest, good enough option. */
      ...
      while (this.map.size() >= this.capacityEstimate && this.pauseQ.isEmpty()) {
        ...
        try {
          ...
          synchronized (this) {
            for (Iterator<Map.Entry<K, V>> iterE = this.map.entrySet().iterator();
                iterE.hasNext(); ) {
              Map.Entry<K, V> entry = iterE.next();
              if (!this.enqueuePause(entry.getKey(), entry.getValue(), null, iterE)) break;
            }
            // startRecordingPuts();
          }
        } catch (ConcurrentModificationException cme) {
      ...
      if (this.map instanceof MultiArrayMap) {
        for (Iterator<Keyable<K>> iterV =
                (Iterator<Keyable<K>>) (((MultiArrayMap<?, ?>) this.map).concurrentIterator());
            iterV.hasNext(); ) {
          Keyable<K> value = iterV.next();
          this.enqueuePause(value.getKey(), null, null, null);
        }
      } else {
        for (Iterator<Map.Entry<K, V>> iterE = this.map.entrySet().iterator(); iterE.hasNext(); ) {
          Map.Entry<K, V> entry = iterE.next();
    ...
    if (!this.pauseQ.isEmpty()) {
      ...
      try {
         ...
         * */
        committed = this.cleanCache ? this.pauseQ.keySet() : this.commit(this.pauseQ);
      } catch (IOException e) {
        e.printStackTrace();
      }
        ...
        for (K key : committed) {
          synchronized (this) {
            ...
            if (this.concurrentRemoves.contains(key)
                ...
                && !this.map.containsKey(key)) this.remove(key); // remove again for good measure
  ...
  public synchronized boolean remove(Object key, Object value) {
    V existing = null;
    if ((existing = this.get(key)) == null || !existing.equals(value)) return false;
    this.remove(key);
    return true;
  }
  ...
  public synchronized boolean replace(K key, V oldValue, V newValue) {
    if (!this.get(key).equals(oldValue)) return false;
  ...
  protected synchronized Set<K> commit(K key) throws IOException {
    ...
    if (this.map.containsKey(key)) singleton.put(key, this.map.get(key));
  ...
  private synchronized void commitAll(Map<K, V> m, boolean clone) {
    ...
    if (m instanceof MultiArrayMap) {
      for (Iterator<V> iter = (Iterator<V>) ((MultiArrayMap) m).concurrentIterator();
          iter.hasNext(); ) {
        V value = iter.next();
        if (value != null) copy.put(((Keyable<K>) value).getKey(), value);
      }
    } else
      for (Object key : m.keySet().toArray()) {
        V value = m.get(key);
        if (value != null) copy.put((K) key, m.get(key));
      }

    while (!copy.isEmpty()) {
      Set<K> committed = null;
      try {
        committed = this.commit(this.extractIterableMap(copy, BATCH_SIZE));
      } catch (IOException e) {
        e.printStackTrace();
      }
      if (committed != null) for (K key : committed) copy.remove(key);
    }
  }
  ...
  /**
   * Will stop the GC thread but will have no other effect. The map can continue to be used with the
   * same semantics.
   *
   * @param commitAll
   */
