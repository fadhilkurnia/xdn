 ...
 * @author V. Arun
 * @param <K>
 * @param <V> This class implements a cuckoo hashmap for storing objects implementing the Keyable<K>
 *     interface. It's main benefit is that it adds ~5 bytes of extra overhead per object (compared
 *     to {@code util.java.HashMap} or other that can use up to hundreds of bytes of additional
 *     overhead).
 *     <p>It supports two ways of iterating over the values in the map. The first is a more
 *     traditional iterator. The second is a non-fail-fast iterator that allows iteration as well as
 *     {@code remove} concurrently with other put/remove operations. The iteration may miss
 *     concurrently added elements (but will not return concurrently removed elements any time after
 *     they have been removed. This non-fail-fast iterator is useful for doing an "approximate"
 *     sweep over the entire map.
 */
public class MultiArrayMap<K, V extends Keyable<K>> implements Iterable<V>, ConcurrentMap<K, V> {
  ...
  public synchronized V put(K key, V value) {
    ...
    for (Object[] array : this.aMap) {
      ...
      if (array[index] == null
          || (array[index] instanceof Keyable<?>
              && key.equals(((Keyable<?>) array[index]).getKey()))) {
        if (array[index] == null) size++;
        else
          log.log(
              Level.FINE,
              "{0} overwrote {1} in [{2},{3}]",
  ...
  public synchronized void clear() {
    for (Object[] array : this.aMap) for (int i = 0; i < array.length; i++) array[i] = null;
    this.hMap.clear();
    for (BitSet bitset : this.bitsets) bitset.clear();
    this.size = 0;
  }
  ...
  public Set<K> keySet() {
    return new Set<K>() {
      ...
      public Object[] toArray() {
        ...
        for (Iterator<V> iterV = MultiArrayMap.this.concurrentIterator(); iterV.hasNext(); ) {
          V value = iterV.next();
          if (value != null) keys.add(value.getKey());
        }
        return keys.toArray();
      }
      ...
      public boolean containsAll(@SuppressWarnings("rawtypes") Collection c) {
        ...
        for (Object o : c) contains = contains && MultiArrayMap.this.containsKey(o);
        return contains;
      }
      ...
      public boolean retainAll(@SuppressWarnings("rawtypes") Collection c) {
        ...
        for (Iterator<V> iterV = MultiArrayMap.this.concurrentIterator(); iterV.hasNext(); ) {
          ...
          if (value != null && !c.contains(value.getKey()))
            removed = removed || (MultiArrayMap.this.remove(value.getKey()) != null);
        }
        return removed;
      }
      ...
      public boolean removeAll(@SuppressWarnings("rawtypes") Collection c) {
        boolean removed = false;
        for (Object o : c) removed = removed || (MultiArrayMap.this.remove(o) != null);
        return removed;
      }
  ...
  public Collection<V> values() {
    return new Set<V>() {
      ...
      public boolean containsAll(Collection<?> c) {
        boolean contains = true;
        for (Object o : c) contains = contains && MultiArrayMap.this.containsValue(o);
        return contains;
      }
      ...
      public boolean removeAll(Collection<?> c) {
        ...
        for (Object o : c)
          removed =
              removed
                  || (o instanceof Keyable<?>
                      && MultiArrayMap.this.remove(((Keyable<?>) o).getKey()) != null);
        return removed;
      }
  ...
  public Set<java.util.Map.Entry<K, V>> entrySet() {
    return new Set<Map.Entry<K, V>>() {
      ...
      @Override
      public boolean contains(Object o) {
        return (o instanceof Map.Entry) && MultiArrayMap.this.containsKey(((Map.Entry) o).getKey());
      }
          ...
          public java.util.Map.Entry<K, V> next() {
            ...
            return new Map.Entry<K, V>() {
              ...
              public V setValue(V newValue) {
                return value != null ? MultiArrayMap.this.put(value.getKey(), newValue) : null;
              }
            };
          }
        };
          ...
          if (value != null)
            entries.add(
                new Map.Entry<K, V>() {
                  ...
                  @Override
                  public V setValue(V newValue) {
                    return MultiArrayMap.this.put(value.getKey(), newValue);
                  }
                });
        }
        return entries;
        ...
        for (Object o : c)
          contains =
              contains
                  ...
                  && (MultiArrayMap.this.containsKey(((Map.Entry) o).getKey()));
        return contains;
      }
        ...
        for (Iterator<V> iterV = MultiArrayMap.this.concurrentIterator(); iterV.hasNext(); ) {
          ...
          for (Object o : c)
            if (key != null
              ...
              removed = removed || MultiArrayMap.this.remove(key) != null;
        }
        return removed;
      }
  ...
  class ConcurrentMAMIterator implements Iterator<V> {
    ...
    /**
     * Note: next() may return null immediately after hasNext() returns true. This behavior is
     * unlike a traditional iterator wherein, if hasNext() returns true, next() necessarily returns
     * a non-null value or throws a {@code ConcurrentModificationException}
     */
    @Override
    public boolean hasNext() {
      if (arrayHasNext()) return true;
      return hMapIndex == -1 ? !hMap.isEmpty() : hMapIndex < hMapValues.length;
    }

    private boolean arrayHasNext() {
      for (int i = level; i < MultiArrayMap.this.aMap.length; i++) {
        if (bitsets[i].nextSetBit(i == level ? index + 1 : 0) >= 0) return true;
      }
      return false;
    }
    ...
    public V next() {
      V value = null;
      if ((value = arrayNext()) != null) return value;
      if (hMapIndex == -1 && (hMapValues = hMap.values().toArray()).length == 0) return null;
      return ++hMapIndex < hMapValues.length ? last = (V) hMapValues[hMapIndex] : null;
    }
    ...
    private V arrayNext() {
      for (; level < MultiArrayMap.this.aMap.length; level++) {
        ...
        if (index >= 0) return last = (V) aMap[level][index];
        else assert (index == -1);
      }
      return null;
    }
  ...
  class MAMIterator extends ConcurrentMAMIterator implements Iterator<V> {
    ...
    public boolean hasNext() {
      ...
      if (super.arrayHasNext()) return true;
    ...
    public V next() {
      if (modCount != expectedModCount) throw new ConcurrentModificationException();
      V value = null;
      if ((value = super.arrayNext()) != null) return value;
      if (hMapIter == null) hMapIter = hMap.values().iterator();
      return hMapIter.next();
    }

    public void remove() {
      if (modCount != expectedModCount) throw new ConcurrentModificationException();
      if (!super.removed()) return;
  ...
  /**
   * @return Non-fail-fast iterator that allows concurrent modification but with the caveat that
   *     next() may return null even though hasNext() returns true. Concurrent add/remove operations
   *     may be overlooked.
   */
