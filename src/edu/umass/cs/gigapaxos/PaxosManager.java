...
/**
 * @param <NodeIDType>
 *     <p>PaxosManager is the primary interface to create and use paxos by creating a paxos
 *     instance.
 *     <p>PaxosManager manages all paxos instances at a node. There is typically one paxos manager
 *     per machine. This class could be static, but it is not so that we can test emulations
 *     involving multiple "machines" within a JVM.
 *     <p>PaxosManager has four functions at a machine that are useful across paxos instances of all
 *     applications on the machine: (1) logging, (2) failure detection, (3) messaging, and (4) paxos
 *     instance mapping. The fourth is key to allowing the manager to demultiplex incoming messages
 *     to the appropriate application paxos instance.
 * @author V. Arun
 */
public class PaxosManager<NodeIDType> {
  ...
  /**
   * aditya: The thread pool is for executing app.execute() in the single node case. Otherwise, in
   * the single node case the app.execute() method would get called from AbstractPaxosLogger thread.
   */
  ...
  private class Outstanding {
    ...
    ConcurrentHashMap<Long, RequestAndCallback> requests =
        USE_GC_MAP
            ? new GCConcurrentHashMap<Long, RequestAndCallback>(
                new GCConcurrentHashMapCallback() {
                  @Override
                  public void callbackGC(Object key, Object value) {
                    PaxosManager.this.callbackRequestTimeout(
                        ((RequestAndCallback) value).requestPacket);
                  }
                },
                REQUEST_TIMEOUT)
            : new ConcurrentHashMap<Long, RequestAndCallback>();
    HashMap<RequestPacket, RequestAndCallback> conflictIDRequests =
        new HashMap<RequestPacket, RequestAndCallback>();
    ConcurrentHashMap<RequestPacket, RequestResponseAndCallback> responses =
        USE_GC_MAP
            ? new GCConcurrentHashMap<RequestPacket, RequestResponseAndCallback>(
                new GCConcurrentHashMapCallback() {
                  @Override
                  public void callbackGC(Object key, Object value) {}
                },
    ...
    private void enqueue(RequestAndCallback rc) {
      assert (rc.requestPacket.getType() != PaxosPacketType.ACCEPT
          || rc.requestPacket.hasRequestValue());
    ...
    private RequestAndCallback dequeue(RequestPacket request) {
      ...
      else return this.conflictIDRequests.remove(request);
    }
  ...
  static final long REQUEST_TIMEOUT = Config.getGlobalLong(PC.REQUEST_TIMEOUT) * 1000;
  ...
  private void GC() {
    ...
    else if (System.currentTimeMillis() - this.outstanding.lastIncremented
        > PaxosManager.FADE_OUTSTANDING_TIMEOUT) {
      if (this.outstanding.requests.size() > MAX_OUTSTANDING_REQUESTS)
        PaxosConfig.log.severe(this + " clearing clogged outstanding queue");
  ...
  static class RequestResponseAndCallback extends RequestAndCallback {
    ...
    RequestResponseAndCallback(RequestPacket request, Request response, ExecutedCallback callback) {
  ...
  private void defaultCallback(
      Request response, InetSocketAddress clientAddress, InetSocketAddress listenAddress) {
    if (response == null || !(response instanceof ClientRequest)) return;
  ...
  protected boolean executed(RequestPacket requestPacket, Request request, boolean sendResponse) {
    RequestAndCallback rc = this.outstanding.dequeue(requestPacket);
    if (rc != null) this.outstanding.totalRequestSize -= rc.requestPacket.lengthEstimate();
    RequestInstrumenter.remove(requestPacket.requestID);
    // cache response
    if (ENABLE_RESPONSE_CACHING && request instanceof ClientRequest)
      this.outstanding.responses.put(
          requestPacket,
          new RequestResponseAndCallback(
              requestPacket, ((ClientRequest) request), rc != null ? rc.callback : null));
    // only called if executed
    if (rc != null && rc.callback != null) rc.callback.executed(request, true);
    else if (sendResponse) this.defaultCallback(requestPacket, request);
  ...
  protected boolean retransmittedRequest(RequestPacket requestPacket) {
    ...
    if (ENABLE_RESPONSE_CACHING
      ...
      RequestAndCallback rc = this.outstanding.dequeue(requestPacket);
      if (rc != null && rc.callback != null) rc.callback.executed(rrc.clientRequest, false);
      else if (rrc.callback != null) rrc.callback.executed(rrc.clientRequest, false);
      else
        this.defaultCallback(
            rrc.clientRequest, requestPacket.getClientAddress(), requestPacket.getListenAddress());
      assert (requestPacket.batchSize() == 0);
    }
    return rrc != null;
  }
   ...
   * @param niot InterfaceNIOTransport or InterfaceMessenger object used for messaging.
   * @param pi InterfaceReplicable application controlled by gigapaxos. Currently, all paxos
   *     instances must correspond to a single umbrella application even though each
   *     createPaxosInstance method explicitly specifies the app and this information is stored
   *     explicitly inside a paxos instance. The reason for the single umbrella app restriction is
   *     that we won't have a pointer to the appropriate app upon recovery otherwise.
   * @param paxosLogFolder Paxos logging folder.
   * @param enableNullCheckpoints Whether null checkpoints are enabled. We need this flag to be
   *     enabled if we intend to reconfigure paxos groups managed by this PaxosManager. Otherwise,
   *     we can not distinguish between a null checkpoint and no checkpoint, so the next epoch
   *     members may be waiting forever for the previous epoch's final state (that happens to be
   *     null). This flag needs to be set at construction time and can not be changed thereafter.
   */
  public PaxosManager(
      NodeIDType id,
        ...
        Executors.newScheduledThreadPool(
            ...
            new ThreadFactory() {
              ...
              public Thread newThread(Runnable r) {
                Thread thread = Executors.defaultThreadFactory().newThread(r);
                thread.setName(
                    PaxosManager.class.getSimpleName()
                        + myID
                        + "appExecuteThreadPool"
                        + thread.getId());
                return thread;
    ...
    this.largeCheckpointer = new LargeCheckpointer(paxosLogFolder, id.toString());
    ...
    this.paxosLogger =
        new SQLPaxosLogger(
            this.myID, id.toString(), paxosLogFolder, this.wrapMessenger(this.messenger));
    this.nullCheckpointsEnabled = enableNullCheckpoints;
    // periodically remove active state for idle paxii
    executor.scheduleWithFixedDelay(
        new Deactivator(), 0, Config.getGlobalInt(PC.DEACTIVATION_PERIOD), TimeUnit.MILLISECONDS);
    this.pendingDigests =
        new PendingDigests(
            this.outstanding.requests,
  ...
  private void initOutstandingMonitor() {
    ...
    this.executor.scheduleWithFixedDelay(
          ...
          public void run() {
            try {
              if ((PaxosManager.this.outstanding.requests.size() > 0
                          || PaxosManager.this.pendingDigests.size() > 0)
                      && (System.currentTimeMillis() - PaxosManager.this.outstanding.lastIncremented
                          > REQUEST_TIMEOUT)
                  || monitorIterval > 0) {
                HashMap<Long, String> instances = new HashMap<Long, String>();
                for (RequestAndCallback rc : PaxosManager.this.outstanding.requests.values())
                  instances.put(
                      rc.requestPacket.requestID,
                      rc.requestPacket.getPaxosID() + ":" + rc.requestPacket.getSummary());
                PaxosConfig.log.log(
                    Level.INFO,
                    "{0} |outstanding|={1}; {2}; |unpaused|={3}; \n|pending|={4}; {5}",
                    new Object[] {
                      PaxosManager.this,
                      PaxosManager.this.outstanding.requests.size(),
                      Util.truncatedLog(instances.entrySet(), 10),
                      PaxosManager.this.pinstances.size(),
        ...
        0,
        monitorIterval > 0 ? monitorIterval : REQUEST_TIMEOUT,
        TimeUnit.SECONDS);
  }

  /**
   * Refer {@link #PaxosManager(Object, Stringifiable, InterfaceNIOTransport, Replicable, String,
   * boolean)} .
   *
   * @param id
   * @param nc
  ...
  public PaxosManager(
      NodeIDType id,
      Stringifiable<NodeIDType> nc,
      InterfaceNIOTransport<NodeIDType, JSONObject> niot,
      Replicable app) {
    this(id, nc, niot, (app), null, PaxosInstanceStateMachine.ENABLE_NULL_CHECKPOINT_STATE);
  }
  ...
  /**
   * Returns members of this paxos instance if it is active, and null otherwise. Note that this
   * means that we return null even if the paxos instance is being created (but has not yet
   * completed recovery). A non-null answer to this method necessarily means that the paxos instance
   * was ready to take more requests at the time of the query.
   *
   * @param paxosID The name of the object or service being managed as a replicated state machine.
   * @param version The reconfiguration version.
  ...
  /**
   * @param paxosID
   * @return Set of members in the paxos instance named paxosID. There can only be one version of a
   *     paxos instance at a node, so we don't really have to specify a version unless we want to
   *     explicitly get the members for a specific version, in which case, {@link
   *     #getReplicaGroup(String, int)} should be used.
   */
  ...
  private void synchronizedNoop(String paxosID, int version) {
    PaxosInstanceStateMachine pism = this.getInstance(paxosID);
    if (pism != null && pism.getVersion() == version) pism.synchronizedNoop();
  }

  /**
   * A method that specifies the minimum number of necessary arguments to create a paxos instance.
   * The version is assumed to be 0 here. The use of this method is encouraged only if
   * reconfiguration is not desired. The initialState argument can be {@code null} if {@link
   * #isNullCheckpointStateEnabled()} is true.
   *
   * @param paxosID
   * @param gms
   * @param initialState
   * @return Returns true if the paxos instance paxosID:version or one with a higher version number
   *     was successfully created.
   */
  public boolean createPaxosInstance(String paxosID, Set<NodeIDType> gms, String initialState) {
    return this.createPaxosInstance(paxosID, 0, gms, myApp, initialState, null, true) != null;
  }
   ...
   */
  public boolean createPaxosInstance(
      String paxosID, int version, Set<NodeIDType> gms, Replicable app, String initialState) {
    return this.createPaxosInstance(paxosID, version, gms, app, initialState, null, true) != null;
  }

  private PaxosInstanceStateMachine createPaxosInstance(
      String paxosID,
      int version,
      Set<NodeIDType> gms,
      Replicable app,
      ...
      HotRestoreInfo hri,
      boolean tryRestore) {
    return this.createPaxosInstance(
        paxosID, version, gms, app, initialState, hri, tryRestore, false);
  }
   ...
   */
  public boolean createPaxosInstance(Map<String, String> nameStates, Set<NodeIDType> gms) {
    waitPinstancesSize();
    synchronized (this) {
      ...
      PaxosConfig.log.log(
          ...
          new Object[] {this, nameStates.size(), Util.truncatedLog(nameStates.entrySet(), 8)});
      if (SNEAKY_BATCH_CREATION)
        this.paxosLogger.insertInitialCheckpoints(nameStates, Util.setToStringSet(gms), members);
      boolean created = true;
      for (String name : nameStates.keySet()) {
        assert (nameStates.get(name) != null);
        created =
            created
                && (SNEAKY_BATCH_CREATION
                        ? this.createPaxosInstance(
                            ...
                            this.myApp,
                            nameStates.get(name),
                            HotRestoreInfo.createHRI(
                                name,
                                members,
                                PaxosInstanceStateMachine.roundRobinCoordinator(name, members, 0)),
                            false)
                        : this.createPaxosInstance(name, gms, nameStates.get(name)))
                    != null;
      }
      return created;
    }
  ...
  private PaxosInstanceStateMachine createPaxosInstance(
    ...
    return this.createPaxosInstanceFinal(
        paxosID, version, gms, app, initialState, hri, tryRestore, missedBirthing);
  }
  ...
  private synchronized PaxosInstanceStateMachine createPaxosInstanceFinal(
    ...
    if (!gms.contains(this.getNodeID()))
      throw new PaxosInstanceCreationException(
          this.getNodeID()
              + " can not create a paxos instance for group "
              + gms
              + " to which it does not belong");

    boolean tryHotRestore = (hasRecovered() && hri == null);
    PaxosInstanceStateMachine pism = this.getInstance(paxosID, tryHotRestore, tryRestore);
    ...
    if (pism == null && equalOrHigherVersionStopped(paxosID, version)) {
      PaxosConfig.log.log(
          ...
          new Object[] {this, paxosID, version, paxosID, this.getVersion(paxosID)});
      return null;
    }
    ...
    try {
      ...
      pism =
          new PaxosInstanceStateMachine(
              paxosID,
              version,
              myID,
              this.integerMap.put(gms),
              app != null ? app : this.myApp,
              initialState,
              this,
              hri,
              missedBirthing);
    ...
    PaxosConfig.log.log(
        ...
        new Object[] {this, hri != null ? "unpaused" : "created", pism.getPaxosIDVersion()});
  ...
  private void syncPaxosInstance(PaxosInstanceStateMachine pism, boolean forceSync) {
    if (pism != null) pism.poke(forceSync);
  }
  ...
  protected void addServers(Set<NodeIDType> nodes) {
    if (this.unstringer instanceof NodeConfig)
      for (NodeIDType node : nodes) {
        this.servers.add(((NodeConfig<NodeIDType>) this.unstringer).getNodeAddress(node));
      }
  }
  ...
  private static final int MAX_OUTSTANDING_REQUESTS =
      Config.getGlobalInt(PC.MAX_OUTSTANDING_REQUESTS);
  private final boolean DISABLE_CC = Config.getGlobalBoolean(PC.DISABLE_CC);
  private final boolean ORDER_PRESERVING_REQUESTS =
      Config.getGlobalBoolean(PC.ORDER_PRESERVING_REQUESTS);
  ...
  class JSONDemultiplexer extends AbstractJSONPacketDemultiplexer {
    ...
    public boolean handleMessage(JSONObject jsonMsg, edu.umass.cs.nio.nioutils.NIOHeader header) {
      try {
        ...
        PaxosManager.this.handleIncomingPacket(
            edu.umass.cs.gigapaxos.paxosutil.PaxosPacketDemultiplexer.toPaxosPacket(
                fixNodeStringToInt(jsonMsg), PaxosManager.this.unstringer));
        return true;
      } catch (JSONException e) {
        log.severe(this + " unable to parse JSON or unable fix node ID string to integer");
        e.printStackTrace();
      } catch (OverloadException oe) {
        if (this.clientFacing) PaxosPacketDemultiplexer.throttleExcessiveLoad();
      }
      return true;
    }
    ...
    public boolean isOrderPreserving(JSONObject msg) {
      ...
      try {
        ...
        log.severe(this + " incurred JSONException while parsing " + msg);
        e.printStackTrace();
      }
      return false;
    }
  ...
  class FastDemultiplexer extends edu.umass.cs.gigapaxos.paxosutil.PaxosPacketDemultiplexerFast {

    public FastDemultiplexer(int numThreads, boolean clientFacing) {
      super(numThreads);
      this.setThreadName(
          PaxosManager.this.intToString(myID) + (clientFacing ? "-clientFacing" : ""));
      this.register(PaxosPacket.PaxosPacketType.PAXOS_PACKET);
    }
    ...
    public boolean handleMessage(Object msg, edu.umass.cs.nio.nioutils.NIOHeader header) {
      ...
      if (msg instanceof net.minidev.json.JSONObject)
        try {
          ...
          assert ((type = PaxosPacket.getPaxosPacketType(jsonMsg)) != PaxosPacketType.ACCEPT
              || jsonMsg.containsKey(RequestPacket.Keys.STRINGIFIED.toString()));
          long t = System.nanoTime();
          PaxosPacket pp =
              edu.umass.cs.gigapaxos.paxosutil.PaxosPacketDemultiplexerFast.toPaxosPacket(
                  fixNodeStringToInt(jsonMsg), PaxosManager.this.unstringer);
          Level level = type == PaxosPacketType.REQUEST ? Level.FINE : Level.FINEST;
          PaxosConfig.log.log(
              level,
              "{0} packet fast demultiplexer received {1}",
              new Object[] {PaxosManager.this, pp.getSummary(PaxosConfig.log.isLoggable(level))});

          if (PaxosMessenger.INSTRUMENT_SERIALIZATION && Util.oneIn(100))
            if (pp.getType() == PaxosPacketType.REQUEST)
              DelayProfiler.updateDelayNano("requestPacketization", t);
    ...
    // @Override
    public boolean isOrderPreservingFIXME_REMOVE(net.minidev.json.JSONObject msg) {
      if (!ORDER_PRESERVING_REQUESTS) return false;
      try {
        // only preserve order for REQUEST or PROPOSAL packets
        PaxosPacketType type = PaxosPacket.getPaxosPacketType(msg);
  ...
  private static final boolean ENABLE_RESPONSE_CACHING =
      Config.getGlobalBoolean(PC.ENABLE_RESPONSE_CACHING);

  private void handleIncomingPacket(PaxosPacket pp) {

    if (ENABLE_RESPONSE_CACHING
        && pp.getType() == PaxosPacketType.REQUEST
        && this.retransmittedRequest(((RequestPacket) pp))) return;
    else if (pp.getType() == PaxosPacketType.BATCHED_PAXOS_PACKET)
      for (PaxosPacket packet : ((BatchedPaxosPacket) pp).getPaxosPackets())
        this.handleIncomingPacket(packet);
    else if (BATCHING_ENABLED) this.enqueueRequest(pp);
    else this.handlePaxosPacket(pp);
  }
  ...
  private void error(RequestPacket req) {
    PaxosConfig.log.warning(this + " received request with no paxosID: " + req.getSummary());
  }
    ...
    try {
      ...
      switch (paxosPacketType) {
        ...
        default: // paxos protocol messages
          ...
          PaxosConfig.log.log(
              level,
              "{0} received paxos message for {1} : {2}",
              new Object[] {
                this,
                pism != null ? pism : "non-existent instance",
                request.getSummary(PaxosConfig.log.isLoggable(level))
              });
          if ((pism != null) && (pism.getVersion() == request.getVersion())) {
            PaxosPacket.PaxosPacketType msgType =
                request != null ? request.getType() : PaxosPacket.PaxosPacketType.NO_TYPE;
            ...
            if (msgType.equals(PaxosPacket.PaxosPacketType.ACCEPT_REPLY)
              ...
              this.appExecuteThreadPool.execute(
                  new Runnable() {
                    public void run() {
                      try {
      ...
      PaxosConfig.log.severe(this.myID + " received bad JSON message: " + request);
      je.printStackTrace();
    } finally {
      setProcessing(false);
    }
  ...
  private String propose(String paxosID, RequestPacket requestPacket, ExecutedCallback callback) {
    ...
    if (pism != null) {
      ...
      PaxosConfig.log.log(
          Level.FINE,
          "{0} proposing to {1}: {2}",
          new Object[] {this, pism.getPaxosIDVersion(), requestPacket.getSummary()});
   ...
   * @param requestPacket
   * @param callback
   * @return The paxosID:version represented as a String to which the request got proposed; null if
   *     no paxos group named paxosID exists locally.
   */
  public String propose(String paxosID, String requestPacket, ExecutedCallback callback) {
    return propose(
        paxosID,
        (new RequestPacket(this.outstanding.generateUnusedID(), requestPacket, false))
  ...
  public String proposeStop(String paxosID, Request request, ExecutedCallback callback) {
    ...
    return this.propose(paxosID, this.getRequestPacket(request, true), callback);
  }

  @SuppressWarnings("deprecation")
  private RequestPacket getRequestPacket(Request request, boolean stop) {
    return request instanceof RequestPacket
            && request.getRequestType().getInt() == PaxosPacket.PaxosPacketType.REQUEST.getInt()
        ?
        // return as-is
        (stop == ((RequestPacket) request).isStopRequest())
            ? (RequestPacket) request
            :
            // fix stop
            new RequestPacket(
                ((RequestPacket) request).requestID,
                ((RequestPacket) request).requestValue,
                stop,
                (RequestPacket) request)
        :
        ...
        request instanceof ClientRequest
            ...
            :
            // default
            new RequestPacket(this.outstanding.generateUnusedID(), request.toString(), stop)
                .setReturnRequestValue();
  }
   ...
   */
  public String proposeStop(
      String paxosID, int version, Request request, ExecutedCallback callback) {
    PaxosInstanceStateMachine pism = this.getInstance(paxosID);
    if (pism != null && pism.getVersion() == version) {
      return this.propose(
          paxosID,
          (RequestPacket) (this.getRequestPacket(request, true).putPaxosID(paxosID, version)),
          callback);
    } else
      PaxosConfig.log.log(
          Level.INFO,
          ...
          new Object[] {
            ...
            paxosID,
            version,
            (pism == null ? " no paxos instance found " : pism.getPaxosIDVersion() + " pre-exists")
          });
    return null;
  }

  /**
   * Proposes a request to stop a specific version of paxosID. There is no way to stop a paxos
   * replicated state machine other than by issuing a stop request. The only way to know for sure
   * that the RSM is stopped is by the application receiving the stop request.
   *
   * @param paxosID
   * @param value
   * @param version
   * @param callback
   * @return The paxosID:version represented as a String to which this request was proposed.
   */
  public String proposeStop(String paxosID, int version, String value, ExecutedCallback callback) {
    ...
    if (pism != null && pism.getVersion() == version) {
      return this.propose(
          paxosID,
          (RequestPacket)
              ((new RequestPacket(this.outstanding.generateUnusedID(), value, true))
                  .setReturnRequestValue()
                  .putPaxosID(paxosID, version)),
          callback);
    } else
      PaxosConfig.log.log(
          Level.INFO,
  ...
  /**
   * Stop the paxos instance named paxosID on this machine irrespective of the version. Note that
   * there can be at most one paxos version with a given name on a machine.
   *
   * @param paxosID
   * @param value
   * @param callback
  ...
  public String proposeStop(String paxosID, String value, ExecutedCallback callback) {
    PaxosInstanceStateMachine pism = this.getInstance(paxosID);
    if (pism == null) return null;
    else return this.proposeStop(paxosID, pism.getVersion(), value, callback);
  }

  /**
   * @param paxosID
   * @param version
   * @return The final state wrapped in a StringContainer. We use StringContainer to distinguish
   *     between null state and no state when null checkpoint state is enabled.
   */
  ...
  /**
   * We are only allowed to delete a stopped paxos instance. There is no public method to force-kill
   * a paxos instance other than by successfully creating a paxos instance with the same name and
   * higher version. The only way to stop a paxos instance is to get a stop request committed.
   *
   * @param paxosID
   * @param version
   * @return Returns true if the paxos instance {@code paxosID:version} exists and is stopped. If
   *     so, the instance will be deleted.
   */
  public boolean deleteStoppedPaxosInstance(String paxosID, int version) {
    ...
    if (pism != null && pism.getVersion() == version)
      ...
      if (pism.synchronizedNoop() && pism.isStopped()) return true;
    return false;
  }
   ...
   * @param paxosID
   * @param version
   * @return Returns true if the final state was successfully deleted or there was nothing to
   *     delete.
   */
  public boolean deleteFinalState(String paxosID, int version) {
    ...
    return this.paxosLogger.deleteEpochFinalCheckpointState(paxosID, version);
  }
  ...
  public PaxosManager<NodeIDType> initClientMessenger(
      InetSocketAddress myAddress, InterfaceNIOTransport<NodeIDType, ?> niot) {
    PaxosManager<NodeIDType> pm = this.initClientMessenger(myAddress, false, niot);
  ...
  private PaxosManager<NodeIDType> initClientMessenger(
      InetSocketAddress myAddress, boolean ssl, InterfaceNIOTransport<NodeIDType, ?> nioTransport) {
    Messenger<InetSocketAddress, JSONObject> cMsgr = null;
    SSLMessenger<NodeIDType, ?> msgr =
        (nioTransport instanceof Messenger ? (SSLMessenger<NodeIDType, ?>) nioTransport : null);
    ...
    try {
      ...
      if (clientPortOffset > 0) {
        InetSocketAddress myAddressOffsetted =
            new InetSocketAddress(myAddress.getAddress(), myAddress.getPort() + clientPortOffset);
        PaxosConfig.log.log(
            Level.INFO,
        ...
        cMsgr =
            new JSONMessenger<InetSocketAddress>(
                createdNIOTransport =
                    new MessageNIOTransport<InetSocketAddress, JSONObject>(
                            ...
                            ssl
                                ? SSLDataProcessingWorker.SSL_MODES.valueOf(
                                    Config.getGlobalString(PC.CLIENT_SSL_MODE))
                                : SSL_MODES.CLEAR)
                        .setName(this.intToString(this.myID)));
        ...
        assert (msgr != null);
        if (ssl) msgr.setSSLClientMessenger(cMsgr);
        else msgr.setClientMessenger(cMsgr);
      }
    } catch (IOException e) {
      e.printStackTrace();
  ...
  public void forceCheckpoint(String paxosID) {
    PaxosInstanceStateMachine pism = this.getInstance(paxosID);
    if (pism != null) pism.forceCheckpoint();
  }
  ...
  private int getNumOutstandingOrQueued() {
    return (this.outstanding.requests.size()) + this.requestBatcher.getQueueSize();
  }
  ...
  protected void incrOutstanding(RequestPacket request) {
    ...
    if (Util.oneIn(10)) DelayProfiler.updateMovAvg("outstanding", this.outstanding.requests.size());
  }
  ...
  /**
   * @return Minimum delay between successive sync decisions for the same paxos instance. This rate
   *     limit prevents unnecessary sync'ing under high load when a nontrivial extent of
   *     out-of-order-ness is expected.
   */
  ...
  /**
   * @return Maximum reordering among received decisions that when exceeded prompts a checkpoint
   *     transfer as opposed to a sync-decisions operation.
   */
  ...
  public final void close() {
    ...
    if (this.myApp instanceof GigapaxosShutdownable)
      ((GigapaxosShutdownable) this.myApp).shutdown();

    for (Iterator<PaxosInstanceStateMachine> pismIter = this.pinstances.concurrentIterator();
        pismIter.hasNext(); )
      PaxosConfig.log.log(
          Level.FINE,
  ...
  private static final boolean EMULATE_UNREPLICATED =
      Config.getGlobalBoolean(PC.EMULATE_UNREPLICATED);

  private final boolean emulateUnreplicated(PaxosPacket request) {
    if (!EMULATE_UNREPLICATED || !(request instanceof RequestPacket)) return false;
    // else will finally return true
    // pretend-execute new requests

    PaxosInstanceStateMachine.execute(
        null, this, myApp, ((RequestPacket) request).setEntryReplica(getMyID()), false);
    return true;
  }

  private static final boolean LAZY_PROPAGATION = Config.getGlobalBoolean(PC.LAZY_PROPAGATION);

  private final boolean emulateLazyPropagation(PaxosPacket request) {
    if (!LAZY_PROPAGATION || !(request instanceof RequestPacket)) return false;

    // else will finally return true
    try {
      // extract newly received requests
      request = ((RequestPacket) request).getEntryReplicaRequestsAsBatch(getMyID())[1];
      ...
      if (request != null) {
        ...
        PaxosInstanceStateMachine pism = this.getInstance(request.getPaxosID());
        MessagingTask mtask =
            pism != null ? new MessagingTask(pism.otherGroupMembers(), request) : null;
        this.send(mtask);
      ...
      PaxosConfig.log.severe(this + " IOException while lazy-propagating " + request.getSummary());
      e.printStackTrace();
    }
    return true;
  }
  ...
  private /* synchronized */ PaxosInstanceStateMachine getInstance(
    ...
    synchronized (this) {
      ...
      if ((pism = pinstances.get(paxosID)) != null) pism.markActive();
    }
    if (pism == null
        && ((tryHotRestore && (pism = this.unpause(paxosID)) != null)
            || (tryRestore && (pism = this.restore(paxosID)) != null)))
      // nothing here
      ;
  ...
  private synchronized void initiateRecovery(NodeIDType id) {
    ...
    PaxosConfig.log.log(Level.INFO, "{0} beginning to recover checkpoints", new Object[] {this});
    ...
    while ((pri = this.paxosLogger.readNextCheckpoint(true)) != null) {
      ...
      try {
        this.recover(
            ...
            pri.getVersion(),
            this.myID,
            getNodesFromStringSet(pri.getMembers()),
            myApp,
            pri.getState());
      } catch (PaxosInstanceCreationException pice) {
        // should we remove this checkpoint?
        pice.printStackTrace();
        PaxosConfig.log.severe(this + " unable to create paxos instance " + pri.getPaxosID());
      }
    ...
    if (!found) {
      PaxosConfig.log.warning(
          "No checkpoint state found for node "
              + id
    ...
    PaxosConfig.log.log(
        Level.INFO, "{0} beginning to roll forward logged messages", new Object[] {this});
    while (this.paxosLogger.initiateReadMessages())
      ; // acquires lock
    String paxosPacketString = null;
    PaxosPacket paxosPacket = null;
    /**
     * Set packetizer for logger. We need this in order to have the benefits of caching the original
     * string form of received accepts to reduce serialization overhead. Without a packetizer, the
     * logger doesn't know how to convert original string node IDs (that came over the network) in
     * the logged messages to integer IDs. The alternative would be to just store integer IDs in
     * logged messages (trusting that we will always be able to have IntegerMap be able to convert
     * back to the original node IDs because checkpoint recovery happens before rolling forward
     * logs), but that means we wouldn't have the stringified caching optimization.
     */
    this.paxosLogger.setPacketizer(
        new AbstractPaxosLogger.PaxosPacketizer() {

          @Override
          protected PaxosPacket stringToPaxosPacket(String str) {
            try {
              PaxosPacket pp =
                  PaxosPacket.getPaxosPacket(
                      PaxosManager.this.fixNodeStringToInt(new JSONObject(str)));
              assert (pp != null) : str;
              return pp;
            } catch (JSONException e) {
              e.printStackTrace();
              try {
                PaxosConfig.log.severe(
                    PaxosManager.this
          ...
          protected PaxosPacket stringToPaxosPacket(byte[] bytes) {
            ...
            try {
              ...
              // else
              return this.stringToPaxosPacket(MessageExtractor.decode(bytes));
    ...
    /**
     * We need this for {@link SQLPaxosLogger} for logging messages with int IDs converted back to
     * the original string IDs. The logger will invoke stringToPaxosPacket(String) above while
     * reading logged messages from disk. Disk I/O is no different from network I/O in that integer
     * IDs have meaning outside of PaxosInstanceStateMachine soft state.
     *
     * <p>We don't always have to convert int to string IDs because if we have {@code stringified}
     * already stored, it is already in a network-friendly form.
     */
    this.paxosLogger.setPaxosPacketStringifier(
        new AbstractPaxosLogger.PaxosPacketStringifier() {

          @Override
          protected String paxosPacketToString(PaxosPacket paxosPacket) {
            PaxosPacket.PaxosPacketType type = paxosPacket.getType();
            ...
            if (type == PaxosPacket.PaxosPacketType.ACCEPT
              ...
              if (paxosPacket instanceof RequestPacket
                  && (stringified = ((RequestPacket) paxosPacket).getStringifiedSelf()) != null)
                return stringified;
              else
                try {
                  net.minidev.json.JSONObject jsonSmart = paxosPacket.toJSONSmart();
                  stringified =
                      jsonSmart != null
                          ? PaxosManager.this.messenger.fixNodeIntToString(jsonSmart).toString()
                          :
    ...
    try {
      while ((paxosPacket = this.paxosLogger.readNextMessage()) != null) {
        ...
        PaxosConfig.log.log(
            ...
            new Object[] {this, paxosPacket.getSummary(PaxosConfig.log.isLoggable(level))});
      ...
      Util.suicide(
          PaxosConfig.log,
          this
              + " recovery interrupted while parsing "
              + paxosPacketString
    ...
    while ((pri = this.paxosLogger.readNextCheckpoint(true)) != null) {
      ...
      if (pism != null) {
        ...
        if (System.currentTimeMillis() - initTime < Config.getGlobalLong(PC.DEACTIVATION_PERIOD))
          pism.poke(true);
      }
  ...
  private static final boolean BATCHED_ACCEPT_REPLIES =
  ...
  private void notifyLoggedDecisions(MessagingTask[] mtasks) {
    ...
    for (Map.Entry<String, Integer> entry :
        PaxosPacketBatcher.getMaxLoggedDecisionMap(mtasks).entrySet())
  ...
  protected void send(MessagingTask mtask, boolean coalesce, boolean logMsg)
    ...
    if (logMsg && mtask instanceof LogMessagingTask) {
      AbstractPaxosLogger.logAndMessage(
          this.paxosLogger, (LogMessagingTask) mtask); // , this.messenger);
    } else {
      this.sendOrLoopback(coalesce ? PaxosManager.this.ppBatcher.coalesce(mtask) : mtask);
    }
  }
  ...
  protected void send(InetSocketAddress sockAddr, Request request, InetSocketAddress listenSockAddr)
      throws JSONException, IOException {
    try {
      this.messenger.sendClient(
          sockAddr,
          request instanceof RequestPacket ? (((RequestPacket) request).toBytes()) : request,
          listenSockAddr);
   ...
   * killed. */
  protected synchronized void kill(PaxosInstanceStateMachine pism, boolean clean) {
    assert (pism != null);
    if (this.isClosed())
      throw new PaxosInstanceDestructionException(this + " has already been closed");
    ...
    PaxosInstanceStateMachine existing = this.getInstance(pism.getPaxosID());
    ...
    while (!pism.kill(clean))
      PaxosConfig.log.severe(
          "Problem stopping paxos instance " + pism.getPaxosID() + ":" + pism.getVersion());
    incrKilled();
    this.softCrash(pism);
    this.corpses.put(pism.getPaxosID(), pism);
    executor.schedule(
        new Cremator(pism.getPaxosID(), this.corpses),
        Config.getGlobalInt(PC.MORGUE_DELAY),
  ...
  protected void crashAndRecover(PaxosInstanceStateMachine pism) {
    if (pism == null || this.isClosed() || !this.pinstances.containsValue(pism)) return;
  ...
  protected synchronized boolean hibernate(PaxosInstanceStateMachine pism) {
    if (pism == null
        ...
        || !this.isHibernateable()) return false;

    // else hibernate
    boolean hibernated = false;
    PaxosConfig.log.log(Level.INFO, "{0} trying to hibernate {1}", new Object[] {this, pism});
    boolean stopped = pism.tryForcedCheckpointAndStop();

    if (stopped) {
      this.softCrash(pism);
      hibernated = true;
      PaxosConfig.log.log(Level.INFO, "{0} sucessfully hibernated {1}", new Object[] {this, pism});
    }
    return hibernated;
  }
  ...
  private synchronized PaxosInstanceStateMachine restore(String paxosID) {
    if (this.isClosed() || !this.isHibernateable()) return null;
    PaxosInstanceStateMachine pism = null;
    if ((pism = this.pinstances.get(paxosID)) != null) {
      return pism;
    }
    PaxosConfig.log.log(
        Level.INFO, "{0} trying to restore instance {1}", new Object[] {this, paxosID});
    ...
    if (pism != null) {
      ...
      PaxosConfig.log.log(
          Level.WARNING, "{0} unable to restore paxos instance {1}", new Object[] {this, paxosID});
    }
    return pism;
  }
  ...
  protected HotRestoreInfo pause(PaxosInstanceStateMachine pism) {
    if (pism == null
        || this.isClosed()
        || !this.pinstances.containsValue(pism)
        || !isPauseEnabled()) return null;
    PaxosConfig.log.log(
        Level.FINE, "{0} trying to pause {1}", new Object[] {this, pism.getPaxosIDVersion()});
    ...
    if (hri != null) {
      ...
      this.softCrash(pism);
      if (Util.oneIn(Integer.MAX_VALUE)) DelayProfiler.updateDelay("pause", pauseInitTime);
      PaxosConfig.log.log(
          Level.FINE, "{0} successfully paused {1}", new Object[] {this, pism.getPaxosIDVersion()});
    } else PaxosConfig.log.log(Level.FINE, "{0} failed to pause {1}", new Object[] {this, pism});
    return hri;
  }
  ...
  protected final Diskable<String, PaxosInstanceStateMachine> disk =
      new Diskable<String, PaxosInstanceStateMachine>() {
        ...
        @Override
        public Set<String> commit(Map<String, PaxosInstanceStateMachine> toCommit)
            throws IOException {
  ...
  protected synchronized Set<String> pause(
    ...
    if (this.isClosed() || !isPauseEnabled()) return null;
    ...
    for (PaxosInstanceStateMachine pism : pauseBatch.values()) {
      ...
      if (hri != null) hriMap.put(paxosID, hri);
    }

    // write paused state to disk
    Map<String, HotRestoreInfo> pausedHRIMap = this.paxosLogger.pause(hriMap);
    for (HotRestoreInfo pausedHRI : pausedHRIMap.values()) hriMap.remove(pausedHRI.paxosID);
    ...
    for (HotRestoreInfo hri : hriMap.values()) {
      ...
      if (rolledBack)
        PaxosConfig.log.log(
            Level.INFO, "{0} rolled back pausing of {1}", new Object[] {this, hri.paxosID});
      else
        PaxosConfig.log.log(
            Level.SEVERE,
  ...
  private /* synchronized */ PaxosInstanceStateMachine unpause(String paxosID) {
    if (this.isClosed() || !this.hasRecovered() || !this.isPauseEnabled()) return null;

    PaxosInstanceStateMachine restored = null;
    if ((restored = this.pinstances.get(paxosID)) != null) return restored;
    ...
    synchronized (this.stringLocker.get(paxosID)) {
      PaxosConfig.log.log(
          Level.FINE, "{0} about to try to unpause instance {1}", new Object[] {this, paxosID});
      ...
      if (hri != null) {
        ...
        PaxosConfig.log.log(
            Level.FINE, "{0} unable to unpause instance {1}", new Object[] {this, paxosID});
    }
   ...
   * forward. */
  private PaxosInstanceStateMachine recover(
      String paxosID, int version, int id, Set<NodeIDType> members, Replicable app, String state) {
    PaxosConfig.log.log(
        Level.FINE, "{0} {1}:{2} {3} recovering", new Object[] {this, paxosID, version, members});
    PaxosInstanceStateMachine pism =
        this.createPaxosInstance(paxosID, version, (members), app, null, null, false);
    return pism;
  }

  private PaxosInstanceStateMachine recover(
      String paxosID, int version, int id, Set<NodeIDType> members, Replicable app) {
  ...
  private void rollForward(String paxosID, int version) {
    if (
    /* !ONE_PASS_RECOVERY || */ this.hasRecovered()) {
      PaxosConfig.log.log(
          Level.FINE, "{0} about to roll forward {1}:{2}", new Object[] {this, paxosID, version});
  ...
  private void findPaxosInstance(PaxosPacket pp) throws JSONException {
    ...
    if (zombie == null || (zombie.getVersion() - pp.getVersion()) < 0) findReplicaGroup(pp);
    else this.logPacketDrop(pp);
  }

  private void logPacketDrop(PaxosPacket pp) {
    PaxosConfig.log.log(
        PaxosPacket.isRecovery(pp) ? Level.FINE : Level.INFO,
  ...
  protected void heardFrom(int id) {
    try {
      ...
      PaxosConfig.log.log(
          Level.INFO, "{0} has no NodeIDType entry for integer {1}", new Object[] {this, id});
    }
  }
  ...
  protected long getDeadTime(int id) {
    return (FD != null ? FD.getDeadTime(this.integerMap.get(id)) : System.currentTimeMillis());
  }
  ...
  private synchronized void timedWaitCanCreateOrExistsOrHigher(
    ...
    try {
      if (!this.canCreateOrExistsOrHigher(paxosID, version)) wait(timeout);
    } catch (InterruptedException ie) {
      ie.printStackTrace();
    }
  }
  ...
  private synchronized void timedWaitForExistence(String paxosID, int version, long timeout) {
    try {
      long waitStartTime = System.currentTimeMillis();
      while (!this.equalOrHigherVersionExists(paxosID, version)
          && System.currentTimeMillis() - waitStartTime < timeout) wait(timeout);
    } catch (InterruptedException ie) {
      ie.printStackTrace();
    }
  }

  protected synchronized void waitCanCreateOrExistsOrHigher(String paxosID, int version) {
    try {
      while (!this.canCreateOrExistsOrHigher(paxosID, version)) wait();
    } catch (InterruptedException ie) {
      ie.printStackTrace();
    }
  }
  ...
  /**
   * This is a common default create paxos instance creation method, i.e., we wait for lower
   * versions to be killed if necessary but, after a timeout, create the new instance forcibly
   * anyway. This method won't create the instance if the same or higher version already exists. One
   * concern with this method is that force killing lower versions in order to start higher versions
   * can cause the epoch final state to be unavailable at *any* node in immediately lower version.
   * It is okay to kill even an immediately preceding instance only if we already have the necessary
   * state (presumably obtained from a stopped immediately preceding instance at some other group
   * member). In the reconfiguration protocol, we must ensure this by starting a new epoch only
   * after receiving a successful acknowledgment for stopping the previous epoch. However, if say
   * only one node has executed the previous epoch's stop and then starts the new epoch and causes
   * other new epoch replicas to force-create the new epoch, and then crashes, the new epoch is
   * stuck.
   *
   * @param paxosID
   * @param version
   ...
   * @param state
   * @param timeout
   * @return Returns true if this paxos instance or one with a higher version number was
   *     successfully created.
   */
  public boolean createPaxosInstanceForcibly(
      String paxosID,
      int version,
      Set<NodeIDType> gms,
      Replicable app,
    ...
    return this.createPaxosInstanceForcibly(paxosID, version, gms, app, state);
  }

  private synchronized boolean createPaxosInstanceForcibly(
      String paxosID, int version, Set<NodeIDType> gms, Replicable app, String state) {
    PaxosInstanceStateMachine pism = this.getInstance(paxosID);
    ...
    if (pism != null && pism.getVersion() - version < 0) {
      PaxosConfig.log.log(
          ...
          new Object[] {this, pism.getPaxosIDVersion(), paxosID, version});
      this.kill(pism); // will succeed or throw exception
    }
    boolean created = this.createPaxosInstance(paxosID, version, gms, app, state);
    return created;
  }
   ...
   * @param paxosID
   * @param version
   * @return Returns true if the paxos instance {@code paxosID:version} exists or a higher version
   *     exists.
   */
  ...
  /**
   * Unlike the method {@link #equalOrHigherVersionExists(String, int)}, this method checks for
   * whether an equal or higher version was previously stopped. We need this check to prevent going
   * back in time, i.e., starting an epoch after it has been stopped.
   *
   * @param paxosID
   * @param version
   * @return
  ...
  private boolean equalOrHigherVersionStopped(String paxosID, int version) {
    ...
    Integer lastStoppedVersion = this.paxosLogger.getEpochFinalCheckpointVersion(paxosID);
    if (lastStoppedVersion != null && lastStoppedVersion - version >= 0) return true;
    return false;
  }
  ...
  private String printLog(String paxosID) {
    return ("State for " + paxosID + ": Checkpoint: " + this.paxosLogger.getStatePacket(paxosID));
  }
  ...
  private void findReplicaGroup(PaxosPacket pp) throws JSONException {
    FindReplicaGroupPacket findGroup =
        new FindReplicaGroupPacket(this.myID, pp); // paxosID and version should be within
    ...
    if (nodeID >= 0) {
      try {
        PaxosConfig.log.log(
            ...
            new Object[] {this, pp.getSummary(), pp.getPaxosID(), node});
      ...
      PaxosConfig.log.log(
          pp instanceof RequestPacket ? Level.INFO : Level.INFO,
          "{0} cant find group member in {1} {2}",
          new Object[] {
            this,
            pp.getSummary(),
            pp instanceof RequestPacket
                ? ((((RequestPacket) pp).getEntryReplica() != this.myID
                        ? "forwarded by entry node " + node
                        : "received from client" + ((RequestPacket) pp).getClientAddress()))
                    + ", which may be because the paxos instance has not yet been created or has"
    ...
    if (findGroup.group == null && findGroup.nodeID != this.myID) {
      ...
      if (pism == null || (pism.getVersion() - findGroup.getVersion()) < 0) {
        ...
        if (created)
          PaxosConfig.log.log(
              Level.INFO,
              "{0} created paxos instance {1}:{2} from nothing because it apparently missed its"
                  + " birthing",
              new Object[] {this, findGroup.getPaxosID(), findGroup.getVersion()});
      }
    }
    try {
  ...
  private static final double PAUSE_SIZE_THRESHOLD =
      Config.getGlobalDouble(PC.PAUSE_SIZE_THRESHOLD);

  private long lastDeactivationAttempt = 0;

  private boolean shouldTryDeactivation() {
    synchronized (this) {
      if ((System.currentTimeMillis() - lastDeactivationAttempt
              < Config.getGlobalLong(PC.DEACTIVATION_PERIOD))
          || (this.pinstances.size() < this.pinstances.capacity() * PAUSE_SIZE_THRESHOLD))
        return false;
  ...
  private void syncAndDeactivate() {
    if (isClosed() || this.pinstances.size() == 0) return;
    ...
    for (Iterator<PaxosInstanceStateMachine> pismIter = this.pinstances.concurrentIterator();
      ...
      if (pism.isLongIdle()
        ...
        PaxosConfig.log.log(
            Level.FINER, "{0} trying to pause {1} [{2}]", new Object[] {this, paxosID, pism});
        ...
        if (batch.size() >= PAUSE_BATCH_SIZE) {
          ...
          if (batchPaused != null) numPaused += batchPaused.size();
          PaxosConfig.log.log(Level.FINE, "{0} paused {1}", new Object[] {this, batchPaused});
  ...
  private void printPauseLog(Collection<String> paused) {
    ...
    PaxosConfig.log.log(
        paused.size() > 0 || this.totalInstancesChanged ? Level.INFO : Level.FINE,
        "{0} deactivated {1} idle instances {2}; has {3} active instances{4}; avg_pause_delay ="
            + " {5}; avg_deactivation_loop_delay = {6}; total_instances_created = {7};"
            + " total_current_instances = {8}",
        new Object[] {
          this,
          paused.size(),
          Util.truncatedLog(paused, PRINT_LOG_SIZE),
          this.pinstances.size(),
          this.pinstances.size() < 10 ? Arrays.asList(this.pinstances.keySet().toArray()) : "",
  ...
  private JSONObject fixNodeStringToInt(JSONObject json) throws JSONException {
    ...
    if (json.has(PaxosPacket.NodeIDKeys.B.toString())) {
      // fix ballot string
      String ballotString = json.getString(PaxosPacket.NodeIDKeys.B.toString());
      NodeIDType nodeID = this.unstringer.valueOf(Ballot.getBallotCoordString(ballotString));
      if (nodeID != null)
        // assert (coordInt != null);
        json.put(
            PaxosPacket.NodeIDKeys.B.toString(),
            new Ballot(Ballot.getBallotNumString(ballotString), this.integerMap.put(nodeID))
                .toString());
  ...
  private net.minidev.json.JSONObject fixNodeStringToInt(net.minidev.json.JSONObject json) {
    // long t = System.nanoTime();
    // FailureDetectionPacket already has generic NodeIDType
    if ((Integer) json.get(PaxosPacket.Keys.PT.toString())
        == PaxosPacket.PaxosPacketType.FAILURE_DETECT.getInt()) return json;

    if (json.containsKey(PaxosPacket.NodeIDKeys.B.toString())) {
      // fix ballot string
      String ballotString = (String) json.get(PaxosPacket.NodeIDKeys.B.toString());
      json.put(
          PaxosPacket.NodeIDKeys.B.toString(),
          new Ballot(
                  Ballot.getBallotNumString(ballotString),
                  this.integerMap.put(
                      this.unstringer.valueOf(Ballot.getBallotCoordString(ballotString))))
              .toString());
    } else if (json.containsKey(PaxosPacket.NodeIDKeys.GROUP.toString())) {
      // fix group string (JSONArray)
      Collection<?> jsonArray = (Collection<?>) json.get(PaxosPacket.NodeIDKeys.GROUP.toString());
  ...
  /**
   * If set to true, {@link PaxosManager} will clear the DB upon creation. This static flag applies
   * only to PaxosManager instances created after this flag has been set to true.
   *
   * @param clean
   */
  ...
  /**
   * This test method is deprecated and will either be removed or significantly revamped. Use
   * TESTPaxosMain instead to run a single machine test with multiple virtual nodes.
   *
   * @param args
  ...
  static void test(String[] args) throws InterruptedException, IOException, JSONException {
    ...
    for (int i = 0; i < numNodes; i++) {
      ...
      JSONNIOTransport<Integer> niot =
          new JSONNIOTransport<Integer>(members[i], snc, new PacketDemultiplexerDefault(), true);
    ...
    for (int i = 0; i < names.length; i++) names[i] = "paxos" + i;

    System.out.println(
        "Creating "
            + numPaxosGroups
            + " paxos groups each with "
            + numNodes
            + " members each, one each at each of the "
            + numNodes
            + " nodes");
    for (int node = 0; node < numNodes; node++) {
      ...
      for (int group = 0; group < numPaxosGroups; group++) {
        ...
        pms[node].createPaxosInstance(
            names[group], 0, Util.arrayToIntSet(members), apps[node], null, null, false);
    ...
    for (int i = 0; i < numRequests; i++) {
      ...
      try {
        ...
        futures[i] =
            (ScheduledFuture<?>)
                execpool.schedule(crtask, (long) scheduledDelay, TimeUnit.MILLISECONDS);
        scheduledDelay += 0;
      } catch (Exception e) {
        e.printStackTrace();
        continue;
    ...
    PaxosConfig.log.info(
        maxRecoveredNode + " has executed up to slot " + (maxRecoverySlot + numRequests));
    ...
    for (int i = 0; i < numNodes; i++) {
      for (int j = i + 1; j < numNodes; j++) {
        ...
        while (committed1 != committed2) {
          if (committed1 > committed2) apps[j].waitToFinish(names[0], committed1);
          else if (committed1 < committed2) apps[i].waitToFinish(names[0], committed2);
          PaxosConfig.log.info(
              "Waiting : (slot1,hash1)=("
                  + committed1
                  + ","
        ...
        assert (apps[i].getHash(names[0]) == apps[j].getHash(names[0]))
            : ("Waiting : (slot1,hash1)=("
                + committed1
                + ","
                + apps[i].getHash(names[0])
    ...
    System.out.println(
        "\n\nTest completed. Executed "
            ...
            + "ms.\n"
            + "\n"
            + "Note that it is possible for the test to be successful even if the number of"
            + " consistently\n"
            + "executed requests is less than the number of received requests as paxos only"
            + " guarantees\n"
            + "consistency, i.e., that all replicas executed requests in the same order, not that"
            + " all requests\n"
            + "issued will get executed. The latter property can be achieved by clients reissuing"
            + " requests\n"
            + "until successfully executed. With reissuals, clients do need to worry about double"
            + " execution,\n"
            + "so they should be careful. A client is not guaranteed to get a failure message if"
            + " the request fails,\n"
            + "e.g., if the replica receiving a request dies immediately. If the client uses a"
            + " timeout to detect\n"
            + "failure and thereupon reissue its request, it is possible that both the original and"
            + " re-issued\n"
            + "requests are executed. Clients can get around this problem by using sequence numbers"
            + " within\n"
            + "their app, reading the current sequence number, and then trying to commit their"
            + " write provided the\n"
            + "sequence number has not changed in the meantime. There are other alternatives, but"
            + " all of these\n"
    ...
    for (PaxosManager<Integer> pm : pms) pm.close();
  }
  ...
  /**
   * A utility method to check the Replicable Application being handled by this Paxos Manager. Note
   * that the handled Application can be wrapped by classes generated by LargeCheckpointer.wrap(.)
   * and TrivialRepliconfigurable(.).
   *
   * @param another Replicable App whose class name we want to compare.
   * @return true if the handled App by this Paxos Manager equals to {@param another} class name.
   */
  public boolean isAppEquals(Replicable another) {
    String expectedReplicableAppClassName = another.getClass().getSimpleName();
    String myReplicableAppClassName = null;

    Replicable myWrappedApp = this.myApp;
    while ((myWrappedApp instanceof LargeCheckpointer.LargeCheckpointerWrappedApp
        || myWrappedApp instanceof TrivialRepliconfigurable)) {

      if (myWrappedApp instanceof LargeCheckpointer.LargeCheckpointerWrappedApp lcpApp) {
        myWrappedApp = lcpApp.getReplicableApp();
        myReplicableAppClassName = lcpApp.getReplicableApp().getClass().getSimpleName();
      }

      if (myWrappedApp instanceof TrivialRepliconfigurable trivialApp) {
        myWrappedApp = trivialApp.app instanceof Replicable ? (Replicable) trivialApp.app : null;
        myReplicableAppClassName = trivialApp.app.getClass().getSimpleName();
      }
    }

    return expectedReplicableAppClassName.equals(myReplicableAppClassName);
  }
  ...
  private static final int DIGEST_THRESHOLD = Config.getGlobalInt(PC.DIGEST_THRESHOLD);
  private static final boolean DIGEST_REQUESTS = Config.getGlobalBoolean(PC.DIGEST_REQUESTS);
  ...
  protected AcceptPacket digest(AcceptPacket multicastAccept) {
    return this.shouldDigest() ? multicastAccept.digest(this.getMessageDigest()) : multicastAccept;
  }
  ...
  private void callbackDigestedAcceptTimeout(AcceptPacket accept) {
    try {
      PaxosConfig.log.log(
          ...
          new Object[] {
            PaxosManager.this, accept.getSummary(PaxosConfig.log.isLoggable(Level.INFO))
          });
      ...
      if (released != null) {
        ...
        try {
          ...
          PaxosManager.this.messenger.send(new MessagingTask(PaxosManager.this.getMyID(), accept));
          PaxosManager.this.send(
              new MessagingTask(
                  accept.ballot.coordinatorID,
                  new AcceptReplyPacket(
                          PaxosManager.this.getMyID(),
                          accept.ballot,
                          accept.slot,
                          ...
                          accept.requestID)
                      .setDigestRequest()
                      .putPaxosID(accept.getPaxosID(), accept.getVersion())));
        } catch (JSONException | IOException e) {
          e.printStackTrace();
        }
    } catch (Exception e) {
  ...
  private void callbackRequestTimeout(RequestPacket request) {
    ...
    if ((accept = PaxosManager.this.pendingDigests.release(request, true, false)) != null)
      try {
        PaxosManager.this.send(
            new MessagingTask(PaxosManager.this.getMyID(), accept), false, false);
      } catch (JSONException | IOException e) {
        PaxosConfig.log.severe(
            PaxosManager.this
    ...
    PaxosConfig.log.log(
        ...
        new Object[] {
          PaxosManager.this,
          request.getSummary(PaxosConfig.log.isLoggable(level)),
          accept != null ? accept.getSummary(PaxosConfig.log.isLoggable(level)) : ""
        });
  }

  /**
   * @param nodes
   * @return Modified argument after filtering out dead nodes using information from its failure
   *     detector.
   */
  public Set<NodeIDType> removeDead(Set<NodeIDType> nodes) {
    for (Iterator<NodeIDType> iter = nodes.iterator(); iter.hasNext(); )
      if (!this.FD.isNodeUp(iter.next())) iter.remove();
    return nodes;
  }

  protected AcceptPacket getPreviouslyIssuedAccept(RequestPacket proposal) {
    RequestAndCallback rc = null;
    if (((rc = this.outstanding.requests.get(proposal.getRequestID())) != null
            || (rc = this.outstanding.conflictIDRequests.get(proposal)) != null)
        && rc.requestPacket instanceof AcceptPacket
        && rc.requestPacket.equals(proposal)) return (AcceptPacket) rc.requestPacket;
    return null;
  }
  ...
  protected AcceptPacket setIssuedAccept(AcceptPacket accept) {
    ...
    RequestAndCallback rc =
        accept != null ? this.outstanding.requests.get(accept.getRequestID()) : null;
    if (rc != null || (rc = this.outstanding.conflictIDRequests.get(accept)) != null)
      rc.setAcceptPacket(accept);
    return accept;
  }

  /**
   * isPaxosCoordinator returns true if the paxos instance that live in this node (i.e.,
   * ActiveReplica) think itself is the current paxos coordinator. Note that, due to asynchrony, it
   * is possible that multiple nodes think itself as an active coordinator.
   *
   * @param paxosID instance ID, name of service (or object) being replicated.
   * @return true if the paxos instanace in this node think itself as a paxos coordinator.
   */
  ...
  /**
   * getPaxosCoordinator returns the node ID, which this node (Active Replica) think as the
   * coordinator. It is possible this node to return the ID of itself if it thinks it is the current
   * coordinator at the time of executing this method. The returned node ID (if not null) can be
   * used by Messenger to send any protocol messages.
   *
   * @param paxosID instance ID, name of service (or object) being replicated.
   * @return the ID of the coordinator node, or null if this node does not know who is the current
   *     coordinator.
   */
