...
/**
 * @author arun
 *     <p>This class is the main server-side class in typical usage scenarios where the app itself
 *     is specified through an external configuration properties file.
 */
public class PaxosServer {
  ...
  PaxosServer(String myID, NodeConfig<String> nodeConfig, String[] args) throws IOException {
    this.messenger =
        (new JSONMessenger<String>(
            (new MessageNIOTransport<String, JSONObject>(
                myID, nodeConfig, ReconfigurationConfig.getServerSSLMode()))));
    Replicable app = this.createApp(args);
    PaxosManager<String> pm =
        startPaxosManager(
            this.messenger,
            app,
    ...
    pm.createPaxosInstance(PaxosConfig.getDefaultServiceName(), nodeConfig.getNodeIDs(), null);
  }

  private PaxosManager<String> startPaxosManager(
      Messenger<String, JSONObject> messenger, Replicable app, InetSocketAddress myAddress) {
    return new PaxosManager<String>(
            messenger.getMyID(),
            PaxosConfig.getDefaultNodeConfig(),
            this.messenger,
            app,
            null,
            true)
        .initClientMessenger(myAddress, messenger);
  }
  ...
  private Replicable createApp(String[] args) {
    ...
    if (PaxosConfig.application != null) {
      try {
        ...
        try {
          ...
          PaxosConfig.getLogger()
              .severe(
                  "App must support a constructor with a single (String[]) argument"
      ...
      PaxosConfig.getLogger()
          .severe(messenger.getMyID() + " unable to create paxos application replica");
    }
  ...
  private static Set<String> processArgs(String[] args, NodeConfig<String> nodeConfig) {
    ...
    for (String arg : args) if (arg.trim().equals("-c")) PaxosManager.startWithCleanDB(true);
    // server names must be at the end of args
    Set<String> servers = new HashSet<String>();
    for (int i = args.length - 1; i >= 0; i--)
      if (nodeConfig.nodeExists(args[i])) servers.add(args[i]);
    return servers;
  }
