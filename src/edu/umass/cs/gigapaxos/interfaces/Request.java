...
public interface Request extends Summarizable {
  ...
  /**
   * Serializes the request to a String. The default toString() method must be overridden.
   *
      ...
      public String toString() {
        return Request.this.getRequestType()
            ...
            + (Request.this instanceof RequestIdentifier
                ? ":" + ((RequestIdentifier) Request.this).getRequestID()
                : "");
      }
    };
  }
   ...
   * AppRequestParser#getRequest(byte[], edu.umass.cs.nio.nioutils.NIOHeader)} , i.e., invoking
   * getRequest(request.toBytes(), header).equals(request).
   *
   * @return byte[] serialized form of this request
   */
  default byte[] toBytes() {
    try {
      return this.toString().getBytes(MessageNIOTransport.NIO_CHARSET_ENCODING);
