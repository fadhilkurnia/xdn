 ...
 * @param <V>
 */
public class RequestCallbackFuture<V>
    implements Callback<Request, V>,
        // RequestCallback,
        RequestFuture<V> {
  ...
  /**
   * @param request The request until whose execution this callback will block.
   * @param callback
   */
   ...
   *
   * @return Response corresponding to the execution of the request supplied in this callback's
   *     constructor.
   * @throws Exception
   */
  ...
  private V waitResponse(Long timeout, TimeUnit unit) throws ExecutionException {
    ...
    synchronized (this) {
      if (this.response == null)
        try {
          if (timeout != null) this.wait(unit.toMillis(timeout));
          else this.wait();
  ...
  public V get(long timeout, TimeUnit unit) throws ExecutionException, TimeoutException {
    ...
    if (!this.processResponseInvoked)
      throw new TimeoutException(
          "Request " + request.getSummary() + " timed out after " + unit.toMillis(timeout) + "ms");
    return response;
  }
