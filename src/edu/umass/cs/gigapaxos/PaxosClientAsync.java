...
/**
 * @author arun
 *     <p>This class is meant to only send and receive {@link RequestPacket} requests. To use other
 *     app-specific request types, use ReconfigurableAppClientAsync.
 */
public class PaxosClientAsync {
  ...
  private final GCConcurrentHashMap<Long, RequestCallback> callbacks =
      new GCConcurrentHashMap<Long, RequestCallback>(
  ...
  class ClientPacketDemultiplexer extends AbstractPacketDemultiplexer<RequestPacket> {
    ...
    public boolean handleMessage(
        RequestPacket message, edu.umass.cs.nio.nioutils.NIOHeader header) {
      ClientRequest response = message.getResponse();
      if (response != null)
        if (callbacks.containsKey(response.getRequestID()))
          callbacks.remove(response.getRequestID()).handleResponse(response);
    ...
    protected RequestPacket processHeader(byte[] message, NIOHeader header) {
      return PaxosPacketDemultiplexerFast.getRequestPacket(message, header);
    }
  ...
  public Long sendRequest(
      String paxosID, String value, InetSocketAddress server, RequestCallback callback)
    ...
    do {
      ...
      prev = this.callbacks.putIfAbsent((long) request.requestID, callback);
    } while (prev != null);
   ...
   * @throws JSONException
   */
  public long sendRequest(String paxosID, String value, RequestCallback callback)
      throws IOException, JSONException {
    Long reqID = null;
    do {
   ...
   * @throws JSONException
   */
  public Long sendRequest(RequestPacket request, InetSocketAddress server, RequestCallback callback)
      throws IOException, JSONException {
    int sent = -1;
  ...
  public Long sendRequest(RequestPacket request, RequestCallback callback)
    ...
    return this.sendRequest(
        request, servers[(int) (Math.random() * this.servers.length)], callback);
  }
  ...
  public PaxosClientAsync(Set<InetSocketAddress> servers) throws IOException {
    ...
    this.servers =
        PaxosConfig.offsetSocketAddresses(servers, PaxosConfig.getClientPortOffset())
