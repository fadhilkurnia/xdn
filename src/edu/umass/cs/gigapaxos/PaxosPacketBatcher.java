...
public class PaxosPacketBatcher extends ConsumerTask<MessagingTask[]> {
      ...
      if (task != null) {
        ...
        switch (task.msgs[0].getType()) {
          ...
          case BATCHED_ACCEPT:
            ...
            break;
          default:
            if (task.msgs[0] instanceof RequestPacket) this.enqueueImpl(task);
        }
      }
  }
  ...
  private boolean enqueueImpl(BatchedCommit commit) {
    ...
    boolean added = false;
    if (cMap.isEmpty()) added = (cMap.put(commit.ballot, (commit)) != null);
    else if (cMap.containsKey(commit.ballot))
      added = cMap.get(commit.ballot).addBatchedCommit(commit);
    else added = (cMap.put(commit.ballot, (commit)) != null);
  ...
  private boolean enqueueImpl(BatchedAccept bAccept) {
    ...
    boolean added = false;
    if (aMap.isEmpty()) added = (aMap.put(bAccept.ballot, (bAccept)) != null);
    else if (aMap.containsKey(bAccept.ballot))
      added = aMap.get(bAccept.ballot).addBatchedAccept(bAccept);
    else added = (aMap.put(bAccept.ballot, (bAccept)) != null);
  ...
  public MessagingTask[] dequeueImpl() {
    ...
    while (!this.requests.isEmpty()
      ...
      if (pkts.add(mtask)) lengthEstimate += ((RequestPacket) mtask.msgs[0]).lengthEstimate();
  ...
  private PaxosPacket dequeueImplAR() {
    ...
    if (!this.acceptReplies.isEmpty()) {
      ...
      if (arMap.isEmpty()) this.acceptReplies.remove(batchedAR.getPaxosID());
    }
    return batchedAR;
  }
  ...
  private PaxosPacket dequeueImplC() {
    ...
    if (!this.commits.isEmpty()) {
      ...
      if (cMap.isEmpty()) this.commits.remove(batchedCommit.getPaxosID());
    }
  ...
  private PaxosPacket dequeueImplA() {
    ...
    if (!this.accepts.isEmpty()) {
      ...
      if (aMap.isEmpty()) this.accepts.remove(batchedAccept.getPaxosID());
    }
  ...
  private static final boolean BATCH_ACROSS_GROUPS =
      Config.getGlobalBoolean(PC.BATCH_ACROSS_GROUPS);
  ...
  public void process(MessagingTask[] tasks) {
    if (BATCH_ACROSS_GROUPS && tasks.length > MIN_PP_BATCH_SIZE)
      for (MessagingTask mtask : this.batch(tasks)) this.send(mtask);
    else for (MessagingTask mtask : tasks) this.send(mtask);
  }
  ...
  private MessagingTask[] batch(MessagingTask[] mtasks) {
    Map<Set<Integer>, BatchedPaxosPacket> grouped =
        new LinkedHashMap<Set<Integer>, BatchedPaxosPacket>();
    for (MessagingTask mtask : mtasks) {
      if (mtask == null || mtask.isEmptyMessaging()) continue;
      {
        Set<Integer> group = Util.arrayToIntSet(mtask.recipients);
        assert (group != null);
        if (!grouped.containsKey(group)) grouped.put(group, new BatchedPaxosPacket(mtask.msgs));
        else grouped.get(group).append(mtask.msgs);
  ...
  private static boolean BATCHED_ACCEPTS =
      Config.getGlobalBoolean(PC.DIGEST_REQUESTS)
          && !Config.getGlobalBoolean(PC.FLIP_BATCHED_ACCEPTS);

  private static boolean BATCHED_ACCEPT_REPLIES =
  ...
  protected MessagingTask coalesce(MessagingTask mtask) {
    if (mtask == null
        ...
        || isUnbatchableRequests(mtask)) return mtask;

    MessagingTask nonLocal = MessagingTask.getNonLoopback(mtask, this.paxosManager.getMyID());
    if (nonLocal == null || nonLocal.isEmptyMessaging()) return mtask;
    MessagingTask local = MessagingTask.getLoopback(mtask, this.paxosManager.getMyID());
    if (local == null || local.isEmptyMessaging())
      ; // no-op

    boolean isAccReply = allPositiveAcceptReplies(mtask),
        isCommit = allCoalescableDecisions(mtask),
        //
        isAccept = allCoalescableAccepts(mtask),
        isRequest = allCoalescableRequests(mtask);
    if (!isAccReply && !isCommit && !isAccept && !isRequest) return mtask;
  ...
  private boolean allLocal(MessagingTask mtask) {
    for (int recipient : mtask.recipients)
      if (recipient != this.paxosManager.getMyID()) return false;
    return true;
  }
  ...
  private static boolean allPositiveAcceptReplies(MessagingTask mtask) {
    for (PaxosPacket ppkt : mtask.msgs)
      if (!(ppkt instanceof AcceptReplyPacket
          && ((AcceptReplyPacket) ppkt).isCoalescable()
          // only positive accept replies are batchable
          && ((AcceptReplyPacket) ppkt).ballot.coordinatorID == mtask.recipients[0])) return false;
    return true;
  }

  private static boolean allCoalescableDecisions(MessagingTask mtask) {
    for (PaxosPacket ppkt : mtask.msgs)
      if (!(ppkt instanceof PValuePacket && ((PValuePacket) ppkt).isCoalescable())) return false;
    return true;
  }

  private static boolean allCoalescableAccepts(MessagingTask mtask) {
    for (PaxosPacket ppkt : mtask.msgs)
      if (!(ppkt instanceof AcceptPacket && !((AcceptPacket) ppkt).hasRequestValue())) return false;
    return true;
  }

  private static boolean allCoalescableRequests(MessagingTask mtask) {
    for (PaxosPacket ppkt : mtask.msgs)
      if (!(ppkt instanceof RequestPacket && ((RequestPacket) ppkt).batchSize() == 0)) return false;
    return true;
  }
  ...
  static class HashMapContainer implements Collection<HashMap<String, HashMap<Ballot, ?>>> {
    private final HashMap<String, HashMap<Ballot, BatchedAcceptReply>> acceptReplies =
        new HashMap<String, HashMap<Ballot, BatchedAcceptReply>>();
    private final HashMap<String, HashMap<Ballot, BatchedCommit>> commits =
        new HashMap<String, HashMap<Ballot, BatchedCommit>>();
    private final HashMap<String, HashMap<Ballot, BatchedAccept>> accepts =
        new HashMap<String, HashMap<Ballot, BatchedAccept>>();
