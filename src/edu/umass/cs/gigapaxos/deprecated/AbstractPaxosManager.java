...
/**
 * This abstract class is deprecated and was used mainly to transition from the old paxos
 * implementation to the new one. It currently exists only for backwards compatibility reasons.
 *
 * @param <NodeIdType>
 */
@Deprecated
public abstract class AbstractPaxosManager<NodeIdType> {
   ...
   * @return true if paxos instance is created. false if another instance with same ID and version
   *     already exists, or size of nodeIDs is less than 3.
   */
  public abstract boolean createPaxosInstance(
      String paxosIDNoVersion, short version, Set<NodeIdType> nodeIDs, Replicable paxosInterface);
   ...
   *
   * <p>ReqeustPacket.clientID is used to distinguish which method proposed this value.
   *
   * @param paxosIDNoVersion paxosID of the paxos group excluding version number
   * @param value request to be proposed
   * @return NULL if no paxos instance with given paxos ID was found, returns paxosIDNoVersion
   *     otherwise.
   */
