...
public abstract class PaxosCoordinator {
  ...
  protected static PaxosCoordinator makeCoordinator(
      PaxosCoordinator c, int bnum, int coord, int[] members, int slot, boolean recovery) {
    boolean sendPrepare = false;
    if (c == null || c.getPCS() == null || (c.getPCS().getBallot().compareTo(bnum, coord)) < 0) {
      // if(c==null)
    ...
    } else if (c.getPCS() != null && (c.getBallot().compareTo(bnum, coord)) == 0 && !c.isActive())
  ...
  protected static PaxosCoordinator createCoordinator(
      int bnum, int coord, int[] members, int slot, boolean recovery, final String myID) {
    PaxosCoordinatorState c =
        new PaxosCoordinatorState(bnum, coord, slot, members, null) {
  ...
  public static boolean isPreemptedFully(PaxosCoordinator c, AcceptReplyPacket acceptReply) {
  ...
  protected static PaxosCoordinator hotRestore(PaxosCoordinator c, HotRestoreInfo hri) {
    ...
    PaxosCoordinator coordinator =
        makeCoordinator(
            c,
            hri.coordBallot.ballotNumber,
            hri.coordBallot.coordinatorID,
            hri.members,
            hri.nextProposalSlot,
            true);
   ...
   * Phase2a
   */
  protected abstract AcceptPacket propose(int[] members, RequestPacket request);

  protected static AcceptPacket propose(PaxosCoordinator c, int[] groupMembers, RequestPacket req) {
    return c != null ? c.propose(groupMembers, req) : null;
  }

  protected static PValuePacket handleAcceptReply(
      PaxosCoordinator c, int[] members, AcceptReplyPacket acceptReply) {
   ...
   * Phase2b
   */
  protected synchronized PValuePacket handleAcceptReply(
    ...
    if (acceptReply.ballot.compareTo(this.getPCS().getBallot()) > 0) {
      if ((preemptedPValue = getPCS().handleAcceptReplyHigherBallot(acceptReply)) != null) {
        /*
        ...
        log.log(
            ...
            new Object[] {
              this.getPCS().getBallot(), acceptReply.getPaxosID(), acceptReply.slotNumber
            });
      ...
      committedPValue = getPCS().handleAcceptReplyMyBallot(members, acceptReply);
    } else
      log.log(
          Level.FINE,
          ...
          new Object[] {
            ...
            acceptReply.getVersion(),
            acceptReply.acceptor,
            acceptReply.ballot,
            this.getPCS().getBallotStr(),
            acceptReply
          });
  ...
  protected abstract PValuePacket handleAcceptReplyMyBallot(
      int[] members, AcceptReplyPacket acceptReply);

  protected abstract PValuePacket handleAcceptReplyHigherBallot(AcceptReplyPacket acceptReply);
    ...
    if (this.getPCS().isPrepareAcceptedByMajority(prepareReply, members)) {
      ...
      this.getPCS()
          .combinePValuesOntoProposals(
              members, prepareReply.getPaxosID(), prepareReply.getVersion());
  ...
  private synchronized ArrayList<ProposalPacket> getPreActivesIfPreempted(
      PrepareReplyPacket prepareReply, int[] members) {
    return (this.exists() && !this.isActive() && this.getPCS().isPreemptable(prepareReply))
        ? this.resignAsCoordinator()
        : null;
  }

  protected static ArrayList<ProposalPacket> getPreActivesIfPreempted(
      PaxosCoordinator c, PrepareReplyPacket prepareReply, int[] members) {
  ...
  private synchronized boolean waitingTooLong() {
    return this.exists() && this.getPCS().testAndSetWaitingTooLong() ? true : false;
  }
  ...
  private synchronized boolean waitingTooLong(int slot) {
    return (this.exists()
            && this.isCommandering(slot)
            && this.getPCS().testAndSetWaitingTooLong(slot))
        ? true
        : false;
  }
  ...
  private synchronized AcceptPacket reissueAcceptIfWaitingTooLong(int slot) {
    return (this.isActive() && this.waitingTooLong(slot))
        ? this.getPCS().reInitCommander(slot)
        : null;
  }

  protected static AcceptPacket reissueAcceptIfWaitingTooLong(PaxosCoordinator c, int slot) {
  ...
  protected void testingInitCoord(int load) {
    if (getPCS() != null) getPCS().testingInitCoord(load);
  }
  ...
  public static void main(String[] args) {
    System.out.println(
        "FAILURE: This is an untested shell class for PaxosCoordinatorState that is tested. "
