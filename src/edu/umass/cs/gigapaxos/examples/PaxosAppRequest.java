...
/**
 * @author arun
 *     <p>A class like this is needed only if the app wants to use request types other than
 *     RequestPacket, which is generally useful only if the app wants to coordinate only some
 *     request types using consensus but process other requests locally at replicas. For using just
 *     gigapaxos to implement linearizability, i.e., coordinating all requests via consensus, this
 *     class is unnecessary. Applications can simply encapsulate requests as {@link RequestPacket};
 *     doing so will also be slightly more efficient than using app-specific request types.
 */
public class PaxosAppRequest extends JSONPacket implements ReplicableRequest, ClientRequest {
   ...
   * @param stop
   */
  public PaxosAppRequest(String name, String value, IntegerPacketType type, boolean stop) {
    this(name, 0, (long) (Math.random() * Long.MAX_VALUE), value, type, stop);
  }
   ...
   * @param stop
   */
  public PaxosAppRequest(String name, long id, String value, IntegerPacketType type, boolean stop) {
  ...
  public PaxosAppRequest(JSONObject json) throws JSONException {
    ...
    this.response =
        json.has(Keys.RESPONSE_VALUE.toString())
            ? json.getString(Keys.REQUEST_VALUE.toString())
            : null;
  }
  ...
  public boolean needsCoordination() {
    return this.getRequestType().equals(PacketType.LOCAL_READ) ? false : true;
  }

  @Override
  public ClientRequest getResponse() {
    return new PaxosAppRequest(
        this.name,
        this.epoch,
        this.requestID,
        this.response == null ? Keys.ACK.toString() : this.response,
        PacketType.getPacketType(type),
        this.stop);
  }
  ...
  /**
   * Simple check to ensure that stringification and back yields the original request.
   *
   * @param args
   */
  public static void main(String[] args) {
    PaxosAppRequest request =
        new PaxosAppRequest(
            "name1", 0, 0, "request1", PaxosAppRequest.PacketType.COORDINATED_WRITE, false);
    System.out.println(request);
    try {
      PaxosAppRequest request2 = (new PaxosAppRequest(request.toJSONObject()));
