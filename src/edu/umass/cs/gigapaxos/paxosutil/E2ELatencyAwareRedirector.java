...
public class E2ELatencyAwareRedirector implements NearestServerSelector {
  ...
  /**
   * The fraction of requests that are sent to a random replica as probes in order to maintain an
   * up-to-date map of latencies to different replicas.
   */
  ...
  final LinkedHashMap<InetSocketAddress, Double> e2eLatencies =
      new LinkedHashMap<InetSocketAddress, Double>() {
  ...
  @SuppressWarnings("serial")
  final LinkedHashMap<InetSocketAddress, Long> lastProbed =
  ...
  public InetSocketAddress getNearest(Set<InetSocketAddress> addresses) {
    ...
    Collections.shuffle(shuffled = new ArrayList<InetSocketAddress>(addresses));
    for (InetSocketAddress address : shuffled) {
      ...
      if (closest == null) closest = address;
  ...
  public void learnSample(InetSocketAddress isa, double latency) {
    ...
    if ((historical = e2eLatencies.putIfAbsent(isa, latency)) != null)
      e2eLatencies.put(isa, Util.movingAverage(latency, historical, ALPHA));
  }
  ...
  public static final int prefixMatch(InetAddress addr1, InetAddress addr2) {
    ...
    byte xor = 0;
    for (i = 0; i < 4; i++)
      if ((xor = (byte) (baddr1[i] ^ baddr2[i])) == 0) match += 8;
      else break;
    int j = 0;
    if (xor != 0)
      for (j = 1; j < 8; j++)
  ...
  /**
   * @return Test class for E2ELatencyAwareRedirector @TODO Commented because of iOS refactoring.
   *     Will be moved to a test path.
   */
  /*
  ...
  */
  ...
  /**
   * @param args @TODO Commented because of iOS refactoring. Test will be moved to a new test path
   */
  /*
