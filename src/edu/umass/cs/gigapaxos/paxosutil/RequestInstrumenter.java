...
/**
 * @author V. Arun
 *     <p>This is a utility class to instrument packet sends and receives to track problems with
 *     paxos requests. This is useful only for single-node gigapaxos testing.
 */
...
public class RequestInstrumenter {
  ...
  public static void received(RequestPacket request, int sender, int receiver) {
    if (DEBUG)
      synchronized (RequestInstrumenter.class) {
        map.put(
            request.requestID,
            (map.containsKey(request.requestID) ? map.get(request.requestID) : "")
                + rcvformat(request.requestID, request, sender, receiver));
      }
  }

  public static void received(AcceptReplyPacket request, int sender, int receiver) {
    if (DEBUG)
      synchronized (RequestInstrumenter.class) {
  ...
  public static void sent(AcceptReplyPacket request, int sender, int receiver) {
    if (DEBUG)
      synchronized (RequestInstrumenter.class) {
        map.put(
            request.getRequestID(),
            (map.containsKey(request.getRequestID()) ? map.get(request.getRequestID()) : "")
  ...
  public static String getLog() {
    if (DEBUG)
      synchronized (RequestInstrumenter.class) {
        String s = "--{";
        for (long qid : map.keySet()) s += "\n" + qid + "=" + map.get(qid);
        return s + (map.isEmpty() ? "}" : "\n}--");
      }
    return "";
  }

  private static String rcvformat(long requestID, PaxosPacket packet, int sender, int receiver) {
    if (DEBUG)
      synchronized (RequestInstrumenter.class) {
        return packet.getType().toString()
            + " ("
            + sender
            + ")   ->"
            + receiver
  ...
  private static String sndformat(long requestID, PaxosPacket packet, int sender, int receiver) {
    if (DEBUG)
      synchronized (RequestInstrumenter.class) {
        return packet.getType().toString()
            + " "
