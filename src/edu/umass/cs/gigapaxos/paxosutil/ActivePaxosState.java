...
/**
 * @author V. Arun
 *     <p>This class maintains state for active paxos instances that is not needed by idle ones,
 *     e.g., for retransmissions or sync requests for missing decisions, so we store this separately
 *     in a smaller hashmap in PaxosManager and clean out long-idle objects periodically.
 *     <p>This class is now deprecated and will be removed. It used to be used by PaxosManager and
 *     PaxosInstanceStateMachine to keep track of the last active time and the last sync'd time.
 *     These were kept in a separate structure because there wasn't an efficient iterator over
...
public class ActivePaxosState {
  ...
  /**
   * @return True if last time sync'd was at least MIN_RESYNC_DELAY in the past.
   */
