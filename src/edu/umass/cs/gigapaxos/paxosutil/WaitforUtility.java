...
/**
 * @author V. Arun
 *     <p>A waitfor utility for integer node identifiers that uses roughly 8(N+1) bytes when waiting
 *     for responses from N nodes. Note that the final array members will not occupy space in each
 *     instance as it will simply be a pointer to the corresponding array in the paxos instance that
 *     created it.
 *     <p>There is a similar Waitfor structure in the utils package that is inspired by this one and
 *     is slightly cleaner and more manageable as it uses sets instead of arrays.
 */
...
public class WaitforUtility {
  ...
  public static void main(String[] args) {
    ...
    System.out.println(
        "Success. The following methods were tested: [heardFrom, heardFromMajority, contains,"
            + " getMembers, getIndex].");
  }
}
