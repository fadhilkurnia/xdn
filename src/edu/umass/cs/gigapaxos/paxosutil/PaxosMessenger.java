...
/**
 * @author V. Arun
 * @param <NodeIDType> This class is simply JSONMessenger adapted to use MessagingTask instead of
 *     GenericMessagingTask and to fix integer node IDs to NodeIDType as needed.
 *     <p>It's a utility because there is nothing paxos specific here.
 */
...
public class PaxosMessenger<NodeIDType> extends JSONMessenger<NodeIDType> {
  ...
  public PaxosMessenger(
      InterfaceNIOTransport<NodeIDType, JSONObject> niot, IntegerMap<NodeIDType> nodeMap) {
  ...
  public void send(MessagingTask[] mtasks) throws JSONException, IOException {
    ...
    for (MessagingTask mtask : mtasks) send(mtask);
  }
  ...
  private Object toJSONObject(PaxosPacket msg) throws JSONException {
    if (cacheStringifiedAccept()
        && msg.getType() == PaxosPacketType.ACCEPT
  ...
  public static final boolean INSTRUMENT_SERIALIZATION =
      ENABLE_INSTRUMENTATION && Config.getGlobalBoolean(PC.INSTRUMENT_SERIALIZATION);
  ...
  private Object toJSONSmartObject(PaxosPacket msg) throws JSONException {
    ...
    if (BYTEIFICATION
        ...
        || msg.getType() == PaxosPacketType.BATCHED_ACCEPT_REPLY) return msg;
    ...
    Object jsonified =
        jsonSmart == null ? fixNodeIntToString(msg.toJSONObject()) : fixNodeIntToString(jsonSmart);
    String stringified = jsonified.toString();
    ...
    if (Util.oneIn(100) && INSTRUMENT_SERIALIZATION && ENABLE_INSTRUMENTATION)
      if (msg.getType() == PaxosPacketType.ACCEPT)
        DelayProfiler.updateDelayNano(
            "acceptStringification", t, ((RequestPacket) msg).batchSize() + 1);
  ...
  private Object[] toObjects(PaxosPacket[] packets) throws JSONException {
    ...
    for (int i = 0; i < packets.length; i++) {
      ...
      assert (!cacheStringifiedAccept()
          ...
          || ((RequestPacket) packets[i]).getStringifiedSelf() != null
          || BYTEIFICATION);
    }
    return objects;
  }
  ...
  public JSONObject fixNodeIntToString(JSONObject json) throws JSONException {
    // long t = System.nanoTime();
    if (!ENABLE_INT_STRING_CONVERSION || IntegerMap.allInt()) return json;
  ...
  public net.minidev.json.JSONObject fixNodeIntToString(net.minidev.json.JSONObject jsonSmart) {
    // long t = System.nanoTime();
    if (!ENABLE_INT_STRING_CONVERSION || IntegerMap.allInt()) return jsonSmart;
    if (jsonSmart.containsKey(PaxosPacket.NodeIDKeys.B.toString())) {
      ...
      Collection<?> jsonArray =
          (Collection<?>) jsonSmart.get(PaxosPacket.NodeIDKeys.GROUP.toString());
