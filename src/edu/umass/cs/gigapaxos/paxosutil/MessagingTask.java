...
/**
 * @author V. Arun
 *     <p>This messaging task means that each msg needs to be sent to all recipients. Note: it does
 *     *NOT* mean that each msg needs to be sent to its corresponding recipient; in fact, the sizes
 *     of the two arrays, recipients and msgs, may be different. This utility class is really just a
 *     container for two arrays.
 */
public class MessagingTask {
  ...
  public MessagingTask(int[] destIDs, PaxosPacket pkt) {
    assert (pkt != null && destIDs != null)
        : "Incorrect usage: MessagingTask should not be instantiated with null messages or"
              + " destinations";
  ...
  public MessagingTask(int destID, PaxosPacket[] pkts) {
    assert (pkts != null && pkts.length > 0 && pkts[0] != null)
        : "Incorrect usage: MessagingTask should not be instantiated with no messages";
  ...
  /**
   * Multicast multiple packets. This is a special type of constructor compared to the others above.
   * This is the only constructor that allows MessagingTask to be instantiated with no messages. We
   * need this as it is convenient to create an empty MessagingTask in a logging-only
  ...
  public MessagingTask(int[] destIDs, PaxosPacket[] pkts) {
    assert (pkts != null && destIDs != null)
        : "Incorrect usage: MessagingTask should not be instantiated with null messages or"
              + " destinations";
    for (Object obj : pkts)
      assert (obj != null)
          : "Incorrect usage: MessagingTask should not be instantiated with null messages";
  ...
  /**
   * @return True if {@link #isEmptyMessaging()} returns true. This method exists so that it can be
   *     overridden unlike {@link #isEmptyMessaging()}.
   */
  ...
  /**
   * @return Whether the list of recipients or messages is null or empty, i.e., there is nothing to
   *     do.
   */
   ...
   *
   * <p>Converts an object array to a PaxosPacket array because there is annoyingly no easy way in
   * Java to convert a collection of a child type to an array of a parent type.
   *
   * @param ppChildArray
  ...
  public static PaxosPacket[] toPaxosPacketArray(Object[] ppChildArray) {
    ...
    for (int i = 0; i < ppChildArray.length; i++)
      assert (ppChildArray[i] != null)
          : "Incorrect usage: MessagingTask should "
    ...
    for (int i = 0; i < ppChildArray.length; i++) {
      assert (ppChildArray[i] instanceof PaxosPacket)
          : "Incorrect usage: MessagingTask can only take PaxosPacket objects";
  ...
  /**
   * @return Singleton array containing {@code this}.
   */
  ...
  public static MessagingTask getLoopback(MessagingTask mtask, int myID) {
    if (mtask == null || mtask.recipients == null || mtask.recipients.length == 0) return null;
    for (int i : mtask.recipients) if (i == myID) return new MessagingTask(myID, mtask.msgs);
    return null;
  }
  ...
  public static MessagingTask getNonLoopback(MessagingTask mtask, int myID) {
    ...
    if (arrayContains(mtask.recipients, myID)) {
      if (mtask.recipients.length > 1)
        return new MessagingTask(Util.filter(mtask.recipients, myID), mtask.msgs);
      else return null;
    }
    return mtask;
  }
  ...
  private static boolean arrayContains(int[] array, int member) {
    for (int a : array) if (a == member) return true;
    return false;
  }
   ...
   * @param array1
   * @param array2
   * @return Combined array with the two input arrays.
   */
  public static MessagingTask[] combine(MessagingTask[] array1, MessagingTask[] array2) {
    int length1 = (array1 != null ? array1.length : 0);
    int length2 = (array2 != null ? array2.length : 0);
    MessagingTask[] mtasks = new MessagingTask[length1 + length2];
    for (int i = 0; i < length1; i++) mtasks[i] = array1[i];
    for (int i = 0; i < length2; i++) mtasks[length1 + i] = array2[i];
    return mtasks;
  }
   ...
   */
  public static MessagingTask[] combine(MessagingTask mtask, MessagingTask[] array) {
    if (mtask == null && array == null) return null;
    else if (mtask != null) return combine(mtask.toArray(), array);
    else return array;
  }
}
