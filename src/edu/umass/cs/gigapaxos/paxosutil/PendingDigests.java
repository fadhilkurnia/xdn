...
public class PendingDigests {
  ...
  /** */
  public abstract static class PendingDigestCallback {
    /**
     * @param accept
     */
    public abstract void callback(AcceptPacket accept);
  }
   ...
   * @param callback
   */
  public PendingDigests(
      Map<Long, RequestAndCallback> rcs, int numMDs, PendingDigestCallback callback) {
    this.requests = rcs;
    // this.mds = new MessageDigest[numMDs];
    this.callback = callback;
    this.accepts =
        (ACCEPT_TIMEOUT == Integer.MAX_VALUE
            ? new ConcurrentHashMap<Long, AcceptPacket>()
            : new GCConcurrentHashMap<Long, AcceptPacket>(
                ...
                ACCEPT_TIMEOUT));
    try {
      for (int i = 0; i < mds.length; i++) mds[i] = MessageDigest.getInstance("MD5");
    } catch (NoSuchAlgorithmException e) {
      Util.suicide("Unable to initialize MessageDigest; exiting");
    }
  }
  ...
  public AcceptPacket match(AcceptPacket accept) {
    ...
    synchronized (this.requests) {
      if ((rc = requests.get(accept.requestID)) == null) this.accepts.put(accept.requestID, accept);
    }

    if (rc != null && rc.getRequestPacket().getPaxosID().equals(accept.getPaxosID())) {
      if (rc.getRequestPacket().digestEquals(accept, mds[(int) (Math.random() * mds.length)])) {
        accept = accept.undigest(rc.getRequestPacket());
        assert (accept.hasRequestValue());
        return accept;
      } else logAnomaly(rc.getRequestPacket(), accept);
    }
  ...
  public AcceptPacket release(RequestPacket request, boolean remove, boolean assertion) {
    ...
    assert (request != null
        && request.getPaxosID() != null
        && (accept == null || accept.getPaxosID() != null));

    if (accept != null && accept.getPaxosID().equals(request.getPaxosID())) {
      if (request.digestEquals(accept, mds[(int) (Math.random() * mds.length)])) {
        ...
        return accept;
      } else this.logAnomaly(request, accept);
    }
    return null;
  }
