...
/**
 * @author arun
 * @param <TaskType> A utility for a standard "consumer" task in a producer/consumer setup.
 */
public abstract class ConsumerTask<TaskType> implements Runnable {
  ...
  private TaskType dequeue() {
    ...
    long millis =
        sleepDuration >= 1 ? (long) sleepDuration : (Math.random() < sleepDuration ? 1 : 0);
    if (millis > 0)
      try {
        Thread.sleep(millis);
  ...
  protected boolean isEmpty() {
    synchronized (lock) {
      if (this.lock instanceof Collection) return ((Collection<?>) this.lock).isEmpty();
      else if (this.lock instanceof Map) return ((Map<?, ?>) this.lock).isEmpty();
      else throw new RuntimeException("ConsumerTask lock must be of type Collection or Map");
    }
  }
  ...
  public void waitToFinish() {
    synchronized (this.lock) {
      while (!this.isEmpty() || this.getProcessing()) this.lockWait();
      this.stop();
    }
  }
  ...
  protected void waitForNotEmptyOrStopped() {
    synchronized (this.lock) {
      while (this.isEmpty() && !isStopped()) this.lockWait();
    }
