...
/**
 * @author arun
 *     <p>A file for just static SQL constants.
 */
public class SQL {
  /**
   * The user name and password here are used by both gigapaxos and reconfiguration. Embedded derby
   * can take any user name and password, so the values here should be for mysql or other DBs
   * supported here.
   *
   * <p>These values are intentionally not configurable using gigapaxos.properties. We might support
   * them using System command-line properties in the future.
   */
  ...
  /** */
  public static final List<String> DUPLICATE_KEY = new ArrayList<String>(Arrays.asList("23505"));

  /** */
  public static final List<String> DUPLICATE_TABLE =
      new ArrayList<String>(Arrays.asList("X0Y32", "42S01", "42000", "42S11"));

  /** */
  public static final List<String> NONEXISTENT_TABLE =
      new ArrayList<String>(Arrays.asList("42Y07", "42Y55", "42X05", "42S02"));
  ...
  public static String getBlobString(int size, SQLType type) {
    switch (type) {
      ...
      case EMBEDDED_H2:
        return " blob(" + size + ")";
      case MYSQL:
        if (size < 65536) return " TEXT ";
        else if (size < (int) Math.pow(2, 24)) return " MEDIUMTEXT ";
        else if (size < (int) Math.pow(2, 32)) return " LONGTEXT ";
    }
    return "";
  }
