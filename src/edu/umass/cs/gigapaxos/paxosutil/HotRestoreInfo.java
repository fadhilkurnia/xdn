...
/**
 * @author V. Arun
 *     <p>A simple utility class to serialize and deserialize paxos state for pause and hot restore.
 */
...
public class HotRestoreInfo {
  ...
  /**
   * Paxos group epoch number. A paxosID:version two-tuple corresponds to a unique replicated state
   * machine.
   */
  ...
  /**
   * An array at the coordinator indicating which nodes have executed or checkpointed up which slot
   * numbers in order to do garbage collection.
   */
  public final int[] nodeSlots;

  public HotRestoreInfo(
      String paxosID,
      int version,
      int[] members,
      int accSlot,
  ...
  public String toString() {
    return paxosID
        ...
        + nextProposalSlot
        + SEP
        + (nodeSlots != null ? Util.arrayOfIntToString(nodeSlots) : "null");
  }

  public boolean isCreateHRI() {
    /**
     * Revert to Original coz FIX below is bad. We do need accSlot=1 as that is the next expected
     * slot after inserting the initial checkpoint (slot 0). With createHRI,
     * PaxosInstanceStateMachine only updates the app state using restore, but does not checkpoint
     * the state as that would have already been done as part of a batch earlier.
     */
  ...
  public static HotRestoreInfo createHRI(String paxosID, int[] members, int coordinator) {
    assert (Util.contains(coordinator, members));
    // revert to Original (see explanation above)
    return new HotRestoreInfo(
        paxosID,
        0,
        members,
        1,
  ...
  public static class HotRestoreInfoTest extends DefaultTest {
    ...
    public void testToStringAndBack() {
      ...
      HotRestoreInfo hri1 =
          new HotRestoreInfo(
              "paxos0", 2, members, 5, new Ballot(3, 4), 3, new Ballot(45, 67), 34, nodeSlots);
