...
/**
 * @author V. Arun
 *     <p>This is a faster demultiplexer than PaxosPacketDemultiplexerJSON. JSON turns out to be the
 *     bottleneck. This class supports json-smart and direct serialization to and from byte[]. The
 *     last option is the fastest. We need faster options only for RequestPacket and AcceptPacket;
 *     for everything else JSON is just fine.
 *     <p>Byteification is a bit harder to maintain, especially the processHeader part, but is worth
 *     it for this critical demultiplexer.
 */
public abstract class PaxosPacketDemultiplexerFast extends AbstractPacketDemultiplexer<Object> {
  /**
   * @param numThreads
   */
  ...
  private static PaxosPacket toPaxosPacket(byte[] bytes)
    ...
    PaxosPacket.PaxosPacketType type =
        bbuf.getInt() == PaxosPacketType.PAXOS_PACKET.getInt()
            ? PaxosPacketType.getPaxosPacketType(bbuf.getInt())
            : null;

    if (type == null) fatal(bytes);
   ...
   */
  public static PaxosPacket toPaxosPacket(
      net.minidev.json.JSONObject jsonS, Stringifiable<?> unstringer) throws JSONException {
    assert (jsonS != null);
    assert (jsonS.get(PaxosPacket.Keys.PT.toString()) != null) : jsonS;
    PaxosPacket.PaxosPacketType type =
        PaxosPacket.PaxosPacketType.getPaxosPacketType(
            (Integer) jsonS.get(PaxosPacket.Keys.PT.toString()));
    if (type == null) fatal(jsonS);
    ...
    switch (type) {
      ...
      default:
        return PaxosPacketDemultiplexer.toPaxosPacket(toJSONObject(jsonS), unstringer);
    }
   ...
   */
  public static RequestPacket getRequestPacket(byte[] message, NIOHeader header) {
    Object packet = processHeaderUtil(message, header);
    if (packet != null && packet instanceof RequestPacket) return (RequestPacket) packet;
    if (JSONPacket.couldBeJSON(message))
      try {
        packet =
            PaxosPacketDemultiplexer.toPaxosPacket(
                new JSONObject(MessageExtractor.decode(message)));
        if (packet != null && packet instanceof RequestPacket) return (RequestPacket) packet;
   ...
   * @throws JSONException
   */
  public static JSONObject toJSONObject(net.minidev.json.JSONObject jsonS) throws JSONException {
    JSONObject json = new JSONObject();
    for (String key : jsonS.keySet()) {
      Object value = jsonS.get(key);
      if (value instanceof Collection<?>) json.put(key, new JSONArray((Collection<?>) value));
  ...
  private static void fatal(Object json) {
    PaxosConfig.getLogger()
        .severe(PaxosPacketDemultiplexerFast.class.getSimpleName() + " received " + json);
    throw new RuntimeException("PaxosPacketDemultiplexer recieved unrecognized paxos packet type");
  }
  ...
  protected Integer getPacketType(Object message) {
    if (message instanceof net.minidev.json.JSONObject)
      return (Integer)
          ((net.minidev.json.JSONObject) message).get(JSONPacket.PACKET_TYPE.toString());
  ...
  private static boolean isByteable(byte[] bytes) {
    ByteBuffer bbuf;
    int type = -1;
    if ((bbuf = ByteBuffer.wrap(bytes, 0, 8)).getInt()
            == PaxosPacket.PaxosPacketType.PAXOS_PACKET.getInt()
        && ((type = bbuf.getInt()) == PaxosPacket.PaxosPacketType.REQUEST.getInt()
            || (type == PaxosPacket.PaxosPacketType.ACCEPT.getInt())
            || type == PaxosPacketType.BATCHED_COMMIT.getInt()
            || type == PaxosPacketType.BATCHED_ACCEPT_REPLY.getInt())) return true;
   ...
   * @param bytes
   * @param header
   * @return A static utility method to convert bytes to RequestPacket with header processing.
   */
  public static final Object processHeaderUtil(byte[] bytes, NIOHeader header) {
    if (isByteable(bytes)) {
      ...
      if (PaxosPacket.getType(bytes) == PaxosPacketType.REQUEST) {
        ...
        try {
          if (noCA) bbuf.put(caddress).putShort(cport);
          else bbuf.position(bbuf.position() + 6);
          if (noLA) bbuf.put(laddress).putShort(lport);
          else bbuf.position(bbuf.position() + 6);
    ...
    net.minidev.json.JSONObject retval =
        MessageExtractor.stampAddressIntoJSONObject(
            header.sndr, header.rcvr, insertStringifiedSelf(json, message));
    ...
    try {
      if (PaxosMessenger.INSTRUMENT_SERIALIZATION && Util.oneIn(100))
        ...
        else if (PaxosPacket.getPaxosPacketType(retval)
            == PaxosPacket.PaxosPacketType.BATCHED_ACCEPT_REPLY)
  ...
  public boolean isOrderPreserving(Object msg) {
    ...
    if (msg instanceof PaxosPacket) return ((PaxosPacket) msg).getType() == PaxosPacketType.REQUEST;
    ...
    if (msg instanceof byte[]) {
      ...
      if (bbuf.getInt() == PaxosPacket.PaxosPacketType.PAXOS_PACKET.getInt()) {
        type = bbuf.getInt();
        if (type == PaxosPacket.PaxosPacketType.REQUEST.getInt()) return true;
      }
    }
    // else
