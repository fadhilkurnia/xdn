...
/**
 * @author V. Arun
 * @param <NodeIDType> This class maintains a map of NodeIDType objects to integers, and provides
 *     some convenience methods to go back and forth between collections of the two kinds. It is
 *     synchronized because PaxosManager and Messenger (used by the BatchLogger thread in
...
public class IntegerMap<NodeIDType> {
  ...
  public int put(NodeIDType node) {
    ...
    if (!node.toString().equals(Integer.valueOf(id).toString())) allInteger = false;

    boolean collision = this.nodeMap.containsKey(id);
    if (collision && (node instanceof String) && ((String) node).equals("" + id)) return id;
    ...
    if (collision)
      while (this.nodeMap.containsKey(id)
        ...
        log.warning("Hash collision: " + node + " != " + this.nodeMap.get(id));
        id++;
      }
  ...
  /**
   * @return True if no non-integer node ID was ever encountered. If so, we can be slightly more
   *     efficient in messaging by avoiding node to int conversion and back.
   */
  ...
  private static String message =
      ": Unable to translate integer ID "
