...
public class LogIndex implements Keyable<String>, Serializable, Pausable {
  ...
  public void GC() {
    ...
    for (Iterator<LogIndexEntry> entryIter = this.log.iterator(); entryIter.hasNext(); ) {
      LogIndexEntry entry = entryIter.next();
      if (entry.slot - this.gcSlot <= 0) entryIter.remove();
    }
    // the only time after initialization when minLogfile is updated
    if (!this.log.isEmpty()) this.minLogfile = this.log.get(0).logfile;
   ...
   */
  public boolean add(int s, int bnum, int bcoord, int type, String file, long offset, int length) {
    return this.add(new LogIndexEntry(s, bnum, bcoord, type, file, offset, length));
  }
  ...
  public boolean modify(LogIndexEntry cur) {
    for (LogIndexEntry entry : this.log) {
      if (entry.slot == cur.slot
          && entry.ballotNum == cur.ballotNum
  ...
  public boolean add(LogIndexEntry entry) {
    if (gcSlot == null || entry.slot - gcSlot <= 0) return false;
    if (this.minLogfile == null) this.minLogfile = entry.logfile;
    if (this.log == null) this.log = new ArrayList<LogIndexEntry>();
  ...
  private ArrayList<LogIndexEntry> getLoggedMessages(int minSlot, Integer maxSlot, int type) {
      ...
      synchronized (this.log) {
        for (LogIndexEntry entry : this.log)
          if ((type == -1 || entry.type == type)
              ...
              && ((maxSlot == null) || (entry.slot - maxSlot <= 0))) messages.add(entry);
      }
    return messages;
  }
   ...
   * @param minSlot
   * @param maxSlot
   * @return List of accepts in the range {@code [minSlot, maxSlot]}, both inclusive.
   */
  public ArrayList<LogIndexEntry> getLoggedAccepts(int minSlot, Integer maxSlot) {
    return this.getLoggedMessages(minSlot, maxSlot, PaxosPacketType.ACCEPT.getInt());
  }

  /**
   * @param minSlot
   * @param maxSlot
   * @return List of decisions in the range {@code [minSlot, maxSlot]}, both inclusive.
   */
  public ArrayList<LogIndexEntry> getLoggedDecisions(int minSlot, int maxSlot) {
    return this.getLoggedMessages(minSlot, maxSlot, PaxosPacketType.DECISION.getInt());
  }
  ...
  public Set<String> getLogfiles() {
    ...
    if (this.log != null) for (LogIndexEntry entry : this.log) logfiles.add(entry.logfile);
    return logfiles;
  }

  /**
   * The ordering of fields below is important for correctness as we are not using keys for fields
   * here.
   *
  ...
  /**
   * @return Oldest log file containing an entry for this paxosID after the last commit to disk.
   */
   ...
   */
  public boolean isLogMsgNeeded(int slot, int ballotNum, int ballotCoord, int type) {
    if (slot - this.gcSlot <= 0) {
      ...
      for (LogIndexEntry entry : this.log) {
        if (entry.ballotNum - ballotNum > 0
            || (entry.ballotNum == ballotNum && entry.ballotCoord - ballotCoord > 0)) return false;
      }
    }
    return true;
  }
  ...
  public Object getSummary(boolean makeObject) {
    if (!makeObject) return null;
    return new Object() {
      public String toString() {
        String s = "";
        s +=
            LogIndex.this.paxosID
                + ":"
                + LogIndex.this.version
                + ":"
                + LogIndex.this.minLogfile
                + ":"
                + LogIndex.this.gcSlot;
        if (LogIndex.this.log != null && !LogIndex.this.log.isEmpty()) {
          s += ":[";
          for (LogIndexEntry entry : LogIndex.this.log) {
            s +=
                (PaxosPacketType.getPaxosPacketType(entry.type).toString().substring(0, 1)
                    + entry.slot);
          }
          s += "]";
        }
  ...
  private static final long DEACTIVATION_PERIOD = Config.getGlobalLong(PC.DEACTIVATION_PERIOD);
  ...
  public static class LogIndexTest extends DefaultTest {
    ...
    public void testRestore() throws JSONException, IOException {
      ...
      System.out.println(
          new String(
              SQLPaxosLogger.inflate(
                  SQLPaxosLogger.deflate(logIndex.toString().getBytes("ISO-8859-1"))),
              "ISO-8859-1"));
    }
  }
}
