...
/**
 * @author V. Arun
 *     <p>Used to get NIO to send paxos packets to PaxosManager. This class has been merged into
 *     PaxosManager now and will be soon deprecated.
 */
public abstract class PaxosPacketDemultiplexer extends AbstractPacketDemultiplexer<JSONObject> {
   ...
   */
  public static PaxosPacket toPaxosPacket(JSONObject json, Stringifiable<?> unstringer)
      throws JSONException {
    PaxosPacket.PaxosPacketType type = PaxosPacket.getPaxosPacketType(json);
    if (type == null) fatal(json);
    ...
    switch (type) {
      ...
      case ACCEPT:
        ...
        break;
      case BATCHED_ACCEPT:
        paxosPacket = (new BatchedAccept(json));
        break;
      case BATCHED_PAXOS_PACKET:
        paxosPacket = (new BatchedPaxosPacket(json));
        break;
      case ACCEPT_REPLY:
      ...
      case BATCHED_ACCEPT_REPLY:
        ...
        if (PaxosMessenger.INSTRUMENT_SERIALIZATION && Util.oneIn(100))
          DelayProfiler.updateDelayNano(
              "batchedAcceptReplyPacketization", t, ((BatchedAcceptReply) paxosPacket).size());
        break;
  ...
  public static void throttleExcessiveLoad() {
    try {
      Thread.sleep(
          THROTTLE_SLEEP >= 1 ? (long) THROTTLE_SLEEP : (Math.random() < THROTTLE_SLEEP ? 1 : 0));
  ...
  private static void fatal(Object json) {
    PaxosConfig.getLogger()
        .severe(PaxosPacketDemultiplexer.class.getSimpleName() + " received " + json);
