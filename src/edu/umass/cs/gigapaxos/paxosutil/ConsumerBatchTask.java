...
/**
 * @author arun
 * @param <TaskType> A utility task to "consume" batched requests.
 */
public abstract class ConsumerBatchTask<TaskType> extends ConsumerTask<TaskType> {
   ...
   * @param peek
   */
  public ConsumerBatchTask(Collection<TaskType> lock, TaskType[] dummy, boolean peek) {
    super(lock);
   ...
   * @param peek
   */
  public ConsumerBatchTask(Map<?, TaskType> lock, TaskType[] dummy, boolean peek) {
    super(lock);
    this.dummy = Arrays.copyOf(dummy, 0);
    this.peek = peek;
  }
  ...
  protected void remove(TaskType[] tasks) {
    synchronized (lock) {
      for (TaskType task : tasks) {
        if (this.lock instanceof Collection) {
          if (((Collection<?>) this.lock).remove(task)) this.lockNotify();
          ;
        } else if (this.lock instanceof Map) {
          if (((Map<?, TaskType>) this.lock).values().remove(task)) this.lockNotify();
          ;
        } else
          throw new RuntimeException("ConsumerBatchTask lock must be of type Collection<TaskType");
      }
  ...
  public void enqueueAndWait(TaskType task) {
    ...
    synchronized (this.lock) {
      while (this.contains(task)) this.lockWait();
    }
  }
