...
/**
 * @author V. Arun
 *     <p>This abstract class exists to make the logger pluggable. An example is a quick and dirty,
 *     fake, memory-based logger DummyPaxosLogger that was implemented long back but has been
 *     deprecated now. Use DerbyPaxosLogger that extends this class for a more scalable, efficient,
...
public abstract class AbstractPaxosLogger {
  ...
  private static Logger log = Logger.getLogger(AbstractPaxosLogger.class.getName());
  ...
  protected AbstractPaxosLogger(int id, String logDir, PaxosMessenger<?> msgr) {
    ...
    (this.batchLogger =
            new BatchedLogger(logQueue = new LinkedList<LogMessagingTask>(), this, this.messenger))
    ...
    (this.collapsingCheckpointer = new Checkpointer(new LinkedHashMap<String, CheckpointTask>()))
        .start(AbstractPaxosLogger.class.getSimpleName() + myID);
    addLogger(this);
  }

  protected abstract static class PaxosPacketizer {
    protected abstract PaxosPacket stringToPaxosPacket(String str) throws JSONException;

    protected abstract PaxosPacket stringToPaxosPacket(byte[] bytes);
  }

  protected abstract static class PaxosPacketStringifier {
    protected abstract String paxosPacketToString(PaxosPacket paxosPacket);
  }
   ...
   * @param logMTask The logAndMessage task.
   * @param messenger The messenger that will send the message after logging.
   * @throws JSONException
   * @throws IOException
   */
  protected static final void logAndMessage(
      AbstractPaxosLogger logger, LogMessagingTask logMTask // , Messenger<?> messenger
  ...
  protected static final void logDecision(AbstractPaxosLogger logger, PValuePacket decision) {
    if (logger.isAboutToClose()) return;
  ...
  private static final boolean BLOCKING_CHECKPOINT =
      Config.getGlobalBoolean(PC.BLOCKING_CHECKPOINT);

  /*
   */
  protected static final String checkpoint(
      AbstractPaxosLogger logger,
      boolean sync,
      String paxosID,
      int version,
      Set<String> members,
      ...
      String state,
      int gcSlot) {
    if (logger.isAboutToClose()) return null;
    ...
    if (BATCH_CHECKPOINTS && !sync) {
      ...
      if (BLOCKING_CHECKPOINT) { // redundant check
        logger.collapsingCheckpointer.enqueueAndWait(
            logger
            .new CheckpointTask(logger, paxosID, version, members, slot, ballot, state, gcSlot));
      } else
        // currently not used
        logger.collapsingCheckpointer.enqueue(
            logger
            .new CheckpointTask(logger, paxosID, version, members, slot, ballot, state, gcSlot));
    } else logger.putCheckpointState(paxosID, version, members, slot, ballot, state, gcSlot);
    return state;
  }
  ...
  protected static final void rollForward(
      AbstractPaxosLogger logger, String paxosID, PaxosMessenger<?> messenger) {
    if (logger.isAboutToClose()) return;
    ...
    for (PaxosPacket paxosMsg : loggedMessages) {
      ...
      try {
        log.log(Level.FINE, "{0}{1}{2}", new Object[] {logger.myID, " rolling forward ", mtask});
        messenger.send(mtask);
      } catch (IOException e) {
        log.severe("IOException encountered while replaying logged message " + paxosMsg);
        e.printStackTrace();
  ...
  protected static final boolean kill(AbstractPaxosLogger logger, String paxosID, int version) {
   ...
   * @param strID
   */
  public static final synchronized void fileLock(Object strID) {
    ...
    try {
      ...
      if (created && PaxosConfig.getPropertiesFile() != null) {
        ...
        raf.write(UtilServer.readFileAsString(PaxosConfig.getPropertiesFile()).getBytes());
      }
  ...
  /**
   * @param paxosID
   * @return Ballot corresponding to the latest (executed request in) checkpoint.
   */
  public abstract Ballot getCheckpointBallot(String paxosID);

  /**
   * @param paxosID
   * @return Slot corresponding to the most recent executed request in the checkpoint.
   */
   ...
   */
  public abstract SlotBallotState getSlotBallotState(
      String paxosID, int version); // useful to enforce version check upon recovery
   ...
   * @param gcSlot
   */
  public abstract void putCheckpointState(
      String paxosID,
      int version,
      Set<String> group,
      int slot,
   ...
   */
  public abstract boolean putCheckpointState(CheckpointTask[] tasks, boolean update);
  ...
  /**
   * Copies the final state at the end of an epoch (or version) into a separate place. Useful for
   * reconfiguring gigapaxos groups.
   *
   * @param paxosID
   * @param version
   * @return True if copied successfully.
   */
  public abstract boolean copyEpochFinalCheckpointState(String paxosID, int version);
   ...
   */
  public abstract StringContainer getEpochFinalCheckpointState(String paxosID, int version);
   ...
   * @param paxosID
   * @param version
   * @return Returns true if the final epoch state exists and is successfully deleted.
   */
  public abstract boolean deleteEpochFinalCheckpointState(String paxosID, int version);
   ...
   * Cursor-based checkpoint reading is useful under recovery when a very large number of
   * checkpoints may have to be read and restored sequentially.
   *
   * @param b
   * @return Returns true if the cursor to start reading checkpoints is successfully created.
   */
  ...
  /**
   * Reads the next checkpoint after the cursor has been created by {@link
   * #initiateReadCheckpoints(boolean)}.
   *
   * @param b
  ...
  /**
   * @return Starts a cursor for sequentially reading all log messages across all paxos groups from
   *     the database in order to replay them. This one-pass recovery is much faster than replaying
   *     log messages for each paxos group one at a time.
   */
  public abstract boolean initiateReadMessages(); // starts a cursor

  /**
   * Reads the next log message after the cursor has been initialized by {@link
   * #initiateReadMessages()}.
   *
  ...
  /** Closes all connections to the database. The database can not be used beyond this point. */
   ...
   * @param paxosID
   * @param version TODO
   * @return Returns true if state was succcessfully removed (or didn't exist anyway).
   */
  ...
  /**
   * Gets a list of all logged messages for the paxos group {@code paxosID}. Used by rollForward. We
   * don't need to bother about the version here because non-version-compliant logged messages if
   * any will be rejected by the paxos instance anyway.
   *
   * @param paxosID
   ...
   */
  public abstract Map<Integer, PValuePacket> getLoggedAccepts(
   ...
   * @throws JSONException
   */
  public abstract ArrayList<PValuePacket> getLoggedDecisions(
      String paxosID, int version, int minSlot, int maxSlot) throws JSONException;
  ...
  protected Map<Integer, PValuePacket> getLoggedAccepts(
   ...
   * @return True if all successfully created.
   */
  public boolean insertInitialCheckpoints(
      Map<String, String> nameStates, Set<String> gms, int[] members) {
    ...
    for (String name : nameStates.keySet()) {
      tasks[i++] =
          new CheckpointTask(
              ...
              gms,
              0,
              new Ballot(0, PaxosInstanceStateMachine.roundRobinCoordinator(name, members, 0)),
              nameStates.get(name),
              0);
    }
  ...
  private class BatchedLogger extends ConsumerBatchTask<LogMessagingTask> {
    ...
    BatchedLogger(
        LinkedList<LogMessagingTask> lock,
        AbstractPaxosLogger logger,
    ...
    public LogMessagingTask dequeueImpl() {
      throw new RuntimeException(
          this.getClass().getName() + ".dequeueImpl() should not have been called");
    }

    @Override
    public void process(LogMessagingTask task) {
      throw new RuntimeException(
          this.getClass().getName() + ".process() should not have been called");
    }
    ...
    public void process(LogMessagingTask[] lmTasks) {
      ...
      for (int i = 0; i < lmTasks.length; i++) packets[i] = lmTasks[i].logMsg;
      ...
      if (!DISABLE_LOGGING && lmTasks.length > 0)
        if (Util.oneIn(10)) DelayProfiler.updateDelay("log", t); // , packets.length);
      ...
      // then message if successfully logged
      {
        try {
          ...
          log.severe("Logged message but could not send response: " + lmTasks);
          e.printStackTrace();
        }
      }
    }
  ...
  static class Main {
    public static void main(String[] args) {
      System.out.println(
          "FAILURE: This class can not be unit-tested, but can be tested using DerbyPaxosLogger");
    }
  }
}
