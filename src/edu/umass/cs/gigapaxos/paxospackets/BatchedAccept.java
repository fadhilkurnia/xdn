...
public class BatchedAccept extends PaxosPacket {
  ...
  public BatchedAccept(JSONObject json) throws JSONException {
    ...
    for (int i = 0; i < jsonArray.length(); i++)
      try {
        ...
        this.slotDigests.put(
            jsonArray.getJSONArray(i).getInt(0),
            jsonArray.getJSONArray(i).getString(1).getBytes(CHARSET));
    ...
    for (int i = 0; i < jsonArray.length(); i++)
      this.slotRequestIDs.put(
          jsonArray.getJSONArray(i).getInt(0), jsonArray.getJSONArray(i).getLong(1));
  ...
  public boolean addBatchedAccept(BatchedAccept bAccept) {
    if (!bAccept.ballot.equals(this.ballot) || !bAccept.paxosID.equals(this.paxosID))
      throw new RuntimeException(
          "Unable to combine " + bAccept.getSummary() + " with " + this.getSummary());
  ...
  public void addAccept(AcceptPacket accept) {
    if (!accept.ballot.equals(this.ballot) || !accept.paxosID.equals(this.paxosID))
      throw new RuntimeException(
          "Unable to combine " + accept.getSummary() + " with " + this.getSummary());
