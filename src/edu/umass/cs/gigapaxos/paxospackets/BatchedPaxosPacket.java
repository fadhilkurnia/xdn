...
/**
 * @author arun
 *     <p>A general-purpose batched paxos packet. This is useful to extract batching benefits even
 *     with many groups just by batching together paxos packets destined to the same set of
 *     destinations. We might as well try to extract blood out of stone.
 */
public class BatchedPaxosPacket extends PaxosPacket {
  ...
  public BatchedPaxosPacket(PaxosPacket[] pkts) {
    ...
    assert (pkts != null);
    this.packetType = PaxosPacketType.BATCHED_PAXOS_PACKET;
    for (PaxosPacket pkt : pkts) packets.add(pkt);
  }
  ...
  public BatchedPaxosPacket(JSONObject json) throws JSONException {
    ...
    for (int i = 0; i < jarray.length(); i++)
      this.packets.add(PaxosPacketDemultiplexer.toPaxosPacket(jarray.getJSONObject(i), null));
  }
   ...
   * @throws JSONException
   */
  public BatchedPaxosPacket(net.minidev.json.JSONObject json) throws JSONException {
    super(json);
    this.packetType = PaxosPacket.getPaxosPacketType(json);
    net.minidev.json.JSONArray jarray =
        (net.minidev.json.JSONArray) json.get(PaxosPacket.Keys.PP.toString());
    for (int i = 0; i < jarray.size(); i++) {
      assert (jarray.get(i) != null) : jarray + " in " + json;
      this.packets.add(
          PaxosPacketDemultiplexerFast.toPaxosPacket(
  ...
  protected JSONObject toJSONObjectImpl() throws JSONException {
    ...
    for (PaxosPacket pp : this.packets) jarray.put(pp.toJSONObject());
  ...
  protected net.minidev.json.JSONObject toJSONSmartImpl() throws JSONException {
    ...
    for (PaxosPacket pp : this.packets) jarray.add(pp.toJSONSmart());
    json.put(PaxosPacket.Keys.PP.toString(), jarray);

    assert (!this.packets.isEmpty());
    assert (((net.minidev.json.JSONArray) json.get(PaxosPacket.Keys.PP.toString())).get(0) != null);
  ...
  public BatchedPaxosPacket append(PaxosPacket[] pps) {
    for (PaxosPacket pp : pps) this.packets.add(pp);
    return this;
  }
