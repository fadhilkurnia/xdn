...
public class BatchedAcceptReply extends AcceptReplyPacket implements Byteable {
  ...
  public JSONObject toJSONObjectImpl() throws JSONException {
    ...
    for (Integer slot : this.slots.keySet()) jarray.put(slot).put(this.slots.get(slot));
  ...
  public BatchedAcceptReply(ByteBuffer bbuf)
    ...
    for (int i = 0; i < numSlots; i++) this.slots.put(bbuf.getInt(), bbuf.getLong());
  ...
  public byte[] toBytes() {
    ...
    try {
      ...
      assert (bbuf.position()
          == SIZEOF_PAXOSPACKET_FIXED + paxosIDLength + SIZEOF_BATCHEDACCEPTREPLY);

      // self
      bbuf.putInt(this.slots.size());
      for (Integer i : slots.keySet()) bbuf.putInt(i).putLong(slots.get(i));
      ...
      if (PaxosMessenger.INSTRUMENT_SERIALIZATION && Util.oneIn(Integer.MAX_VALUE))
        DelayProfiler.updateDelayNano("batchedAcceptReplyByteification", t, this.slots.size());
      return bbuf.array();
  ...
  public boolean addAcceptReply(AcceptReplyPacket ar) {
    if (!ar.ballot.equals(this.ballot) || !this.paxosID.equals(ar.paxosID))
      throw new RuntimeException(
          "Unable to combine " + ar.getSummary() + " with " + this.getSummary());
  ...
  protected String getSummaryString() {
    return acceptor
        ...
        + (this.slots.size() == 1
            ? this.slots
            : "[" + this.slots.firstEntry() + "..." + this.slots.lastEntry() + "]")
        + "("
        + maxCheckpointedSlot
        + ")";
  }
  ...
  public static void main(String[] args) {
    ...
    try {
      AcceptReplyPacket ar1 = new AcceptReplyPacket(23, new Ballot(0, 234), 1, -1);
      ar1.putPaxosID("pid1", 0);
      AcceptReplyPacket ar2 = new AcceptReplyPacket(23, new Ballot(0, 234), 2, -1);
      ...
      assert (bar.toJSONObject()
          .toString()
          .equals(new BatchedAcceptReply(bar.toJSONObject()).toString()));
      BatchedAcceptReply bar1 = new BatchedAcceptReply(ByteBuffer.wrap(bar.toBytes()));
      System.out.println(bar1);
      assert (bar.toString()
          .equals(new BatchedAcceptReply(ByteBuffer.wrap(bar.toBytes())).toString()));
