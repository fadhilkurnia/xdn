...
public class RequestPacket extends PaxosPacket implements Request, ClientRequest, Byteable {
  ...
  private static final MessageDigest[] mds =
      new MessageDigest[Config.getGlobalInt(PC.NUM_MESSAGE_DIGESTS)];
  ...
  /**
   * These JSON keys are rather specific to RequestPacket or for debugging, so they are here as
   * opposed to PaxosPacket. Application developers don't have to worry about these.
   */
  public static enum Keys {
    ...
    /**
     * Listening socket address on which request received. Used to decide which messenger to use to
     * send back response.
     */
    LA,

    /**
     * Boolean query flag to specify whether this request came into the system as a string or
     * RequestPacket.
     */
    QF,
  ...
  /**
   * A unique requestID for each request. Paxos doesn't actually check or care whether two requests
   * with the same ID are identical. This field is useful for asynchronous clients to associate
   * responses with requests.
   *
   * <p>FIXME: change to long.
   */
  ...
  /**
   * The actual request body. The client will get back this string if that is what it sent to paxos.
   * If it issued a RequestPacket, then it will get back the whole RequestPacket back.
   */
  ...
  static enum Fields implements GetType {
    ...
    // address fields
    clientAddress(InetSocketAddress.class),
    listenAddress(InetSocketAddress.class),

    // non-final
    entryReplica(int.class),
    entryTime(long.class),
    shouldReturnRequestValue(boolean.class),
    forwardCount(int.class),

    // digest related fields
    broadcasted(boolean.class),
    digest((new byte[0]).getClass()),

    // highly variable length fields
    requestValue(String.class),
    responseValue(String.class),
    batched((new RequestPacket[0]).getClass());
  ...
  public int getClientID() {
    if (this.clientAddress != null) return this.clientAddress.getPort();
    return -1;
  }
  ...
  public RequestPacket addDebugInfo(String str) {
    if (DEBUG)
      this.debugInfo =
          (this.debugInfo == null ? "" : this.debugInfo) + makeDebugInfo(str, this.getEntryTime());
    return this;
  }
  ...
  public RequestPacket addDebugInfoDeep(String str) {
    if (DEBUG)
      if (this.addDebugInfo(str).batched != null)
        for (RequestPacket req : this.batched) req.addDebugInfo(str);
    return this;
  }

  public RequestPacket addDebugInfo(String str, int nodeID) {
    if (DEBUG) this.addDebugInfo(str + nodeID);
    return this;
  }

  public RequestPacket addDebugInfoDeep(String str, int nodeID) {
    if (DEBUG) this.addDebugInfoDeep(str + nodeID);
    return this;
  }
  ...
  public static boolean addDebugInfo(JSONObject msg, String str, int nodeID) throws JSONException {
    ...
    if (DEBUG && msg.has(Keys.DBG.toString()) && msg.has(Keys.CT.toString())) {
      str = str + nodeID;
      String debug =
          msg.getString(Keys.DBG.toString()) + makeDebugInfo(str, msg.getLong(Keys.CT.toString()));
      added = true;
  ...
  public RequestPacket(JSONObject json) throws JSONException {
    ...
    this.forwarderID =
        (json.has(RequestPacket.Keys.FWDR.toString())
    ...
    JSONArray batchedJSON =
        json.has(Keys.BATCH.toString()) ? json.getJSONArray(Keys.BATCH.toString()) : null;
    ...
    this.stringifiedSelf =
        json.has(Keys.STRINGIFIED.toString())
  ...
  public RequestPacket(net.minidev.json.JSONObject json) throws JSONException {
    ...
    this.packetType = PaxosPacketType.REQUEST;
    this.stop =
        json.containsKey(Keys.STOP.toString()) ? (Boolean) json.get(Keys.STOP.toString()) : false;
    this.requestID = Util.toLong(json.get(Keys.QID.toString()));
    this.requestValue = (String) json.get(Keys.QV.toString());

    this.responseValue =
        json.containsKey(Keys.RV.toString()) ? (String) json.get(Keys.RV.toString()) : null;
    this.entryTime = Util.toLong(json.get(Keys.ET.toString()));
    this.forwardCount =
        (json.containsKey(Keys.NFWDS.toString()) ? (Integer) json.get(Keys.NFWDS.toString()) : 0);
    this.forwarderID =
        (json.containsKey(RequestPacket.Keys.FWDR.toString())
            ? (Integer) json.get(RequestPacket.Keys.FWDR.toString())
            : IntegerMap.NULL_INT_NODE);
    this.debugInfo =
        (json.containsKey(Keys.DBG.toString()) ? (String) json.get(Keys.DBG.toString()) : "");

    this.clientAddress =
        (json.containsKey(Keys.CA.toString())
            ? Util.getInetSocketAddressFromString((String) (json.get(Keys.CA.toString())))
            : JSONNIOTransport.getSenderAddressJSONSmart(json));
    this.listenAddress =
        (json.containsKey(Keys.LA.toString())
    ...
    this.shouldReturnRequestValue =
        json.containsKey(Keys.QF.toString()) ? (Boolean) json.get(Keys.QF.toString()) : false;

    // unwrap latched along batch
    Collection<?> batchedJSON =
        json.containsKey(Keys.BATCH.toString())
    ...
    this.stringifiedSelf =
        json.containsKey(Keys.STRINGIFIED.toString())
            ...
            : null;
    assert (PaxosPacket.getPaxosPacketType(json) != PaxosPacketType.ACCEPT
            || this.stringifiedSelf != null)
        : PaxosPacket.getPaxosPacketType(json) + ":" + json;
  ...
  public JSONObject toJSONObjectImpl() throws JSONException {
    ...
    if (forwardCount > 0) json.put(Keys.NFWDS.toString(), this.forwardCount);
    ...
    if (this.clientAddress != null) json.put(Keys.CA.toString(), this.clientAddress);
    if (this.listenAddress != null) json.put(Keys.LA.toString(), this.listenAddress);

    if (this.shouldReturnRequestValue) json.put(Keys.QF.toString(), this.shouldReturnRequestValue);
  ...
  public net.minidev.json.JSONObject toJSONSmartImpl() throws JSONException {
    ...
    if (this.responseValue != null) json.put(Keys.RV.toString(), this.responseValue);
    json.put(Keys.ET.toString(), this.entryTime);
    if (forwardCount > 0) json.put(Keys.NFWDS.toString(), this.forwardCount);
    if (this.stop) json.put(Keys.STOP.toString(), this.stop);
    if (DEBUG) {
      json.put(RequestPacket.Keys.FWDR.toString(), this.forwarderID);
      if (this.debugInfo != null) json.put(Keys.DBG.toString(), this.debugInfo);
    }
    json.put(PaxosPacket.NodeIDKeys.E.toString(), this.entryReplica);
    if (this.clientAddress != null) json.put(Keys.CA.toString(), this.clientAddress.toString());
    if (this.listenAddress != null) json.put(Keys.LA.toString(), this.listenAddress.toString());
    if (this.shouldReturnRequestValue) json.put(Keys.QF.toString(), this.shouldReturnRequestValue);
    // convert latched along batch to json array
    if (this.batched != null && this.batched.length > 0) {
  ...
  /**
   * The weird constant above is to try to avoid mistakes in the painful (but totally worth it)
   * byte'ification method below. Using bytes as opposed to json strings makes a non-trivial
   * difference (~2x over json-smart and >4x over org.json. So we just chuck json libraries and use
   * our own byte[] serializer for select packets.
   *
   * <p>The serialization overhead really matters most for RequestPacket and AcceptPacket. Every
   * request, even with batching, must be deserialized by the coordinator and must be serialized
   * back while sending out the AcceptPacket. The critical path is the following at a coordinator
   * and is incurred at least in part even with batching for every request: (1) receive request, (2)
   * send accept, (3) receive accept_replies, (4) send commit Accordingly, we use byteification for
   * {@link RequestPacket}, {@link AcceptPacket}, {@link BatchedAcceptReply} and {@link
   * BatchedCommit}.
   */
  protected byte[] toBytes(boolean instrument) {
    ...
    if ((this.getType() == PaxosPacketType.REQUEST || this.getType() == PaxosPacketType.ACCEPT)
        ...
        && !instrument) return this.byteifiedSelf;
    ...
    try {
       ...
       * address. */
      bbuf.put(
          this.clientAddress != null ? this.clientAddress.getAddress().getAddress() : new byte[4]);
      ...
      bbuf.put(
          this.listenAddress != null ? this.listenAddress.getAddress().getAddress() : new byte[4]);
      // 0 (not -1) means invalid port
      ...
      if (this.digest != null) bbuf.put(this.digest);
      ...
      // requestValue
      byte[] reqValBytes =
          this.requestValue != null ? this.requestValue.getBytes(CHARSET) : new byte[0];
      ...
      // responseValue
      byte[] respValBytes =
          this.responseValue != null ? this.responseValue.getBytes(CHARSET) : new byte[0];
      ...
      if (this.getType() == PaxosPacketType.REQUEST) this.byteifiedSelf = exactBytes;
  ...
  public RequestPacket(ByteBuffer bbuf) throws UnsupportedEncodingException, UnknownHostException {
    ...
    this.clientAddress =
        cport != 0 ? new InetSocketAddress(InetAddress.getByAddress(ca), cport) : null;
    byte[] la = new byte[4];
    bbuf.get(la);
    int lport = (int) bbuf.getShort();
    lport = lport >= 0 ? lport : lport + 2 * (Short.MAX_VALUE + 1);
    this.listenAddress =
        lport != 0 ? new InetSocketAddress(InetAddress.getByAddress(la), lport) : null;
    exactLength += (4 + 2 + 4 + 2);
    ...
    if (digestLength > 0) bbuf.get(this.digest = new byte[digestLength]);
  ...
  /**
   * Learned the hard way that using org.json to stringify is an order of magnitude slower with
   * large request values compared to manually inserting the string like below. json-smart is fast
   * enough for our purposes but still doesn't beat direct string construction like below.
   *
   * <p>Note: we need to account for batching carefully here. So, we use a json array of strings as
   * opposed to json objects for batched requests above. Otherwise, this method will only use string
   * construction for the first request in a batch.
   */
  public String toString() {
    try {
      if (this.packetType == PaxosPacketType.ACCEPT)
        if (this.stringifiedSelf != null) return this.stringifiedSelf;
        else {
  ...
  private boolean isAnyBatchedRequestStop() {
    ...
    for (RequestPacket req : this.batched) if (req.isStopRequest()) return true;
    return false;
  }
  ...
  public RequestPacket latchToBatch(RequestPacket[] reqs) {
    ...
    if (this.batched == null) this.batched = allThreaded;
    else this.batched = concatenate(this.batched, allThreaded);
    for (int i = 0; i < this.batched.length; i++) assert (!this.batched[i].isBatched());
    return this;
  }
  ...
  private static RequestPacket[] toArray(RequestPacket[] reqs) {
    ...
    assert (count == totalSize)
        : count + " != " + totalSize + " while unraveling " + print(reqArrayList);
    return allThreaded;
  }
  ...
  private static int size(ArrayList<RequestPacket[]> reqArrayList) {
    int size = 0;
    for (RequestPacket[] reqArray : reqArrayList) size += reqArray.length;
    return size;
  }
  ...
  protected RequestPacket getFirstOnly() {
    if (this.batchSize() == 0) return this;
    // else
    RequestPacket req = (
        // retain slot in first coz it is useful for testing
        this instanceof ProposalPacket
            ? new ProposalPacket(((ProposalPacket) this).slot, this)
  ...
  public RequestPacket getACK() {
    ...
    RequestPacket reply =
        new RequestPacket(this.requestID, ResponseCodes.ACK.toString(), this.stop, this);
    reply.batched = null;
  ...
  public String[] getRequestValues() {
    ...
    if (this.shouldReturnRequestValue) {
      ...
      if (this.batched != null)
        for (int i = 0; i < this.batched.length; i++) {
          reqValues[i + 1] =
              this.batched[i].shouldReturnRequestValue
                  ? this.batched[i].requestValue
  ...
  public RequestPacket[] getRequestPackets() {
    ...
    for (int i = 0; i < this.batchSize(); i++) reqs[i + 1] = this.batched[i];
    return reqs;
  }
  ...
  public RequestPacket[] getEntryReplicaRequestsAsBatch(int id) {
    ...
    for (int i = 0; i < reqArray.length; i++)
      if (reqArray[i].getEntryReplica() == IntegerMap.NULL_INT_NODE
          || reqArray[i].getEntryReplica() == id) noEntryReplicaRequests.add(reqArray[i]);
      else entryReplicaRequests.add(reqArray[i]);
  ...
  protected String getSummaryString() {
    return requestID
        ...
        + ":"
        + "["
        + (this.requestValue != null && NO_OP.equals(this.requestValue)
            ? NO_OP
  ...
  // testing
  private static PValuePacket getRandomPValue(
      String paxosID, int version, int slot, Ballot ballot, boolean stop, InetSocketAddress isa) {
    PValuePacket pvalue =
        new PValuePacket(
            ballot,
            new ProposalPacket(
                slot, new RequestPacket("some_request", stop).setClientAddress(isa)));
   ...
   * The value needs to be an upper bound on the sum total of all of the gunk in PValuePacket other
   * than the requestValue itself, i.e., the size of a no-op decision.
   */
      ...
      length =
          (samplePValue == null
                  ? (samplePValue =
                      (PValuePacket)
                          (getRandomPValue(
                              TC.TEST_GUID.toString(),
                              0,
                              3142,
                              new Ballot(23, 2178),
                              true,
                              new InetSocketAddress("128.119.245.40", 2000))))
                  : samplePValue)
              .toJSONObject()
              .toString()
              .length();
  ...
  public int lengthEstimate() {
    ...
    if (this.isBatched()) for (RequestPacket req : this.batched) len += req.lengthEstimate();
    return len;
  }
  ...
  public String printBatched() {
    ...
    s += this.getSummary();
    if (this.batchSize() > 0)
      for (int i = 0; i < this.batchSize(); i++) s += "\n  " + this.batched[i].getSummary();
    return s;
  }
  ...
  public void setResponse(String response) {
    if (this.responseValue == null) this.responseValue = response;
  ...
  /**
   * We use this address instead of a null client socket address or listen address so that a null
   * address doesn't get overwritten by a bad legitimate address when a request is forwarded across
   * servers. Port 9 is a privileged port and is used for the Discard protocol.
   *
   * <p>We don't use this anymore.
   */
  protected static final InetSocketAddress NULL_SOCKADDR =
  ...
  public boolean equals(Object obj) {
    ...
    obj instanceof RequestPacket
        ...
        && (this.clientAddress == req.clientAddress
            || this.clientAddress != null && this.clientAddress.equals(req.clientAddress))
  ...
  static {
    if (Config.getGlobalBoolean(PC.ENABLE_STATIC_CHECKS)) doubleCheckFields();
  }
