...
/**
 * @author arun
 *     <p>A sync reply packet contains commits missing at the sending node (nodeID). The receiver is
 *     expected to send to the sender the commits it is reporting as missing in this sync reply.
 */
...
public final class SyncDecisionsPacket extends PaxosPacket {
  ...
  public SyncDecisionsPacket(
      int nodeID, int maxDecisionSlot, ArrayList<Integer> missingSlotNumbers, boolean flag) {
  ...
  public SyncDecisionsPacket(JSONObject json) throws JSONException {
    ...
    if (json.has(PaxosPacket.Keys.MISS.toString()))
      missingSlotNumbers =
          Util.JSONArrayToArrayListInteger(json.getJSONArray(PaxosPacket.Keys.MISS.toString()));
  ...
  public JSONObject toJSONObjectImpl() throws JSONException {
    ...
    if (missingSlotNumbers != null && missingSlotNumbers.size() > 0)
      json.put(PaxosPacket.Keys.MISS.toString(), new JSONArray(missingSlotNumbers));
    return json;
  }
  ...
  protected String getSummaryString() {
    return nodeID
        ...
        + ", "
        + this.missingSlotNumbers.get(this.missingSlotNumbers.size() - 1)
        + "]";
  }
}
