...
public class BatchedCommit extends PaxosPacket implements Byteable {
  ...
  /**
   * Note: The order of the fields can not be changed. No new fields can be added or existing fields
   * removed unless the bytebuffer based constructor and toBytes() methods are accordingly modified.
   * Use the JSON mode for adding temporary debugging fields if needed.
   */
  ...
  static enum Fields implements GetType {
    ballot(Ballot.class),
    medianCheckpointedSlot(int.class),
    slots(TreeSet.class),
    group(Set.class);
    final Class<?> type;
  ...
  public BatchedCommit(JSONObject json) throws JSONException {
    ...
    for (int i = 0; i < jsonArray.length(); i++) this.group.add(jsonArray.getInt(i));

    jsonArray = json.getJSONArray(PaxosPacket.Keys.SLOTS.toString());
    for (int i = 0; i < jsonArray.length(); i++) this.slots.add(jsonArray.getInt(i));
  }
  ...
  public boolean addBatchedCommit(BatchedCommit bCommit) {
    if (!bCommit.ballot.equals(this.ballot) || !bCommit.paxosID.equals(this.paxosID))
      throw new RuntimeException(
          "Unable to combine " + bCommit.getSummary() + " with " + this.getSummary());
  ...
  public boolean addCommit(PValuePacket commit) {
    if (!commit.ballot.equals(this.ballot) || !commit.paxosID.equals(this.paxosID))
      throw new RuntimeException(
          "Unable to combine " + commit.getSummary() + " with " + this.getSummary());
   ...
   */
  public BatchedCommit(ByteBuffer bbuf) throws UnsupportedEncodingException, UnknownHostException {
    super(bbuf);
    this.ballot = new Ballot(bbuf.getInt(), bbuf.getInt());
    this.medianCheckpointedSlot = bbuf.getInt();
    int numSlots = bbuf.getInt();
    for (int i = 0; i < numSlots; i++) this.slots.add(bbuf.getInt());
    this.group = new HashSet<Integer>();
    int groupSize = bbuf.getInt();
    for (int i = 0; i < groupSize; i++) this.group.add(bbuf.getInt());
  }

  /**
   * Includes everything without {@link #slots}, {@link #group} and the two integers for their
   * respective sizes. This is a bit inconsistent with {@link PaxosPacket#SIZEOF_PAXOSPACKET_FIXED}
   * where the size integer is included in the fixed length. This is because we have two variable
   * length sets in BatchedCommit, so it is more convenient to count both sizes as part of those
   * variable length sets.
   */
  protected static final int SIZEOF_BATCHEDCOMMIT_FIXED =
      8 // int,int ballot
  ...
  public byte[] toBytes() {
    ...
    if (this.byteifiedSelf != null) return this.byteifiedSelf;
    ...
    try {
          ...
          ByteBuffer.wrap(
              new byte[
                  ...
                  SIZEOF_PAXOSPACKET_FIXED
                      + paxosIDLength
                      // self
                      + SIZEOF_BATCHEDCOMMIT_FIXED
                      + 4 * (this.slots.size() + 1 + this.group.size() + 1)]);
      super.toBytes(bbuf);
      ...
      for (Integer i : slots) bbuf.putInt(i);
      bbuf.putInt(this.group.size());
      for (Integer i : this.group) bbuf.putInt(i);
