...
public class AcceptReplyPacket extends PaxosPacket {
  ...
  /**
   * Maximum slot up to which this node has checkpointed state, a value that is used for garbage
   * collection of logs.
   */
   ...
   * @param ar
   */
  public AcceptReplyPacket(
      int nodeID,
      Ballot ballot,
      int slotNumber,
      int maxCheckpointedSlot,
      long requestID,
      AcceptReplyPacket ar) {
    super(ar);
    ...
    this.requestID =
        ar != null
            ? (ar instanceof BatchedAcceptReply
                ? ((BatchedAcceptReply) ar).getRequestID(slotNumber)
                : ar.requestID)
            : requestID;
  }
   ...
   * @param maxCheckpointedSlot
   */
  public AcceptReplyPacket(int nodeID, Ballot ballot, int slotNumber, int maxCheckpointedSlot) {
   ...
   */
  public AcceptReplyPacket(
      int nodeID, Ballot ballot, int slotNumber, int maxCheckpointedSlot, long requestID) {
  ...
  protected static final int SIZEOF_ACCEPTREPLY =
      4 // acceptor
          + 8 // int,int ballot
          + 4 // int slotNumber
  ...
  static enum Fields implements GetType {
    acceptor(int.class),
    ballot(Ballot.class),
    slotNumber(int.class),
    maxCheckpointedSlot(int.class),
    requestID(long.class),
    undigestRequest(boolean.class);
    final Class<?> type;
  ...
  public AcceptReplyPacket(ByteBuffer bbuf)
    ...
    if (bbuf.get() == (byte) 1) this.setDigestRequest();
    assert (bbuf.position() == SIZEOF_PAXOSPACKET_FIXED + paxosIDLength + SIZEOF_ACCEPTREPLY)
        : bbuf.position()
            + " != ["
  ...
  protected static final int SIZEOF_BATCHEDACCEPTREPLY = 4 + 8 + 4 + 4 + 8 + 1;

  public ByteBuffer toBytes(ByteBuffer bbuf) throws UnsupportedEncodingException {
    super.toBytes(bbuf);
    assert (bbuf.position()
        == SIZEOF_PAXOSPACKET_FIXED + this.getPaxosID().getBytes(CHARSET).length);
    bbuf.putInt(this.acceptor)
        .putInt(this.ballot.ballotNumber)
        .putInt(this.ballot.coordinatorID)
        .putInt(this.slotNumber)
        .putInt(this.maxCheckpointedSlot)
  ...
  protected String getSummaryString() {
    return acceptor + "->" + ballot + ", " + slotNumber + "(" + maxCheckpointedSlot + ")";
  }
  ...
  public JSONObject toJSONObjectImpl() throws JSONException {
    ...
    if (this.undigestRequest) json.put(PaxosPacket.Keys.NACK.toString(), this.undigestRequest);
    return json;
  }
  ...
  /**
   * @return Whether this packet is coalescable, which is true when its undigestRequest flag is
   *     false.
   */
