...
/**
 * @author arun
 *     <p>This is a funky packet that is used to create paxos groups at nodes that missed its
 *     birthing, say, because they were down then. If a node receives a paxos packet for which it
 *     has no state, it uses this packet to find the group membership and create the paxos instance
 *     locally.
 */
...
public class FindReplicaGroupPacket extends PaxosPacket {
  ...
  public static int getNodeID(JSONObject msg) throws JSONException {
    ...
    if (msg.has(PaxosPacket.Keys.PT.toString())) {
      PaxosPacketType msgType =
          PaxosPacketType.getPaxosPacketType(msg.getInt(PaxosPacket.Keys.PT.toString()));
      switch (msgType) {
        case ACCEPT:
        ...
        case ACCEPT_REPLY:
          ...
          break;
        case PREPARE:
          id = (new Ballot(msg.getString(PaxosPacket.NodeIDKeys.B.toString()))).coordinatorID;
