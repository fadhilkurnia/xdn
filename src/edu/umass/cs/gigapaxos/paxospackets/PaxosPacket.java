...
/**
 * @author arun
 *     <p>The parent class for all paxos packet types. Every paxos packet has a minimum of three
 *     fields: packet type, paxos group ID, version.
 */
public abstract class PaxosPacket extends JSONPacket implements Summarizable {
  ...
  /**
   * These are keys involving NodeIDType that need to be "fixed" by Messenger just before sending
   * and by PaxosPacketDemultiplexer just after being received. The former fix involves a
   * int->NodeIDType->String conversion and the latter involves a String->NodeIDType->int
   * conversion. The former requires IntegerMap<NodeIDType> and the latter requires
  ...
  public enum PaxosPacketType implements IntegerPacketType {
    ...
    /**
     * Request to retrieve missing decisions. A replica sends out this request when it has received
     * a decision with slot number n+K but not n for a sufficiently large K. The value of K is
     * generally a constant fixed at initialization time to a value based on the expected, normal
     * length of the outstanding request pipeline. However, a paxos instance additionally tries to
     * sync more aggressively at creation time, i.e., the effective K is small, e.g., 1, because it
     * is more likely that different instances got created at different times and some instances
  ...
  public static PaxosPacketType getPaxosPacketType(JSONObject json) throws JSONException {
    return PaxosPacketType.getPaxosPacketType(json.getInt(PaxosPacket.Keys.PT.toString()));
  }
  ...
  public static PaxosPacketType getPaxosPacketType(net.minidev.json.JSONObject json)
    ...
    if (json.get(PaxosPacket.Keys.PT.toString()) != null)
      return PaxosPacketType.getPaxosPacketType((Integer) json.get(PaxosPacket.Keys.PT.toString()));
    else return null;
  }
  ...
  // testing
  protected PaxosPacket(net.minidev.json.JSONObject json) throws JSONException {
    super(
        PaxosPacket.PaxosPacketType.getPaxosPacketType((Integer) json.get(JSONPacket.PACKET_TYPE)));
    if (json != null) {
      ...
      if (json.containsKey(PaxosPacket.Keys.V.toString()))
        this.version = (Integer) json.get(PaxosPacket.Keys.V.toString());
    }
  }

  protected static String CHARSET = "ISO-8859-1";
  protected static final boolean BYTEIFICATION = Config.getGlobalBoolean(PC.BYTEIFICATION);
  ...
  protected ByteBuffer toBytes(ByteBuffer bbuf) throws UnsupportedEncodingException {
    ...
    assert (bbuf.position() == SIZEOF_PAXOSPACKET_FIXED + paxosIDBytes.length)
        : bbuf.position() + " != " + SIZEOF_PAXOSPACKET_FIXED + paxosIDBytes.length;
    return bbuf;
  }
  ...
  public JSONObject toJSONObject() throws JSONException {
    ...
    JSONPacket.putPacketType(json, PaxosPacket.PaxosPacketType.PAXOS_PACKET.getInt());
    ...
    for (String name : Util.getNames(child)) json.put(name, child.get(name));
  ...
  public net.minidev.json.JSONObject toJSONSmart() throws JSONException {
    ...
    // tells Packet that this is a PaxosPacket
    json.put(PaxosPacket.PACKET_TYPE, PaxosPacket.PaxosPacketType.PAXOS_PACKET.getInt());
    // the specific type of PaxosPacket
    json.put(PaxosPacket.Keys.PT.toString(), this.packetType.getInt());
    json.put(PaxosPacket.Keys.ID.toString(), this.paxosID);
    json.put(PaxosPacket.Keys.V.toString(), this.version);
    ...
    if (child != null) {
      for (String name : (child).keySet()) json.put(name, child.get(name));
    } else return null;
  ...
  protected static boolean isStatic(Field field) {
    ...
    for (String token : tokens) if (token.equals("static")) return true;
    return false;
  }

  private static final boolean STRICT_ADDRESS_CHECKS =
  ...
  static void checkFields(Class<?> clazz, GetType[] expFields) {
    ...
    boolean print = PaxosConfig.getLogger().isLoggable(Level.FINE);
    if (print) System.out.println("Checking fields for " + clazz);
    for (int i = 0, j = 0; i < fields.length; j += (isStatic(fields[i]) ? 0 : 1), i++) {
      if (isStatic(fields[i])) continue;
      if (j >= expFields.length) break;
      if (print) System.out.println("    " + fields[j]);
      boolean match = true;
  ...
  public static PaxosPacketType getType(byte[] bytes) {
    ...
    ByteBuffer bbuf = ByteBuffer.wrap(bytes, 0, 8);
    return bbuf.getInt() == PaxosPacketType.PAXOS_PACKET.getInt()
            && (type = PaxosPacketType.getPaxosPacketType(bbuf.getInt())) != null
        ? type
        : null;
  }
