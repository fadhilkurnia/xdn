...
/**
 * @author arun
 *     <p>A <slot, ballot, request> three-tuple. The slot and request are within the proposal here.
 *     A pvalue is used internally by paxos acceptors and coordinators. It also acts as a DECISION
 *     packet as every committed request must be associated with some slot, and ballot.
 */
@SuppressWarnings("javadoc")
public class PValuePacket extends ProposalPacket {

  /**
   * A ballot is a {@code <ballotNumber:ballotCoordinator>} two-tuple. A PValuePacket is a <ballot,
   * proposal> two-tuple, or equivalently a <ballot, slot, request> three-tuple, or equivalently a
   * <ballotNumber: balltoCoordinator, slot, request> four-tuple.
   */
  ...
  /**
   * For garbage collection, similar to that in AcceptPacket. This is the slot up to which the
   * median node (or above) has checkpointed when nodes in the paxos replica group are ordered by
   * their macCheckpointedSlot.
   */
  ...
  static enum Fields implements GetType {
    ballot(Ballot.class),
    recovery(boolean.class),
    medianCheckpointedSlot(int.class),
    noCoalesce(boolean.class);
  ...
  public PValuePacket(net.minidev.json.JSONObject json) throws JSONException {
    ...
    this.recovery =
        json.containsKey(PaxosPacket.Keys.RCVRY.toString())
            ? (Boolean) json.get(PaxosPacket.Keys.RCVRY.toString())
            : false;
    this.packetType = PaxosPacket.getPaxosPacketType(json);
    this.noCoalesce =
        json.containsKey(PaxosPacket.Keys.NO_COALESCE.toString())
