...
/**
 * @author arun
 *     <p>A utility class to consume batched requests.
 */
public class RequestBatcher extends ConsumerTask<RequestPacket> {

  private static final int MAX_BATCH_SIZE = Config.getGlobalInt(PC.MAX_BATCH_SIZE);
  private static final double MIN_BATCH_SLEEP_DURATION =
      Config.getGlobalDouble(PC.BATCH_SLEEP_DURATION);
  private static final double BATCH_OVERHEAD = Config.getGlobalDouble(PC.BATCH_OVERHEAD);
  ...
  /**
   * @param lock Used for synchronization by abstract ConsumerTask<TaskType>.
   * @param paxosManager Needed to consume requests by invoking {@code
   *     paxosManager.handleIncomingPacketInternal}.
   */
  private RequestBatcher(
      HashMap<String, LinkedBlockingQueue<RequestPacket>> lock, PaxosManager<?> paxosManager) {
    super(lock);
    this.batched = lock;
  ...
  protected static synchronized void updateSleepDuration(long entryTime) {
    agreementLatency =
        Util.movingAverage(((double) (System.currentTimeMillis() - entryTime)), agreementLatency);
    if (ENABLE_INSTRUMENTATION && Util.oneIn(10)) DelayProfiler.updateDelay("latency", entryTime);
  }
  ...
  public void enqueueImpl(RequestPacket task) {
    ...
    if (taskList == null) taskList = new LinkedBlockingQueue<RequestPacket>();
    taskList.add(task);
  ...
  private double computeSleepDuration() {
    return (Math.max(this.avgNumQGroups, this.batched.size()) < MAX_GROUPS_FOR_BATCH_SLEEP
        ? Math.min(
                MAX_BATCH_SLEEP_DURATION,
                MIN_BATCH_SLEEP_DURATION + agreementLatency * BATCH_OVERHEAD)
            / (Math.max(this.avgNumQGroups, this.batched.size()))
        : 0);
  }
  ...
  /**
   * This load throttling mechanism will propagate the exception so that the client facing
   * demultiplexer will catch it and take the necessary action to throttle the incoming request
   * load.
   *
   * @param paxosID
   */
  ...
  public RequestPacket dequeueImpl() {
    if (this.batched.isEmpty()) return null;

    // pluck first list (each grouped by paxosID)
    Iterator<Entry<String, LinkedBlockingQueue<RequestPacket>>> mapEntryIter =
        this.batched.entrySet().iterator();
    Entry<String, LinkedBlockingQueue<RequestPacket>> firstEntry = mapEntryIter.next();
    ...
    while (reqPktIter.hasNext()) {
      ...
      ((totalByteLength += next.lengthEstimate())
              > (SQLPaxosLogger.isLoggingEnabled()
                  ? Math.min(NIOTransport.MAX_PAYLOAD_SIZE, SQLPaxosLogger.MAX_LOG_MESSAGE_SIZE)
                  : NIOTransport.MAX_PAYLOAD_SIZE))
          // batch size limit would be reached
          || ((totalBatchSize += next.batchSize() + 1) > MAX_BATCH_SIZE)) break;
    ...
    if (!batch.isEmpty()) first.latchToBatch(batch.toArray(new RequestPacket[0]));

    // remove first list if all plucked
    if (firstEntry.getValue().isEmpty()) // !reqPktIter.hasNext())
    mapEntryIter.remove();

    if (ENABLE_INSTRUMENTATION && Util.oneIn(20)) DelayProfiler.updateMovAvg("#queued", queueSize);
