...
public class TESTPaxosShutdownThread extends Thread {
  ...
  public static final ConcurrentHashMap<Integer, PValuePacket> decisions101 =
      new ConcurrentHashMap<Integer, PValuePacket>();
  public static final LinkedBlockingQueue<Integer> decisions102 =
      new LinkedBlockingQueue<Integer>();

  public static final ConcurrentHashMap<Integer, PValuePacket> accepts101 =
      new ConcurrentHashMap<Integer, PValuePacket>();
  ...
  public void run() {
    ...
    Set<RequestPacket> missing =
        TESTPaxosClient.getMissingRequests(testClients.toArray(new TESTPaxosClient[0]));
    if (RequestInstrumenter.DEBUG) {
      String sep = "-------\n";
      for (RequestPacket req : missing) {
        // logging doesn't seem to work here
        System.err.println(sep + (RequestInstrumenter.getLog(req.requestID)));
      }
      System.out.println("\n#missing=" + missing.size());
    } else if (!missing.isEmpty())
      System.out.println("Request instrumentation (RequestInstrumenter.DEBUG) not turned on.");
  }
