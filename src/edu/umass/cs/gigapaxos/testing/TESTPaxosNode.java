...
/**
 * @author V. Arun
 *     <p>The main server node class for gigapaxos testing.
 */
public class TESTPaxosNode {
  ...
  private PaxosManager<Integer> startPaxosManagerAndApp(
      int id, NodeConfig<Integer> nc, boolean local) {
    try {
      // shared between app and paxos manager only for testing
      JSONMessenger<Integer> niot = null;
      this.pm =
          new PaxosManager<Integer>(
              id,
              nc,
              (niot =
                  new JSONMessenger<Integer>(
                      new MessageNIOTransport<Integer, JSONObject>(
                          id,
                          nc,
                          new PacketDemultiplexerDefault(),
                          true,
                          SSLDataProcessingWorker.SSL_MODES.valueOf(
                              Config.getGlobal(PC.SERVER_SSL_MODE).toString())))),
              (this.app = new TESTPaxosApp(niot)),
              null,
              true);
      pm.initClientMessenger(
          new InetSocketAddress(nc.getNodeAddress(myID), nc.getNodePort(myID)), niot);
  ...
  protected void createNonDefaultGroupInstanes(int numGroups) {
    if (numGroups > 10000 && Config.getGlobalBoolean(TC.BATCH_CREATION_ENABLED)) {
      this.createNonDefaultGroupInstanes(numGroups, Config.getGlobalInt(TC.BATCH_CREATION_SIZE));
      return;
    }
    // else
    int j = 1;
    ...
    for (int i = Config.getGlobalInt(TC.PRE_CONFIGURED_GROUPS); i < numGroups; i++) {
      ...
      for (int id : TESTPaxosConfig.getDefaultGroup()) {
        ...
        if (id == myID)
          this.getPaxosManager()
              .createPaxosInstance(groupID, 0, group, (ReplicableDeprecated) null, null);
      }
      ...
      for (int id : TESTPaxosConfig.getDefaultGroup()) {
        try {
          if (id == myID)
            this.getPaxosManager()
                .createPaxosInstance(
                    nameStates, Util.arrayToIntSet(TESTPaxosConfig.getDefaultGroup()));
        } catch (Exception e) {
          // ignore
        }
      }
  ...
  private static int processArgs(String[] args) {
    ...
    int myID = -1;
    try {
      myID = (args != null && args.length > 0 ? Integer.parseInt(args[args.length - 1]) : -1);
  ...
  public static void main(String[] args) {
    try {
      ...
      System.out.println(
          "Creating " + Config.getGlobalInt(TC.PRE_CONFIGURED_GROUPS) + " default groups");
      me.createDefaultGroupInstances();
      System.out.println(
          "Creating "
              + ((numGroups - Config.getGlobalInt(TC.PRE_CONFIGURED_GROUPS)) > 0
