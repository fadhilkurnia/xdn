...
/**
 * @author V. Arun
 *     <p>The main class for testing gigapaxos on a single machine with virtual nodes.
 */
...
public class TESTPaxosMain {
  ...
  public void testPaxos(boolean recovery) {
    ...
    try {
      ...
      // runs
      TESTPaxosConfig.setRandomGroups(Config.getGlobalInt(TC.PRE_CONFIGURED_GROUPS));
      ...
      for (int id : tpMain.nodes.keySet()) {
        ...
        tpMain.nodes.get(id).createNonDefaultGroupInstanes(Config.getGlobalInt(TC.NUM_GROUPS));
      }
  ...
  /**
   * Basic test with recovery. The timeout here should be at least 4*NUM_REQUESTS/TOTAL_LOAD + a
   * recovery time that depends upon NUM_REQUESTS*NUM_NODES*T, where T is roughly 1/10^3, so, for
   * example, for 33K requests and 3 nodes roughly corresponds to a recovery time of 33K*3*1/10^3 ~=
   * 100 seconds.
   *
   * @param args
            ...
            2
                ...
                2
                    ...
                    * 0.3
                    / 1000);
    Assert.assertTrue(
        "Increase the timeout for this test to at least " + runtimeEstimate * 1000,
        timeoutMillis >= runtimeEstimate);
    testPaxos(false);
    Thread.sleep(1000);
