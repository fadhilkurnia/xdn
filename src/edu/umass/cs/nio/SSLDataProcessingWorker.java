...
/**
 * @author arun
 *     <p>This class does SSL wrap/unwrap functions respectively right before NIO performs an
 *     (unencrypted) outgoing network write and right after NIO receives (encrypted) incoming
 *     network reads. It is really not a "message extractor", just an InterfaceDataProcessingWorker
 *     + wrap functionality, but it implements InterfaceMessageExtractor so that it is easy for
 *     MessageNIOTransport to conduct its functions without worrying about whether
 *     NIOTransport.worker is InterfaceMessageExtractor or not.
 *     <p>Nevertheless, this class can be used simply with NIOTransport for secure byte stream
 *     communication as InterfaceMessageExtractor is also an InterfaceDataProcessingWorker.
 */
public class SSLDataProcessingWorker implements InterfaceMessageExtractor {
  ...
  private ConcurrentHashMap<SelectableChannel, AbstractNIOSSL> sslMap =
      new ConcurrentHashMap<SelectableChannel, AbstractNIOSSL>();
  ...
  protected SSLDataProcessingWorker(
      DataProcessingWorker worker, SSLDataProcessingWorker.SSL_MODES sslMode, String myID)
  ...
  protected SSLDataProcessingWorker setHandshakeCallback(HandshakeCallback callback) {
  ...
  public void processData(SocketChannel channel, ByteBuffer encrypted) {
    ...
    log.log(
        ...
        new Object[] {this, encrypted.remaining(), channel});
    // unwrap SSL
  ...
  protected boolean isHandshakeComplete(SocketChannel socketChannel) {
    ...
    return nioSSL != null ? ((NonBlockingSSLImpl) nioSSL).isHandshakeComplete() : false;
  }

  protected boolean register(SelectionKey key, boolean isClient) throws IOException {
    ...
    log.log(
        ...
        new Object[] {this, (isClient ? "client" : "server"), key.channel()});
    this.sslMap.put(key.channel(), new NonBlockingSSLImpl(key, engine, this.taskWorkers));
    return true;
  }
  ...
  class NonBlockingSSLImpl extends AbstractNIOSSL {
    ...
    NonBlockingSSLImpl(SelectionKey key, SSLEngine engine, Executor taskWorkers) {
    ...
    public void onClosed() {
      log.log(
          Level.FINE, "{0} cleaning up closed SSL channel {1}", new Object[] {this, key.channel()});
      cleanup(key);
    }
    ...
    public void onOutboundData(ByteBuffer encrypted) {
      ...
      try {
         ...
         */
        for (int attempts = 0;
            (attempts < 1 || !this.isHandshakeComplete()) && encrypted.hasRemaining();
            attempts++) channel.write(encrypted);
        ...
        if (encrypted.hasRemaining())
          log.log(
              Level.FINE,
              "{0} failed to bulk-write {1} bytes despite multiple attempts ({2} bytes left"
                  + " unsent)",
              new Object[] {
                this, totalLength, encrypted.remaining(),
              });
  ...
  public void addPacketDemultiplexer(AbstractPacketDemultiplexer<?> pd) {
    if (this.decryptedWorker instanceof InterfaceMessageExtractor)
      ((InterfaceMessageExtractor) this.decryptedWorker).addPacketDemultiplexer(pd);
  }

  @Override
  public void precedePacketDemultiplexer(AbstractPacketDemultiplexer<?> pd) {
    if (this.decryptedWorker instanceof InterfaceMessageExtractor)
      ((InterfaceMessageExtractor) this.decryptedWorker).precedePacketDemultiplexer(pd);
  }

  @Override
  public void processLocalMessage(InetSocketAddress sockAddr, byte[] msg) {
    if (this.decryptedWorker instanceof InterfaceMessageExtractor)
      ((InterfaceMessageExtractor) this.decryptedWorker).processLocalMessage(sockAddr, msg);
  }

  private void extractMessages(SelectionKey key, ByteBuffer incoming) {
    ByteBuffer bbuf = null;
    try {
      while (incoming.hasRemaining() && (bbuf = this.extractMessage(key, incoming)) != null) {
  ...
  private ByteBuffer extractMessage(SelectionKey key, ByteBuffer incoming) throws IOException {
    NIOTransport.AlternatingByteBuffer abbuf =
        (NIOTransport.AlternatingByteBuffer) key.attachment();
    assert (abbuf != null);
    if (abbuf.headerBuf.remaining() > 0) {
      ...
      if (abbuf.headerBuf.remaining() == 0) {
        abbuf.bodyBuf =
            ByteBuffer.allocate(NIOTransport.getPayloadLength((ByteBuffer) abbuf.headerBuf.flip()));
