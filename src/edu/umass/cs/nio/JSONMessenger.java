...
/**
 * @author V. Arun
 * @param <NodeIDType> This class has support for retransmissions with exponential backoff. But you
 *     can't rely on this backoff for anything other than ephemeral traffic bursts. If you are
 *     overloaded, you are overloaded, so you must just reduce the load.
 */
public class JSONMessenger<NodeIDType> implements SSLMessenger<NodeIDType, JSONObject> {

  /**
   * The JSON key for the time when the message was sent. Used only for instrumentation purposes by
  ...
  public JSONMessenger(final InterfaceNIOTransport<NodeIDType, JSONObject> niot, int numWorkers) {
    ...
    if (niot instanceof JSONMessenger) this.execpool = ((JSONMessenger<NodeIDType>) niot).execpool;
    else
      this.execpool =
          Executors.newScheduledThreadPool(
              ...
              new ThreadFactory() {
                ...
                public Thread newThread(Runnable r) {
                  ...
                  thread.setName(
                      JSONMessenger.class.getSimpleName() + niot.getMyID() + thread.getName());
                  return thread;
                }
              });
    ...
    for (int i = 0; i < workers.length; i++) {
      try {
        ...
        this.workers[i].setName(
            JSONMessenger.class.getSimpleName() + niot.getMyID() + "_send_worker" + i);
      } catch (IOException e) {
        this.workers[i] = null;
        e.printStackTrace();
      }
  ...
  /**
   * Send returns void because it is the "ultimate" send. It will retransmit if necessary. It is
   * inconvenient for senders to worry about retransmission anyway. We may need to retransmit
   * despite using TCP-based NIO because NIO is designed to be non-blocking, so it may sometimes
   * drop messages when asked to send but the channel is congested. We use the return value of NIO
   * send to decide whether to retransmit.
   */
  @SuppressWarnings("unchecked")
  protected void send(GenericMessagingTask<NodeIDType, ?> mtask, boolean useWorkers)
    ...
    for (Object msg : mtask.msgs) {
      ...
      byte[] msgBytes =
          msg instanceof byte[]
              ? (byte[]) msg
              : msg instanceof Byteable
                  ? ((Byteable) msg).toBytes()
      ...
      for (int r = 0; r < mtask.recipients.length; r++) {
        ...
        if (sent > 0) {
          log.log(
              ...
              new Object[] {
                ...
                log.isLoggable(level)
                    ? (msg instanceof Summarizable
                        ? ((Summarizable) msg).getSummary(log.isLoggable(level))
                        : message != null
                            ? Util.truncate(message, 32, 32)
                            : Util.truncate(new Stringer(msgBytes), 32, 32))
                    : msgBytes
              });
        } else if (sent == 0) {
          log.log(
          ...
          Retransmitter rtxTask =
              new Retransmitter((mtask.recipients[r]), msgBytes, RTX_DELAY, useWorkers);
          // can't block here, so have to ignore returned future
          execpool.schedule(rtxTask, RTX_DELAY, TimeUnit.MILLISECONDS);
        } else {
          assert (sent == -1) : sent;
          boolean isPresent = this.getNodeConfig().nodeExists((NodeIDType) mtask.recipients[r]);
          log.log(
              isPresent ? Level.WARNING : Level.INFO,
              "{0} failed to send message to node {1} [isPresent={4}]:[connected={2}]: {3}",
              new Object[] {
                this.nioTransport.getMyID(),
                mtask.recipients[r],
                !this.nioTransport.isDisconnected((NodeIDType) mtask.recipients[r]),
                msg,
                isPresent
              });
        }
  ...
  private int specialCaseSend(Object id, byte[] msgBytes, boolean useWorkers) throws IOException {
    ...
    else return this.sendToID((NodeIDType) id, msgBytes, useWorkers);
  }

  /**
   * We need this because NIO may drop messages when congested. Thankfully, it tells us when it does
   * that. The task below exponentially backs off with each retransmission. We are probably doomed
   * anyway if this class is invoked except rarely.
   */
   ...
   * @param id
   * @param jsonData
   * @return Return value indicates the number of bytes sent. A value of -1 indicates an error.
   ...
   * @param address
   * @param jsonData
   * @return Refer {@link #sendToID(Object, JSONObject) sendToID(Object, JSONObject)}.
   * @throws IOException
   */
  @Override
  ...
  /**
   * @param pd The supplied packet demultiplexer is appended to end of the existing list. Messages
   *     will be processed by the first demultiplexer that has registered for processing the
   *     corresponding packet type and only by the first demultiplexer.
   *     <p>Note that there is no way to remove demultiplexers. All the necessary packet
   *     demultiplexers must be determined at design time. It is strongly recommended that all
   *     demultiplexers process exclusive sets of packet types. Relying on the order of chained
   *     demultiplexers is a bad idea.
   */
  @Override
  ...
  private AddressMessenger<JSONObject> getClientMessengerInternal() {
    return this.clientMessenger != null
  ...
  private AddressMessenger<JSONObject> getSSLClientMessengerInternal() {
    return this.sslClientMessenger != null
  ...
  public static class JSONObjectByteableWrapper extends JSONObjectWrapper implements Byteable {
    ...
    public byte[] toBytes() {
      try {
        return obj instanceof Byteable
            ? ((Byteable) obj).toBytes()
  ...
  /**
   * A hack that relies on the fact that NIO treats JSONObject as no different from any other object
   * in that it invokes toString() and then getBytes(.) to serialize and send it over the network.
   * It is necessary that the other end receiving this object be able to reconstruct it from a
   * byte[], string, or JSONObject.
   *
   * <p>Automatically tries to use clientMessenger.
   *
   * @param sockAddr
   * @param message
   ...
   * @throws IOException
   */
  public int sendClient(
      InetSocketAddress sockAddr, Object message, InetSocketAddress listenSocketAddress)
      throws JSONException, IOException {
    AddressMessenger<JSONObject> msgr = this.getClientMessenger(listenSocketAddress);
    msgr = msgr != null ? msgr : this;
    if (message instanceof byte[]) return msgr.sendToAddress(sockAddr, (byte[]) message);
    else if (message instanceof Byteable)
      return msgr.sendToAddress(sockAddr, ((Byteable) message).toBytes());
    else return msgr.sendToAddress(sockAddr, new JSONObjectWrapper(message));
  }
  ...
  public AddressMessenger<JSONObject> getClientMessenger(InetSocketAddress listenSockAddr) {
    ...
    if (msgr instanceof InterfaceNIOTransport
        && ((InterfaceNIOTransport<?, ?>) msgr).getListeningSocketAddress().getPort()
            == (listenSockAddr.getPort())) return msgr;
    // else
