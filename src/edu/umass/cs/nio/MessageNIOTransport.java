 ...
 * @author V. Arun
 * @param <NodeIDType>
 * @param <MessageType> This class exists primarily as a wrapper around NIOTransport to support
 *     messages. NIOTransport is for general-purpose NIO byte stream communication between numbered
 *     nodes as specified by the NodeConfig interface and a data processing worker as specified by
 *     the DataProcessingWorker interface that handles a byte stream. This class provides the
 *     abstraction of messages and a corresponding PacketDemultiplexer that handles messages instead
 *     of a continuous byte stream.
 *     <p>MessageType can be any type whose toString() method results in a meaningful serialization
 *     of a MessageType instance, i.e., the packet demultiplexer on the other end must be able to
 *     reconstruct the MessageType object from the String. The one exception is byte[] that is
 *     supported here. However, MessageType can not extend Object[] or be any primitive array other
 *     than byte[]. If byte[] is MessageType, the corresponding
 *     AbstractPacketDemultiplexer.getMessage(byte[]) method should return a String decoded from the
 *     byte[] assuming ISO-8859-1 encoding.
 *     <p>This class short-circuits local sends by directly sending it to the packet demultiplexer.
 */
public class MessageNIOTransport<NodeIDType, MessageType> extends NIOTransport<NodeIDType>
  ...
  /**
   * JSON key corresponding to sender IP address. Relevant only if {@code MessageType} is
   * JSONObject. These can be optimized to have just a single short key, e.g., "SA" for sender
   * socket address, but it doesn't really matter because all four fields below are stamped only at
   * the receiver, so they don't induce more network traffic or (de-)serialization overhead.
   */
  @Deprecated public static final String SNDR_IP_FIELD = "_SIP";

  /**
   * JSON key corresponding to sender port number. Relevant only if {@code MessageType} is
   * JSONObject.
   */
  @Deprecated public static final String SNDR_PORT_FIELD = "_SPORT";

  /**
   * JSON key corresponding to sender socket address. Relevant only if {@code MessageType} is
   * JSONObject.
   */
  public static final String SNDR_ADDRESS_FIELD = "_SNDR_ADDRESS";

  /**
   * JSON key corresponding to receiver IP address. Relevant only if {@code MessageType} is
   * JSONObject.
   */
  @Deprecated public static final String RCVR_IP_FIELD = "_RCVR_IP_ADDRESS";

  /**
   * JSON key corresponding to receiver port number. Relevant only if {@code MessageType} is
   * JSONObject.
   */
  @Deprecated public static final String RCVR_PORT_FIELD = "_RCVR_TCP_PORT";

  /**
   * JSON key corresponding to receiver socket address. Relevant only if {@code MessageType} is
   * JSONObject.
   */
   ...
   *
   * @param id My node ID.
   * @param nodeConfig A map from all nodes' IDs to their respective socket addresses.
   * @throws IOException
   */
   ...
   * @throws IOException
   */
  public MessageNIOTransport(
      NodeIDType id, NodeConfig<NodeIDType> nodeConfig, SSLDataProcessingWorker.SSL_MODES sslMode)
      throws IOException {
    // Note: Default extractor will not do any useful demultiplexing
   ...
   * @param id My node ID.
   * @param nodeConfig A map from all nodes' IDs to their respective socket addresses.
   * @param pd The packet demultiplexer to handle received messages.
   * @param start If a server thread must be automatically started upon construction. If false, the
   *     caller must explicitly invoke (new Thread(JSONNIOTransport)).start() to start the server.
   * @throws IOException
   */
  public MessageNIOTransport(
      NodeIDType id,
  ...
  public MessageNIOTransport(AbstractPacketDemultiplexer<?> pd) throws IOException {
    super(null, 0, new MessageExtractor(pd), SSLDataProcessingWorker.SSL_MODES.CLEAR);
  }

  /**
   * Used only for testing. The private nature of this method means that external users can no
   * longer explicitly specify the message extractor that can now only be {@link
   * edu.umass.cs.nio.MessageExtractor}.
   *
   * @param id My node ID.
   * @param nodeConfig A map from all nodes' IDs to their respective socket addresses.
   * @param worker The message extractor.
   * @throws IOException
   */
  private MessageNIOTransport(
      NodeIDType id, NodeConfig<NodeIDType> nodeConfig, MessageExtractor worker)
      throws IOException {
    // Switched order of the latter two arguments
  ...
  /**
   * @param pd The demultiplxer to add at the end of the current chain.
   */
  ...
  /**
   * @param pd The demultiplxer to add before the current chain.
   */
   ...
   * @param id
   * @param msg
   * @return Refer {@link JSONMessenger#sendToID(Object, JSONObject) JSONMessenger.sendToID(Object,
   *     JSONObject)}.
   * @throws IOException
   */
  @Override
  ...
  public int sendToAddress(InetSocketAddress isa, MessageType msg) throws IOException {
    ...
    if (msg instanceof byte[]) return this.sendUnderlying(isa, (byte[]) msg);
    else if (msg instanceof Byteable) return this.sendUnderlying(isa, ((Byteable) msg).toBytes());
  ...
  /**
   * @param json
   * @return Socket address of the sender recorded in this JSON message at receipt time.
   */
  public static final InetSocketAddress getSenderAddress(JSONObject json) {
    ...
    try {
      InetSocketAddress isa =
          json.has(MessageNIOTransport.SNDR_ADDRESS_FIELD)
   ...
   */
  public static final InetSocketAddress getSenderAddress(byte[] bytes) throws UnknownHostException {
  ...
  public static final InetSocketAddress getSenderAddressJSONSmart(
    ...
    InetSocketAddress isa =
        json.containsKey(MessageNIOTransport.SNDR_ADDRESS_FIELD)
            ? Util.getInetSocketAddressFromString(
                (String) json.get(MessageNIOTransport.SNDR_ADDRESS_FIELD))
            : null;
    return isa;
  }
  ...
  /**
   * @param json
   * @return Socket address of the recorded recorded in this JSON message at receipt time. Sometimes
   *     a sender needs to know on which of its possibly multiple listening sockets this message was
   *     received, so we insert it into the packet at receipt time.
   */
  public static final InetSocketAddress getReceiverAddress(JSONObject json) {
    ...
    try {
      isa =
          json.has(MessageNIOTransport.RCVR_ADDRESS_FIELD)
              ? Util.getInetSocketAddressFromStringStrict(
  ...
  public static final InetSocketAddress getReceiverAddressJSONSmart(
      net.minidev.json.JSONObject json) {
    InetSocketAddress isa =
        json.containsKey(MessageNIOTransport.RCVR_ADDRESS_FIELD)
            ? Util.getInetSocketAddressFromString(
                (String) json.get(MessageNIOTransport.RCVR_ADDRESS_FIELD))
            : null;
  ...
  protected int sendToIDInternal(NodeIDType destID, MessageType msg) throws IOException {
    if (destID.equals(this.myID)) return sendLocal(msg);
    // else
    if (msg instanceof byte[]) return this.sendUnderlying(destID, (byte[]) msg);
  ...
  /**
   * We send even byte arrays encoded as strings because it is easier to do demultiplexing (in
   * JSONMesageExtractor) working with strings than byte arrays. Using a fixed encoding only ensures
   * that going from a string to bytes and back yields the same string. But we also want the
   * property here that going from a byte array to a string and back yields the same byte array.
   * This property is not ensured by all encodings but is ensured by ISO-8859-1, so we use that
   * encoding.
   *
   * <p>Note: If we really just need to send a byte stream and don't care about receiving all of the
   * bytes written as a single unit, we should not be using MessageNIOTransport in the first place,
   * and just use NIOTransport directly instead.
   */
  public static final String NIO_CHARSET_ENCODING = "ISO-8859-1";

  /**
   * These methods are really redundant wrappers around the corresponding NIOTransport methods, but
   * they exist so that there is one place where all NIO sends actually happen.
   */
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      ((MessageNIOTransport<Integer, JSONObject>) niots[0])
          .sendToIDInternal(1, JSONify(msgNum++, "Hello back from 0 to 1"));
      ((MessageNIOTransport<Integer, JSONObject>) niots[0])
          .sendToIDInternal(1, JSONify(msgNum++, "Second hello back from 0 to 1"));
      try {
        Thread.sleep(1000);
      } catch (Exception e) {
        e.printStackTrace();
      ...
      ((MessageNIOTransport<Integer, JSONObject>) niots[0])
          .sendToIDInternal(1, JSONify(msgNum++, "Third hello back from 0 to 1"));
      ((MessageNIOTransport<Integer, JSONObject>) niots[1])
          .sendToIDInternal(0, JSONify(msgNum++, "Thank you for all the hellos back from 1 to 0"));
      // //////////////////////////////////////////////////////////////////////
      int seqTestNum = 1;
      Thread.sleep(2000);
      ...
      for (int i = 0; i < nNodes * seqTestNum; i++) {
        ...
        ((MessageNIOTransport<Integer, JSONObject>) niots[k])
            .sendToIDInternal(j, JSONify(msgNum++, "Hello from " + k + " to " + j));
      }
      ...
      MessageNIOTransport<Integer, JSONObject> concurrentSender =
          new MessageNIOTransport<Integer, JSONObject>(nNodes, snc, workers[nNodes]);
