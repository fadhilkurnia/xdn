...
public abstract class AbstractNIOSSL implements Runnable {
  private static final int DEFAULT_BUFFER_SIZE = NIOTransport.WRITE_BUFFER_SIZE;

  /**
   * There isn't much benefit to increasing the buffer size and it does introduce a speed bump, so
   * best to leave MAX_FACTOR at 1, effectively disabling dynamic buffer expansion.
   */
  private static final int MAX_FACTOR = 1;

  private static final int MAX_BUFFER_SIZE = MAX_FACTOR * NIOTransport.WRITE_BUFFER_SIZE;
  private static final int MAX_DST_BUFFER_SIZE = MAX_FACTOR * NIOTransport.WRITE_BUFFER_SIZE;
   ...
   * @param myID
   */
  public AbstractNIOSSL(SelectionKey key, SSLEngine engine, Executor taskWorkers, String myID) {
  ...
  public synchronized void nioSend(final ByteBuffer unencrypted) {
    try {
      ...
      log.log(
          Level.INFO,
          "{0} increased wrapSrc buffer size to {1}",
  ...
  public synchronized void notifyReceived(ByteBuffer encrypted) {
    ...
    try {
      ...
      log.log(
          Level.INFO,
          "{0} increased unwrapSrc buffer size to {1}",
  ...
  /**
   * Trying to put buf2 into buf1 by creating a new buffer with enough space. Using this method
   * seems to cause nio to get stuck at high sending rates. Unclear why. This method currently does
   * not get invoked during runtime.
   *
   * @param buf1
   * @param buf2
   * @return
  ...
  private ByteBuffer getBiggerBuffer(ByteBuffer buf1, ByteBuffer buf2) {
    ...
    if (biggerSize > MAX_BUFFER_SIZE) {
      log.log(Level.WARNING, "{0} reached maximum allowed buffer size limit", new Object[] {this});
              ...
              public void run() {
                try {
                  ...
                  log.log(
                      ...
                      new Object[] {this, sslTask, (System.nanoTime() - t0) / 1000000});
  ...
  private synchronized boolean wrap() {
    ...
    switch (wrapResult.getStatus()) {
      case OK:
        if (wrapDst.position() > 0) this.drainOutbound();
        break;
      ...
      case BUFFER_OVERFLOW:
        log.log(
            Level.INFO,
            "{0} wrap BUFFER_OVERFLOW: Wrapped data is coming faster than the network can send it"
                + " out.",
            new Object[] {this});
        int biggerSize = engine.getSession().getApplicationBufferSize() + wrapDst.capacity();
        ...
        if (biggerSize < MAX_DST_BUFFER_SIZE) {
          ...
          log.log(
              Level.INFO,
              "{0} increased wrapDst buffer size to {1}",
          ...
          new Runnable() {
            ...
            public void run() {
              while (wrapDst.position() > 0) {
                ...
                if (wrapDst.position() == prev) Thread.yield();
              }
            }
          };
  ...
  private synchronized boolean unwrap() {
    ...
    switch (unwrapResult.getStatus()) {
      case OK:
        if (unwrapDst.position() > 0) this.pullInbound();
        break;
      ...
      case BUFFER_OVERFLOW:
        log.log(
            ...
            new Object[] {this});
        int biggerSize = engine.getSession().getApplicationBufferSize() + unwrapDst.capacity();
      ...
      case BUFFER_UNDERFLOW:
        log.log(Level.FINEST, "{0} unwrap BUFFER_UNDERFLOW", new Object[] {this});
        return false;
    }
    ...
    if (version.startsWith("1.8")) {
      ...
      try {
        ...
        */
        ...
        if (cleaner == null)
          try {
            Field cleanerField = bbuf.getClass().getDeclaredField("cleaner");
