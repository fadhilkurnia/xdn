 ...
 * @author V. Arun
 */
public abstract class AbstractJSONPacketDemultiplexer
    extends AbstractPacketDemultiplexer<JSONObject> implements PacketDemultiplexer<JSONObject> {

  /**
   * @param threadPoolSize Refer documentation for {@link
   *     AbstractPacketDemultiplexer#setThreadPoolSize(int) setThreadPoolsize(int)}.
   */
  ...
  protected Integer getPacketType(JSONObject json) {
    try {
      if (json != null) return JSONPacket.getPacketType(json);
  ...
  protected JSONObject processHeader(byte[] message, NIOHeader header, boolean cacheStringified) {
    return processHeader(message, 0, header, cacheStringified);
  }

  protected JSONObject processHeader(
      byte[] message, int offset, NIOHeader header, boolean cacheStringified) {
   ...
   */
  public static final JSONObject processHeaderStatic(
      byte[] message, int offset, NIOHeader header, boolean cacheStringified) {
    try {
