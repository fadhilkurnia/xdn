...
/**
 * @author arun
 *     <p>A simple local tester for SSL. Run this class in two separate terminals, one with argument
 *     100, and the other with argument 101. You also need to configure SSL parameters like
 *     keyStore, trustStore and their passwords for the JVM.
 */
  ...
  public static void main(String[] args) {
      ...
      public void processData(SocketChannel socket, ByteBuffer incoming) {
        ...
        if (sendReply) {
          ...
          if (count == numTestMessages * msgSize)
            System.out.println(
                "Request receipt rate after receiving *ALL* "
                    + msgCount
                    + " requests and "
                    + count
                    + " bytes = "
    ...
    try {
...

      {
        Thread.sleep(10000);
        numBytesSent +=
            niot1.send(isa2, ("*******poke*******" + gibberish).getBytes(), batchSize) + 8;
      }
