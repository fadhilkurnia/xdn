...
public class NIOTester {
  ...
  public static void main(String[] args) throws UnsupportedEncodingException {
    ...
    final InetSocketAddress isa1 =
        new InetSocketAddress(snc.getNodeAddress(id1), snc.getNodePort(id1));
    InetSocketAddress isa2 = new InetSocketAddress(snc.getNodeAddress(id2), snc.getNodePort(id2));
    ...
    while (gibberish.length() < size) gibberish += gibberish;
    ...
    class PDEcho extends AbstractPacketDemultiplexer<String> {
      ...
      public boolean handleMessage(String message, edu.umass.cs.nio.nioutils.NIOHeader header) {
        ...
        try {
          if (sendReply) {
            if (twoWay) while (niot2.send(isa1, replyBytes, batchSize) <= 0) Thread.yield();
            ;
          }
        } catch (Exception e) {
          e.printStackTrace();
        ...
        if (count == numTestMessages * msgSize) {
          System.out.println(
              "Receiver rate after receiving all "
                  + count / msgSize
                  + " requests = "
                  + Util.df(count / msgSize * 1000.0 / (System.currentTimeMillis() - t))
                  + "/sec ");
        }
        return true;
      }
    ...
    class PDSender extends AbstractPacketDemultiplexer<String> {
      ...
      public boolean handleMessage(String message, edu.umass.cs.nio.nioutils.NIOHeader header) {
        ...
        if (count == numTestMessages / replyRatio * replySize) {
          System.out.println(
              "Response rate after ALL "
                  + count / replySize
                  + " responses = "
                  + Util.df(count / replySize * 1000.0 / (System.currentTimeMillis() - t))
                  + "/sec "
    ...
    try {
      ...
      niot1 = new MessageNIOTransport<Integer, String>(id1, snc, new PDSender(), true);

      niot2 = new MessageNIOTransport<Integer, String>(id2, snc, new PDEcho(), true);
      ...
      for (int i = 0; i < numTestMessages / batchSize; i++) {
        int curSent = 0;
        while ((curSent = niot1.send(isa2, gibberish.getBytes("ISO-8859-1"), batchSize)) <= 0)
          Thread.yield();
        ;
        totalSent += curSent;
        r.record();
      ...
      System.out.println(
          "Sent "
              ...
              + totalSent
              + " bytes at "
              + Util.df(numTestMessages * 1000.0 / (System.currentTimeMillis() - t))
              + "/sec");
