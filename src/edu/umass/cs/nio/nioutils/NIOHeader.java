...
public class NIOHeader {
   ...
   */
  public static NIOHeader getNIOHeader(byte[] bytes) throws UnknownHostException {
    ByteBuffer bbuf = ByteBuffer.wrap(bytes, 0, 12);
    byte[] sip = new byte[4];
    bbuf.get(sip, 0, 4);
    int sport = (int) bbuf.getShort();
    if (sport < 0) sport += 2 * (Short.MAX_VALUE + 1);

    byte[] dip = new byte[4];
    bbuf.get(dip, 0, 4);
    int dport = (int) bbuf.getShort();
    if (dport < 0) dport += 2 * (Short.MAX_VALUE + 1);
    return new NIOHeader(
        new InetSocketAddress(InetAddress.getByAddress(sip), sport),
        new InetSocketAddress(InetAddress.getByAddress(dip), dport));
  }
  ...
  /**
   * @return {@code this} as a 12 byte array with 6 bytes for each IP, port pair.
   */
  public byte[] toBytes() {
    ByteBuffer bbuf = ByteBuffer.wrap(new byte[12]);
    bbuf.put(this.sndr != null ? this.sndr.getAddress().getAddress() : new byte[4]);
    bbuf.putShort(this.sndr != null ? (short) this.sndr.getPort() : 0);
    bbuf.put(this.rcvr != null ? this.rcvr.getAddress().getAddress() : new byte[4]);
