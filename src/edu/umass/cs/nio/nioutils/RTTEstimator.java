...
public class RTTEstimator {
  ...
  /**
   * byte will take us only up to 255ms. This array is static because it might as well be shared by
   * all nodes on a single physical machine and certainly within a JVM.
   */
  ...
  public static long record(InetAddress address, long rtt) {
    ...
    return rtts[index] =
        (byte)
            Util.movingAverage(
                rtt,
                rtts[index] != 0 ? (rtts[index] < 0 ? rtts[index] + 255 : rtts[index]) : rtt,
                ALPHA);
  }
  ...
  public static long getRTT(InetAddress address) {
    int index = addrToPrefixInt(address);
    if (bitset.get(index)) return rtts[addrToPrefixInt(address)];
    else return -1;
  }
  ...
  /**
   * @param address
   * @return RTT to address via a blocking ping. The implementation is JVM and platform dependent.
   * @throws IOException
   */
  public static long ping(InetAddress address) throws IOException {
    ...
    if (delay < PING_TIMEOUT) return delay;
    return -1;
  }

  /**
   * Utility methods to fetch Amazon EC2 information although that isn't used by the estimation
  ...
  private static ConcurrentHashMap<String, String> awsIPToRegion =
   ...
   */
  public static JSONObject getAmazonIPToRegionFile() throws IOException, JSONException {
    URL url = new URL(AWS_IP_RANGES);
    InputStream is = url.openStream();
    int ptr = 0;
    StringBuffer buffer = new StringBuffer();
    while ((ptr = is.read()) != -1) buffer.append((char) ptr);
    if (buffer.length() > 0) return awsIPRanges = new JSONObject(buffer.toString());
    return null;
  }
  ...
  protected static ConcurrentHashMap<String, String> populateAWSIPToRegionMap() {
    ...
    if (awsIPRanges != null) {
      try {
        JSONArray jarray =
            awsIPRanges.has(AWS_PREFIXES_KEY) ? awsIPRanges.getJSONArray(AWS_PREFIXES_KEY) : null;
        for (int i = 0; i < jarray.length(); i++)
          awsIPToRegion.put(
              (jarray.getJSONObject(i).getString(AWS_IP_PREFIX_KEY)),
              jarray.getJSONObject(i).getString(AWS_REGION_KEY));
  ...
  private static LinkedHashMap<InetAddress, Long> testMap =
      new LinkedHashMap<InetAddress, Long>() {
        /** */
  ...
  /**
   * @return String representation of bounded-size map; used only for instrumentation.
   */
  ...
  private static LinkedHashMap<Integer, LinkedHashSet<InetAddress>> closest =
      new LinkedHashMap<Integer, LinkedHashSet<InetAddress>>() {
        private static final long serialVersionUID = 1L;

        protected boolean removeEldestEntry(Map.Entry<Integer, LinkedHashSet<InetAddress>> eldest) {
   ...
   * @param nearestMap
   */
  public static void closest(InetSocketAddress sender, Map<InetAddress, Long> nearestMap) {
    closest.put(
        addrToPrefixInt(sender.getAddress()), new LinkedHashSet<InetAddress>(nearestMap.keySet()));
  }
