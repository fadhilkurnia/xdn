...
/**
 * @author V. Arun
 *     <p>Helps instrument read/write stats in NIOTransport. Used for testing and instrumentation
 *     purposes only.
 */
public class NIOInstrumenter {
    ...
    if (enabled)
      synchronized (NIOInstrumenter.class) {
        if (msg.has(JSONMessenger.SENT_TIME)) {
          averageDelay =
              Util.movingAverage(
                  System.currentTimeMillis() - msg.getLong(JSONMessenger.SENT_TIME), averageDelay);
          lastUpdated = System.currentTimeMillis();
        }
      }
  }
  ...
  /**
   * Will monitor for long {@link AbstractPacketDemultiplexer#handleMessage(Object, NIOHeader)}
   * instances.
   */
  public static void monitorHandleMessage() {
    {
      log.log(
          Level.INFO,
          "Initializing handleMessageMonitor with threshold " + THRESHOLD + " period " + PERIOD);
  ...
  public String toString() {
    ...
    return s
        ...
        + totalConnAccepted
        + "]"
        + "\nJSONMessageWorker: [totalJSONRcvd = "
        + totalJSONRcvd
        + "]"
        + "]";
  }
