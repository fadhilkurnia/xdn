...
/**
 * @author V. Arun
 * @param <NodeIDType> This class is a simple utility to quickly set up a simple node config object
 *     when port numbers can take "don't care" values. It is also especially convenient for a local
 *     (single-machine) setup. This class is mainly useful for quick testing.
 */
public class SampleNodeConfig<NodeIDType> implements NodeConfig<NodeIDType> {
  ...
  /**
   * @param defaultPort Assigns port numbers to nodes starting from defaultPort
   */
  ...
  /**
   * The caller can either specify the number of nodes, nNodes, or specify a set of integer node IDs
   * explicitly. In the former case, nNodes from 0 to nNodes-1 will the node IDs. In the latter
   * case, the explicit set of node IDs will be used.
   *
   * @param startNodeID
  ...
  /**
   * Maps each node ID to a port number. If ID is an integer, then the port number is calculated as
   * {@code defaultport + ID}, else it is {@code defaultPort + ID.hashCode()}. The port calculated
   * above modulo 65536 is returned in order to ensure legitimate port numbers.
   */
  @Override
