...
public abstract class JSONPacket {
  ...
  /**
   * @return JSONObject corresponding to fields in classes extending this class.
   * @throws JSONException
   */
  protected abstract JSONObject toJSONObjectImpl() throws JSONException;

  /**
   * @return JSONObject corresponding to this class' (including subclasses) fields.
   * @throws JSONException
   */
  ...
  public static final Integer getPacketType(JSONObject json) throws JSONException {
    if (json.has(PACKET_TYPE)) return json.getInt(PACKET_TYPE);
    else return null;
  }
  ...
  /**
   * @param str
   * @return True if {@code str} could possibly (but not necessarily) be in JSON format. This sanity
   *     check is quicker compared to taking a JSONException hit.
   */
  ...
  /**
   * @param bytes
   * @return True if this {@code bytes} could be possibly (but not necessarily) be in JSON format
   *     assuming the default {@link MessageExtractor} encoding.
   */
