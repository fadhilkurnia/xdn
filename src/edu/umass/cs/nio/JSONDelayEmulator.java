...
public class JSONDelayEmulator {
  ...
  /**
   * Receiver calls this method to toString delay that is to be emulated, and delays processing the
   * packet by that amount. We put the delay to be emulated inside the json at the sender side
   * because the receiver side does not know which node ID sent the packet, and hence cannot know
   * how much to delay the packet
   *
   * @param jsonData
    ...
    if (JSONDelayEmulator.EMULATE_RECEIVER_DELAYS) {
      if (JSONDelayEmulator.USE_CONFIG_FILE_INFO && id != null) {
        ...
        if (id instanceof Integer)
          ...
          delay =
              ((InterfaceDelayEmulator<Integer>) pingNodeConfig).getEmulatedDelay((Integer) id) / 2;
        else if (id instanceof String)
          delay =
              ((InterfaceDelayEmulator<String>) pingNodeConfig).getEmulatedDelay((String) id) / 2;
        else if (id instanceof SocketAddress)
          delay =
              ((InterfaceDelayEmulator<SocketAddress>) pingNodeConfig)
                      .getEmulatedDelay((SocketAddress) id)
                  / 2;
