...
func main() {
	...
	r.POST("/kv", setValue)
	r.PUT("/kv", updateValue)
	r.DELETE("/kv/:key", deleteValue)
	r.POST("/kv/blindwrite", blindWrite) // Blind write route

	// Start the server
	log.Printf("Starting restkv at :8080")
...
// setValue adds a new key-value pair
func setValue(c *gin.Context) {
	...
	if err := db.First(&kv, "key = ?", kv.Key).Error; err == nil {
...
// updateValue updates an existing key-value pair
