...
impl TiKeyValueStore {
    /// Background writer task: processes one write op at a time, each in its own
    /// transaction. Serialization eliminates optimistic 2PC write conflicts.
    async fn writer_loop(
        client: TransactionClient,
        mut rx: mpsc::UnboundedReceiver<WriteOp>,
    ) {
        while let Some(op) = rx.recv().await {
            let result = Self::execute_one(&client, &op).await;
            let reply = match op {
